...
| Text Intelligence Service | News Synthesis V1 plus separate SEC authority and optional model boundary | canonical rendered News/SEC identities, local model artifacts, optional local LLM endpoint | `news_synthesis_v1`, SEC V5 labels, and request classification responses | notice-driven plus reconciliation | Runs News Synthesis V1 for News and V5 only for SEC, independently of optional models. It must not poll providers or write canonical News/SEC rows. |
