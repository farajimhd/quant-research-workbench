...
| `research/mlops` is overloaded. | Operational pipelines, research utilities, migration scripts, and runbooks all accumulated in one folder. | SEC, Benzinga, reference-data, q_live migration, and market SIP workflows now live under `pipelines/`; market SIP wrappers have been removed from `research/mlops`. |
