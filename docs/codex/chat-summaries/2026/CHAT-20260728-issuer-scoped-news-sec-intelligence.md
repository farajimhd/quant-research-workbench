...
`scoped_text_labeling_v3` keeps one canonical rendered publication while
...
- V3 News issuer/event scoping and article-local alias disambiguation.
- V2 SEC relevant-section extraction with event and issuer provenance.
- V3 deterministic semantic concept authority.
- Exact expected-outcome certification with five News and five SEC Markdown
  audits under the machine runtime root.
- Dry-run-by-default, bounded, resumable persistence into
  `scoped_text_labels_v3`, `scoped_content_relations_v1`, and the V3 build
  status table.
...
- 39 targeted unit tests passed.
- Modified Python modules compiled.
- `git diff --check` passed.
- Exact ClickHouse-backed certification passed all 10 audits with zero
...
No historical V3 backfill was executed. After optional audit review, run:
