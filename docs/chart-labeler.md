...
Open **Trading Workspaces → Labeler** (`#labeler`). Select a completed market date and regular or extended session. Every ticker selection starts with the **1h** view. Use **No opportunity & next** to finish a negative review, or switch to a finer view (including **100ms**) and select **Long range** or **Short range**. Click the entry candle, then the exit candle. The selected open and close are reference prices, not executable fills.
...
The browser flag uses isolated deterministic API fixtures. It exercises hourly-first screening, 100ms long/short marking, timestamp preservation across views, undo/redo, deletion, failed-save recovery, endpoint dragging, completion, next ticker, and revisiting saved labels. It does not create production labels or place trades. Live QMD compatibility requires a separate real-data check.
