...
- Scanner, Signal Stream, and Watchlist body rows use one fixed 42 px logical height before global UI scaling. Logos render at 28 px inside the 34 px identity cell, while News and SEC recency glyphs render at 15 px; rows without either asset retain the same height and alignment.
