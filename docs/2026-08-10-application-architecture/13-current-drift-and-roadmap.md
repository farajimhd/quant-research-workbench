...
| OMS | Durable Portfolio reservation verification, persisted intent idempotency, adaptive execution/protection, partial-fill handling and broker reconciliation are implemented | Complete remaining operator projections and multi-host command fencing if deployed beyond one broker-session authority | No unreserved intent can enter order planning; broker deployment remains separately authorized |
