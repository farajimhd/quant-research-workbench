            ...
            for index, scenario in enumerate(scenarios, start=1):
                ...
                try:
                    ...
                    if args.structural_detector_fixture:
                        evidence = json.loads(Path(args.structural_detector_fixture).read_text())
                        # Results are generated by the real backend detector over
                        # canonical candles. Only transport is intercepted here,
                        ...
                        def detector_response(route):
                            ...
                            if evidence.get('contract') in ('structural-candle-detector-4','structural-candle-detector-5','structural-candle-detector-6','structural-candle-detector-7','structural-candle-detector-8','structural-candle-detector-9','structural-candle-detector-10'):
                        ...
                        }""", {'rows': visible, 'scale': scenario['scale']})
                        panel = page.locator('#structural-indicator-fixture')
                        supertrend_control=panel.get_by_role('button',name=re.compile(r'^Supertrend 10'))
                        supertrend_control.wait_for(state='visible')
                        supertrend_control.click()
                        page.get_by_role('dialog',name='Supertrend settings').wait_for(state='visible')
                        if page.get_by_label('Supertrend ATR period',exact=True).input_value()!='10':
                            raise RuntimeError('Supertrend default period changed')
                        page.screenshot(path=str(screenshot_path.with_name(screenshot_path.stem+'__supertrend-settings.png')),full_page=True)
                        ...
                        page.get_by_role('checkbox',name='Supertrend',exact=True).uncheck()
                        supertrend_control.wait_for(state='hidden')
                        page.get_by_role('checkbox',name='Supertrend',exact=True).check()
                        supertrend_control.wait_for(state='visible')
                        ...
                        if evidence.get('contract') in ('structural-candle-detector-7','structural-candle-detector-8','structural-candle-detector-9','structural-candle-detector-10'):
                        ...
                        if evidence.get('contract') in ('structural-candle-detector-8','structural-candle-detector-9','structural-candle-detector-10'):
                            ...
                            if evidence.get('contract')=='structural-candle-detector-10' and not all(text in inspector.inner_text() for text in ('Broader context','Entry thesis','Room basis')):
                                raise RuntimeError('Position thesis or conditional level path missing')
                        ...
                        expected_families=15 if evidence.get('contract') in ('structural-candle-detector-7','structural-candle-detector-8','structural-candle-detector-9','structural-candle-detector-10') else 14 if evidence.get('contract')=='structural-candle-detector-6' else 13
                        if inspector.locator('tbody tr').count()!=expected_families:
                            raise RuntimeError('Candle inspector lost label families')
                        if evidence.get('contract') in ('structural-candle-detector-6','structural-candle-detector-7','structural-candle-detector-8','structural-candle-detector-9','structural-candle-detector-10'):
                        ...
                        page.wait_for_function("reasons => [...document.querySelectorAll('.structural-label-layer .structural-candle-label')].some(n=>reasons.some(reason=>n.textContent.includes(reason)))",arg=list({r['technical_signal']['reason'].replace('_',' ') for r in evidence['rows'] if r['technical_signal']['action'].endswith(('_enter','_exit'))}))
                        panel.get_by_role('button',name='Structural detector settings',exact=True).click()
                    ...
                    if args.staged_strategy_fixture:
                        ...
                        # Structural indicator now owns chart labels; strategy
                        # evidence remains available in the activity timeline.
                        page.locator('#staged-strategy-fixture').get_by_role('button', name='Structural detector', exact=True).wait_for()
                    ...
                    if args.level_book_v7:
                        page.get_by_role('checkbox',name='Show reaction book',exact=True).check()
                        summary=page.locator('.reaction-book-settings summary')
                        summary.filter(has_text='V7 ·').wait_for(timeout=60000)
                        summary.click()
                        page.get_by_label('Level book source',exact=True).select_option('v7')
                        page.get_by_text('Current-day confirmed levels',exact=True).wait_for()
                        page.screenshot(path=str(screenshot_path.with_name(screenshot_path.stem+'__v7-settings.png')),full_page=True)
                        page.get_by_role('checkbox',name='Current-day confirmed levels',exact=True).uncheck()
                        page.get_by_role('checkbox',name='Current-day confirmed levels',exact=True).check()
                        summary.click()
                        page.screenshot(path=str(screenshot_path.with_name(screenshot_path.stem+'__v7-levels.png')),full_page=True)
                    ...
                    if args.swing_structure_fixture:
                        ...
                        toggle.click()
                    if args.hindsight_actions:
                        from action_values_review import review_action_values
                        metrics['hindsight_actions']=review_action_values(page,screenshot_path)
                    ...
                    if args.backtest_presets and scenario['page']=='backtest-trading':
                        ...
                        if page.get_by_label('Start time',exact=True).input_value()!='04:00:00':raise RuntimeError('Ticker changed selected period')
                        if page.get_by_label('End time',exact=True).input_value()!='04:30:00':raise RuntimeError('Ticker changed selected period')
                        ...
                        if page.get_by_label('Start time',exact=True).is_disabled():raise RuntimeError('Batch time controls must remain editable')
                        ...
                        if page.get_by_label('Start time',exact=True).input_value()!='16:00:00' or page.get_by_label('End time',exact=True).input_value()!='20:00:00':raise RuntimeError('After-hours period was not applied')
                        page.get_by_role('button',name='Ticker preset',exact=True).click()
                    ...
                    if args.backtest_warmup_presentation and scenario['page']=='backtest-trading':
                        ...
                        trigger.click()
                        dialog=page.get_by_role('dialog',name='Backtest preparation',exact=True)
                        dialog.wait_for(state='visible')
                        progress=header.get_by_role('progressbar')
                        if progress.get_attribute('aria-label')=='Backtest warm-up progress':
                            counts=dialog.locator('dl > div').nth(1).locator('dd').inner_text().replace(',','').split('/')
                            completed,total=map(int,counts)
                        ...
                        if header.locator('details').count():raise RuntimeError('Inline diagnostics expanded the header')
                        if abs(header.bounding_box()['height']-before)>1:raise RuntimeError('Details changed header height')
                        page.screenshot(path=str(screenshot_path.with_name(screenshot_path.stem+'__details.png')))
                        page.keyboard.press('Escape')
                    ...
                    if args.structural_detector_fixture:
                        ...
                        page.screenshot(path=str(screenshot_path.with_name(screenshot_path.stem+'__failed.png')),full_page=True)
                    if args.hindsight_actions:
                        print('Action-value review failure: '+repr(exc),flush=True)
                        page.screenshot(path=str(screenshot_path.with_name(screenshot_path.stem+'__failed.png')),full_page=True)
                    capture_failures += 1
                    result.update({
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument('--hindsight-actions',action='store_true',help='validate real action-value labels, price spectra, inventory inspector and keyboard controls')
    result.add_argument('--hindsight-action-chart',help='canonical QMD chart response for isolated real ChartPanel validation; action endpoint remains live')
    ...
    result.add_argument('--level-book-v7', action='store_true', help='validate prepared V7 historical and causal streaming levels through backend endpoints')
