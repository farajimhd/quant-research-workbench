...
const CHART_INDICATORS: ChartDisplayItem[] = [
  ...
  displayIndicator("indicator.qmd_liquidity_levels", "QMD Liquidity Support & Resistance", "volume_liquidity", ["liquidity_support_price", "liquidity_support_strength", "liquidity_support_confidence", "liquidity_resistance_price", "liquidity_resistance_strength", "liquidity_resistance_confidence"], "price", {
    ...
    readingGuide: "Treat each level as an area, not an exact tick. The current SUP and RES are identified by compact native tags at their prices on the right axis; redundant current labels are not drawn over candles. Historical SUP and RES tags sit outside their source candle, with resistance evidence above and support evidence below. Read region density for accumulated strength, then read border weight and color intensity for confidence. A green level below price matters only if bids replenish or selling fails to move the midpoint; the red case is symmetric.",
    ...
    components: [
      ...
      { label: "SUP / RES history", description: "Historical tags are offset outside their source candle. Current in-plot detail labels are intentionally omitted to preserve the price bars.", tone: "neutral" },
  ...
  displayIndicator("indicator.market_structure_levels", "Market Structure Levels", "price_action", ["structure_session_high", "structure_session_low", "structure_premarket_high", "structure_premarket_low", "structure_opening_range_high", "structure_opening_range_low", "structure_swing_high", "structure_swing_low", "structure_bos_price", "structure_bos_direction", "structure_choch_price", "structure_choch_direction", "structure_luld_upper", "structure_luld_lower", "structure_52_week_high", "structure_52_week_low", "structure_prior_month_high", "structure_prior_month_low", "structure_prior_month_close", "structure_volume_poc", "structure_nearest_round"], "price", {
    ...
    readingGuide: "Start with session and premarket extremes, then opening range. Current in-plot detail labels are omitted so candles remain readable; selected current levels instead publish compact native tags beside their right-axis prices. Premarket H/L, opening-range H/L, and POC enable axis tags by default, while every eligible level group can be configured independently in the legend. Historical tags such as SwH, SwL, BoS, and CHoCH are placed outside their source candle: highs and bullish breaks above, lows and bearish breaks below, and neutral references to the left. A BoS or CHoCH connector begins where the broken swing first became causally confirmed and ends at the closing bar that broke it; this preserves the relationship without backpainting the original pivot. These deterministic structure levels do not expose a confidence score, so their weight and opacity remain neutral rather than implying one.",
