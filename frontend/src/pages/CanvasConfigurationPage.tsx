...
const CHART_INDICATORS: ChartDisplayItem[] = [
  ...
  displayIndicator("indicator.qmd_liquidity_levels", "QMD Liquidity Support & Resistance", "volume_liquidity", ["liquidity_support_price", "liquidity_support_strength", "liquidity_support_confidence", "liquidity_resistance_price", "liquidity_resistance_strength", "liquidity_resistance_confidence"], "price", {
    ...
    readingGuide: "Treat each level as an area, not an exact tick. The current SUP and RES are identified by compact native tags at their prices on the right axis; redundant current labels are not drawn over candles. Historical SUP and RES text is attached to open space along the evidence span, above resistance and below support. The renderer tries several positions along the line and omits the text when none is clear of candle bodies and wicks. Read region density for accumulated strength, then read border weight and color intensity for confidence. A green level below price matters only if bids replenish or selling fails to move the midpoint; the red case is symmetric.",
    ...
    components: [
      ...
      { label: "SUP / RES history", description: "Historical text sits along the level span rather than beside a candle. Resistance labels use the space above the line and support labels the space below; labels are skipped when the span has no candle-clear position.", tone: "neutral" },
  ...
  displayIndicator("indicator.market_structure_levels", "Market Structure Levels", "price_action", ["structure_session_high", "structure_session_low", "structure_premarket_high", "structure_premarket_low", "structure_opening_range_high", "structure_opening_range_low", "structure_swing_high", "structure_swing_low", "structure_bos_price", "structure_bos_direction", "structure_choch_price", "structure_choch_direction", "structure_luld_upper", "structure_luld_lower", "structure_52_week_high", "structure_52_week_low", "structure_prior_month_high", "structure_prior_month_low", "structure_prior_month_close", "structure_volume_poc", "structure_nearest_round"], "price", {
    ...
    readingGuide: "Start with session and premarket extremes, then opening range. Current in-plot detail labels are omitted so candles remain readable; selected current levels instead publish compact native tags beside their right-axis prices. Premarket H/L, opening-range H/L, and POC enable axis tags by default, while every eligible level group can be configured independently in the legend. Historical SwH and SwL text sits along its confirmed level span. A BoS or CHoCH connector begins where the broken swing first became causally confirmed and ends at the closing bar that broke it; its label sits between those endpoints, above bullish connectors and below bearish connectors. Event glyphs remain outside the breaking candle. Every line label tries several positions and is omitted when none is clear of candle bodies, wicks, and other labels. This preserves the causal relationship without backpainting the original pivot or obscuring price. These deterministic structure levels do not expose a confidence score, so their weight and opacity remain neutral rather than implying one.",
    ...
    components: [
      ...
      { label: "SwH / SwL", description: "Confirmed swing high and low markers. They appear when the pivot becomes known, after two following bars, and their optional text is placed along the historical level span rather than on a candle.", tone: "info" },
      { label: "BoS", description: "Break of Structure: a close through a confirmed swing in the established trend direction. The square stays outside the breaking candle; BoS+ or BoS- is placed in clear space along the connector back to the confirmed swing state.", tone: "buy" },
      { label: "CHoCH", description: "Change of Character: the first confirmed-swing break against the prior structure direction. The X stays outside the breaking candle; CHoCH+ or CHoCH- is placed in clear space along the dashed connector back to the confirmed swing state.", tone: "warning" },
