...
const CHART_INDICATORS: ChartDisplayItem[] = [
  ...
  displayIndicator("indicator.qmd_generic_structure", "QMD Generic Structure", "price_action", [
    ...
    detailedDescription: "QMD derives price structure only from ordered eligible trade prices. Consolidated NBBO updates still maintain displayed-liquidity evidence and zones, but a quote that was never executed cannot create a swing, BoS, or CHoCH. The engine extracts three simultaneous price-response scales—micro, tactical, and context—and clusters confirmed traded-price pivots into persistent support and resistance zones. The chart samples this same event state at each bar end; candle OHLC does not define or recalculate the structure.",
    calculation: "The adaptive base reversal threshold has a two-tick or 0.5-basis-point floor, follows the larger of 1.25 times recent spread and 1.5 times recent eligible-trade movement, and is capped at the larger of 25 basis points or four ticks so a transient quote cannot turn Micro into a broad swing scale. Micro, tactical, and context use 1×, 3×, and 8× that base. A pivot is published only after eligible traded price reverses by the scale threshold. A break requires traded price beyond the frozen pivot plus scale-specific persistence. A break with the prior structure is BoS; the first accepted break against it is CHoCH.",
