...
function ReplayControls({ onExit, onRunChange, run }: { onExit: () => void; onRunChange: (run: CanvasReplayRun) => void; run: CanvasReplayRun }) {
  ...
  const transportPreparing = active && !navigationActive && run.runtime_ready !== true;
  ...
  return <div className="replay-canvas-controls" aria-label="Replay controls">
    ...
    <div className={`replay-run-state${navigationActive || transportPreparing ? " is-navigation" : ""}`} data-status={run.status}><i aria-hidden="true" /><span><strong aria-live="polite">{navigationPreparing ? "Preparing causal scan" : navigationActive ? "Finding next action" : transportPreparing ? preparingLabel : run.status.replaceAll("_", " ")}</strong><small>{navigationPreparing ? `Loading signal + Watchlist history · ${navigationElapsedSeconds(run.navigation_search?.started_at, wallClock)}s` : navigationActive ? `${compactEventCount(run.navigation_search?.scanned_events)} market events · ${formatReplayClock(run.current_time)} ET · ${navigationElapsedSeconds(run.navigation_search?.started_at, wallClock)}s` : transportPreparing ? preparingDetail : run.navigation_action ? `${run.navigation_action.ticker ? `${run.navigation_action.ticker} · ` : ""}${run.navigation_action.label}` : run.status === "warming" ? `${compactEventCount(run.warmup_events)} events` : `${Math.round(run.progress * 100)}%`}</small></span></div>
