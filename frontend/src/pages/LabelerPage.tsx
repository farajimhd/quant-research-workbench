...
import type { LabelCandle, LabelRange } from "../app/components/ChartLabelOverlay";
...
export function LabelerPage() {
  const [day, setDay] = useState(() => new Date(Date.now() - 86400000).toLocaleDateString("en-CA", { timeZone: "America/New_York" }));
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch (reason) { if (!controller.signal.aborted) { setError(message(reason)); setProgress("Chart coverage incomplete"); } }
    })();
    return () => controller.abort();
  }, [scope, reload]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const navigation = (event: Event) => { if (locked) { event.preventDefault(); setError("Save or discard the pending draft before leaving Labeler."); } };
    window.addEventListener("beforeunload", guard);
    window.addEventListener("workspace-before-navigate", navigation);
    return () => { window.removeEventListener("beforeunload", guard); window.removeEventListener("workspace-before-navigate", navigation); };
  }, [locked]);
  ...
  async function persist(next: LabelRange[], status = "in_progress") {
    ...
    try {
      ...
      setReview(saved); setDraft(null); setSaveState(`Saved · revision ${saved.revision}`);
  ...
  function pick(candle: LabelCandle) {
    ...
    edit([...ranges, { id, direction: mode, annotation_timeframe: timeframe, entry_timestamp: new Date(Math.round(pending.time * 1000)).toISOString(), exit_timestamp: new Date(Math.round(candle.endTime! * 1000)).toISOString(), entry_price: pending.open, exit_price: candle.close }]);
  ...
  const payload = useMemo<ChartPayload>(() => ({ ...emptyPayload, timeframe, candles: bars.map(b => ({ time: b.start_ms / 1000, endTime: b.end_ms / 1000, isClosed: true, open: b.open, high: b.high, low: b.low, close: b.close })), volume: bars.map(b => ({ time: b.start_ms / 1000, value: b.volume, color: getComputedStyle(document.documentElement).getPropertyValue("--chart-text").trim() })) }), [bars, timeframe]);
  const chosen = ranges.find(r => r.id === selected);
  const ready = complete && !locked && bars.length > 0 && !!review;
  return <div className="labeler-page">
    ...
    <div className="labeler-workspace">
      ...
      <main className="labeler-main"><div className="labeler-status" role="status"><strong>{ticker || "Choose a session"}</strong><span>{review?.status.replaceAll("_", " ")}</span><span>{progress}</span><span>{saveState}</span></div>
        {error ? <div className="labeler-error" role="alert">{error}{draft && !busy ? <><button onClick={() => void persist(draft, draftStatus)}>Retry save</button><button onClick={() => { setDraft(null); setReload(v => v + 1); }}>Discard unsaved draft and reload</button></> : null}</div> : null}
        {complete && bars.length > 0 ? <ChartPanel key={`${day}:${session}:${ticker}:${timeframe}`} ticker={ticker} timeframe={timeframe} timeframes={frames} payload={payload} featureOptions={[]} indicatorOptions={[]} visibleColumns={[]} visibleSupervisionGroups={[]} onTickerChange={() => {}} onTimeframeChange={value => { if (!locked) setTimeframe(value); }} onVisibleColumnsChange={() => {}} onVisibleSupervisionGroupsChange={() => {}} tickerEditable={false} toolbarVariant="compact" showIndicatorControls={false} showSupervisionControls={false} baseHeight={540}
          loading={!!ticker && !complete && !error} deferInitialFitUntilLoaded initialFitMode="last_market_day" appearanceDefaults={{ legendGutterVisible: false, rightLegendGutterVisible: false }} settingsStorageKey="chart-labeler"
          labeling={{ ranges, active: ready && mode !== "select", selected, pendingTime: pending ? Math.round(pending.time * 1000) : undefined, onPick: pick, onSelect: setSelected, onMove: move }}
          toolbarActions={<div className="labeler-tools">{(["select", "LONG", "SHORT"] as const).map(tool => <button disabled={!ready} key={tool} aria-pressed={mode === tool} onClick={() => { setMode(tool); setPending(null); }}>{tool === "select" ? "Select" : `${tool === "LONG" ? "Long" : "Short"} range`}</button>)}
            ...
            <button disabled={!ready || !chosen} onClick={() => { edit(ranges.filter(r => r.id !== selected)); setSelected(null); }}>Delete</button></div>} /> : <div className="labeler-chart-placeholder" role="status">{complete ? "No observed candles in this session. A negative review cannot be certified." : progress || "Select a ticker to load its hourly chart."}</div>}
        ...
        <footer className="labeler-footer"><span>Unfinished coverage stays unknown. Reference prices are not executable fills.</span><button disabled={!ready || !ranges.length} onClick={() => void finish("completed")}>Complete & next</button><button disabled={!ready || ranges.length > 0} onClick={() => void finish("no_opportunity")}>No opportunity & next</button></footer>
      </main>
    </div>
  </div>;
}
