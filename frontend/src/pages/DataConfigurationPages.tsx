  ...
  return <article className="rule-set-document">
    ...
    <section className="rule-condition-list">{ruleSet.conditions.map((condition, index) => {
      ...
      return <div className="rule-condition-row rule-condition-editable" key={condition.condition_id}>
        ...
        }} optionLimit={0} options={!source && condition.left_source_id ? [{ description: "Unregistered definition referenced by this draft.", label: condition.left_source_id, value: condition.left_source_id }, ...definitionOptions] : definitionOptions} presentation="catalog" searchable searchPlaceholder="Search data definitions…" showAllOnOpen value={condition.left_source_id} /><em>{condition.left_source_id}{condition.left_timeframe ? ` · ${condition.left_timeframe}` : ""}</em></div>
