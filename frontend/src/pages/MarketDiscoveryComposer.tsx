import { Activity, ArrowDown, ArrowRight, ArrowUp, Check, Columns3, ListFilter, Pencil, Plus, ScanSearch, Search, Trash2 } from "lucide-react";
...
type AggregationContract = { allowed?: string[]; default?: string; function?: string; mode?: "intrinsic" | "none" | "required" };
type ColumnDefinition = { available_intervals?: string[]; column_id: string; description: string; dimensions?: { aggregation?: AggregationContract; interval_semantics?: string }; field_ref?: string; name: string; sortable?: boolean; source_id: string; source_kind?: "data_field" | "rule_set" | string; semantic_type: string; value_type: string };
...
type BaseComposition = { column_aggregations?: Record<string, string>; column_intervals?: Record<string, IntervalValue>; columns: string[]; description: string; inclusion_operator: "all" | "any"; inclusion_rule_sets: string[]; name: string; refresh_interval_ms: number };
type Composition = BaseComposition & { maximum_size: number; ranking_aggregation?: string; ranking_direction: "ascending" | "descending"; ranking_field: string; ranking_field_ref?: string; ranking_interval?: IntervalValue };
...
type SignalStream = BaseComposition & { cooldown_ms: number; enabled: boolean; episode_role?: "start" | "milestone"; episode_ttl_ms?: number; maximum_events: number; occurrence_source?: "rule_evaluator" | "qmd_live_market_state" | "qmd_squeeze_episode"; origin?: string; protected?: boolean; rearm_policy: "after_false" | "after_cooldown"; revision: number; signal_stream_id: string; source_id: string; source_scan_id: string; source_type: "core_scan" | "watchlist" | "news_events"; trigger_policy: "false_to_true"; watchlist_routes: SignalRoute[] };
export type MarketDiscoveryConfiguration = { atomic_fields?: unknown[]; calculation_catalog?: unknown[]; classifications?: unknown[]; data_field_plan?: { field_refs?: string[]; timeframes?: string[] }; data_fields?: unknown[]; column_catalog: ColumnDefinition[]; core_scan: CoreScan; field_catalog: DataDefinition[]; rule_sets: RuleSet[]; security_universe: { description: string; name: string }; signal_streams: SignalStream[]; watchlists: Watchlist[] };
...
export function MarketDiscoveryComposer({ onChange, section }: { onChange: (value: MarketDiscoveryConfiguration) => void; section: MarketDiscoveryConfiguration }) {
  ...
  const selectedRankingIntervals = selectedRankingColumn?.available_intervals ?? [];
  const selectedRankingAggregation = selectedRankingColumn?.dimensions?.aggregation;
  const selectedSignalSource = selectedSignalStream ? `${selectedSignalStream.source_type || "core_scan"}:${selectedSignalStream.source_id || selectedSignalStream.source_scan_id || section.core_scan.scan_id}` : "";
  const signalSourceOptions: InventoryFilterOption[] = [
    { description: "Evaluate every eligible ticker in the untruncated QMD Core Scan population.", group: "Full market", label: `${section.core_scan.name} · all eligible tickers`, value: `core_scan:${section.core_scan.scan_id}` },
    ...(selectedSignalStream?.source_type === "news_events" ? [{ description: "Issuer-scoped, causally available News Synthesis V1 publications.", group: "Event intelligence", label: "News Synthesis V1 events", value: "news_events:q_live.news_synthesis_v1" }] : []),
    ...section.watchlists.map((row) => ({ description: row.enabled ? `Evaluate only the current members of ${row.name}.` : `${row.name} is disabled and cannot supply candidates until enabled.`, disabled: !row.enabled || row.availability === "integration_pending", group: "Watchlists", label: row.name, value: `watchlist:${row.watchlist_id}` })),
  ];
  ...
  function replaceColumns(columns: string[]) {
    ...
    const column_aggregations: Record<string, string> = {};
    columns.forEach((columnId) => {
      ...
      const aggregation = section.column_catalog.find((row) => row.column_id === columnId)?.dimensions?.aggregation;
      if (aggregation?.mode === "required") column_aggregations[columnId] = selected.column_aggregations?.[columnId] || aggregation.default || aggregation.allowed?.[0] || "";
    });
    replace({ ...selected, columns, column_aggregations, column_intervals });
  }
  ...
  return <div className="market-discovery-composer">
    ...
    <main className="market-discovery-definition">
      ...
      {!selectedWatchlist && !selectedSignalStream ? <section className="market-discovery-source"><span>Population</span><strong>{section.security_universe.name}</strong><p>{section.security_universe.description} The runtime keeps this population untruncated; Canvas applies its own presentation limit.</p></section> : selectedWatchlist ? <section className="market-discovery-source"><span>Source scanner</span><strong>{section.core_scan.name} · all eligible tickers</strong><p>Membership is resolved from the complete Core Scan candidate population before the Watchlist ranking and row limit are applied.</p></section> : null}

      {selectedSignalStream ? <>
        <section className="market-discovery-settings market-discovery-signal-setup">
          <header><span>Execution and session</span><p>These parameters define where candidates come from, when rules run, and how today’s immutable occurrence list behaves.</p></header>
          <div>
            <label className="market-discovery-wide-control"><span>Candidate source</span><InventoryFilterSelect ariaLabel="Signal Stream candidate source" className="configuration-lookup-button" disabled={selectedSignalStream.protected} onChange={(value) => { const separator = value.indexOf(":"); const source_type = value.slice(0, separator) as SignalStream["source_type"]; const source_id = value.slice(separator + 1); replaceSignalStream({ source_id, source_scan_id: section.core_scan.scan_id, source_type }); }} options={signalSourceOptions} presentation="catalog" searchable showAllOnOpen value={selectedSignalSource} /><small>{selectedSignalStream.source_type === "news_events" ? "Each validated issuer publication is an immutable candidate event; current QMD market fields are frozen with it." : "Core Scan evaluates every eligible ticker. A Watchlist evaluates only its current mutable members."}</small></label>
            <div className="market-discovery-setting-readonly"><span>Trading-day window</span><strong>04:00–20:00 ET</strong><small>Canvas shows only occurrences from today’s premarket start through the current time. The view clears after after-hours; durable audit records remain retained.</small></div>
            <div className="market-discovery-setting-readonly"><span>Trigger</span><strong>False → true edge</strong><small>A row is appended only when the selected Rule Sets transition into a match, preventing repeated rows on every refresh.</small></div>
            <label><span>Evaluation cadence</span><div className="market-discovery-unit-input"><input min="1000" onChange={(event) => replaceSignalStream({ refresh_interval_ms: Math.max(1000, Number(event.target.value)) })} step="100" type="number" value={selectedSignalStream.refresh_interval_ms} /><em>ms</em></div><small>The requested refresh cadence. Runtime coordinates all active discovery definitions at the fastest configured cadence, with a one-second floor.</small></label>
            <label><span>Rearm policy</span><InventoryFilterSelect ariaLabel="Signal Stream rearm policy" className="configuration-lookup-button" onChange={(value) => replaceSignalStream({ rearm_policy: value as SignalStream["rearm_policy"] })} options={[{ description: "The symbol may emit again only after its rules first become false.", label: "After rules become false", value: "after_false" }, { description: "The symbol may emit again after the cooldown even if its rules remain true.", label: "After cooldown while true", value: "after_cooldown" }]} presentation="catalog" showAllOnOpen value={selectedSignalStream.rearm_policy} /><small>Controls when one symbol is eligible to append another occurrence during the same trading day.</small></label>
            {selectedSignalStream.rearm_policy === "after_cooldown" ? <label><span>Cooldown</span><div className="market-discovery-unit-input"><input min="1" onChange={(event) => replaceSignalStream({ cooldown_ms: Math.max(1, Number(event.target.value)) })} type="number" value={selectedSignalStream.cooldown_ms} /><em>ms</em></div><small>Minimum elapsed time before a continuously matching symbol may emit another occurrence.</small></label> : null}
            <label><span>Maximum session rows</span><input min="1" onChange={(event) => replaceSignalStream({ maximum_events: Math.max(1, Number(event.target.value)) })} type="number" value={selectedSignalStream.maximum_events} /><small>Bounds rows returned to Canvas for this stream; it does not delete the durable occurrence audit.</small></label>
            <label className="market-discovery-toggle"><span><strong>Runtime enabled</strong><small>When off, this definition remains configured but creates no QMD demand or occurrences.</small></span><input checked={selectedSignalStream.enabled} onChange={(event) => replaceSignalStream({ enabled: event.target.checked })} type="checkbox" /></label>
          </div>
        </section>
        <ReferenceSection description="Each occurrence may admit its symbol into selected Watchlists. Membership remains mutable and follows the destination policy; the occurrence remains immutable." empty="No Watchlist routing configured. This stream will record occurrences without changing Watchlist membership." kind="Watchlist" onChange={(ids) => replaceSignalStream({ watchlist_routes: ids.map((watchlist_id) => selectedSignalStream.watchlist_routes.find((route) => route.watchlist_id === watchlist_id) ?? { membership_expiry: "end_of_trading_day", membership_ttl_ms: 300000, watchlist_id }) })} options={watchlistOptions(section.watchlists.filter((row) => !(selectedSignalStream.source_type === "watchlist" && row.watchlist_id === selectedSignalStream.source_id)), selectedSignalStream.watchlist_routes.map((route) => route.watchlist_id))} selected={selectedSignalStream.watchlist_routes.map((route) => route.watchlist_id)} title="Watchlist routing" />
      </> : null}
      ...
      {!selectedSignalStream ? <section className="market-discovery-settings">
        ...
        <div>
          <label><span>Ranking Data Field</span><InventoryFilterSelect ariaLabel="Ranking Data Field" onChange={(ranking_field_ref) => { const column = section.column_catalog.find((row) => row.field_ref === ranking_field_ref); const intervals = column?.available_intervals ?? []; const aggregation = column?.dimensions?.aggregation; replace({ ...(selected as Composition), ranking_aggregation: aggregation?.mode === "required" ? aggregation.default || aggregation.allowed?.[0] || "" : "", ranking_field_ref, ranking_field: column?.source_id ?? "", ranking_interval: intervals.length ? preferredInterval(intervals) : "" }); }} optionLimit={0} options={rankingOptions(section.column_catalog)} presentation="catalog" searchable searchPlaceholder="Search Data Fields…" showAllOnOpen value={selectedRankingRef} />{selectedRankingIntervals.length ? <IntervalSelect ariaLabel="Ranking interval" className="market-discovery-column-interval" intervals={selectedRankingIntervals} onChange={(ranking_interval) => replace({ ...(selected as Composition), ranking_interval })} value={(selected as Composition).ranking_interval || preferredInterval(selectedRankingIntervals)} /> : selectedRankingColumn ? <small className="market-discovery-field-context">No interval · current or anchored value</small> : null}{selectedRankingAggregation?.mode === "required" ? <AggregationSelect ariaLabel="Ranking aggregation" contract={selectedRankingAggregation} onChange={(ranking_aggregation) => replace({ ...(selected as Composition), ranking_aggregation })} value={(selected as Composition).ranking_aggregation} /> : null}</label>
      ...
      <ReferenceSection
        ...
        kind="Column"
        columnIntervals={selected.column_intervals ?? {}}
        columnAggregations={selected.column_aggregations ?? {}}
        onChange={replaceColumns}
        onIntervalChange={(columnId, interval) => replace({ ...selected, column_intervals: { ...(selected.column_intervals ?? {}), [columnId]: interval } })}
        onAggregationChange={(columnId, aggregation) => replace({ ...selected, column_aggregations: { ...(selected.column_aggregations ?? {}), [columnId]: aggregation } })}
...
function ReferenceSection({ columnAggregations = {}, columnIntervals = {}, description, empty, kind, onAggregationChange, onChange, onIntervalChange, options, selected, title }: { columnAggregations?: Record<string, string>; columnIntervals?: Record<string, IntervalValue>; description: string; empty: string; kind: string; onAggregationChange?: (columnId: string, aggregation: string) => void; onChange: (ids: string[]) => void; onIntervalChange?: (columnId: string, interval: IntervalSpec) => void; options: InventoryFilterOption[]; selected: string[]; title: string }) {
  ...
  const available = options.filter((row) => !selected.includes(row.value) && !(row.family && selectedFamilies.has(row.family)));
  ...
  return <section className="market-discovery-references">
    ...
    {selected.length ? <div className="market-discovery-reference-list">{selected.map((id, index) => { const option = byId.get(id); const reference = option?.reference || id; const intervals = option?.intervals ?? []; const displayName = option?.label ?? id; const contract = (option as InventoryFilterOption & { aggregation?: AggregationContract }).aggregation; const move = (offset: number) => { const target = index + offset; if (target < 0 || target >= selected.length) return; const reordered = [...selected]; [reordered[index], reordered[target]] = [reordered[target], reordered[index]]; onChange(reordered); }; return <article key={id}><span><strong>{displayName}</strong><small>{option?.description || id}</small>{kind === "Column" && intervals.length ? <IntervalSelect ariaLabel={`${displayName} interval`} className="market-discovery-column-interval" intervals={intervals} onChange={(interval) => onIntervalChange?.(id, interval)} value={columnIntervals[id] || preferredInterval(intervals)} /> : null}{kind === "Column" && contract?.mode === "required" ? <AggregationSelect ariaLabel={`${displayName} aggregation`} contract={contract} onChange={(aggregation) => onAggregationChange?.(id, aggregation)} value={columnAggregations[id]} /> : null}<details className="market-discovery-technical-identity"><summary>Technical identity</summary><code title={reference}>{reference}</code>{reference !== id ? <em>{kind === "Column" ? `Canvas column · ${id}` : id}</em> : null}</details></span><div className="market-discovery-reference-actions">{kind === "Column" ? <><button aria-label={`Move ${displayName} up`} disabled={index === 0} onClick={() => move(-1)} title="Move column up" type="button"><ArrowUp size={14} /></button><button aria-label={`Move ${displayName} down`} disabled={index === selected.length - 1} onClick={() => move(1)} title="Move column down" type="button"><ArrowDown size={14} /></button></> : null}<button aria-label={`Remove ${displayName}`} className="danger" onClick={() => onChange(selected.filter((value) => value !== id))} title={`Remove ${displayName}`} type="button"><Trash2 size={14} /></button></div></article>; })}</div> : <p className="market-discovery-empty-reference">{empty}</p>}
  </section>;
}
...
function columnOptions(columns: ColumnDefinition[], selected: string[]): InventoryFilterOption[] { return columns.map((row) => ({ aggregation: row.dimensions?.aggregation, description: row.description, family: row.source_id, group: row.source_kind === "rule_set" ? "Rule Set Results" : "Data Field Outputs", intervals: row.available_intervals, label: row.name, reference: row.source_kind === "rule_set" ? `rule-set://${row.source_id}` : row.field_ref || row.source_id, subgroup: row.source_kind === "rule_set" ? "Boolean results" : readable(row.semantic_type), value: row.column_id } as InventoryFilterOption & { aggregation?: AggregationContract })).sort((a, b) => Number(selected.includes(b.value)) - Number(selected.includes(a.value)) || a.label.localeCompare(b.label)); }
function rankingOptions(columns: ColumnDefinition[]): InventoryFilterOption[] { return columns.filter((row) => row.sortable && row.source_kind === "data_field" && row.field_ref).map((row) => ({ description: row.description, family: row.source_id, group: "Data Field Outputs", intervals: row.available_intervals, label: row.name, reference: row.field_ref, subgroup: readable(row.semantic_type), value: row.field_ref! })).sort((a, b) => a.label.localeCompare(b.label)); }
function readable(value: string) { return value.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()); }
function AggregationSelect({ ariaLabel, contract, onChange, value }: { ariaLabel: string; contract: AggregationContract; onChange: (value: string) => void; value?: string }) { const allowed = contract.allowed ?? []; return <InventoryFilterSelect ariaLabel={ariaLabel} className="market-discovery-column-aggregation" onChange={onChange} options={allowed.map((item) => ({ description: `${readable(item)} of events in the selected window.`, label: readable(item === "volume_weighted_mean" ? "volume weighted average" : item), value: item }))} presentation="catalog" showAllOnOpen value={value || contract.default || allowed[0] || ""} />; }
