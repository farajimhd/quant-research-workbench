...
type SignalStream = BaseComposition & { cooldown_ms: number; enabled: boolean; maximum_events: number; origin?: string; rearm_policy: "after_false" | "after_cooldown"; revision: number; signal_stream_id: string; source_id: string; source_scan_id: string; source_type: "core_scan" | "watchlist"; trigger_policy: "false_to_true"; watchlist_routes: SignalRoute[] };
    ...
    <main className="market-discovery-definition">
      ...
      {selectedSignalStream ? <>
        <section className="market-discovery-settings market-discovery-signal-setup">
          ...
          <div>
            <label className="market-discovery-wide-control"><span>Candidate source</span><InventoryFilterSelect ariaLabel="Signal Stream candidate source" className="configuration-lookup-button" onChange={(value) => { const separator = value.indexOf(":"); const source_type = value.slice(0, separator) as SignalStream["source_type"]; const source_id = value.slice(separator + 1); replaceSignalStream({ source_id, source_scan_id: section.core_scan.scan_id, source_type }); }} options={signalSourceOptions} presentation="catalog" searchable showAllOnOpen value={selectedSignalSource} /><small>Core Scan evaluates every eligible ticker. A Watchlist evaluates only its current mutable members.</small></label>
