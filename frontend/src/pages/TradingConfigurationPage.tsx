      ...
      <section className="strategy-authoring-stage discovery-guided-stage">
        ...
        {guidedStep === "watchlists" ? watchlistView === "select" ? <div className="discovery-watchlist-selection">
          ...
          <section className="discovery-watchlist-question">
            ...
            {watchlistQuestion === "calculations" ? <><header><h2>Which focused calculations should run for members?</h2></header><fieldset className="discovery-calculation-choices"><legend>Focused calculations</legend><p>QMD leases these higher-cost calculations only for seeded candidates and current Watchlist members, then releases unused state.</p><div>{section.core_scan.calculations.filter((row) => row.tier === "watchlist" && row.enabled).map((capability, index) => { const checked = selectedWatchlist.calculations.includes(capability.capability_id); return <label data-selected={checked} key={capability.capability_id}><span className="discovery-capability-index">{index + 1}</span><span className="discovery-calculation-card-copy"><strong>{capability.name}</strong><small>{capability.description}</small></span><input checked={checked} onChange={(event) => replaceWatchlist({ ...selectedWatchlist, calculations: event.target.checked ? [...selectedWatchlist.calculations, capability.capability_id] : selectedWatchlist.calculations.filter((id) => id !== capability.capability_id) })} type="checkbox" /></label>; })}</div></fieldset></> : null}
...
function WatchlistRuleChoices({ fields, onChange, ruleSets, watchlist }: { fields: DiscoveryField[]; onChange: (value: WatchlistConfig) => void; ruleSets: RuleSetDefinition[]; watchlist: WatchlistConfig }) {
  ...
  return <div className="discovery-rule-catalog">
    ...
    <div className="discovery-rule-card-list">{visibleRules.map((ruleSet, index) => {
      ...
      return <article data-assignment="active" key={ruleSet.rule_set_id}><span className="discovery-capability-index">{index + 1}</span><div><header><strong>{ruleSet.name}</strong><span>{types.length ? types.map((type) => type === "system" ? "System" : capabilityTypeLabel(type as DiscoveryCapability["capability_type"])).join(" · ") : "Rule set"}</span></header>{ruleSet.description ? <p>{ruleSet.description}</p> : null}<RuleSetMeaning catalog={catalog} ruleSet={ruleSet} /></div><button aria-label={`Remove ${ruleSet.name}`} className="button compact danger discovery-rule-remove" onClick={() => removeRule(ruleSet.rule_set_id)} type="button"><Trash2 size={13} /> Remove</button></article>;
