...
function abstractionKindForCapability(type: DiscoveryCapability["capability_type"]): AbstractionKind {
  return {
    event: "signal",
...
function collectRuleExpressionIds(expression?: RuleExpression): string[] {
...
function collectLifecycleRuleSetIds(lifecycle?: StrategyLifecycle): string[] {
  if (!lifecycle) return [];
...
function GuidedConfiguration({ approved, draft, label, omsStage, onChange, onContinue, onLabelChange, onOmsStageChange, onPublish, onSwitchToExpert, publishing, revisions, section }: {
  ...
  if (step === "portfolio") questions.push(
    <GuidedQuestion description="Choose the Run Plan-to-account mandate to configure, then connect it to the governed account and its account-wide safety policy." key="portfolio-account" label="Which Portfolio mandate are you configuring?" status={mandate.enabled ? "Enabled" : "Disabled"}><div className="guided-form-grid"><SelectField help="Each mandate gives one Run Plan bounded authority over one synchronized account." label="Portfolio mandate" onChange={selectMandate} options={draft.portfolio.mandates.map((row) => ({ description: `${deploymentName(draft.assignments, row.run_plan_id)} → ${accountName(draft.accounts, row.account_key)}`, label: deploymentName(draft.assignments, row.run_plan_id), value: row.mandate_id }))} value={mandate.mandate_id} /><SelectField help="The synchronized account whose cash, positions, buying power, reservations, and broker state govern every request." label="Account" onChange={(account_key) => replaceMandate({ ...mandate, account_key })} options={draft.accounts.bindings.map((row) => ({ description: `${readableLabel(row.account_class)} account`, label: row.name, value: row.account_key }))} value={mandate.account_key} /><SelectField help="The reusable account-level limits and permissions applied to this account before mandate-specific limits." label="Account safety policy" onChange={(portfolio_policy_id) => replaceAccount({ ...account, portfolio_policy_id })} options={draft.portfolio.policies.map((row) => ({ label: String(row.name ?? row.policy_id), value: String(row.policy_id) }))} value={account.portfolio_policy_id} /><BooleanField help="Disabled mandates remain saved but cannot approve new exposure for this Run Plan and account." label="Mandate enabled" onChange={(enabled) => replaceMandate({ ...mandate, enabled })} value={mandate.enabled} /></div></GuidedQuestion>,
  ...
  if (step === "execution") questions.push(
    <GuidedQuestion description="Choose the reusable OMS profile to configure. It groups the default entry, exit, routing, and broker-held protection choices applied after Portfolio approves an action." key="execution-profile" label="Which OMS profile are you configuring?" status={omsProfile.editable ? "Editable profile" : "Protected profile"}><div className="guided-form-grid"><SelectField help="Run Plans reference this stable OMS profile; changing the selection changes which profile this walkthrough edits." label="OMS profile" onChange={selectOmsProfile} options={draft.oms.profiles.map((row) => ({ description: row.description, label: row.name, value: row.profile_id }))} value={omsProfile.profile_id} /><SelectField help="Default broker-neutral policy used to work approved entry quantity." label="Entry execution policy" onChange={(entry_execution_policy_id) => replaceOmsProfile({ ...omsProfile, settings: { ...omsProfile.settings, entry_execution_policy_id } })} options={draft.oms.execution_policies.map((row) => ({ description: row.description, label: readableLabel(row.name), value: row.policy_id }))} value={omsProfile.settings.entry_execution_policy_id} /><SelectField help="Default policy used for risk-reducing and final exits." label="Exit execution policy" onChange={(exit_execution_policy_id) => replaceOmsProfile({ ...omsProfile, settings: { ...omsProfile.settings, exit_execution_policy_id } })} options={draft.oms.execution_policies.map((row) => ({ description: row.description, label: readableLabel(row.name), value: row.policy_id }))} value={omsProfile.settings.exit_execution_policy_id} /><SelectField help="Broker-held protection design attached to new fills unless a Strategy requests another allowed profile." label="Protection profile" onChange={(protection_profile_id) => replaceOmsProfile({ ...omsProfile, settings: { ...omsProfile.settings, protection_profile_id } })} options={draft.oms.protection_profiles.map((row) => ({ description: row.description, label: row.name, value: row.profile_id }))} value={omsProfile.settings.protection_profile_id} /></div></GuidedQuestion>,
    ...
    <GuidedQuestion description="This is the actual policy behind labels such as Patient, Balanced, or Adaptive. It identifies the quote authority and what happens when only part of the approved quantity fills." key="execution-behavior" label="Configure policy behavior and partial fills" status={readableLabel(executionPolicy.name)}><div className="guided-form-grid"><SelectField help="The bounded execution behavior implemented by OMS for this policy identity." label="Policy behavior" onChange={(name) => replaceExecutionPolicy({ ...executionPolicy, name })} options={["passive", "midpoint", "adaptive_patient", "adaptive_regular", "adaptive_urgent", "adaptive_very_urgent", "immediate_with_limit", "ibkr_native_adaptive", "cancel_if_not_filled"].map((value) => ({ label: readableLabel(value), value }))} value={executionPolicy.name} /><SelectField help="Authoritative quote feed used for pricing and repricing. A stale or unavailable feed fails closed." label="Quote source" onChange={(quote_source) => replaceExecutionPolicy({ ...executionPolicy, quote_source: quote_source as ExecutionPolicyConfig["quote_source"] })} options={["qmd", "ibkr", "simulated"].map((value) => ({ label: readableLabel(value), value }))} value={executionPolicy.quote_source} /><SelectField help="Action applied only to the broker-confirmed unfilled remainder after fills are reconciled." label="Partial-fill policy" onChange={(partial_fill_policy) => replaceExecutionPolicy({ ...executionPolicy, partial_fill_policy: partial_fill_policy as ExecutionPolicyConfig["partial_fill_policy"] })} options={[{ description: "Continue working only the confirmed remainder.", label: "Complete remainder", value: "complete_remainder" }, { description: "Keep the fill and stop pursuing the remainder.", label: "Accept partial", value: "accept_partial" }, { description: "Cancel the remainder while retaining confirmed fills.", label: "Cancel remainder", value: "cancel_remainder" }]} value={executionPolicy.partial_fill_policy} /></div></GuidedQuestion>,
  ...
  if (step === "protection") questions.push(
    <GuidedQuestion description="Protection stays broker-visible and independently reconciled while a position is open. The group below defines how adds, profit reductions, and protection failures are handled." key="protection-profile" label="Choose and review the protection group" status="Configured"><div className="guided-form-grid"><SelectField help="Reusable broker-held stop, target, and trailing design attached to approved fills." label="Protection profile" onChange={(protection_profile_id) => replaceOmsProfile({ ...omsProfile, settings: { ...omsProfile.settings, protection_profile_id } })} options={draft.oms.protection_profiles.map((row) => ({ description: row.description, label: row.name, value: row.profile_id }))} value={omsProfile.settings.protection_profile_id} /><div className="configuration-fixed-value"><span>Configured slices</span><strong>{protectionProfile.slices.length}</strong><small>Every filled unit must be covered by exactly one slice.</small></div></div></GuidedQuestion>,
  ...
  if (step === "accounts") questions.push(
    <GuidedQuestion description="Choose the stable account binding to configure. Run Plans and Portfolio mandates reference this key while runtime resolves either deterministic simulated state or an externally discovered IBKR account." key="account-selection" label="Which account binding are you configuring?" status={account.enabled ? "Enabled" : "Disabled"}><div className="guided-form-grid"><SelectField help="Changing this selection changes which account the following questions edit; it does not reassign any Portfolio mandate." label="Account binding" onChange={selectAccount} options={draft.accounts.bindings.map((row) => ({ description: `${readableLabel(row.account_class)} · ${row.modes.map(readableLabel).join(", ")}`, label: row.name, value: row.account_key }))} value={account.account_key} /><TextField help="Operator-facing name shown in configuration, Run Plans, and runtime evidence." label="Account name" onChange={(name) => replaceAccount({ ...account, name })} value={account.name} /><BooleanField help="Disabled bindings remain saved but cannot be selected by a new runtime." label="Account enabled" onChange={(enabled) => replaceAccount({ ...account, enabled })} value={account.enabled} /><div className="configuration-fixed-value"><span>Stable account key</span><strong>{account.account_key}</strong><small>Run Plans, mandates, and runtime state retain this identity.</small></div></div></GuidedQuestion>,
...
function guidedQuestionRailLabel(key: string, fallback: string) {
  const labels: Record<string, string> = {
    "portfolio-account": "Account & policy", "portfolio-mandate-limits": "Mandate limits", "portfolio-assignment": "Assignment", "portfolio-replacement": "Replacement", "portfolio-authority": "Authority", "portfolio-capital-policy": "Capital", "portfolio-exposure-policy": "Exposure", "portfolio-risk-policy": "Risk & loss", "portfolio-permissions": "Permissions", "portfolio-allowlists": "Allowlists", "portfolio-operational": "Operations",
    "execution-profile": "Defaults", "execution-urgency": "Urgency", "execution-behavior": "Behavior", "execution-price-envelope": "Price bounds", "execution-timing": "Timing",
...
function MarketDiscoveryStudio({ onChange, section }: { onChange: (value: MarketDiscoverySection) => void; section: MarketDiscoverySection }) {
  ...
  useEffect(() => {
    let active = true;
    const refresh = () => api<WatchlistRuntimeSnapshot>("/api/market-discovery/watchlists/runtime")
      .then((payload) => {
        if (!active) return;
        setWatchlistRuntime(payload);
        setWatchlistRuntimeError("");
      })
      .catch((reason) => {
        if (!active) return;
        setWatchlistRuntimeError(reason instanceof Error ? reason.message : String(reason));
      });
    void refresh();
  ...
  function addWatchlist() {
    ...
    const next: WatchlistConfig = {
      ...
      source_scan_id: section.core_scan.scan_id, inclusion_rule_sets: [], inclusion_operator: "all", exclusion_rule_sets: [], ranking_field: "liquidity-rank", ranking_direction: "descending",
      maximum_size: 100, refresh_interval_ms: 1000, membership_expiry: "end_of_trading_day", membership_ttl_ms: 300000, manual_inclusions: [], manual_exclusions: [],
      columns: section.column_catalog.filter((row) => row.default_visible).map((row) => row.column_id),
      calculations: section.core_scan.calculations.filter((row) => row.tier === "watchlist" && row.enabled).map((row) => row.capability_id), membership_history: [], origin: "user", template: false,
    };
  ...
  const computationRequirementSummary = watchlistRuntime?.computation_requirements
    ? ` Unified planner view: ${watchlistRuntime.computation_requirements.live_requirement_count} live and ${watchlistRuntime.computation_requirements.offline_requirement_count} offline requirements${watchlistRuntime.computation_requirements.complete ? "" : " (partial authority coverage)"}.`
    : "";
  const watchlistRuntimeMessage = watchlistRuntimeError
    ? `Runtime status unavailable: ${watchlistRuntimeError}`
    : watchlistRuntime?.status === "ready"
      ? `Causal membership resolver ready at ${new Date(watchlistRuntime.as_of).toLocaleTimeString()}: ${watchlistRuntime.member_count} members across ${watchlistRuntime.watchlist_count} Watchlists.${watchlistRuntime.computation_demand ? ` QMD focused demand: ${watchlistRuntime.computation_demand.active_symbol_count} symbols, ${watchlistRuntime.computation_demand.active_target_count} leases, ${watchlistRuntime.computation_demand.active_requirement_count ?? "unreported"} effective requirements, ${watchlistRuntime.computation_demand.estimated_demand_units} weighted units${watchlistRuntime.computation_demand.deduplicated_demand_units_saved ? ` after removing ${watchlistRuntime.computation_demand.deduplicated_demand_units_saved} duplicate units` : ""}.` : ""}${computationRequirementSummary}`
      : `The resolver is waiting for its first Core Scan snapshot. Live and Paper remain fail closed; configured Watchlists and manual overrides are retained.${computationRequirementSummary}`;
  ...
  return <div className="strategy-studio-workspace market-discovery-studio">
    ...
    {mode === "catalog" ? <div className="configuration-workbench strategy-editor-catalog discovery-capability-workbench">
      <aside className="strategy-parameter-catalog">
        ...
        <label className="strategy-parameter-search"><Search aria-hidden="true" size={15} /><input aria-label="Search QMD capabilities" onChange={(event) => setCapabilityQuery(event.target.value)} placeholder="Search names, fields, inputs" type="search" value={capabilityQuery} /></label>
      ...
      <main className="strategy-parameter-detail-page discovery-capability-detail"><header><span>Registry-driven integration contract</span><h2>Enrichment provenance and causal availability</h2><p>These records identify where each value comes from, when it becomes legal to use, how freshness and coverage are evaluated, and why a value may be null.</p></header><div className="discovery-capability-matrix abstraction-card-grid">{visibleEnrichmentFields.slice(0, 100).map((field, index) => <AbstractionCard description={`Available at ${field.available_at}. Nulls: ${field.null_reasons.map(readableLabel).join(", ")}.`} identity={field.field_id} key={field.field_id} kind="field" metadata={[{ label: "Owner / source", value: `${field.owner} · ${field.source_path}` }, { label: "Point-in-time plan", value: field.query_plan_id }, { label: "Freshness", value: field.freshness_policy }, { label: "Coverage", value: field.coverage_query_plan }, { label: "Historical support", value: readableLabel(field.historical_support) }]} ordinal={index + 1} registryId={field.field_id} status={`${readableLabel(field.group)} · ${readableLabel(field.status)}`} title={field.label} unavailable={field.status === "integration_pending"} />)}</div>{visibleEnrichmentFields.length > 100 ? <p className="configuration-safety-note"><CircleHelp size={15} />Showing the first 100 matching fields. Narrow the registry search to inspect another field family.</p> : null}</main>
      ...
      <section className="strategy-authoring-stage discovery-guided-stage">
        {guidedStep === "universal" ? <>
          ...
          <div className="discovery-capability-matrix abstraction-card-grid">
            {universalCapabilities.map((capability, index) => <AbstractionCard description={capability.description} identity={capability.capability_id} key={capability.capability_id} kind="processing_step" metadata={[{ label: "Inputs", value: capability.inputs.join(", ") }, { label: "Outputs", value: capability.fields.join(", ") }, { label: "Authority", value: `${capability.owner} · v${capability.implementation_version}` }, { label: "Cadence", value: readableLabel(capability.cadence) }, { label: "Persistence", value: readableLabel(capability.persistence_policy) }, { label: "Consumers", value: capability.consumers.map(capabilityScopeLabel).join(", ") || "System-owned downstreams" }]} ordinal={index + 1} registryId={capability.capability_id} selected status={`Required · ${readableLabel(capability.operational_status)}`} title={capability.name} />)}
          </div>
        </> : null}
        {guidedStep === "core" ? <>
          ...
          <div className="discovery-capability-matrix abstraction-card-grid">
            {filteredDiscoveryCapabilities.map((capability, index) => {
              ...
              return <AbstractionCard actions={<>{removable && runnable ? <button aria-label={`Modify ${capability.name}`} className="button compact secondary" onClick={() => setEditingCapabilityId(capability.capability_id)} type="button"><PencilLine size={13} /> Modify</button> : null}{removable && capability.enabled ? <button aria-label={`Remove ${capability.name}`} className="button compact danger" onClick={() => setDiscoveryCapabilityEnabled(capability.capability_id, false)} type="button"><Trash2 size={13} /> Remove</button> : null}</>} description={capability.calculation || capability.description} identity={capability.capability_id} key={capability.capability_id} kind={abstractionKindForCapability(capability.capability_type)} metadata={[{ label: "Inputs", value: capability.inputs.join(", ") || "Service owned" }, { label: "Outputs", value: `${capability.fields.slice(0, 8).join(", ") || capability.output_type}${capability.fields.length > 8 ? ` +${capability.fields.length - 8} more` : ""}` }, { label: "Scanner columns", value: capability.scanner_columns.map((column) => column.name).join(", ") || "Not published" }, { label: "Authority", value: `${capability.owner} · v${capability.implementation_version}` }, { label: capability.enabled ? "Active cadence" : "Supported cadence", value: (capability.enabled ? capability.selected_timeframes : capability.timeframes).join(", ") || readableLabel(capability.cadence) }, { label: "Consumers", value: capability.consumers.map(capabilityScopeLabel).join(", ") || "No registered consumer scope" }]} ordinal={index + 1} registryId={capability.capability_id} selected={capability.enabled} status={`${configurationStatus} · ${capability.priority.toUpperCase()}`} title={capability.name} unavailable={!runnable} />;
            })}
        ...
        {guidedStep === "watchlists" ? watchlistView === "select" ? <div className="discovery-watchlist-selection">
          ...
          <section className="discovery-watchlist-available" aria-label="Available Watchlists">
            ...
            <div className="abstraction-card-grid">{section.watchlists.map((watchlist) => <AbstractionCard actions={<button className="button compact" onClick={() => configureWatchlist(watchlist.watchlist_id)} type="button">Configure <ArrowRight size={13} /></button>} description={<>{watchlist.description || "No description yet."}{watchlist.availability_detail ? <small className="discovery-watchlist-availability">{watchlist.availability_detail}</small> : null}</>} identity={watchlist.watchlist_id} key={watchlist.watchlist_id} kind="watchlist" metadata={[{ label: "Maximum members", value: watchlist.maximum_size }, { label: "Columns", value: watchlist.columns?.length ?? 0 }, { label: "Rule sets", value: watchlist.inclusion_rule_sets.length }]} selected={watchlist.enabled} status={watchlist.template ? "System template" : watchlist.availability === "integration_pending" ? "Integration pending" : watchlist.enabled ? "Enabled" : "Disabled"} title={watchlist.name} unavailable={watchlist.availability === "integration_pending"}><WatchlistRuleSummary capabilities={discoveryCapabilities} compact ruleSets={section.rule_sets} watchlist={watchlist} /></AbstractionCard>)}{section.watchlists.length === 0 ? <EmptyState title="No Watchlists configured" detail="Create a Watchlist to define how QMD narrows Core Scan candidates for strategies." /> : null}</div>
          </section>
          <p className="configuration-safety-note discovery-runtime-notice" data-ready={watchlistRuntime?.status === "ready" ? "true" : "false"}>{watchlistRuntime?.status === "ready" ? <BadgeCheck size={15} /> : <TriangleAlert size={15} />}{watchlistRuntimeMessage}</p>
          ...
          <div className="discovery-watchlist-step-navigation">
            ...
            <nav aria-label="Watchlist configuration steps">{([['identity', '1', 'Identity'], ['rules', '2', 'Rules'], ['ranking', '3', 'Ranking'], ['columns', '4', 'Columns'], ['timing', '5', 'Timing'], ['overrides', '6', 'Overrides'], ['calculations', '7', 'Calculations'], ['review', '8', 'Review']] as const).map(([id, number, label], index) => <button aria-current={watchlistQuestion === id ? "step" : undefined} key={id} onClick={() => setWatchlistQuestionIndex(index)} type="button"><span>{number}</span><strong>{label}</strong></button>)}</nav>
          ...
          <section className="discovery-watchlist-question">
            ...
            {watchlistQuestion === "rules" ? <><header><h2>Which rule sets should candidates pass?</h2></header><WatchlistRuleChoices fields={section.field_catalog} onChange={replaceWatchlist} ruleSets={section.rule_sets} watchlist={selectedWatchlist} /></> : null}
            ...
            {watchlistQuestion === "timing" ? <><header><h2>How often should membership be refreshed and expire?</h2></header><div className="configuration-field-grid"><NumberField help="How often QMD resolves the Watchlist from its source scan." label="Refresh interval" minimum={1} onChange={(refresh_interval_ms) => replaceWatchlist({ ...selectedWatchlist, refresh_interval_ms })} step={100} unit="ms" value={selectedWatchlist.refresh_interval_ms} /><fieldset className="discovery-expiry-policy"><legend>Membership expiry</legend><p>A symbol leaves at the end of its New York trading day by default. Choose an override only when this Watchlist requires different persistence.</p><div>{[["end_of_trading_day", "End of trading day", "Remove the symbol at the applicable New York session boundary."], ["time_to_live", "Time to live", "Remove membership when it is not reconfirmed within a fixed duration."], ["never", "No automatic expiry", "Keep membership until a rule or manual action removes it."]].map(([value, label, detail], index) => { const checked = selectedWatchlist.membership_expiry === value; return <label data-selected={checked} key={value}><span className="discovery-capability-index">{index + 1}</span><span className="discovery-expiry-card-copy"><strong>{label}</strong><small>{detail}</small></span><input checked={checked} name={`expiry-${selectedWatchlist.watchlist_id}`} onChange={() => replaceWatchlist({ ...selectedWatchlist, membership_expiry: value as WatchlistConfig["membership_expiry"] })} type="radio" /></label>; })}</div></fieldset>{selectedWatchlist.membership_expiry === "time_to_live" ? <NumberField help="Positive duration without a confirming refresh before membership expires." label="Membership TTL" minimum={1} onChange={(membership_ttl_ms) => replaceWatchlist({ ...selectedWatchlist, membership_ttl_ms })} step={1000} unit="ms" value={selectedWatchlist.membership_ttl_ms} /> : null}</div></> : null}
            ...
            {watchlistQuestion === "calculations" ? <><header><h2>Which focused derivations should run for members?</h2></header><fieldset className="discovery-calculation-choices"><legend>Focused derivations</legend><p>QMD leases these higher-cost derivations only for seeded candidates and current Watchlist members, then releases unused state.</p><div className="abstraction-card-grid">{section.core_scan.calculations.filter((row) => row.tier === "watchlist" && row.enabled).map((capability, index) => { const checked = selectedWatchlist.calculations.includes(capability.capability_id); return <AbstractionCard compact control={<input checked={checked} onChange={(event) => replaceWatchlist({ ...selectedWatchlist, calculations: event.target.checked ? [...selectedWatchlist.calculations, capability.capability_id] : selectedWatchlist.calculations.filter((id) => id !== capability.capability_id) })} type="checkbox" />} description={capability.description} identity={capability.capability_id} key={capability.capability_id} kind={abstractionKindForCapability(capability.capability_type)} metadata={[{ label: "Authority", value: capability.owner }, { label: "Cadence", value: readableLabel(capability.cadence) }]} ordinal={index + 1} registryId={capability.capability_id} selected={checked} status={checked ? "Selected" : "Available"} title={capability.name} />; })}</div></fieldset></> : null}
            {watchlistQuestion === "review" ? <><header><h2>Review this Watchlist</h2></header><div className="discovery-watchlist-review"><article><span>Source</span><strong>{section.core_scan.name}</strong><small>Core Scan candidates</small></article><article><span>Membership</span><strong>Top {selectedWatchlist.maximum_size} · {selectedWatchlist.ranking_direction === "descending" ? "highest" : "lowest"} first</strong><small>Refreshes every {selectedWatchlist.refresh_interval_ms} ms · {selectedWatchlist.membership_expiry === "end_of_trading_day" ? "expires at trading-day end" : selectedWatchlist.membership_expiry === "time_to_live" ? `expires after ${selectedWatchlist.membership_ttl_ms} ms` : "no automatic expiry"}</small></article><article><span>Rules</span><strong>{selectedWatchlist.inclusion_rule_sets.length} active rule set{selectedWatchlist.inclusion_rule_sets.length === 1 ? "" : "s"}</strong><small>{selectedWatchlist.manual_inclusions.length} manual inclusions · {selectedWatchlist.manual_exclusions.length} manual exclusions</small></article><article><span>Visible data</span><strong>{selectedWatchlist.columns.length} columns</strong><small>{selectedWatchlist.calculations.length} focused calculations · {selectedWatchlist.enabled ? "enabled" : "disabled"}</small></article></div></> : null}
          </section>
    ...
    <div className="modal-backdrop discovery-capability-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}>
      <section aria-describedby={descriptionId} aria-labelledby={titleId} aria-modal="true" className="modal-panel discovery-capability-dialog discovery-core-approval-dialog" role="dialog">
        ...
        <div className="discovery-capability-dialog-body">
          <AbstractionCard compact description={capability.calculation || capability.description} identity={capability.capability_id} kind={abstractionKindForCapability(capability.capability_type)} metadata={[{ label: "Cost", value: readableLabel(capability.cost_class) }, { label: "Priority", value: capability.priority.toUpperCase() }]} status="Pending approval" title={capability.name} />
  ...
  return createPortal(
    <div className="modal-backdrop discovery-capability-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}>
      <section aria-describedby={descriptionId} aria-labelledby={titleId} aria-modal="true" className="modal-panel discovery-capability-dialog" role="dialog">
        ...
        <div className="discovery-capability-dialog-body">
          <AbstractionCard compact description={capability.calculation || capability.description} identity={capability.capability_id} kind={abstractionKindForCapability(capability.capability_type)} metadata={[{ label: "Inputs", value: capability.inputs.join(", ") || "Service owned" }, { label: "Outputs", value: capability.fields.join(", ") || capability.output_type }, { label: "Priority", value: capability.priority.toUpperCase() }]} selected={draft.enabled} status={draft.enabled ? "Active" : "Inactive"} title={capability.name} />
...
function WatchlistRuleSummary({ capabilities, compact = false, ruleSets, watchlist }: { capabilities: DiscoveryCapability[]; compact?: boolean; ruleSets: RuleSetDefinition[]; watchlist: WatchlistConfig }) {
  ...
  const ruleById = new Map(ruleSets.map((ruleSet) => [ruleSet.rule_set_id, ruleSet]));
  const included = (watchlist.inclusion_rule_sets ?? []).map((ruleSetId) => ruleById.get(ruleSetId)).filter((ruleSet): ruleSet is RuleSetDefinition => Boolean(ruleSet));
  const ranking = capabilities.find((capability) => capability.capability_id === watchlist.ranking_field);
  const inclusionLogic = watchlist.inclusion_operator === "any" ? "A candidate may pass any selected rule" : "A candidate must pass every selected rule";
  return <span className={`discovery-watchlist-rule-summary${compact ? " compact" : ""}`}>
    <span className="discovery-watchlist-rule-summary-heading"><strong>Selection logic</strong><small>{inclusionLogic}</small></span>
    <span className="discovery-watchlist-rule-summary-list">
      {included.map((ruleSet, index) => <span data-purpose="active" key={ruleSet.rule_set_id}><em>{index ? (watchlist.inclusion_operator === "any" ? "OR" : "AND") : "RULE"}</em><span><strong>{ruleSet.name}</strong>{!compact && ruleSet.description ? <small>{ruleSet.description}</small> : null}<small>{ruleSetMeaning(ruleSet, catalog)}</small></span></span>)}
      {!included.length ? <span data-purpose="none"><em>NO RULES</em><span><small>All Core Scan candidates remain eligible until rules are added.</small></span></span> : null}
    </span>
    <span className="discovery-watchlist-rule-outcome"><strong>Then rank</strong><small>{ranking?.name ?? readableLabel(watchlist.ranking_field)}, {watchlist.ranking_direction === "ascending" ? "lowest first" : "highest first"}; keep the top {watchlist.maximum_size}.</small></span>
  </span>;
}

function WatchlistColumnChoices({ columns, onChange, selected }: { columns: WatchlistColumn[]; onChange: (value: string[]) => void; selected: string[] }) {
  ...
  return <section className="discovery-column-catalog">
    <header><div><span>Watchlist columns</span><strong>{selected.length} selected</strong><small>DB-, SEC-, QMD-, and Text Intelligence-managed facts retain their source authority.</small></div><label><Search size={15} /><input aria-label="Search Watchlist columns" onChange={(event) => setQuery(event.target.value)} placeholder="Search columns" type="search" value={query} /></label></header>
    ...
    <div className="discovery-column-card-list abstraction-card-grid">{visible.map((column, index) => {
      ...
      return <AbstractionCard compact control={<input checked={checked} disabled={column.column_id === "symbol" || !available} onChange={() => toggle(column.column_id)} type="checkbox" />} description={column.description} identity={column.column_id} key={column.column_id} kind="column" metadata={[{ label: "Value type", value: readableLabel(column.value_type) }, { label: "Authority", value: column.source }, { label: "Query plan", value: column.query_plan_id }]} ordinal={index + 1} registryId={`column.${column.column_id}`} selected={checked} status={readableLabel(column.implementation_status)} title={column.name} unavailable={!available} />;
    })}{!visible.length ? <EmptyState title="No matching columns" detail="Change the search or source filter." /> : null}</div>
  </section>;
}

function WatchlistRuleChoices({ fields, onChange, ruleSets, watchlist }: { fields: DiscoveryField[]; onChange: (value: WatchlistConfig) => void; ruleSets: RuleSetDefinition[]; watchlist: WatchlistConfig }) {
  const [query, setQuery] = useState("");
  ...
  const watchlistRules = ruleSets.filter((ruleSet) => ruleSet.scope === "watchlist" || watchlist.inclusion_rule_sets.includes(ruleSet.rule_set_id));
  ...
  const selectedRules = watchlistRules.filter((ruleSet) => watchlist.inclusion_rule_sets.includes(ruleSet.rule_set_id));
  const availableRules = watchlistRules.filter((ruleSet) => !watchlist.inclusion_rule_sets.includes(ruleSet.rule_set_id));
  const visibleRules = selectedRules.filter((ruleSet) => {
    const search = query.trim().toLocaleLowerCase();
    return !search || [ruleSet.name, ruleSet.description, ruleSetMeaning(ruleSet, catalog)].some((value) => value.toLocaleLowerCase().includes(search));
  });
  function addRule() {
    if (!ruleToAddId || !availableRules.some((ruleSet) => ruleSet.rule_set_id === ruleToAddId)) return;
    onChange({
      ...watchlist,
      inclusion_rule_sets: [...watchlist.inclusion_rule_sets, ruleToAddId],
      exclusion_rule_sets: [],
    });
    setRuleToAddId("");
  }
  function removeRule(ruleSetId: string) {
    onChange({ ...watchlist, inclusion_rule_sets: watchlist.inclusion_rule_sets.filter((id) => id !== ruleSetId), exclusion_rule_sets: [] });
  }
  return <div className="discovery-rule-catalog">
    <header><div><span>Rule sets</span><strong>{selectedRules.length} active · {availableRules.length} available</strong><small>Add a QMD rule set to this Watchlist. Remove its card to stop applying it.</small></div><label><Search size={15} /><input aria-label="Search active Watchlist rules" onChange={(event) => setQuery(event.target.value)} placeholder="Search active rules" type="search" value={query} /></label><div className="discovery-rule-add"><InventoryFilterSelect ariaLabel="Rule set to add" className="configuration-lookup-button" onChange={setRuleToAddId} options={[{ description: availableRules.length ? "Choose a QMD registry rule set to add to this Watchlist." : "Every available QMD rule set is already active.", label: availableRules.length ? "Choose a rule set" : "No rule sets available", value: "" }, ...availableRules.map((ruleSet) => ({ description: ruleSet.description || ruleSetMeaning(ruleSet, catalog), label: ruleSet.name, value: ruleSet.rule_set_id }))]} searchable={availableRules.length > 7} searchPlaceholder="Find a QMD rule set…" showAllOnOpen value={ruleToAddId} /><button className="button compact" disabled={!ruleToAddId} onClick={addRule} type="button"><Plus size={14} /> Add rule set</button></div></header>
    <div className="discovery-rule-card-list abstraction-card-grid">{visibleRules.map((ruleSet, index) => {
      const types = ruleSourceTypes(ruleSet);
      return <AbstractionCard actions={<button aria-label={`Remove ${ruleSet.name}`} className="button compact danger discovery-rule-remove" onClick={() => removeRule(ruleSet.rule_set_id)} type="button"><Trash2 size={13} /> Remove</button>} description={ruleSet.description} identity={ruleSet.rule_set_id} key={ruleSet.rule_set_id} kind="rule_set" metadata={[{ label: "Sources", value: types.length ? types.map((type) => type === "system" ? "System" : capabilityTypeLabel(type as DiscoveryCapability["capability_type"])).join(" · ") : "Registered fields" }, { label: "Conditions", value: ruleSet.conditions.length }]} ordinal={index + 1} selected status="Active" title={ruleSet.name}><RuleSetMeaning catalog={catalog} ruleSet={ruleSet} /></AbstractionCard>;
    })}{!visibleRules.length ? <EmptyState title={selectedRules.length ? "No matching active rules" : "No rule sets added"} detail={selectedRules.length ? "Clear the search to show the active rule sets." : "Choose a QMD registry rule set above and add it to this Watchlist."} /> : null}</div>
  </div>;
}
...
function DecisionRulesEditor({ catalog = [], onChange, onRuleSetEdit = () => undefined, ruleSetCatalog = [], rules, stageName, summary, title }: {
  catalog?: StrategyInput[];
...
function formatRuleThreshold(value: unknown, source: StrategyInput | undefined | null): string {
...
function ConfigurationSectionStudio({ draft, guided, onChange, onDraftChange, section }: {
  ...
  return <div className="strategy-studio-workspace configuration-section-studio">
    <nav className="strategy-editor-toolbar">
      <span><strong>{copy.title}</strong><small>{view === "guided" ? copy.guided : copy.managed}</small></span>
      <div className="configuration-section-toolbar-actions">
        {hasSeparateStructureView ? <button className="button compact configuration-structure-button" onClick={() => setView(view === "structure" ? "guided" : "structure")} type="button"><Settings2 size={14} /> {view === "structure" ? "Back to guided setup" : "Advanced structure"}</button> : null}
      </div>
    </nav>
