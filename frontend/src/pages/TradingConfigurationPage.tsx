...
function GuidedConfiguration({ approved, draft, label, omsStage, onChange, onContinue, onLabelChange, onOmsStageChange, onPublish, onSwitchToExpert, publishing, revisions, section }: {
  ...
  function addGuidedAccount() {
    ...
    const next: AccountBinding = { account_key, name: "New account", source_account_id: "replay", account_class: "simulated", base_currency: "USD", session_key: "replay", portfolio_policy_id: String(draft.portfolio.policies[0]?.policy_id ?? "default"), enabled: true, modes: ["replay"] };
    onChange("accounts", { bindings: [...draft.accounts.bindings, next] });
    selectAccount(account_key);
  }

  function removeGuidedAccount() {
    if (draft.accounts.bindings.length <= 1 || draft.portfolio.mandates.some((row) => row.account_key === account.account_key)) return;
