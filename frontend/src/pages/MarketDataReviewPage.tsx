...
const SCANNER_FILTER_PRESETS = [SCANNER_MOMENTUM_FILTER_PRESET, SCANNER_SETUP_FILTER_PRESET, SCANNER_EARLY_MOVE_FILTER_PRESET, SCANNER_SPREAD_FILTER_PRESET];
...
function ScannerTab({ catalog, scope, records }: { catalog: CatalogPayload | null; scope: Scope; records: RecordRow[] }) {
  ...
  const defaultFeatures = useMemo(() => featureOptions.filter((group) => ["core", "session", "momentum", "volume_liquidity", "price_action"].includes(group)), [featureOptions]);
  ...
  return (
    <section className="panel table-fill-panel" ref={fillPanel.ref} style={fillPanel.style}>
      ...
      <DataTable
        ...
        filterPresets={SCANNER_FILTER_PRESETS}
        onRowClick={
  ...
  function createPreset() {
    const preset: ScannerFormulaPreset = {
      columns: [
        {
          enabled: true,
          expression: "last_close - last_open",
              ...
              activePreset.columns.map((column, index) => (
                <div className="scanner-formula-row" key={column.id}>
                  ...
                  <div className="field scanner-formula-expression">
                    ...
                    <input placeholder="zscore(last_macd_line - last_macd_signal)" value={column.expression} onChange={(event) => updateFormula(column.id, { expression: event.target.value })} />
                  </div>
