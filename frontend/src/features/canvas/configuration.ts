...
export const INDICATOR_GUIDES: Record<string, ChartCatalogKnowledge> = {
  ...
  "indicator.vwap": indicatorGuide("Compare price with the extended session's exact eligible-trade volume-weighted average. VWAP is the purple price overlay, starts at 04:00 ET, and continues through 09:30 without resetting.", "From the 04:00 ET anchor, cumulatively divide eligible trade-price notional Σ(price × size) by eligible share volume Σ(size). QMD preserves those additive primitives when it aggregates bars.", "Price holding above a rising VWAP suggests buyers are accepting progressively higher prices; a reclaim that persists is stronger than a brief cross.", "Price holding below a falling VWAP suggests sellers control the session auction; repeated rejection at VWAP reinforces that evidence.", "At the same causal cutoff, session VWAP is identical on every intraday timeframe because it is recombined from exact eligible-trade notional and volume rather than recalculated from timeframe-specific OHLC values.", ["VWAP is a benchmark, not automatic support or resistance.", "Opening and closing auctions or a few very large prints can materially shift it.", "An external-platform comparison must use the same 04:00 ET anchor, extended-hours scope, eligible-trade rules, and market-data feed."]),
  "indicator.execution_vwap": indicatorGuide("Compare executable momentum price with the causal session VWAP of eligible prints that occurred inside the prevailing NBBO. It is the blue price overlay and uses a maximum one-second quote age.", "From the 04:00 ET anchor, accumulate price × size and size only for volume-eligible trades at or between the latest valid bid and ask, provided that quote is no more than one second old.", "Price above execution VWAP shows that the actionable tape is accepting higher prices without letting off-market prints delay a breakout decision.", "Price below execution VWAP shows deterioration in the actionable tape and can participate in the below-entry protection ladder.", "The additive inside-NBBO volume and notional are preserved in every bar, so the result is invariant across intraday chart timeframes at the same causal cutoff.", ["Execution VWAP complements rather than replaces consolidated VWAP.", "Trades without a valid recent NBBO are excluded instead of guessed.", "It is a trading-decision benchmark, not a guarantee that a market order fills at the displayed value."]),
  ...
  "indicator.price_vwap": indicatorGuide("Read the percentage distance between price and the 04:00 ET anchored session VWAP to measure where the current auction sits versus extended-session volume-weighted consensus.", "100 × (close − session VWAP) / session VWAP.", "A sustained positive distance with rising VWAP indicates acceptance above session value.", "A sustained negative distance with falling VWAP indicates acceptance below session value.", "Close follows the selected chart timeframe, while the exact eligible-trade session VWAP remains invariant across intraday timeframes at the same causal cutoff.", ["A large distance can be momentum or temporary extension.", "Premarket prints can retain substantial influence after the regular open, especially in actively traded movers."]),
...
export const CHART_INDICATORS: ChartDisplayItem[] = [
  ...
  displayIndicator("indicator.execution_vwap", "Execution VWAP", "volume_liquidity", ["execution_vwap"]),
...
export const INDICATOR_SERIES = [
  ...
  { column: "execution_vwap", color: "var(--info)", displayItemId: "indicator.execution_vwap", label: "Exec VWAP", pane: "price" },
