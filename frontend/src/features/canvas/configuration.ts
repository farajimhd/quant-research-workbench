...
export const INDICATOR_GUIDES: Record<string, ChartCatalogKnowledge> = {
  ...
  "indicator.price_vwap": indicatorGuide("Read the percentage distance between price and the sole 04:00 ET anchored VWAP authority.", "100 × (close − VWAP) / VWAP, where VWAP includes only eligible prints inside a fresh prevailing NBBO.", "A sustained positive distance indicates acceptance above actionable session value.", "A sustained negative distance indicates deterioration below actionable session value.", "The same execution-eligible VWAP authority is used across chart and strategy surfaces.", ["A large distance can be momentum or temporary extension.", "Trades without a valid recent NBBO are excluded rather than guessed."]),
...
export const CHART_INDICATORS: ChartDisplayItem[] = [
  ...
  displayIndicator("indicator.ema_acceleration", "EMA second derivative", "momentum", [], "ema-acceleration", indicatorGuide(
    "Measure acceleration or deceleration of a configurable EMA of candle closes.",
    "EMA uses alpha = 2/(length+1), initialized with the first length-candle SMA. Curvature = EMA[t] - 2*EMA[t-1] + EMA[t-2]. Time-based units use adjacent candle-close intervals; bps units divide by the current EMA and multiply by 10,000.",
    "Positive curvature means the EMA slope is increasing; a falling EMA can still have positive curvature.",
    "Negative curvature means the EMA slope is decreasing; a rising EMA can still have negative curvature.",
    "Length is in selected-timeframe candles. Only completed candles at or before the chart cursor enter the calculation. The first output needs length + 2 candles.",
    ["Acceleration alone is not a reversal signal.", "Second differences amplify noise.", "This chart indicator uses loaded candle history. Loading earlier candles adds warmup and can change earlier EMA estimates."],
  )),
...
export const INDICATOR_SERIES = [
  ...
  { column: "price_vs_execution_vwap_pct", color: "var(--info)", displayItemId: "indicator.price_vwap", label: "Price vs VWAP", pane: "distance" },
