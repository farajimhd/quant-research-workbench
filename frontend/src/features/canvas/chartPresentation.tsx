...
export function ChartPreview({
  ...
  usePollingTask({
    ...
    task: async (signal) => {
      const parameters = new URLSearchParams({ as_of: changeAsOf,
  ...
  const detectorDecisions = useMemo(() => strategyPresentationAvailable
    ? detectorRows(chartTrading?.strategy_chart_activity ?? [], linkContext.symbol, changeAsOf) : [],
  ...
  const payload = useMemo<ChartPayload>(() => {
    ...
    return {
      ...
      strategy_references: strategyPresentationAvailable ? strategyReferencePresentation(
        chartTrading?.strategy_chart_activity ?? [], linkContext.symbol, changeAsOf) : [],
...
export function historicalMarketLevelZones(
  ...
  if (visibleIndicators.includes("indicator.qmd_generic_structure")) {
    // Local swing events remain independent. The level book is exclusively V7.
  ...
  if (visibleIndicators.includes("indicator.qmd_unified_structure")) {
    // V7 is projected by the QMD-backed chart primitive; never draw legacy book rows.
  }
...
function pushUnifiedStructureLevels(
  ...
  segments.forEach(({ end, latest, level, start }) => {
    ...
    zones.push({
      ...
      tickerRelativeQualityDistributionHash: level.ticker_relative_quality_distribution_hash,
      tickerRelativeQualityPopulationSize: Math.max(0, Number(level.ticker_relative_quality_population_size) || 0),
      tickerRelativeQualityReferenceSession: level.ticker_relative_quality_reference_session,
      tickerRelativeQualityRevision: level.ticker_relative_quality_revision,
...
function isQmdUnifiedStructureLevel(value: unknown): value is QmdUnifiedStructureLevel {
  ...
  // Legacy V5 hashes resistance only; the symmetric contract hashes both roles.
