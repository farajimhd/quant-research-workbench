...
function PositionsPreview({ data, onSymbolSelect, settings }: { data: CanonicalTradingPreview; onSymbolSelect?: (symbol: string) => void; settings: ContainerSettings["positions"] }) {
  ...
  const openColumns = settings.showPnl ? ["symbol", "side", "quantity", "average_price", "mark", "return_pct", "market_value", "unrealized_pnl", "max_unrealized_pnl", "orders", "fills", "account", "requested_at", "opened_at"] : ["symbol", "side", "quantity", "average_price", "mark", "market_value", "orders", "fills", "account", "requested_at", "opened_at"];
  return <section className="trading-preview trading-position-manager"><TradingFreshness data={data} />
    ...
    <div className="trading-summary-strip"><TradingMetric label="Open positions" value={String(openRows.length)} /><TradingMetric label="Winning" value={`${winners}/${openRows.length}`} tone={winners ? "positive" : "neutral"} /><TradingMetric label="Open P&L" value={signedMoney(netPnl)} tone={numberTone(netPnl)} /><TradingMetric label="Peak unrealized" value={money(maxUnrealizedPnl)} tone={maxUnrealizedPnl > 0 ? "positive" : "neutral"} /><TradingMetric label="Gross exposure" value={money(grossValue)} /></div>
