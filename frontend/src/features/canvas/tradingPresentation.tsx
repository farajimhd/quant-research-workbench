...
function TradingJournalPreview({ data, settings }: { data: CanonicalTradingPreview; settings: ContainerSettings["performance_journal"] }) {
  ...
  return <section className="trading-preview performance-journal">
    ...
    <div className="performance-kpi-grid">
      ...
      <JournalMetric detail="Sum of each open position's maximum favorable unrealized P&L observed during its current lifecycle." label="Peak unrealized" tone={Number(data.performance_snapshot?.max_unrealized_pnl || 0) > 0 ? "positive" : "neutral"} value={money(data.performance_snapshot?.max_unrealized_pnl)} />
