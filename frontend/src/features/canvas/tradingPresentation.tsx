...
function PortfolioPreview({ data, settings }: { data: CanonicalTradingPreview; settings: ContainerSettings["portfolio"] }) {
  ...
  return <section className="trading-preview trading-portfolio-preview">
    ...
    <div className="trading-primary-metrics">
      ...
      {settings.showPnl ? <TradingMetric label="Open unrealized" value={signedMoney(metrics.unrealized_pnl)} tone={numberTone(metrics.unrealized_pnl)} /> : null}
...
function PositionsPreview({ data, onSymbolSelect, settings }: { data: CanonicalTradingPreview; onSymbolSelect?: (symbol: string) => void; settings: ContainerSettings["positions"] }) {
  ...
  const openRows = lifecycleRows.filter(({ lifecycle }) => String(lifecycle.status) === "open").map(({ lifecycle, symbol, account, relatedOrders, relatedExecutions, relatedActivity }) => {
    ...
    const openUnrealized = position?.unrealized_pnl;
    const peakUnrealized = position?.max_unrealized_pnl ?? Math.max(0, Number(openUnrealized || 0));
    return { account, symbol, open_unrealized: openUnrealized, peak_unrealized: peakUnrealized, side: lifecycle.side, quantity, average_price: lifecycle.entry_price, mark: position?.market_price, return_pct: returnPct, market_value: position?.market_value, unrealized_pnl: openUnrealized, max_unrealized_pnl: peakUnrealized, realized_pnl: lifecycle.gross_pnl, orders: relatedOrders.length, fills: relatedExecutions.length, requested_at: lifecycle.requested_at, opened_at: lifecycle.opened_at, _position: position ?? {}, _lifecycle: lifecycle, _orders: relatedOrders, _executions: relatedExecutions, _activity: relatedActivity };
  });
  ...
  const maxUnrealizedPnl = openRows.reduce((total, row) => total + Number(row.max_unrealized_pnl || 0), 0);
  ...
  const openColumns = settings.showPnl ? ["symbol", "open_unrealized", "peak_unrealized", "side", "quantity", "average_price", "mark", "return_pct", "market_value", "orders", "fills", "account", "requested_at", "opened_at"] : ["symbol", "side", "quantity", "average_price", "mark", "market_value", "orders", "fills", "account", "requested_at", "opened_at"];
  return <section className="trading-preview trading-position-manager"><TradingFreshness data={data} />
    ...
    <div className="trading-summary-strip"><TradingMetric label="Open positions" value={String(openRows.length)} /><TradingMetric label="Winning" value={`${winners}/${openRows.length}`} tone={winners ? "positive" : "neutral"} /><TradingMetric label="Open unrealized" value={signedMoney(netPnl)} tone={numberTone(netPnl)} /><TradingMetric label="Peak unrealized" value={money(maxUnrealizedPnl)} tone={maxUnrealizedPnl > 0 ? "positive" : "neutral"} /><TradingMetric label="Gross exposure" value={money(grossValue)} /></div>
...
function performanceLifecycleRow(data: CanonicalTradingPreview, lifecycle: PreviewRow) {
  const symbol = String(nestedValue(lifecycle,"instrument","symbol") || "");
  const position = data.positions.find(row => row.account_id === lifecycle.account_id && nestedValue(row,"instrument","instrument_id") === nestedValue(lifecycle,"instrument","instrument_id"));
  const executionIds = new Set((lifecycle.execution_ids as string[] | undefined) ?? []);
  const executions = data.executions.filter(row => row.account_id === lifecycle.account_id && executionIds.has(String(row.execution_id)));
  const orderIds = new Set((lifecycle.order_ids as string[] | undefined) ?? []);
  const orders = data.orders.filter(row => row.account_id === lifecycle.account_id && orderIds.has(String(row.broker_order_id)));
...
function JournalPnlCandleChart({ candles, onTimeframeChange, timeframe }: { candles: PnlCandle[]; onTimeframeChange: (value: PnlCandleTimeframe) => void; timeframe: PnlCandleTimeframe }) {
  ...
  const plot = { bottom: 204, left: 64, right: width - 18, top: 20 };
