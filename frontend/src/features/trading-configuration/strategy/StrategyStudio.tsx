...
export function GuidedStrategyConfiguration({ draft, onChange, onContinue, onProfileChange, profile }: {
  ...
  let questions: GuidedStrategyQuestionDefinition[] = [
    ...
    ...(profile.lifecycle.phase_modes.reentry === "automatic" ? [{
      id: "reentry-guardrails", section: "Reentry", title: "What limits reentry timing and frequency?", description: "These campaign-level guardrails control evidence freshness, minimum waiting time, and whether successful reentries are capped.", guide: "A rejected or unfilled request does not consume an attempt. Fresh confirmation prevents evidence from the prior entry from being silently reused.", content: <div className="guided-form-grid"><BooleanField help="Require confirmation evidence with a timestamp newer than the previous confirmed entry." label="Require fresh confirmation" onChange={(require_new_confirmation) => replaceReentry({ ...reentry, require_new_confirmation })} value={reentry.require_new_confirmation} /><NumberField help="Set the minimum elapsed time after a confirmed full exit before another reentry may become eligible." label="Cooldown" minimum={0} onChange={(cooldown_ms) => replaceReentry({ ...reentry, cooldown_ms })} step={100} unit="ms" value={reentry.cooldown_ms} /><BooleanField help="Continue evaluating reentry for the lifetime of the campaign instead of completing after a fixed number of fills." label="Unlimited reentries" onChange={(unlimited_attempts) => replaceReentry({ ...reentry, unlimited_attempts })} value={Boolean(reentry.unlimited_attempts)} />{!reentry.unlimited_attempts ? <NumberField help="Set the maximum number of confirmed reentry fills allowed during one ticker campaign." label="Maximum attempts" minimum={0} onChange={(maximum_attempts) => replaceReentry({ ...reentry, maximum_attempts })} step={1} unit="entries" value={reentry.maximum_attempts} /> : null}</div>,
    }, {
...
export function StrategyProfileFeaturePreview({ profile }: { profile: StrategyProfile }) {
  ...
  return <div className="strategy-feature-preview">
    ...
    <dl>
      ...
      <div><dt>Position lifecycle</dt><dd><strong>{addCount} add action{addCount === 1 ? "" : "s"} · {exitCount} strategic exit{exitCount === 1 ? "" : "s"}</strong><span>{profile.lifecycle.phase_modes.reentry === "automatic" ? profile.lifecycle.reentry.unlimited_attempts ? "Reentry while campaign remains active" : `Reentry up to ${profile.lifecycle.reentry.maximum_attempts} times` : "Reentry manual"}</span></dd></div>
...
export function strategySetupRows(profile: StrategyProfile) {
  return [
    ...
    { label: "Reentry", value: profile.lifecycle.phase_modes.reentry === "automatic" ? `${profile.lifecycle.reentry.unlimited_attempts ? "Unlimited" : `${profile.lifecycle.reentry.maximum_attempts} attempts`} · ${profile.lifecycle.reentry.cooldown_ms} ms` : "Manual" },
...
export function blankStrategyProfile(source: StrategyProfile, draft: Draft, definition?: StrategyDefinition): StrategyProfile {
  ...
  return {
    ...
    lifecycle: {
      ...
      reentry: { action_id: `position.enter_${source.lifecycle.trading_behavior.side}`, capital_request: deepClone(capitalRequest), cooldown_ms: 0, enabled: true, maximum_attempts: 0, unlimited_attempts: false, order_intent: deepClone(orderIntent), require_new_confirmation: true, rules: { blockers: emptyStage(), confirmation: emptyStage(), opportunity: emptyStage() } },
...
export function ReentryAuthoringSurface({ activePage, draft, mode, onModeChange, onPageChange, onProfileChange, onReplaceReentry, onRuleSetEdit, profile, ruleSets, section }: {
  ...
  return <div className="strategy-entry-layout strategy-lifecycle-layout">
    <div className="strategy-entry-question-surface">
      ...
      {activePage === "reentry_policy" ? <div className="strategy-entry-fields"><div className="guided-form-grid"><BooleanField help="Require confirmation evidence newer than the evidence used by the previous confirmed entry." label="Require new confirmation" onChange={(require_new_confirmation) => onReplaceReentry({ ...reentry, require_new_confirmation })} value={reentry.require_new_confirmation} /><NumberField help="Minimum time after a confirmed full exit before reentry may become eligible." label="Cooldown" minimum={0} onChange={(cooldown_ms) => onReplaceReentry({ ...reentry, cooldown_ms })} step={100} unit="ms" value={reentry.cooldown_ms} /><BooleanField help="Keep reentry eligible for the lifetime of the campaign rather than completing after a fixed number of fills." label="Unlimited reentries" onChange={(unlimited_attempts) => onReplaceReentry({ ...reentry, unlimited_attempts })} value={Boolean(reentry.unlimited_attempts)} />{!reentry.unlimited_attempts ? <NumberField help="Maximum confirmed reentry fills during one ticker campaign." label="Maximum attempts" minimum={0} onChange={(maximum_attempts) => onReplaceReentry({ ...reentry, maximum_attempts })} step={1} unit="entries" value={reentry.maximum_attempts} /> : null}</div></div> : null}
  ...
  return (
    <>
      ...
      <div className="configuration-field-grid">
        ...
        <BooleanField help="Keep reentry eligible for the lifetime of the campaign rather than completing after a fixed number of fills." label="Unlimited reentries" onChange={(unlimited_attempts) => update({ ...reentry, unlimited_attempts })} value={Boolean(reentry.unlimited_attempts)} />
        {!reentry.unlimited_attempts ? <NumberField help="Maximum reentries during one ticker campaign. Zero allows only the initial entry." label="Maximum attempts" minimum={0} onChange={(maximum_attempts) => update({ ...reentry, maximum_attempts })} step={1} unit="entries" value={reentry.maximum_attempts} /> : null}
      </div>
      <DecisionRulesEditor
        catalog={catalog}
