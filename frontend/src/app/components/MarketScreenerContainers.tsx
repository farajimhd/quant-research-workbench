...
function MarketListTable({
  ...
  const matchedRows = useMemo(() => filterRowsByConditions(rows, columnFilters, filterColumns, filterMatchMode).filter((row) => {
    if (deferredQuery && !Object.values(row).some((value) => String(value ?? "").toLowerCase().includes(deferredQuery))) return false;
    return true;
  }).sort((left, right) => compareValues(left[sort.column], right[sort.column]) * (sort.direction === "asc" ? 1 : -1)), [columnFilters, deferredQuery, filterColumns, filterMatchMode, rows, sort]);
  ...
  return <div className="market-list-table-shell" ref={tableShellRef}>
    <div className="market-list-toolbar-stack"><div className="market-list-toolbar">
      ...
      <div className="market-list-pagination" aria-label={`${title} pages`}>
        <span>{matchedRows.length === rows.length ? `${matchedRows.length} rows` : `${matchedRows.length} of ${rows.length}`} · page {page + 1} of {pageCount}</span>
        ...
        <button aria-label="Next page" disabled={page + 1 >= pageCount} onClick={() => setPage((current) => Math.min(pageCount - 1, current + 1))} type="button"><ChevronRight size={13} /></button>
      </div>
...
function renderMarketCell(row: ScreenerRow, column: string, presentations: ReturnType<typeof useTickerPresentations>, customColumns: ScannerCustomColumn[], catalog = FIELD_CATALOG, companyInIdentity = false) {
  ...
  if (column === "ticker" || column === "symbol") {
    ...
    return <SecurityIdentityCell companyName={companyName} country={String(row.country ?? presentations[ticker]?.country ?? "")} halted={row.market_is_halted ?? row.is_halted ?? row.trading_status ?? presentations[ticker]?.market_is_halted ?? presentations[ticker]?.trading_status} logoUrl={String(row.logo_url ?? presentations[ticker]?.logo_url ?? "")} secCount={row.sec_count} secLabels={row.sec_labels} secRecency={preferRecentRecency(row.sec_recency, presentations[ticker]?.sec_recency)} secReviewStatus={row.sec_review_status} secSynthesisCount={row.sec_synthesis_count} ticker={ticker} trailing={hasNewsMarker(row) ? <NewsMarketCell compact row={row} ticker={ticker} /> : null} />;
  }
