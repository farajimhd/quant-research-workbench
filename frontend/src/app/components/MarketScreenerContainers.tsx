import { ArrowDown, ArrowUp, ArrowUpDown, Check, ChevronLeft, Columns3, Filter, Flame, ListFilter, Plus, Search, Star, Trash2, X } from "lucide-react";
...
const FIELD_CATALOG: FieldDefinition[] = [
  ...
  field("exchange", "Exchange", "Security", "raw", "text", "Primary listing exchange at the workspace clock."),
  ...
  field("gap_pct", "Opening gap", "Market state", "derived", "percent", "Opening price versus the previous regular-session close."),
  ...
  field("relative_volume", "Relative volume", "Liquidity", "derived", "number", "Current cumulative volume divided by the comparable historical baseline."),
  field("dollar_volume", "Dollar volume", "Liquidity", "derived", "money", "Executed share volume multiplied by representative price."),
  field("spread_pct", "Spread", "Liquidity", "derived", "percent", "Current NBBO spread divided by midpoint."),
  ...
  field("health_score", "Stock health", "Fundamentals", "derived", "number", "Evidence-weighted stock-health score from the Stock Facts authority."),
  field("financial_strength", "Financial strength", "Fundamentals", "derived", "number", "Financial-statement strength score with explicit evidence coverage."),
  field("valuation_regime", "Valuation", "Fundamentals", "derived", "text", "Historical valuation regime; not an analyst forward estimate."),
  field("live_news_recency", "News state", "News & SEC", "derived", "text", "Hot, cold, old, or none at the workspace clock."),
  ...
  field("sec_recency", "SEC state", "News & SEC", "derived", "text", "Hot, cold, old, or none from filing acceptance time."),
  ...
  field("qmd_signal", "QMD signal", "Signals", "derived", "number", "Canonical combined QMD microstructure score."),
  field("qmd_confidence", "QMD confidence", "Signals", "derived", "percent", "Evidence confidence for the combined QMD signal."),
...
const SCANNER_PRESETS: Record<string, string[]> = {
  ...
  Momentum: ["ticker", "last", "change_5m_pct", "change_pct", "relative_volume", "trade_count", "qmd_signal", "qmd_confidence"],
  Intelligence: ["ticker", "last", "change_pct", "live_news_recency", "live_news_count", "news_labels", "sec_recency", "sec_count", "sec_labels"],
  Fundamentals: ["ticker", "last", "market_cap", "float_shares", "short_crowding_pct", "health_score", "financial_strength", "valuation_regime"],
};
...
export function MarketScannerContainer({ asOf, meta, onSettingsChange, rows, settings }: { asOf: string; meta?: ScannerSnapshotMeta; onSettingsChange: (patch: Partial<MarketScannerSettings>) => void; rows: ScreenerRow[]; settings: MarketScannerSettings }) {
  ...
  return <MarketListSurface
    asOf={asOf}
    columns={settings.columns.length ? settings.columns : SCANNER_PRESETS[settings.preset] ?? SCANNER_PRESETS.Overview}
...
function MarketListSurface({ asOf, columns, empty, eyebrow, limit, onColumnsChange, onPresetChange, preset, presets, rows, subtitle, title }: { asOf: string; columns: string[]; empty: string; eyebrow: string; limit: number; onColumnsChange: (columns: string[]) => void; onPresetChange: (preset: string) => void; preset: string; presets: string[]; rows: ScreenerRow[]; subtitle: string; title: string }) {
  return <section className="market-list-surface" aria-label={title}>
    ...
    <MarketListTable columns={columns} empty={empty} limit={limit} onColumnsChange={onColumnsChange} rows={rows} title={title} />
  </section>;
}

function MarketListTable({ columns, empty, limit, onColumnsChange, rowAction, rows, title }: { columns: string[]; empty: string; limit: number; onColumnsChange: (columns: string[]) => void; rowAction?: (row: ScreenerRow) => ReactNode; rows: ScreenerRow[]; title: string }) {
  ...
  const visibleRows = useMemo(() => rows.filter((row) => {
    ...
    if (filterMode === "news" && !["hot", "cold"].includes(String(row.live_news_recency ?? "").toLowerCase())) return false;
    return true;
  ...
  return <div className="market-list-table-shell">
    <div className="market-list-toolbar">
      ...
      <label className="market-list-filter"><Filter size={13} /><select aria-label={`Filter ${title}`} onChange={(event) => setFilterMode(event.target.value)} value={filterMode}><option value="all">All rows</option><option value="advancing">Advancing</option><option value="declining">Declining</option><option value="news">Recent news</option></select></label>
    ...
    {columnPickerOpen ? <ColumnPicker columns={columns} onChange={onColumnsChange} onClose={() => setColumnPickerOpen(false)} /> : null}
  </div>;
}

function ColumnPicker({ columns, onChange, onClose }: { columns: string[]; onChange: (columns: string[]) => void; onClose: () => void }) {
  ...
  function toggle(key: string) { onChange(columns.includes(key) ? columns.filter((column) => column !== key) : [...columns, key]); }
  return <aside aria-label="Add scanner columns" className="market-column-picker">
    ...
    <div className="market-column-picker-body">
      ...
      <section className={deferredQuery ? "search-results" : undefined}><button className="market-column-back" onClick={() => { setQuery(""); setGroup(groups[0]); }} type="button"><ChevronLeft size={14} /> {deferredQuery ? "All groups" : group}</button>{matches.map((item) => <button className={columns.includes(item.key) ? "selected" : undefined} key={item.key} onClick={() => toggle(item.key)} type="button"><i>{columns.includes(item.key) ? <Check size={12} /> : null}</i><span><strong>{item.label}</strong><small>{item.description}</small></span><em data-kind={item.kind}>{item.kind}</em></button>)}</section>
    </div>
  </aside>;
}
...
function renderMarketCell(row: ScreenerRow, column: string, presentations: ReturnType<typeof useTickerPresentations>) {
  ...
  if (column === "ticker") return <TickerIdentity logoUrl={presentations[String(value)]?.logo_url} ticker={String(value ?? "")} />;
  if (column === "event_time") return value ? <MarketTime value={String(value)} /> : "—";
  if (["live_news_recency", "sec_recency", "direction", "source", "valuation_regime"].includes(column)) return value ? <span className={`market-list-badge ${String(value).toLowerCase().replace(/[^a-z]+/g, "-")}`}>{column === "live_news_recency" && value === "hot" ? <Flame size={11} /> : null}{String(value).replaceAll("_", " ")}</span> : "—";
