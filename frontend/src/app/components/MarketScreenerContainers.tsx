...
export function SignalStreamContainer({ asOf, live, onSettingsChange, onTickerSelect, runId, scannerRows = [], settings }: { asOf: string; live: boolean; onSettingsChange: (patch: Partial<SignalStreamSettings>) => void; onTickerSelect: (ticker: string) => void; runId?: string; scannerRows?: ScreenerRow[]; settings: SignalStreamSettings }) {
  ...
  const lockedColumns = marketMonitorColumns(streamColumns, true);
  ...
  return <section className="market-list-surface watchlist-surface signal-stream-surface" aria-label={`${stream?.name ?? "Signal Stream"} signal stream`}>
    ...
    <nav aria-label="Signal Streams" className="watchlist-tabs" role="tablist">
      {visibleStreams.map((row) => { const selected = row.signal_stream_id === stream?.signal_stream_id; return <span className={selected ? "active" : undefined} key={row.signal_stream_id}><button aria-description={row.description || undefined} aria-selected={selected} onClick={() => selectStream(row.signal_stream_id)} role="tab" type="button">{row.name}</button><button aria-label={`Remove ${row.name} tab`} className="watchlist-tab-remove" onClick={() => removeStream(row.signal_stream_id)} type="button"><X size={10} /></button></span>; })}
...
export function WatchUniverseContainer({ asOf, live = false, onSettingsChange, onTickerSelect, runtime, scannerRows, settings }: { asOf: string; live?: boolean; onSettingsChange: (update: Partial<WatchUniverseSettings> | ((current: WatchUniverseSettings) => Partial<WatchUniverseSettings>)) => void; onTickerSelect: (ticker: string) => void; runtime: WatchlistRuntimeResponse | null; scannerRows: ScreenerRow[]; settings: WatchUniverseSettings }) {
  const { catalog: fieldCatalog, discovery } = useDiscoveryPresentation();
  ...
  const lockedColumns = marketMonitorColumns(watchlist?.columns ?? ["symbol"]);
  const columns = canonicalDiscoveryColumns([...lockedColumns, ...settings.columns]);
  ...
  return <section className="market-list-surface watchlist-surface" aria-label={`${watchlist?.name ?? "Watchlist"} watchlist`}>
    ...
    <nav aria-label="QMD Watchlists" className="watchlist-tabs" role="tablist">
      {visibleWatchlists.map((row) => {
        ...
        return <span className={selected ? "active" : undefined} key={row.watchlist_id}><button aria-description={row.description || undefined} aria-selected={selected} onClick={() => selectWatchlist(row.watchlist_id)} role="tab" type="button">{row.name}</button><button aria-label={`Remove ${row.name} tab`} className="watchlist-tab-remove" onClick={() => removeWatchlist(row.watchlist_id)} type="button"><X size={10} /></button></span>;
      })}
      <button aria-description={availableWatchlists.length ? "Add a QMD Watchlist to this container" : "All configured Watchlists are already open"} aria-expanded={addingWatchlist} aria-label="Add Watchlist tab" className="watchlist-tab-add" disabled={!availableWatchlists.length} onClick={() => setAddingWatchlist((open) => !open)} role="tab" type="button"><Plus size={12} /><span>Add</span></button>
    </nav>
...
export function StrategyActivityContainer({ asOf, focusSequence, historicalPage, historicalRows, loadAllHistory = false, onSettingsChange, onTickerSelect, runId, settings }: { asOf: string; focusSequence?: number; historicalPage?: StrategyActivityPage; historicalRows?: ScreenerRow[]; loadAllHistory?: boolean; onSettingsChange: (patch: Partial<StrategyActivitySettings>) => void; onTickerSelect: (ticker: string) => void; runId?: string; settings: StrategyActivitySettings }) {
  ...
  const selectionScope = runId ?? "";
  const selectedSummary = selection?.scope === selectionScope ? selection.row : null;
  const selectedRecordId = selectedSummary ? strategyActivityRowKey(selectedSummary) : "";
  const exactRecordId = String(selectedSummary?.record_id || "");
  const detailCache = useMemo(() => new Map<string, ScreenerRow>(), [selectionScope]);
  const toggleSelection = (row: ScreenerRow) => setSelection((current) =>
    current?.scope === selectionScope && strategyActivityRowKey(current.row) === strategyActivityRowKey(row)
      ? null : { scope: selectionScope, row });
  ...
  const activitySummary = useMemo(() => {
    ...
    waits.forEach((row) => {
      const reason = String(row.reason_code || row.reason || "Unspecified");
  ...
  useEffect(() => {
    ...
    // Journal records are immutable. Projection refreshes must neither restart
    // this request nor discard evidence when the same event is reopened.
    const cached = detailCache.get(exactRecordId);
    if (cached) {
    ...
    api<StrategyActivityResponse>(`/api/trading/strategy-activity?${query}`, { signal: controller.signal, timeoutMs: runId ? 30000 : 10000 })
      .then((response) => {
        if (controller.signal.aborted) return;
        const detail = response.rows[0] ?? null;
        if (detail) detailCache.set(exactRecordId, detail);
    ...
    return () => controller.abort();
  }, [detailCache, exactRecordId, runId, selectedRecordId]);
  ...
  return <section className="market-list-surface strategy-activity-surface" aria-label="Strategy activity">
    ...
    <div className="strategy-activity-content" ref={contentRef} style={{ "--strategy-inspector-width": `${inspectorWidth}%` } as CSSProperties}>
      ...
      {selectedRecordId ? <StrategyActivityInspector error={detailError} loading={detailLoading} onClose={() => setSelection(null)} row={selectedDetail?.record_id === exactRecordId ? selectedDetail : selectedSummary} /> : <div className="strategy-activity-inspector-empty"><FileCheck2 size={18} /><span><strong>Inspect any event</strong><small>Open a row to see its strategy revision, exact reason, gate checks, thresholds, and point-in-time evidence.</small></span></div>}
    </div>
  </section>;
}
...
function StrategyActivityInspector({ error, loading, onClose, row }: { error: string; loading: boolean; onClose: () => void; row: ScreenerRow | null }) {
  ...
  return <aside className="strategy-activity-inspector" aria-label="Strategy event details">
    <header><div><span>Event evidence</span><strong>{action}</strong></div><button aria-label="Close strategy event details" onClick={onClose} type="button"><X size={16} /> Close</button></header>
...
function MarketListTable({
  ...
  const matchedRows = useMemo(() => {
    ...
    return [sortedRows[pinnedIndex], ...sortedRows.slice(0, pinnedIndex), ...sortedRows.slice(pinnedIndex + 1)];
  }, [columnFilters, deferredQuery, filterColumns, filterMatchMode, pinnedSequence, rows, sort]);
  ...
  const nextPage = async () => {
    ...
    if (!moreAvailable || !onRequestMore || moreLoading) return;
    if (await onRequestMore()) setPage((current) => current + 1);
  };
  ...
  return <div className="market-list-table-shell" ref={tableShellRef}>
...
function normalizeScannerRows(rows: ScreenerRow[]) {
  return rows.map((row) => {
    ...
    return {
      ...
      news_intelligence: newsIntelligenceConfidence(row),
      ticker,
    };
  });
}
...
function renderMarketCell(row: ScreenerRow, column: string, presentations: ReturnType<typeof useTickerPresentations>, customColumns: ScannerCustomColumn[], catalog = FIELD_CATALOG, companyInIdentity = false, onTickerSelect?: (ticker: string) => void) {
...
function NewsMarketCell({ compact = false, row, ticker }: { compact?: boolean; row: ScreenerRow; ticker: string }) {
  ...
  const pending = [row.news_ai_review_state, row.news_ai_reaction_state].some((value) => ["queued", "labeling", "running"].includes(String(value ?? "").toLowerCase()));
  ...
  return <>
    <button aria-label={`Open ${ticker} news timeline. ${details}. ${Math.round(confidence)}% strongest confidence.`} className="market-news-summary" data-compact={compact ? "true" : "false"} onClick={(event) => { event.stopPropagation(); const rect = event.currentTarget.getBoundingClientRect(); event.currentTarget.dispatchEvent(new CustomEvent(OPEN_TICKER_NEWS_EVENT, { bubbles: true, detail: { anchor: { bottom: rect.bottom, left: rect.left, right: rect.right, top: rect.top }, ticker } })); }} type="button">
...
function marketMonitorColumns(columns: string[], chronological = false) {
  ...
  const priority = MARKET_MONITOR_COLUMN_PRIORITY.filter((column) => available.has(column));
  return canonicalDiscoveryColumns([
    ...(chronological ? ["event_time"] : []),
    ...priority,
    ...canonical.filter((column) => column !== "event_time" && !priority.includes(column)),
  ]);
}
