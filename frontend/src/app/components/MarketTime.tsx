...
export function MarketTime({ className = "", dateStyle = "full", includeDate = false, includeSeconds = false, includeSubseconds = false, layout = "stacked", showVancouver = true, value }: { className?: string; dateStyle?: "full" | "short"; includeDate?: boolean; includeSeconds?: boolean; includeSubseconds?: boolean; layout?: "inline" | "stacked"; showVancouver?: boolean; value: string | number | Date }) {
  ...
  const label = `${exchangeDate ? `${exchangeDate}, ` : ""}${exchangeTime} ET${showVancouver ? `; ${vancouverTime} Vancouver` : ""}`;
  return <time aria-label={label} className={`market-time market-time-${layout}${className ? ` ${className}` : ""}`} dateTime={typeof value === "string" ? value : date.toISOString()}>
    ...
    {showVancouver ? <small className="market-time-secondary">VAN {vancouverTime}</small> : null}
  </time>;
}
