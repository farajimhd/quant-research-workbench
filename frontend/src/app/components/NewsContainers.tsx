  ...
  return <section className="news-all" aria-label="All news">
    <form className="news-query-bar" onSubmit={(event) => { event.preventDefault(); setCommittedSearch(search.trim()); }}>
      <div className="news-query-primary">
        <label className="news-search"><Search size={13} /><input aria-label="Search all news" onChange={(event) => setSearch(event.target.value)} placeholder="Search ticker, headline…" value={search} /></label>
        ...
        <label><span>Top</span><select aria-label="News result limit" onChange={(event) => onSettingsChange({ limit: Number(event.target.value) })} value={settings.limit}><option value={25}>Top 25</option><option value={50}>Top 50</option><option value={100}>Top 100</option><option value={250}>Top 250</option></select></label>
        ...
        <label><span>Text</span><select aria-label="News text coverage" onChange={(event) => onSettingsChange({ content: event.target.value })} value={settings.content}><option value="all">All text</option><option value="full">Full text</option><option value="title">Title only</option></select></label>
      ...
      <div className="intelligence-feed news-intelligence-feed" role="list">
        ...
        {displayRows.map((row) => {
          ...
          return <article className="intelligence-feed-row news-intelligence-grid" data-direction={directionValue} key={row.canonical_news_id} role="listitem">
            ...
            <div className="intelligence-sentiment-cell"><ScopedDirection compact summary={row.scoped_summary} /></div>
