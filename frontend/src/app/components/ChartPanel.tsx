...
const ChartPanelCore = forwardRef<ChartPanelHandle, ChartPanelProps>(({
  ...
  const swingStructure = useSwingStructure(ticker, hindsightSessionDate);
  ...
  function drawCurrentRegions() {
    ...
    swingStructurePrimitiveRef.current?.setState(swing.segments,
      ...
      Math.min(swing.end, (timeline.at(-1)?.time ?? 0) + swingDuration), swing.lineOpacity, swing.bandOpacity);
  ...
  function cleanupChartRuntime() {
    ...
    if(reactionBookPrimitiveRef.current && candleRef.current)candleRef.current.detachPrimitive(reactionBookPrimitiveRef.current);
    reactionBookPrimitiveRef.current=null;
  ...
  return (
    ...
    >
      <div className={toolbarVariant === "compact" ? "chart-component-toolbar compact" : "chart-component-toolbar"}>
        ...
        {!labeling ? <>{hindsight.controls}{levelReaction.controls}{structuralDetector.controls}{supertrendIndicator.controls}</> : null}
        <button
