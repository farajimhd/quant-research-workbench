...
function HealthSparkline({ history }: { history: MetricHistoryPayload | null }) {
  ...
  return <div className="facts-health-history">
    <header><span>Historical health trajectory</span><small className={!points.length ? "inline-loading-message" : undefined}>{!points.length ? <span className="loading-spinner" aria-hidden="true" /> : null}{points.length ? `${formatHistoryDate(points[0].at)} → ${formatHistoryDate(points[points.length - 1].at)}` : "Loading history…"}</small></header>
