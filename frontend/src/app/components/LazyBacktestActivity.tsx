...
export function LazyBacktestActivity(props: Props) {
  ...
  const { runId, settings } = props;
  const [held, setHeld] = useState<{ runId?: string; asOf: string; sequence?: number } | null>(null);
  const heldScope = held?.runId === runId ? held : null;
  const asOf = heldScope?.asOf ?? props.asOf;
  const sequence = heldScope ? heldScope.sequence : props.throughSequence;
  const key = JSON.stringify([runId, asOf, sequence, settings.strategyId, settings.ticker, settings.eventType]);
  ...
  useEffect(() => {
    ...
    const scope = `${runId}:${asOf}:${sequence}`;
    if (fence.current.scope !== scope) { fence.current = { scope, sequence }; cache.current.clear(); }
  ...
  return <div className="backtest-lazy-activity" aria-busy={loading}>
    <div className="backtest-view-controls" aria-label="Activity history pages">
      ...
      <button className="button secondary compact" disabled={loading || !page || page.complete} onClick={() => { setHeld({ runId, asOf, sequence }); setCursor({ key, offset: page?.next_offset ?? offset + 200 }); }}>Older events</button>
      {heldScope ? <><span>Browsing saved activity page</span><button className="button secondary compact" onClick={() => { setHeld(null); setCursor({ key: "", offset: 0 }); }}>Latest events</button></> : null}
    </div>
