...
export function SecDetailContainer({ asOf, canvasId, requestedCik, requestedAccession }: { asOf: string; canvasId: string; requestedCik?: string; requestedAccession?: string }) {
  ...
  if (!key) return <SecEmpty label="Choose a filing in All SEC or Ticker SEC to read it here." />; if (loading && !detail) return <LoadingState fill label="Loading filing" />; if (error && !detail) return <SecEmpty label={error} />; if (!detail) return null;
