...
type SecScopedSummary = { classified?: boolean; content_role: string; event_concepts: string[]; forecast_trigger_eligible: boolean; issuer_count?: number; issuer_history_context_eligible: boolean; label_count?: number; labeling_version?: string; quality_flags?: string[]; reaction_evaluation_eligible: boolean; semantic_direction: string; semantic_score?: number | null; source_origin: string };
...
export function SecDetailContainer({ asOf, canvasId, requestedCik, requestedAccession }: { asOf: string; canvasId: string; requestedCik?: string; requestedAccession?: string }) {
  ...
  return <article className="news-reader sec-reader"><header className="news-reader-hero"><div className="news-reader-kicker"><TemperatureTag tone={tone} /><SecFilingTime row={row} /><SecLabel label={row.filing_label_text} /><span>{row.form_type}</span>{row.impact_score ? <ImpactBadge label={row.impact_label} score={row.impact_score} /> : null}<SecGuideButton compact /></div><div className="news-reader-title-row"><h1>{row.disclosure_title || row.company_name}</h1>{row.scoped_summary ? <div className="news-reader-primary-direction"><SecDirection prominent summary={row.scoped_summary} /></div> : null}</div>{row.disclosure_title ? <p className="sec-reader-company">{row.company_name}</p> : null}<div className="news-reader-byline"><span>CIK {row.cik} · {row.accession_number}</span><TickerList presentations={presentations} tickers={tickers} /></div>{row.scoped_summary ? <SecDetailOverview row={row} summary={row.scoped_summary} /> : <div className="news-label-pending">{detail.intelligence_status === "unavailable" ? "Filing intelligence temporarily unavailable" : "Filing intelligence pending"}</div>}{row.items?.length ? <div className="news-reader-tags" aria-label="Filing items">{row.items.map((item) => <span key={item}>Item {item}</span>)}</div> : null}</header>
