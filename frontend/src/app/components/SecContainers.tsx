import { BookOpen, Check, Clock3, ExternalLink, FileCheck2, FileText, Flame, RefreshCw, Search, Snowflake } from "lucide-react";
  ...
  return <section className="news-all sec-all" aria-label="All SEC filings">
    <form className="news-query-bar" onSubmit={(event) => { event.preventDefault(); setCommitted(search.trim()); }}>
      <div className="news-query-primary">
        ...
        <InventoryFilterSelect ariaLabel="SEC time window" onChange={(value) => value === "custom" ? onSettingsChange({ rangeMode: "custom" }) : onSettingsChange({ lookbackHours: Number(value), rangeMode: "preset" })} options={SEC_WINDOW_OPTIONS} value={settings.rangeMode === "custom" ? "custom" : settings.lookbackHours} />
        ...
        <InventoryFilterSelect ariaLabel="SEC result limit" onChange={(value) => onSettingsChange({ limit: Number(value) })} options={SEC_LIMIT_OPTIONS} value={settings.limit} />
        ...
        <InventoryFilterSelect ariaLabel="SEC filing label" onChange={(value) => onSettingsChange({ label: value })} options={[{ value: "", label: "All labels" }, ...labels.map((label) => ({ value: label.id, label: label.label }))]} value={settings.label} />
        <InventoryFilterSelect ariaLabel="SEC impact score" onChange={(value) => setFilters({ ...filters, impact: value })} options={SEC_IMPACT_OPTIONS} value={filters.impact} />
        <label><span>Security scope</span><input aria-label="SEC security scope" onChange={(event) => setFilters({ ...filters, scope: event.target.value })} placeholder="Any scope" value={filters.scope} /></label>
        <SecEligibilityFilters filters={filters.eligibility} onChange={(eligibility) => setFilters({ ...filters, eligibility })} />
        <InventoryFilterSelect ariaLabel="SEC label state" onChange={(value) => setFilters({ ...filters, labelState: value })} options={SEC_LABEL_STATE_OPTIONS} value={filters.labelState} />
        <InventoryFilterSelect ariaLabel="SEC filing content" onChange={(value) => onSettingsChange({ content: value })} options={SEC_CONTENT_OPTIONS} value={settings.content} />
        <SecGuideButton />
    ...
    <div className="news-table-wrap intelligence-feed-scroll"><div className="intelligence-feed sec-intelligence-feed" role="list"><div className="intelligence-feed-header sec-intelligence-grid" role="row"><span>Accepted</span><span>Ticker</span><span>Filing / disclosure</span><span>Category</span><span>Impact</span><span>Security scope</span><span>Role</span><span>Origin</span><SentimentSortButton onChange={setSentimentSort} order={sentimentSort} /><span>Forecast</span><span>Reaction</span><span>History</span><span>Prior context</span><span>Follow-up</span><span>Content</span></div>{displayRows.map((row) => {
      ...
      return <article className="intelligence-feed-row sec-intelligence-grid" data-direction={directionValue} key={`${row.cik}-${row.accession_number}`} role="listitem">
        ...
        <div className="sec-label-value">{humanize(row.scoped_summary?.content_role || "Pending")}</div>
        <div className="sec-label-value">{humanize(row.scoped_summary?.source_origin || "Pending")}</div>
        <div className="intelligence-sentiment-cell"><SecDirection compact summary={row.scoped_summary} /></div>
        <SecEligibilityCell active={row.scoped_summary?.forecast_trigger_eligible} />
        <SecEligibilityCell active={row.scoped_summary?.reaction_evaluation_eligible} />
        <SecEligibilityCell active={row.scoped_summary?.issuer_history_context_eligible} />
...
function TickerList({ presentations, tickers = [] }: { presentations: ReturnType<typeof useTickerPresentations>; tickers?: string[] }) { return <span className="news-tickers">{tickers.slice(0, 3).map((ticker) => <b key={ticker}><TickerIdentity logoUrl={presentations[ticker]?.logo_url} showLogoPlaceholder ticker={ticker} /></b>)}{tickers.length > 3 ? <b>+{tickers.length - 3}</b> : !tickers.length ? "—" : null}</span>; }
