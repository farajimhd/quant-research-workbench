...
const INITIAL_LABELS: SecLabel[] = [
  ["results", "Results & reports"], ["material_event", "Material event"], ["insider_ownership", "Insider ownership"], ["holder_ownership", "Large-holder ownership"], ["offering_capital", "Offering & capital"], ["governance_proxy", "Governance & proxy"], ["merger_tender", "M&A & tender"], ["registration", "Registration"], ["compliance_notice", "Compliance notice"], ["fund_disclosure", "Fund disclosure"], ["other", "Other filing"],
...
export function AllSecContainer({ asOf, onSettingsChange, settings }: { asOf: string; onSettingsChange: (patch: Partial<SecSettings>) => void; settings: SecSettings }) {
  ...
  return <section className="news-all sec-all" aria-label="All SEC filings">
    ...
    <div className="news-table-wrap"><table className="news-table sec-table"><thead><tr><th>Accepted</th><th>Ticker</th><th>Filing</th><th>Company / disclosure</th><th>Items</th><th>Content</th></tr></thead><tbody>{state.rows.map((row) => <tr key={`${row.cik}-${row.accession_number}`}>
      <td><MarketTime className="news-row-time" dateStyle="short" includeDate value={row.accepted_at_utc} /></td><td><TickerList presentations={presentations} tickers={row.tickers} /></td>
      ...
      <td><button className="news-headline-button" onClick={() => openSecPage(row)} type="button"><strong>{row.company_name}</strong><small>{row.accession_number}</small></button></td>
...
export function TickerSecContainer({ asOf, onSymbolChange, settings, symbol }: { asOf: string; onSymbolChange?: (symbol: string) => void; settings: { lookbackHours: number }; symbol: string }) {
  ...
  const core = state.rows.filter((row) => ["results", "material_event", "offering_capital", "governance_proxy", "merger_tender", "compliance_notice"].includes(row.filing_label));
...
export function SecDetailContainer({ asOf, canvasId, requestedCik, requestedAccession }: { asOf: string; canvasId: string; requestedCik?: string; requestedAccession?: string }) {
  ...
  return <article className="news-reader sec-reader"><header><div className="news-reader-kicker"><TemperatureTag tone={tone} /><MarketTime includeDate value={row.accepted_at_utc} /><SecLabel label={row.filing_label_text} tone={tone} /><span>{row.form_type}</span></div><h1>{row.company_name}</h1><div className="news-reader-byline"><span>CIK {row.cik} · {row.accession_number}</span><TickerList presentations={presentations} tickers={tickers} /></div>{row.items?.length ? <div className="news-reader-tags">{row.items.map((item) => <span key={item}>Item {item}</span>)}</div> : null}<details className="news-classification-evidence"><summary>Why this label</summary><p>{(row.label_evidence ?? []).join(" · ")}</p></details></header>
    <section className="sec-document-inventory"><div className="sec-reader-section-title"><div><strong>Filing documents</strong><small>{detail.documents.length} source documents · {detail.texts.length} readable texts</small></div></div><div>{detail.documents.map((document) => <button data-active={document.document_id === selectedText?.document_id ? "true" : "false"} key={document.document_id} onClick={() => setDocumentId(document.document_id)} type="button"><FileText size={14} /><span><strong>{document.document_name}</strong><small>{document.document_type} · {document.document_role}</small></span>{safeUrl(document.document_url) ? <a href={document.document_url} onClick={(event) => event.stopPropagation()} rel="noreferrer" target="_blank"><ExternalLink size={12} /></a> : null}</button>)}</div></section>
    {selectedText?.text ? <section className="news-reader-body sec-reader-body"><div className="sec-reader-section-title"><div><strong>Readable filing</strong><small>{selectedText.text_kind} · {formatCount(selectedText.text_char_count)} characters</small></div></div>{paragraphs(selectedText.text).map((paragraph, index) => <p key={`${index}-${paragraph.slice(0, 18)}`}>{paragraph}</p>)}</section> : <SecEmpty label="No readable text is available for the selected document." />}
    <section className="sec-facts"><div className="sec-reader-section-title"><div><strong>XBRL facts</strong><small>{detail.facts.length} point-in-time facts</small></div></div>{detail.facts.length ? <div className="news-table-wrap"><table className="news-table"><thead><tr><th>Concept</th><th>Value</th><th>Period</th></tr></thead><tbody>{detail.facts.slice(0, 80).map((fact, index) => <tr key={`${fact.tag}-${index}`}><td>{fact.tag}</td><td>{fact.value} <small>{fact.unit_code}</small></td><td>{fact.fiscal_period || fact.period_end_date || "—"}</td></tr>)}</tbody></table></div> : <SecEmpty label="This filing has no linked XBRL facts." />}</section>
