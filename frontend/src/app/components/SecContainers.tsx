...
type SecScopedLabel = { content_role: string; event_concepts: string[]; evidence_scope: string; forecast_trigger_eligible: boolean; issuer_history_context_eligible: boolean; issuer_relationship?: string; issuer_role: string; quality_flags?: string[]; reaction_evaluation_eligible: boolean; semantic_direction: string; semantic_evidence_text: string; semantic_score?: number | null; source_origin: string; ticker: string; time_orientation?: string };
type SecScopedSummary = { content_role: string; event_concepts: string[]; forecast_trigger_eligible: boolean; issuer_history_context_eligible: boolean; reaction_evaluation_eligible: boolean; semantic_direction: string; semantic_score?: number | null; source_origin: string };
...
export function AllSecContainer({ asOf, onSettingsChange, settings }: { asOf: string; onSettingsChange: (patch: Partial<SecSettings>) => void; settings: SecSettings }) {
  ...
  return <section className="news-all sec-all" aria-label="All SEC filings">
    <form className="news-query-bar" onSubmit={(event) => { event.preventDefault(); setCommitted(search.trim()); }}>
      ...
      <label><span>Window</span><select aria-label="SEC time window" onChange={(event) => event.target.value === "custom" ? onSettingsChange({ rangeMode: "custom" }) : onSettingsChange({ lookbackHours: Number(event.target.value), rangeMode: "preset" })} value={settings.rangeMode === "custom" ? "custom" : settings.lookbackHours}><option value={24}>24 hours</option><option value={72}>3 days</option><option value={168}>7 days</option><option value={720}>30 days</option><option value={8760}>1 year</option><option value={43800}>5 years</option><option value="custom">Custom dates</option></select></label>
      {settings.rangeMode === "custom" ? <><label><span>From</span><input aria-label="SEC range start date" onChange={(event) => onSettingsChange({ startDate: event.target.value })} type="date" value={settings.startDate} /></label><label><span>Through</span><input aria-label="SEC range end date" onChange={(event) => onSettingsChange({ endDate: event.target.value })} type="date" value={settings.endDate} /></label></> : null}
      <label><span>Top</span><select aria-label="SEC result limit" onChange={(event) => onSettingsChange({ limit: Number(event.target.value) })} value={settings.limit}><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option><option value={200}>200</option></select></label>
    ...
    <div className="news-table-wrap"><table className="news-table sec-table sec-intelligence-table"><thead><tr><th>Accepted</th><th>Tickers</th><th>Role</th><th>Direction</th><th>Event</th><th>Filing / disclosure</th><th>Eligibility</th><th>Content</th></tr></thead><tbody>{state.rows.map((row) => <tr key={`${row.cik}-${row.accession_number}`}>
      <td><span className="sec-time-state"><SecFilingTime row={row} /><TemperatureTag tone={temperature(row, Date.parse(state.asOf || asOf))} /></span></td><td><TickerList presentations={presentations} tickers={row.tickers} /></td>
      <td><SecScopedRole summary={row.scoped_summary} fallback={row.filing_label_text} /></td><td><SecDirection summary={row.scoped_summary} salient /></td><td><SecConcepts concepts={row.scoped_summary?.event_concepts} /></td>
      <td><button className="news-headline-button" onClick={() => openSecPage(row, state.queryId)} type="button"><strong>{row.form_type} · {row.company_name}</strong><small>{row.disclosure_title || row.accession_number}</small></button></td>
      <td><SecEligibility summary={row.scoped_summary} /></td><td><ContentState row={row} /></td>
...
export function TickerSecContainer({ asOf, onSymbolChange, settings, symbol }: { asOf: string; onSymbolChange?: (symbol: string) => void; settings: { lookbackHours: number }; symbol: string }) {
  const state = useSecQuery({ asOf, refreshKey: 0, search: "", settings: { content: "all", endDate: "", label: "", limit: 100, lookbackHours: settings.lookbackHours, rangeMode: "preset", startDate: "", ticker: symbol } });
  const presentations = useTickerPresentations([symbol]);
  const actionable = state.rows.filter((row) => row.scoped_summary?.forecast_trigger_eligible);
  const context = state.rows.filter((row) => !row.scoped_summary?.forecast_trigger_eligible && row.scoped_summary?.issuer_history_context_eligible);
  const other = state.rows.filter((row) => !actionable.includes(row) && !context.includes(row));
  return <section className="ticker-news ticker-sec" aria-label={`${symbol} SEC filings`}><header><div><TickerIdentityWithChange asOf={state.asOf || asOf} className="ticker-news-symbol" inputAriaLabel="Ticker SEC symbol" logoUrl={presentations[symbol]?.logo_url} onTickerChange={onSymbolChange} ticker={symbol} /><span>Accepted disclosures</span></div><span className="sec-ticker-header-actions"><small>{state.rows.length} filings · through <MarketTime value={state.asOf || asOf} /></small><SecGuideButton compact /></span></header><SecStatus compact state={state} /><div className="ticker-news-feed"><TickerSecSection asOf={state.asOf || asOf} label="Actionable disclosures" queryId={state.queryId} rows={actionable} /><TickerSecSection asOf={state.asOf || asOf} label="Issuer history & context" queryId={state.queryId} rows={context} /><TickerSecSection asOf={state.asOf || asOf} label="Administrative & other" queryId={state.queryId} rows={other} />{!state.loading && !state.rows.length ? <SecEmpty label={`No ${symbol} filings in this window.`} /> : null}</div></section>;
}

function TickerSecSection({ asOf, label, queryId, rows }: { asOf: string; label: string; queryId: string; rows: SecRow[] }) { return <section className="ticker-news-section"><header><strong>{label}</strong><span>{rows.length}</span></header>{rows.map((row) => { const tone = temperature(row, Date.parse(asOf)); const Icon = temperatureIcon(tone); const direction = normalizeSemanticDirection(row.scoped_summary?.semantic_direction); return <article data-direction={direction} data-tone={tone} key={row.accession_number}><div className="ticker-news-marker"><Icon size={14} /></div><div className="ticker-event-time"><SecFilingTime row={row} /><TemperatureTag tone={tone} /></div><div className="ticker-event-content"><div className="ticker-news-meta"><SecDirection summary={row.scoped_summary} salient /><SecScopedRole fallback={row.filing_label_text} summary={row.scoped_summary} /><SecConcepts concepts={row.scoped_summary?.event_concepts} /></div><button className="ticker-news-open" onClick={() => openSecPage(row, queryId)} type="button"><strong>{row.form_type} · {row.company_name}</strong><p>{row.disclosure_title || (row.items ?? []).join(" · ") || row.accession_number}</p></button></div></article>; })}</section>; }
...
export function SecDetailContainer({ asOf, canvasId, requestedCik, requestedAccession }: { asOf: string; canvasId: string; requestedCik?: string; requestedAccession?: string }) {
  ...
  const row = detail.filing; const tickers = detailTickers; const selectedDocument = detail.documents.find((document) => document.document_id === documentId); const renderedText = detail.texts.find((text) => text.document_id === documentId); const originalText = detail.originals.find((text) => text.document_id === documentId); const tone = temperature(row, Date.parse(asOf));
  ...
  return <article className="news-reader sec-reader"><header className="news-reader-hero"><div className="news-reader-kicker"><TemperatureTag tone={tone} /><SecFilingTime row={row} /><SecLabel label={row.filing_label_text} /><span>{row.form_type}</span>{row.impact_score ? <ImpactBadge label={row.impact_label} score={row.impact_score} /> : null}<SecGuideButton compact /></div><h1>{row.disclosure_title || row.company_name}</h1>{row.disclosure_title ? <p className="sec-reader-company">{row.company_name}</p> : null}<div className="news-reader-byline"><span>CIK {row.cik} · {row.accession_number}</span><TickerList presentations={presentations} tickers={tickers} /></div>{row.scoped_summary ? <SecDetailOverview row={row} summary={row.scoped_summary} /> : <div className="news-label-pending">{detail.intelligence_status === "unavailable" ? "Filing intelligence temporarily unavailable" : "Filing intelligence pending"}</div>}{row.items?.length ? <div className="news-reader-tags" aria-label="Filing items">{row.items.map((item) => <span key={item}>Item {item}</span>)}</div> : null}<details className="news-classification-evidence"><summary>Read classification evidence</summary><p>{(row.label_evidence ?? []).join(" · ")}</p>{row.impact_rationale ? <p>{row.impact_rationale}</p> : null}</details></header>
    <SecIntelligenceDetail presentations={presentations} row={row} status={detail.intelligence_status} />
...
function useSecQuery({ asOf, refreshKey, search, settings }: { asOf: string; refreshKey: number; search: string; settings: SecSettings }) {
  ...
  const load = useCallback(async (before = "", beforeAccession = "", pageAsOf = "") => {
    ...
    const response = await api<SecPayloadWire>(`/api/trading/sec${query({ as_of: pageAsOf || asOf, before: before || undefined, before_accession: beforeAccession || undefined, content: settings.content, end_date: customReady ? settings.endDate : undefined, label: settings.label || undefined, limit: settings.limit, lookback_hours: settings.lookbackHours, query_id: before ? queryIdRef.current : undefined, search: search || undefined, start_date: customReady ? settings.startDate : undefined, ticker: settings.ticker || undefined })}`, { timeoutMs: 30000 });
  ...
  }, [asOf, search, settings.content, settings.endDate, settings.label, settings.limit, settings.lookbackHours, settings.rangeMode, settings.startDate, settings.ticker]);
...
function SecDirection({ prominent = false, salient = false, summary }: { prominent?: boolean; salient?: boolean; summary?: SecScopedSummary | null }) { return <SemanticDirectionMetric direction={summary?.semantic_direction} prominent={prominent || salient} score={summary?.semantic_score} />; }
...
function TemperatureTag({ tone }: { tone: Temperature }) { const Icon = temperatureIcon(tone); const label = tone === "dated" ? "Date only" : tone[0].toUpperCase() + tone.slice(1); return <span className="news-temperature" data-tone={tone}><Icon size={12} /><em>{label}</em></span>; }
...
function SecScopedLabelPanel({ label, presentations, row }: { label: SecScopedLabel; presentations: Record<string, TickerPresentation>; row: SecRow }) {
  const summary: SecScopedSummary = { content_role: label.content_role, event_concepts: label.event_concepts, forecast_trigger_eligible: label.forecast_trigger_eligible, issuer_history_context_eligible: label.issuer_history_context_eligible, reaction_evaluation_eligible: label.reaction_evaluation_eligible, semantic_direction: label.semantic_direction, semantic_score: label.semantic_score, source_origin: label.source_origin };
...
function temperature(row: SecRow, asOf: number): Temperature { if (row.event_time_quality === "date_only") return "dated"; const age = Math.max(0, (asOf - Date.parse(row.accepted_at_utc)) / 60000); return age <= 240 ? "hot" : age <= 1440 ? "cold" : "old"; }
...
function openSecPage(row: SecRow, queryId: string) { ensureSecReaderCanvas(); const selection = { acceptedAt: row.accepted_at_utc, key: `${row.cik}/${row.accession_number}`, queryId }; selectSec(SEC_READER_CANVAS_ID, selection); const url = new URL(focusCanvasUrl(SEC_READER_CANVAS_ID, "sec_detail")); url.searchParams.set("sec_cik", row.cik); url.searchParams.set("sec_accession", row.accession_number); window.open(url.toString(), "quant-sec-reader"); }
