## Laptop And Workstation Storage Authority

These paths are mandatory. Do not infer a different storage root from the
current working directory or from where a launcher happens to be copied.

- The laptop source-of-truth repository is:
  `D:\TradingCodes\quant-research-workbench`
- The laptop operational root is:
  `D:\TradingML`
- All artifacts generated while an agent works, tests, audits, profiles, or
  otherwise operates on this laptop must be written under:
  `D:\TradingML\runtimes`
- Laptop-generated artifacts include temporary files, test and smoke-test
  output, audit output, logs, metrics, checkpoints, W&B files, manifests,
  caches, plots, screenshots, prepared data, downloaded runtime dependencies,
  and any other generated evidence or intermediate output. Do not put these
  artifacts inside the source repository.
- Durable source-controlled deliverables remain in the laptop repository. These
  include source code, tests, launchers, configuration definitions,
  documentation, and intentionally committed small fixtures. A generated
  report or fixture belongs in the repository only when the user explicitly
  requests it as a durable project deliverable.

- The workstation root is:
  `\\DESKTOP-SAAI85T\Workstation-D\TradingML`
- Its required subdirectories have separate responsibilities:
  - `\\DESKTOP-SAAI85T\Workstation-D\TradingML\codes` contains synchronized
    executable code copies. It is not the source of truth.
  - `\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes` contains workstation
    run directories and every artifact generated by workstation execution.
  - `\\DESKTOP-SAAI85T\Workstation-D\TradingML\secrets` contains workstation
    secrets. Never copy secret files or values into the repository, code
    copies, runtime outputs, logs, manifests, or prompts.

- Execution location determines the output authority:
  - A script running on the laptop must write generated output under
    `D:\TradingML\runtimes`.
  - A script running on the workstation must write generated output under
    `\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes`.
- Code changes must be made in the laptop source-of-truth repository first,
  validated there when possible, committed, pushed, and only then synchronized
  into the appropriate workstation `codes` runtime copy.
- If sandbox permissions or connectivity prevent access to the required
...
4. Workstation runtime setup:
   ...
   After implementing and validating a version locally, sync the required code
   to a self-contained folder under:
   `\\DESKTOP-SAAI85T\Workstation-D\TradingML\codes`

   The synchronized code folder should contain enough code to run without
   depending on the laptop path:
   ...
   Workstation-copied files must be adjusted so generated output defaults point
   under `\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes`, while data roots
   and environment discovery paths resolve to workstation-available locations.
   Do not leave laptop-only absolute paths unless they are intentionally valid
   on the workstation.
...
5. Experiment outputs should be organized under a single run directory per job.
   ...
   On the laptop, that run directory must be under
   `D:\TradingML\runtimes`. On the workstation, it must be under
   `\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes`. Runtime folder names
   can be stable. Experiment identity should come from the run manifest and run
   name, not from the runtime folder name.
