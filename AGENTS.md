...
- ClickHouse `default` storage is reserved exclusively for backups. Never create or persist operational tables, canonical data, derived data, or checkpoints on the `default` disk or a policy that routes to it (including the `hdd` alias). Use the explicit workload storage policy: structural checkpoints and QMD operational data belong on `live_market_ssd`; canonical SIP data uses its designated policy. Validate both the table policy and actual part placement before starting writers or campaigns; fail closed if the required policy is absent or placement is incorrect. Never silently fall back to `default`. Existing misplaced data requires an explicit, restart-safe migration with integrity verification; changing a setting alone is not proof that existing parts moved.
