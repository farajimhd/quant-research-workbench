    ...
    def test_projection_and_shared_chart(self):
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                page.get_by_text('Data status (2)', exact=True).click()
                ...
                output = Path(os.environ['MACD_REVIEW_OUTPUT'])
                output.mkdir(parents=True, exist_ok=True)
                for theme, scale, width in [(theme, scale, 1500 if scale == 1 else 1000) for theme in ['light', 'dark'] for scale in [.8, .9, 1, 1.1, 1.25]]:
                ...
                page.get_by_text('Data status (3)', exact=True).click()
                page.get_by_text('Calculation details', exact=True).click()
                dialog = page.get_by_role('dialog', name='Multi-timeframe MACD difference', exact=True)
                ...
                page.screenshot(path=str(output / 'macd-settings-compact-expanded.png'))
                ...
                (output / 'result.json').write_text(json.dumps({'vectors': 9, 'themes': ['light', 'dark'], 'scales': [.8, .9, 1, 1.1, 1.25], 'errors': errors}), encoding='utf-8')
