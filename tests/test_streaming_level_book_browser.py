...
@pytest.mark.parametrize('ticker,clock',[('JUNS','07:18:40'),('SUGP','04:15:20')])
def test_v7_chart_completed_clock_and_rewind(ticker,clock):
    ...
    with sync_playwright() as pw:
        ...
        try:
            page=browser.new_page(viewport={'width':1600,'height':1000});responses=[]
            ...
            page.evaluate("""async ({bars,stamp,ticker})=>{
              ...
              document.getElementById('root').style.display='none';const node=document.createElement('div');node.style.height='950px';document.body.appendChild(node);
              ...
              window.v7Paint=(at)=>root.render(React.createElement(ChartPreview,{
                ...
                chartSettings:{symbol:ticker,timeframe:'1s',visibleIndicators:[],showVolume:true,showSplitEvents:false},linkContext:{symbol:ticker},symbolEditable:false,showTradeAnnotations:false,baseHeight:700,
