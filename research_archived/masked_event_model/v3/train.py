...
REPO_ROOT = next(
    (
        ...
        if (parent / "research" / "masked_event_model" / "v3").exists()
    ),
...
from research.masked_event_model.v3.config import DataConfig, ExperimentConfig, MaskConfig, ModelConfig, ProbeConfig, TrainConfig
from research.masked_event_model.v3.data import EventChunkDataset, discover_chunk_files, target_horizons_from_columns
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train masked event autoencoder v3.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"Model: d={config.model.d_model} emb={config.model.embedding_dim} heads={config.model.n_heads} qlayers={config.model.quote_event_layers} tlayers={config.model.temporal_layers} decoder={config.model.decoder_layers}", flush=True)
    print(
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v3-d{args.d_model}-emb{args.embedding_dim}-e{args.quote_event_layers}-t{args.temporal_layers}-d{args.decoder_layers}-mask{int(args.mask_ratio * 100)}-chunk{args.chunk_ms}-nov2025"
...
if __name__ == "__main__":
    try:
        ...
        print("*** FATAL masked_event_model.v3.train error. Full traceback:", flush=True)
        traceback.print_exc()
        raise
