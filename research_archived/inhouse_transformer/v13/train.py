...
from research.inhouse_transformer.v13.config import (  # noqa: E402
    DataConfig,
    ExperimentConfig,
    ModelConfig,
    TrainConfig,
)
from research.inhouse_transformer.v13.data import (  # noqa: E402
    ANCHOR_NAMES,
...
from research.inhouse_transformer.v13.metrics import MetricAccumulator, append_jsonl  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--five-min-bucket-minutes", type=int, default=defaults.data.five_min_bucket_minutes)
    parser.add_argument("--five-min-lookback-minutes", type=int, default=defaults.data.five_min_lookback_minutes)
    parser.add_argument("--thirty-min-bucket-minutes", type=int, default=defaults.data.thirty_min_bucket_minutes)
    parser.add_argument("--thirty-min-lookback-minutes", type=int, default=defaults.data.thirty_min_lookback_minutes)
    parser.add_argument(
        "--anchor-history-sessions",
        type=int,
        default=defaults.data.anchor_history_sessions,
        help="Prior sessions loaded before the first requested session for same-minute and day/week anchors.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Main transformer target format. v13 defaults to return_bps so the model predicts the next move "
            "instead of a future price level."
        ),
    )
    parser.add_argument(
        "--target-columns",
    ...
    parser.add_argument("--wandb-project", default="May2026-1m-timeseries-cache-v2")
...
def config_from_args(args: argparse.Namespace) -> ExperimentConfig:
    ...
    data = DataConfig(
        ...
        five_min_bucket_minutes=args.five_min_bucket_minutes,
        five_min_lookback_minutes=args.five_min_lookback_minutes,
        thirty_min_bucket_minutes=args.thirty_min_bucket_minutes,
        thirty_min_lookback_minutes=args.thirty_min_lookback_minutes,
...
def main() -> None:
    ...
    print(
        "Multiscale context: "
        f"1m={config.data.context_length} "
        f"5m={math.ceil(config.data.five_min_lookback_minutes / config.data.five_min_bucket_minutes)} "
        f"30m={math.ceil(config.data.thirty_min_lookback_minutes / config.data.thirty_min_bucket_minutes)} "
        f"anchors={len(ANCHOR_NAMES)}",
        flush=True,
    )
    ...
    model = FeatureTemporalTransformer(
        ...
        five_min_context_length=math.ceil(config.data.five_min_lookback_minutes / config.data.five_min_bucket_minutes),
        thirty_min_feature_count=len(MULTISCALE_FEATURE_COLUMNS),
        thirty_min_context_length=math.ceil(config.data.thirty_min_lookback_minutes / config.data.thirty_min_bucket_minutes),
    ...
    while True:
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=config.train.amp and device.type == "cuda"):
            prediction, direction_logits = model_from_batch(model, batch)
...
def infer_timeline_batch_rows(
    *,
    ...
    with torch.autocast(
        ...
        prediction, _ = model_from_batch(model, device_batch)
...
def model_from_batch(model: torch.nn.Module, batch: dict[str, Any]) -> tuple[torch.Tensor, torch.Tensor]:
    return model(
        batch["values"],
        batch["time_features"],
        batch["five_min_values"],
        batch["five_min_time_features"],
        batch["thirty_min_values"],
        batch["thirty_min_time_features"],
        batch["anchor_values"],
...
def raise_on_nonfinite_loss(loss: torch.Tensor, *, label: str, step: int, batch: dict[str, torch.Tensor]) -> None:
    ...
    for key in (
        ...
        "time_features",
        "five_min_values",
        "five_min_time_features",
        "thirty_min_values",
        "thirty_min_time_features",
        "anchor_values",
        "anchor_time_features",
        "targets",
    ):
...
def config_to_dict(config: ExperimentConfig) -> dict[str, Any]:
    return {
        ...
        "data": {
            ...
            "context_length": config.data.context_length,
            "five_min_bucket_minutes": config.data.five_min_bucket_minutes,
            "five_min_lookback_minutes": config.data.five_min_lookback_minutes,
            "thirty_min_bucket_minutes": config.data.thirty_min_bucket_minutes,
            "thirty_min_lookback_minutes": config.data.thirty_min_lookback_minutes,
...
def load_torch_stack() -> None:
    ...
    try:
        ...
        from research.inhouse_transformer.v13.model import (
