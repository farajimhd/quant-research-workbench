...
from research.inhouse_transformer.v19.config import (  # noqa: E402
    DataConfig,
    ExperimentConfig,
    ModelConfig,
    TrainConfig,
...
forecast_loss = None
LOG_RULE = "*" * 96
EXPERIMENT_VERSION = "v19"
MODEL_ARCHITECTURE_NAME = "concat_market_time_tokens_all_time_zscore"
...
METRIC_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "loss": {
        ...
        "unit": "target units",
        "interpretation": "Down is better. For v19 this is binary cross entropy over sign plus magnitude bits.",
    },
    "regression_loss": {
    ...
    "h1_hard_decoded_mae_bps": {
        ...
        "unit": "bps",
        "interpretation": "Down is better. This is the legacy v19 decoded prediction error.",
    },
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Main transformer target format. v19 uses sign plus 12 magnitude bits for log-return bps."
        ),
    )
    parser.add_argument(
        "--target-columns",
    ...
    parser.add_argument(
        ...
        choices=["window_zscore_all"],
        default=defaults.data.time_feature_normalization,
        help="Time-feature normalization. v19 z-scores every time feature within each context window.",
    )
...
def config_from_args(args: argparse.Namespace) -> ExperimentConfig:
    ...
    data = DataConfig(
        ...
        time_feature_normalization=args.time_feature_normalization,
...
def input_experiment_name(config: ExperimentConfig) -> str:
    if config.data.input_normalization == "window_zscore_only":
        input_name = "expanded-time-conditioning-window-zscore"
    else:
        input_name = config.data.input_normalization.replace("_", "-")
    return f"{input_name}-all-time-zscore"
...
def main() -> None:
    ...
    print(f"Time feature normalization: {config.data.time_feature_normalization}", flush=True)
    print(
...
def config_to_dict(config: ExperimentConfig) -> dict[str, Any]:
    return {
        ...
        "data": {
            ...
            "time_feature_normalization": config.data.time_feature_normalization,
...
def load_torch_stack() -> None:
    ...
    try:
        ...
        from research.inhouse_transformer.v19.model import (
