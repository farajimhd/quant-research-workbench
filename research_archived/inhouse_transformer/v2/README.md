# In-House 1m Bar Transformer Baseline v2

This module trains a scratch PyTorch transformer baseline on provider-built 1-minute bars.

This folder is a versioned experiment. `../v1` preserves the actual-price z-score
target experiment that showed potential. v2 changes the default objective so the
model predicts the next move rather than a future price level.
...
The v2 main transformer defaults to `--target-mode return_bps`. Targets are the
...
When `--overfit-batches` is used, the script logs `overfit_timeline_predictions/*` W&B charts after the final test pass. It selects three tickers from the cached training batches, reloads their chronological session data, and overlays dashed predicted h1 close against solid target h1 close for each ticker, plus a table with the underlying values.

Run a small dry run:

```powershell
python research\inhouse_transformer\v2\train.py --dry-run --count-coverage --tickers USO --batch-size 128 --max-batches-per-session 2
```

Run the default experiment:

```powershell
python research\inhouse_transformer\v2\train.py --device cuda --batch-size 1024 --epochs 1
```

Run the main transformer one-session overfit test with wandb logging:

```powershell
python research\inhouse_transformer\v2\train.py --device cuda --overfit-session 2024-01-22 --target-columns close --horizon 1 --batch-size 1024 --epochs 200 --eval-steps 25 --logging-steps 25 --validation-window-count 8192 --test-window-count 8192 --warmup-steps 0 --wandb-entity mehdifaraji --wandb-project May2026-1m-timeseries-forecasting
```

The default W&B run name starts with `v2-` and includes `raw-window-zscore-only` and `return_bps` so it can be compared directly against v1.
