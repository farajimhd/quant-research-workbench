# In-House 1m Bar Transformer Baseline v20

This module trains a scratch PyTorch transformer baseline on provider-built 1-minute bars.

This folder is a versioned experiment. v20 starts from `../v16` and keeps the
same model architecture, optimizer, scheduler, target encoding, and loss. The
only intended experiment change is the data representation: missing 1-minute
trade bars are represented as synthetic clock bars after the ticker's first real
bar, up to a bounded staleness limit.
...
quote_imbalance, quote_valid_ratio, is_synthetic_bar, minutes_since_trade
```

Synthetic no-trade bars are built as:

```text
open = high = low = close = latest real close
volume = 0
transactions = 0
is_synthetic_bar = 1
minutes_since_trade = minutes since the latest real trade bar
```

Quote state fields are forward-filled from the latest available row. Rows are
kept only while `minutes_since_trade <= --max-synthetic-minutes` which defaults
to `30`. The loader also requires the whole context plus prediction horizon to
be contiguous 1-minute clock time, so samples do not silently jump across stale
gaps.
...
Spread bps, quote imbalance, quote valid ratio, `is_synthetic_bar`, and
`minutes_since_trade` are left in their existing units before the final
per-window z-score.
...
v7/v20 add scaled decoded market-time fields to that same separate tensor:
...
anchored to the origin bar or per-window z-scored. In v20, each time feature is
...
The v20 main transformer defaults to `--target-mode binary_magnitude_bps`.
...
```powershell
python research\inhouse_transformer\v20\train.py --dry-run --count-coverage --tickers USO --batch-size 128 --max-batches-per-session 2
```

Run the default experiment:

```powershell
python research\inhouse_transformer\v20\train.py --device cuda --batch-size 1024 --epochs 1
```

Run the main transformer one-session overfit test with wandb logging:

```powershell
python research\inhouse_transformer\v20\train.py --device cuda --overfit-session 2024-01-22 --batch-size 512 --epochs 200 --eval-steps 25 --logging-steps 25 --validation-window-count 5000 --test-window-count 10000 --allow-target-across-session --wandb-entity mehdifaraji --wandb-project May2026-1m-timeseries-v14-variants
```

The default W&B run name starts with `v20-` and includes
...
`v14-overfit-baseline-b512`, the script rewrites that prefix to `v20-` and
prints the normalized name before W&B init.

Run the flat MLP overfit sanity test:

```powershell
python research\inhouse_transformer\v20\train_mlp.py --device cuda --tickers USO --train-start-date 2024-01-22 --train-end-date 2024-01-22 --validation-start-date 2026-01-02 --validation-end-date 2026-01-02 --batch-size 256 --overfit-batches 4 --epochs 50 --eval-steps 25
```
...
```text
D:\TradingData\quant-research-workbench\market_data\models\inhouse_transformer\v20
```
