...
PROFILED_TRAINING_PATH = (
    "v22 full pretraining, v12-style per-masked-event MLP decoder, fixed-grid bottleneck, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Full v22 pretraining over the larger event sample cache. By default it "
...
def print_plan(values: dict[str, Any], train_shards, validation_shards, validation_batches: int, steps_per_epoch: int, argv: list[str]) -> None:
    ...
    print("v22 full pretraining over sample-cache shards", flush=True)
