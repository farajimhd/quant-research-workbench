# Masked Event Model v22

v22 is the event-token masked autoencoder variant that keeps v20's fixed-mask
...
under `research/masked_event_model/v22`.
...
the decoder sees the representation. v22 scatters the variable visible-token
...
v22 does not multiply semantic bit weights into the objective. The training loss
...
W&B project: June2026-event-token-mae-v22-mlp-decoder
```

By default v22 generates a fresh effective seed at process start, then records
...
```powershell
python research\masked_event_model\v22\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\v22\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\v22\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\v22\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\v22\train_10shard_long.py --fresh-start
```
...
```powershell
python research\masked_event_model\v22\train_full_pretrain_mixed_cache.py
```
...
```powershell
python research\masked_event_model\v22\train_full_pretrain_xonly_continue.py
```
...
```text
\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes\masked_event_model\v22\pretrain\v22-fullpretrain-mixedcache-fixedmask070-emb32-bs8192-lr4e4-3epochs-freshrng-from-step260352\checkpoints\checkpoint_latest.pt
```
...
Use this focused launcher to compare v22 against the v20/v21 capacity-test runs.
It runs only:
...
```powershell
python D:\TradingML\codes\masked_event_model\v22\research\masked_event_model\v22\run_grouped_bottleneck_probe.py
```

Dry run:

```powershell
python D:\TradingML\codes\masked_event_model\v22\research\masked_event_model\v22\run_grouped_bottleneck_probe.py --print-only
```

## Embedding Capacity And Bottleneck Precision Test

The v22 capacity launcher runs one controlled architecture variant. It first
...
```powershell
python D:\TradingML\codes\masked_event_model\v22\research\masked_event_model\v22\run_embedding_precision_probe.py
```
