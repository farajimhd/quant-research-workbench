...
$runtimeCandidate = Join-Path $scriptDir 'research\masked_event_model\v3\train_linear_probe.py'
...
Write-Host 'Starting masked event v3 linear probe at' (Get-Date -Format o)
Write-Host 'Runtime root:' $runtimeRoot
Write-Host 'Log:' $logPath
Write-Host 'W&B mode:' $wandbMode
$oldErrorActionPreference = $ErrorActionPreference
$ErrorActionPreference = 'Continue'
& "c:\Users\Mehdi\miniconda3\envs\ml4t\python.exe" -u (Join-Path $runtimeRoot 'research\masked_event_model\v3\train_linear_probe.py') `
  --pretrain-run-name "mem-v3-d384-emb256-e2-t8-d4-mask70-chunk500-b256-nov2025" `
