...
RUNTIME_ROOT = next(
    (
        ...
        if (parent / "research" / "masked_event_model" / "v3" / "train_linear_probe.py").exists()
    ),
...
from research.masked_event_model.v3 import train_linear_probe


PROBE_ARGS: dict[str, Any] = {
    "pretrain_run_name": "mem-v3-d384-emb256-e2-t8-d4-mask70-chunk500-b256-nov2025",
...
def print_help() -> None:
    print("masked event v3 linear-probe launcher")
    print()
    print("Run with defaults:")
    print("  python research\\masked_event_model\\v3\\run_linear_probe.py")
    print()
    print("Optional overrides:")
    print("  python research\\masked_event_model\\v3\\run_linear_probe.py --checkpoint-path D:\\path\\checkpoints\\checkpoint_latest.pt")
    print()
...
def run_probe(**overrides: Any) -> None:
    ...
    print("Starting masked event v3 linear probe from Python launcher.", flush=True)
    print(f"Runtime root: {RUNTIME_ROOT}", flush=True)
    print("Equivalent CLI:", flush=True)
    print("python -m research.masked_event_model.v3.train_linear_probe " + " ".join(argv), flush=True)
