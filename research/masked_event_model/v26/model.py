...
from research.masked_event_model.v26.config import ModelConfig
from research.masked_event_model.v26.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
# R = reusable branch tokens (CLS + header + eight 16-event groups).
...
class EventMAEOutput:
    ...
    # Shape: [B, 14, 8]. Raw decoder logits for each compact header bit.
    header_bit_logits: torch.Tensor
    # Shape: [B, 14]. Original uint8 chunk header bytes.
    target_header_uint8: torch.Tensor
    # Shape: [B, R, Z]. Exportable structured branch-token representation for downstream models.
    ...
    def header_bit_probs(self) -> torch.Tensor:
        # Input shape: [B, 14, 8]. Output shape: [B, 14, 8].
...
class FixedGridChunkBottleneck(nn.Module):
    """Create exported branch tokens from semantic fixed-grid groups.
    ...
    v26 differs from v25 after this scatter step: instead of merging the
    branches back into one vector, it exports the branch tokens directly:

    `[CLS, header, event_group_00_015, ..., event_group_112_127]`.

    The decoder attends to these ten tokens, and downstream models can consume
    the same structured representation without losing semantic slot identity in
    an early merge.
    """
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        self.branch_count = 2 + self.event_group_count
    ...
    def forward(
        ...
        if self.config.bottleneck_force_fp32 and fixed_tokens.is_cuda:
            ...
            with torch.amp.autocast("cuda", enabled=False):
                branch_tokens = self.project_fixed_token_grid(fixed_tokens.float())
            # Input shape: [B, R, Z]. Output shape: [B, R, Z].
            return self.chunk_embedding_output(branch_tokens)
        branch_tokens = self.project_fixed_token_grid(fixed_tokens)
        # Input shape: [B, R, Z]. Output shape: [B, R, Z].
        return self.chunk_embedding_output(branch_tokens)

    def project_fixed_token_grid(self, fixed_tokens: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, 130, D]. Output shape: [B, R, Z].
        branch_embeddings = [
        ...
        return torch.stack(branch_embeddings, dim=1)
...
class PerMaskedEventMlpDecoder(nn.Module):
    """Reconstruct masked event bytes by attending to exported branch tokens.

    Each masked event position forms a query. The reusable branch tokens form
    compact decoder memory. This keeps the decoder from seeing raw masked
    events, but avoids forcing all reconstruction through one merged vector.
    """

    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        super().__init__()
        self.branch_tokens_to_decoder_memory = nn.Sequential(
            OrderedDict(
                [
                    ("branch_token_embedding_to_decoder_width", nn.Linear(config.embedding_dim, config.d_model)),
                    ("branch_token_decoder_memory_gelu", nn.GELU()),
                    ("branch_token_decoder_memory_layer_norm", nn.LayerNorm(config.d_model)),
                ]
            )
        )
        self.masked_event_position_embedding_for_decoder = MaskedEventPositionEmbedding(events_per_chunk, config.d_model)
        self.masked_position_query_layer_norm = nn.LayerNorm(config.d_model)
        self.branch_memory_cross_attention = nn.MultiheadAttention(
            embed_dim=config.d_model,
            num_heads=config.n_heads,
            dropout=config.dropout,
            batch_first=True,
        )
        self.position_memory_cross_attention_layer_norm = nn.LayerNorm(config.d_model)
    ...
    def forward(self, chunk_embedding: torch.Tensor, masked_event_indices: torch.Tensor) -> torch.Tensor:
        if chunk_embedding.ndim != 3:
            raise ValueError(f"v26 decoder expects branch tokens [B,R,Z], got {tuple(chunk_embedding.shape)}")
        # Input shape: [B, R, Z]. Output shape: [B, R, D].
        branch_memory = self.branch_tokens_to_decoder_memory(chunk_embedding)
        # Input shape: [B, M]. Output shape: [B, M, D].
        masked_position_query = self.masked_position_query_layer_norm(
            self.masked_event_position_embedding_for_decoder(masked_event_indices)
        )
        # Input shapes: queries [B, M, D], memory [B, R, D]. Output shape: [B, M, D].
        attended_memory, _ = self.branch_memory_cross_attention(
            query=masked_position_query,
            key=branch_memory,
            value=branch_memory,
            need_weights=False,
        )
        # Input shape: [B, M, D]. Output shape: [B, M, D].
        decoder_input = self.position_memory_cross_attention_layer_norm(masked_position_query + attended_memory)
...
class HeaderMlpDecoder(nn.Module):
    """Reconstruct the compact chunk header from the reusable header branch.
    ...
    `[B, R, Z]` branch-token embedding to carry anchor-price, spread, count,
    flag, and timing information that downstream models may need.
    """

    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.chunk_embedding_to_header_bit_logits = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("header_decoder_to_14x8_bit_logits", nn.Linear(config.d_model, HEADER_BYTES * BITS_PER_BYTE)),
                ]
            )
        )
    ...
    def forward(self, chunk_embedding: torch.Tensor) -> torch.Tensor:
        if chunk_embedding.ndim != 3:
            raise ValueError(f"v26 header decoder expects branch tokens [B,R,Z], got {tuple(chunk_embedding.shape)}")
        # Input shape: [B, R, Z]. Output shape after selecting header branch: [B, Z].
        header_branch_token = chunk_embedding[:, 1, :]
        # Input shape: [B, Z]. Output shape before view: [B, 112].
        logits = self.chunk_embedding_to_header_bit_logits(header_branch_token)
        # Input shape: [B, 112]. Output shape: [B, 14, 8].
        return logits.view(logits.shape[0], HEADER_BYTES, BITS_PER_BYTE)


class EventChunkEncoder(nn.Module):
    """Standalone encoder that ends at reusable `[B, 10, embedding_dim]` branch tokens.
    ...
    def forward(self, header_uint8: torch.Tensor, events_uint8: torch.Tensor) -> torch.Tensor:
        ...
        # Input shapes: header [B, 14], events [B, E, 16]. Output shapes: encoded [B, 2 + E, D], embedding [B, R, Z].
    ...
    def encode_tokens(
        ...
        *,
        ...
        # Input shapes: encoded [B, T, D], indices [B, V or E]. Output shape: [B, R, Z].
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    compute and mirrors MAE-style training. The decoder then attends from masked
    position queries to the ten exported branch tokens, which prevents the
    decoder from reconstructing events by reading high-dimensional encoder
    states directly while preserving semantic memory slots.
    """
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v26 currently supports input_representation='bit' only")
    ...
    def forward(
        ...
        # Input shapes: header [B, 14], events [B, E, 16], masks [B, M]/[B, V]. Output shapes: [B, T, D], [B, R, Z], [B, M, 16].
        encoded_tokens, chunk_embedding, target_events = self.encode_tokens_for_training(
            header_uint8, events_uint8, masks, mask_config
        )
        # Input shape: [B, R, Z]. Output shapes: event [B, M, 16, 8], header [B, 14, 8].
        event_logits, header_logits = self.decode_from_chunk_embedding(chunk_embedding, masks)
    ...
    def decode_from_chunk_embedding(self, chunk_embedding: torch.Tensor, masks: EventMaskBatch) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        keeps the BF16 speed benefit. The disposable decoder attends from
        masked-position queries to `[B, R, Z]` branch-token memory.
        """

        if self.config.decoder_force_fp32 and chunk_embedding.is_cuda:
            with torch.amp.autocast("cuda", enabled=False):
                # Input shapes: chunk embedding [B, R, Z], masks [B, M].
                # Output shapes: event [B, M, 16, 8], header [B, 14, 8].
                return self.decode_reconstruction_targets(chunk_embedding.float(), masks)

        # Input shapes: chunk embedding [B, R, Z], masks [B, M].
        # Output shapes: event [B, M, 16, 8], header [B, 14, 8].
        return self.decode_reconstruction_targets(chunk_embedding, masks)
    ...
    def encode_tokens_for_training(
        ...
        # Input shapes: header [B, 14], events [B, E, 16], visible indices [B, V]. Output shapes: [B, 2 + V, D], [B, R, Z].
    ...
    def decode_masked_events(self, chunk_embedding: torch.Tensor, masks: EventMaskBatch) -> torch.Tensor:
        ...
        # after the exported branch-token embedding has already been formed.
        # Input shapes: chunk embedding [B, R, Z], masked indices [B, M]. Output shape: [B, M, 16, 8].
        return self.per_masked_event_mlp_decoder(chunk_embedding, masks.masked_event_indices)

    def decode_header(self, chunk_embedding: torch.Tensor) -> torch.Tensor:
        # Input shape: chunk embedding [B, R, Z]. Output shape: [B, 14, 8].
        return self.header_mlp_decoder(chunk_embedding)

    def decode_reconstruction_targets(self, chunk_embedding: torch.Tensor, masks: EventMaskBatch) -> tuple[torch.Tensor, torch.Tensor]:
        # Input shapes: chunk embedding [B, R, Z], masks [B, M].
        # Output shape: [B, M, 16, 8].
        event_logits = self.decode_masked_events(chunk_embedding, masks)
        # Input shape: chunk embedding [B, R, Z]. Output shape: [B, 14, 8].
        header_logits = self.decode_header(chunk_embedding)
