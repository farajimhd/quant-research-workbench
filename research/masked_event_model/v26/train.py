...
from research.masked_event_model.v26.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v26.losses import masked_event_bce_loss
from research.masked_event_model.v26.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v26.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v26.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v26 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v26; reconstruction uses branch-token cross-attention plus a per-masked-event MLP.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v26; reconstruction uses branch-token cross-attention plus a per-masked-event MLP.", flush=True)
    ...
    metric_logger = JsonlMetricLogger(run_paths.metrics_path, wandb_run, wandb_key_mapper=v26_wandb_metric_key)
    write_run_manifest(
...
def validation_metric_key(train_key: str) -> str:
    ...
    checkpoint monitors. W&B receives a second mapping in `v26_wandb_metric_key`
...
def v26_wandb_metric_key(metric_key: str) -> str:
    """Put v26 pretraining metrics into explicit W&B panel groups."""
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v26",
        "=" * 80,
        ...
        f"Chunk bottleneck: zero-initialized fixed grid [B, 130, d_model] -> branch tokens [B, 10, {details['embedding_dim']}]",
        f"Decoder: masked event position queries cross-attend to branch tokens -> per-masked-event logits",
        f"Output event bit logits: [B, masked_events, 16, 8]",
        f"Output header bit logits: [B, 14, 8]",
        f"Embedding: [B, 10, {details['embedding_dim']}]",
        "",
...
def build_model_mermaid(config: ExperimentConfig) -> str:
    ...
    GRID --> EMB[\"branch-token embedding<br/>B x 10 x {config.model.embedding_dim}\"]
    EMB --> MEM[\"project branch tokens<br/>B x 10 x d_model\"]
    EMB --> HDEC[\"header MLP decoder<br/>B x 14 x 8 logits\"]
    M --> MPOS[\"decoder-only masked event position embedding<br/>B x {masked} x d_model\"]
    MEM --> XATTN[\"cross-attention<br/>masked position queries attend branch memory\"]
    MPOS --> XATTN
    XATTN --> HEAD[\"per-masked-event MLP<br/>B x {masked} x 16 x 8 logits\"]
    HEAD --> LOSS[\"BCE on masked event bits\"]
    HDEC --> HLOSS[\"BCE on full header bits\"]
"""
...
class MaskedTrainingSummaryWrapper(torch.nn.Module):
    ...
    def forward(self, header_uint8: torch.Tensor, events_uint8: torch.Tensor) -> torch.Tensor:
        ...
        header_bit_logits = self.header_mlp_decoder(chunk_embedding)
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v26-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
...
def format_metrics(step: int, metrics: dict[str, float]) -> str:
    keys = [
        ...
        "pretrain/header_byte_exact_acc_pct",
