...
class MaskConfig:
    ...
    # v26 keeps v9's fixed-mask schedule: every training batch masks the same
...
class ModelConfig:
    ...
    # Accepted for launcher compatibility with earlier versions. v26 ignores
    # this value because reconstruction uses a per-masked-event MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v26 uses a branch-token cross-attention decoder. Its default keeps the
    # decoder inside the active AMP dtype. Pass --decoder-force-fp32 when
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v26-branch-token-decoder"
