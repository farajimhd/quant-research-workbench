...
from research.masked_event_model.v21.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v21.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v21.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v21 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v21; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v21; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    ...
    metric_logger = JsonlMetricLogger(run_paths.metrics_path, wandb_run, wandb_key_mapper=v21_wandb_metric_key)
    write_run_manifest(
...
def validation_metric_key(train_key: str) -> str:
    ...
    checkpoint monitors. W&B receives a second mapping in `v21_wandb_metric_key`
...
def v21_wandb_metric_key(metric_key: str) -> str:
    """Put v21 pretraining metrics into explicit W&B panel groups."""
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v21",
        "=" * 80,
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v21-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
