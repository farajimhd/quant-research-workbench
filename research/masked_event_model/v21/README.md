# Masked Event Model v21

v21 is the event-token masked autoencoder variant that keeps v20's fixed-mask
...
under `research/masked_event_model/v21`.
...
the decoder sees the representation. v21 scatters the variable visible-token
...
The v21 bottleneck tests whether preserving coarse semantic groups helps the
...
v21 does not multiply semantic bit weights into the objective. The training loss
...
W&B project: June2026-event-token-mae-v21-mlp-decoder
```

By default v21 generates a fresh effective seed at process start, then records
...
```powershell
python research\masked_event_model\v21\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\v21\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\v21\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\v21\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\v21\train_10shard_long.py --fresh-start
```
...
```powershell
python research\masked_event_model\v21\train_full_pretrain_mixed_cache.py
```
...
```powershell
python research\masked_event_model\v21\train_full_pretrain_xonly_continue.py
```
...
```text
\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes\masked_event_model\v21\pretrain\v21-fullpretrain-mixedcache-fixedmask070-emb32-bs8192-lr4e4-3epochs-freshrng-from-step260352\checkpoints\checkpoint_latest.pt
```
...
```powershell
python D:\TradingML\codes\masked_event_model\v21\research\masked_event_model\v21\run_embedding_precision_probe.py
```
...
By default this v21 launcher uses BF16 AMP and keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\v21\train_10shard_long.py --fresh-start --run-name v21-fixedgrid-fp16decoder-fixedmask070-emb32-bs8192-10shards --amp-dtype fp16
```
