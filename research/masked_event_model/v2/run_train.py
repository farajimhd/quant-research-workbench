...
RUNTIME_ROOT = next(
    (
        ...
        if (parent / "research" / "masked_event_model" / "v2" / "train.py").exists()
    ),
...
from research.masked_event_model.v2 import train
...
def print_help() -> None:
    print("masked event v2 Python launcher")
    print()
    print("Run with the defaults embedded in this file:")
    print("  python research\\masked_event_model\\v2\\run_train.py")
    print()
    print("Optional one-off overrides:")
    print("  python research\\masked_event_model\\v2\\run_train.py --batch-size 512 --dry-run")
    print()
...
def run_train(**overrides: Any) -> None:
    ...
    print("Starting masked event v2 training from Python launcher.", flush=True)
    print(f"Runtime root: {RUNTIME_ROOT}", flush=True)
    print("Equivalent CLI:", flush=True)
    print("python -m research.masked_event_model.v2.train " + " ".join(argv), flush=True)
    train.main(argv)
