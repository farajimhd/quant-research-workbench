...
from research.masked_event_model.v13.config import ModelConfig
from research.masked_event_model.v13.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class ChunkEmbeddingToDecoderMemory(nn.Module):
    ...
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.chunk_embedding_to_decoder_width = nn.Sequential(
            OrderedDict(
                [
                    ("chunk_embedding_to_decoder_width", nn.Linear(config.embedding_dim, config.d_model)),
                    ("chunk_embedding_decoder_memory_gelu", nn.GELU()),
                    ("chunk_embedding_decoder_memory_layer_norm", nn.LayerNorm(config.d_model)),
                ]
            )
        )
        ...
        self.masked_query_feed_forward = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("masked_query_ffn_output_gelu", nn.GELU()),
                    ("masked_query_ffn_output_layer_norm", nn.LayerNorm(config.d_model)),
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v13 currently supports input_representation='bit' only")
