# Masked Event Model v13

v13 is based on v9 and keeps the same fixed-mask, unweighted-loss,
masked-query cross-attention decoder setup. Its only intentional architecture
change is adding activations after custom linear bridge/projection layers where
v9 left them linear-only. It is version-local: model, masking, loss, progress,
training, and profiling code live under `research/masked_event_model/v13`.
...
v13 is the activation-augmented v9 variant. Every batch masks exactly 70% of the 128
...
bytes can also receive low-rate bit corruption for robustness.
...
The decoder FFN also applies GELU and LayerNorm after its contraction back to
`d_model`. The final `16 x 8` bit projection intentionally remains linear
because BCE-with-logits expects unconstrained raw logits.

Loss is ordinary binary cross entropy with logits over masked event bits only.
v13 does not multiply semantic bit weights into the objective. The training loss
...
W&B project: June2026-event-token-mae-v13-activations
```
...
```powershell
python research\masked_event_model\v13\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\v13\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\v13\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\v13\train_10shard_long.py --fresh-start
```
