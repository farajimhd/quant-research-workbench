...
class ChunkEmbeddingBottleneck(nn.Module):
    ...
    def forward(self, encoded_tokens: torch.Tensor) -> torch.Tensor:
        ...
        # Input shape: [B, T, D]. Output shape after squeeze: [B, T].
        attention_logits = self.encoded_token_to_attention_logit(encoded_tokens).squeeze(-1)
        # Input shape: [B, T]. Output shape: [B, T].
        attention_weights = torch.softmax(attention_logits, dim=1)
        # Input shapes: token embeddings [B, T, Z], weights [B, T]. Output shape after weighted sum: [B, Z].
...
class EventChunkEncoder(nn.Module):
    ...
    encoding, and the attention-pooling chunk embedding bottleneck. It
    deliberately has no decoder, no masked-event query tokens, and no
    reconstruction head, so it can be exported and loaded by downstream models
    without pulling in pretraining machinery.
    """
