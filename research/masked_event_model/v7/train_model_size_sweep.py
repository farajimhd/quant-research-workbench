...
from research.masked_event_model.v7.config import TrainConfig  # noqa: E402
from research.masked_event_model.v7.train import JOB_TYPE, MODEL_FAMILY, MODEL_VERSION  # noqa: E402
...
def main() -> None:
    ...
    print("=" * 96, flush=True)
    print("v7 event-token MAE model-size profile sweep", flush=True)
...
def build_train_command(args: argparse.Namespace, run: SweepRun) -> list[str]:
    ...
    command = [
        sys.executable,
        "-u",
        str(REPO_ROOT / "research" / "masked_event_model" / "v7" / "train.py"),
        "--data-source",
        "sample_cache",
        "--sample-cache-root",
        args.cache_root,
