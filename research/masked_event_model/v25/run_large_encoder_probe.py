...
DEFAULT_PRETRAIN_OUTPUT_ROOT = Path(r"D:\TradingML\runtimes\masked_event_model\\v25\pretrain_capacity_tests")
...
DEFAULT_RUN_PREFIX = "v25-largeencoder-1shard5ep"
DEFAULT_D_MODEL = 384
DEFAULT_N_HEADS = 12
DEFAULT_ENCODER_LAYERS = 12
DEFAULT_DECODER_LAYERS = 5
DEFAULT_FFN_MULT = 4
...
def main() -> int:
    ...
    print("v25 large-encoder embedding benchmark", flush=True)
    ...
    print(
        ...
        f"decoder_layers={args.decoder_layers} ffn_mult={args.ffn_mult} dropout={args.dropout}",
        flush=True,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sequentially pretrain v25 large-encoder emb32 and emb128 variants on one x-only shard, "
            "then run the same temporal v1 linear probe for each resulting checkpoint. "
            "This script is self-contained so active v25 grouped/probe launchers are not changed."
        )
    )
