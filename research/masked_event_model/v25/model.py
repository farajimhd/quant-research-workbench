...
from research.masked_event_model.v25.config import ModelConfig
from research.masked_event_model.v25.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class FixedGridChunkBottleneck(nn.Module):
    ...
    v25 differs from v20 after this scatter step: instead of flattening all 130
...
class HeaderMlpDecoder(nn.Module):
    """Reconstruct the compact chunk header from only the reusable chunk embedding.
    ...
    timing information that downstream models may need.
    """
    ...
    def forward(self, chunk_embedding: torch.Tensor) -> torch.Tensor:
        ...
        logits = self.chunk_embedding_to_header_bit_logits(chunk_embedding)
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v25 currently supports input_representation='bit' only")
    ...
    def forward(
        ...
        # Input shape: [B, Z]. Output shapes: event [B, M, 16, 8], header [B, 14, 8].
    ...
    def decode_header(self, chunk_embedding: torch.Tensor) -> torch.Tensor:
        # Input shape: chunk embedding [B, Z]. Output shape: [B, 14, 8].
