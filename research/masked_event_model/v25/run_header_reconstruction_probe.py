...
DEFAULT_RUN_PREFIX = "v25-headerrecon-emb32-1shard5ep"
...
def main() -> int:
    ...
    print("v25 header-reconstruction emb32 benchmark", flush=True)
    print("pretrain: one x-only shard, header+masked-event reconstruction, 5 epochs by default", flush=True)
    print("probe: temporal v1 labeled-cache linear probe from the resulting v25 encoder checkpoint", flush=True)
    print("=" * 104, flush=True)
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description=(
            "Run the v25 header-reconstruction experiment: emb32 BF16 "
