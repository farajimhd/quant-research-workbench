...
class MaskConfig:
    ...
    # v19 keeps v9's fixed-mask schedule: every training batch masks the same
...
class ModelConfig:
    ...
    # Accepted for launcher compatibility with earlier versions. v19 ignores
    # this value because reconstruction uses a per-masked-event MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v19 is the explicit low-cost MLP decoder experiment, so its default keeps
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v19-mlp-decoder"
