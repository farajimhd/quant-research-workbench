# Masked Event Model v19

v19 is the event-token masked autoencoder variant that keeps the v12 fixed-mask
training path and cheap per-masked-event MLP decoder, but replaces v12's
mean-only chunk bottleneck with a richer cheap summary bottleneck. It is
version-local: model, masking, loss, progress, training, and profiling code live
under `research/masked_event_model/v19`.
...
the decoder sees the representation. v19 concatenates five cheap summaries and
maps them through a small MLP:

```text
encoded tokens [B, token_count, d_model]
  -> mean(tokens) [B, d_model]
  -> max(tokens) [B, d_model]
  -> last_event [B, d_model]
  -> header [B, d_model]
  -> CLS [B, d_model]
  -> concat [B, 5 * d_model]
  -> MLP [B, embedding_dim]
...
gives the embedding direct access to global average state, extreme activations,
the most recent visible event, header context, and the transformer's CLS
summary. The decoder is intentionally small and independent per masked event:
...
v19 does not multiply semantic bit weights into the objective. The training loss
...
W&B project: June2026-event-token-mae-v19-mlp-decoder
```
...
```powershell
python research\masked_event_model\v19\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\v19\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\v19\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\v19\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\v19\train_10shard_long.py --fresh-start
```

By default this v19 launcher keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\v19\train_10shard_long.py --fresh-start --run-name v19-meanmaxlastpool-fp16decoder-fixedmask070-emb32-bs4096-10shards --amp-dtype fp16
```
