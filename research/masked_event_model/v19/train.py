...
from research.masked_event_model.v19.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v19.losses import masked_event_bce_loss
from research.masked_event_model.v19.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v19.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v19.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v19 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v19; the MLP decoder only processes masked events.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v19; reconstruction uses a per-masked-event MLP decoder.", flush=True)
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v19",
        "=" * 80,
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v19-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
