...
from research.masked_event_model.v17.config import ModelConfig
from research.masked_event_model.v17.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class ChunkEmbeddingBottleneck(nn.Module):
    ...
    bottleneck now projects every encoded token to embedding space and max-pools
    all projected tokens, so the strongest header/event activations contribute to the
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v17 currently supports input_representation='bit' only")
