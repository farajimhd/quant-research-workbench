...
class MaskConfig:
    ...
    # v23 keeps v9's fixed-mask schedule: every training batch masks the same
    # 70% of event tokens while the decoder architecture changes to a residual
...
class ModelConfig:
    ...
    # Accepted for launcher compatibility with earlier versions. v23 ignores
    # this value because reconstruction uses a per-masked-event residual MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v23 keeps the decoder inside the active AMP dtype by default. Pass
    # --decoder-force-fp32 when comparing against the conservative FP32 decoder
    # path.
    ...
    bottleneck_branch_embedding_dim: int = 8
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v23-residual-decoder"
