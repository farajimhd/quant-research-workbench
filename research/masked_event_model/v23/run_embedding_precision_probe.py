...
DEFAULT_PRETRAIN_OUTPUT_ROOT = Path(r"D:\TradingML\runtimes\masked_event_model\v23\pretrain_capacity_tests")
...
DEFAULT_RUN_PREFIX = "v23-capacity-1shard5ep"
...
def main() -> int:
    ...
    print("v23 grouped bottleneck residual-decoder probe", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Pretrain the v23 grouped branch-concat bottleneck with residual decoder on one x-only shard, "
            "then run the same temporal v1 linear probe for the resulting checkpoint."
        )
    )
