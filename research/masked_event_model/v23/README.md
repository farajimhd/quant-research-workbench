# Masked Event Model v23

v23 is the event-token masked autoencoder variant that keeps v20's fixed-mask
training path and cheap MLP decoder, but changes the fixed-grid chunk
bottleneck from one global flattened projection into semantic branch MLPs. It is
version-local: model, masking, loss, progress, training, and profiling code live
under `research/masked_event_model/v23`.
...
the decoder sees the representation. v23 scatters the variable visible-token
...
The v23 bottleneck keeps v22's branch-concat layout at the original smaller
branch width and tests whether a stronger residual decoder improves the
pretraining signal. The fixed grid is split into 10 nonlinear branches. Each
branch has two nonlinear layers: input -> 64 hidden features -> 8 output
features. The final chunk embedding is the direct concatenation of the 10 branch
outputs, so its width is 80.
...
The decoder keeps the same input/output contract as the v12 low-cost path but
uses two residual MLP blocks before the final bit-logit head. It receives the
...
chunk_context + masked_event_position_embedding
  -> two residual per-masked-event MLP blocks
...
v23 does not multiply semantic bit weights into the objective. The training loss
...
W&B project: June2026-event-token-mae-v23-residual-decoder
```

By default v23 generates a fresh effective seed at process start, then records
...
```powershell
python research\masked_event_model\v23\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\v23\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\v23\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\v23\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\v23\train_10shard_long.py --fresh-start
```
...
```powershell
python research\masked_event_model\v23\train_full_pretrain_mixed_cache.py
```
...
```powershell
python research\masked_event_model\v23\train_full_pretrain_xonly_continue.py
```
...
```text
\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes\masked_event_model\v23\pretrain\v23-fullpretrain-mixedcache-fixedmask070-emb80-bs8192-lr4e4-3epochs-freshrng-from-step260352\checkpoints\checkpoint_latest.pt
```
...
Use this focused launcher to compare v23 against the v20/v21 capacity-test runs.
It runs only:

```text
v23 branch-concat bottleneck with residual decoder
embedding_dim=80
AMP dtype=bf16
batch_size=4096
...
```powershell
python D:\TradingML\codes\masked_event_model\v23\research\masked_event_model\v23\run_grouped_bottleneck_probe.py
```

Dry run:

```powershell
python D:\TradingML\codes\masked_event_model\v23\research\masked_event_model\v23\run_grouped_bottleneck_probe.py --print-only
```

## Embedding Capacity And Bottleneck Precision Test

The v23 capacity launcher runs one controlled architecture variant. It first
pretrains from scratch on one x-only pretraining shard for 5 epochs at
`batch_size=4096`, then immediately runs the same temporal v1 linear probe on
the labeled cache:

```text
emb80 bf16 branch-concat bottleneck with residual decoder
```
...
```powershell
python D:\TradingML\codes\masked_event_model\v23\research\masked_event_model\v23\run_embedding_precision_probe.py
```

Use `--print-only` first to inspect all generated pretrain and linear-probe
commands. The only valid v23 capacity variant is `emb80-bf16`.

By default this v23 launcher uses BF16 AMP and keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\v23\train_10shard_long.py --fresh-start --run-name v23-fixedgrid-fp16decoder-fixedmask070-emb80-bs8192-10shards --amp-dtype fp16
```
