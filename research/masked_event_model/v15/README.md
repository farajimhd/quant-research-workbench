# Masked Event Model v15

v15 is the byte-input ablation derived from v12. It keeps the fixed-mask
training path and cheap per-masked-event MLP decoder, but changes the encoder
input from expanded bits to normalized byte values. It is version-local: model,
masking, loss, progress, training, and profiling code live under
`research/masked_event_model/v15`.
...
The encoder input transform is:

```text
header_uint8 [B, 14]
  -> header_scaled = header / 127.5 - 1.0
  -> header features [B, 14]

visible_events_uint8 [B, visible_events, 16]
  -> per sample and byte-position z-score across visible_events
  -> event features [B, visible_events, 16]
```

Normalization statistics are computed only from the visible events that enter
the encoder. Masked target events are not included in the mean or variance, so
the autoencoder cannot leak masked bytes through normalization.
...
v15 does not multiply semantic bit weights into the objective. The training loss
...
input representation: byte_zscore
...
W&B project: June2026-event-token-mae-v15-byte-zscore
```
...
```powershell
python research\masked_event_model\v15\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\v15\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\v15\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\v15\train_10shard_long.py --fresh-start
```

By default this v15 launcher keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\v15\train_10shard_long.py --fresh-start --run-name v15-bytezscore-mlpdecoder-fp16decoder-fixedmask070-emb32-bs4096-10shards --amp-dtype fp16
```
