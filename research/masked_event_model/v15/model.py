...
from research.masked_event_model.v15.config import ModelConfig
from research.masked_event_model.v15.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class UInt8HeaderBytesToScaledFeatures(nn.Module):
    """Scale the single chunk header from uint8 bytes to centered continuous features.

    Header bytes do not have an event axis, so per-sample z-scoring would be
    ill-defined. v15 keeps the transform deterministic and leakage-free by
    mapping each byte from `[0, 255]` to approximately `[-1, 1]`.
    """

    def forward(self, header_uint8: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, 14]. Output shape: [B, 14] in [-1, 1].
        return header_uint8.to(torch.float32).div(127.5).sub(1.0)


class UInt8EventBytesToWindowZScoreFeatures(nn.Module):
    """Normalize visible event bytes per sample and per byte position.

    The statistics are computed across the visible event axis only, so masked
    events do not leak into the encoder through normalization means or variances.
    At production time all events are visible, so the same operation normalizes
    across the full event chunk.
    """
    ...
    def forward(self, events_uint8: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, V, 16]. Output shape before normalization: [B, V, 16].
        events_float = events_uint8.to(torch.float32)
        # Input shape: [B, V, 16]. Output shape: [B, 1, 16].
        mean = events_float.mean(dim=1, keepdim=True)
        # Input shapes: events/mean [B, V, 16]/[B, 1, 16]. Output shape: [B, 1, 16].
        variance = events_float.sub(mean).square().mean(dim=1, keepdim=True)
        # Input shapes: centered [B, V, 16], variance [B, 1, 16]. Output shape: [B, V, 16].
        return events_float.sub(mean).mul(torch.rsqrt(variance.add(self.eps)))
...
class HeaderTokenEncoder(nn.Module):
    ...
    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.header_bytes_to_scaled_features = UInt8HeaderBytesToScaledFeatures()
        self.header_scaled_bytes_to_model_token = nn.Sequential(
            OrderedDict(
                [
                    ("header_14_scaled_bytes_to_model_width", nn.Linear(HEADER_BYTES, config.d_model)),
    ...
    def forward(self, header_uint8: torch.Tensor) -> torch.Tensor:
        ...
        header_features = self.header_bytes_to_scaled_features(header_uint8)
        # Input shape: [B, 14]. Output shape after unsqueeze: [B, 1, D].
        header_token = self.header_scaled_bytes_to_model_token(header_features).unsqueeze(1)
...
class EventTokenEncoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        super().__init__()
        self.event_bytes_to_window_zscore_features = UInt8EventBytesToWindowZScoreFeatures()
        self.event_zscore_bytes_to_model_tokens = nn.Sequential(
            OrderedDict(
                [
                    ("event_16_zscore_bytes_to_model_width", nn.Linear(EVENT_BYTES, config.d_model)),
    ...
    def forward(self, visible_events_uint8: torch.Tensor, visible_event_indices: torch.Tensor) -> torch.Tensor:
        ...
        event_features = self.event_bytes_to_window_zscore_features(visible_events_uint8)
        # Input shape: [B, V, 16]. Output shape: [B, V, D].
        event_tokens = self.event_zscore_bytes_to_model_tokens(event_features)
...
class ChunkEmbeddingBottleneck(nn.Module):
    ...
    Earlier variants used only the projected CLS token as the chunk embedding. That
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "byte_zscore":
            raise ValueError("v15 currently supports input_representation='byte_zscore' only")
