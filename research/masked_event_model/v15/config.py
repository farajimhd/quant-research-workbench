...
class MaskConfig:
    ...
    # v15 keeps v9's fixed-mask schedule: every training batch masks the same
...
@dataclass(slots=True)
class ModelConfig:
    input_representation: str = "byte_zscore"
    d_byte: int = 24
    d_model: int = 128
    embedding_dim: int = 32
    n_heads: int = 4
    encoder_layers: int = 6
    # Accepted for launcher compatibility with earlier versions. v15 ignores
    # this value because reconstruction uses a per-masked-event MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v15 is the explicit low-cost MLP decoder experiment, so its default keeps
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v15-byte-zscore"
