...
from research.masked_event_model.v15.config import TrainConfig  # noqa: E402
from research.masked_event_model.v15.train import JOB_TYPE, MODEL_FAMILY, MODEL_VERSION  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Sequentially profile v15 medium/large model sizes with 32-dim embeddings.")
    ...
    parser.add_argument("--run-prefix", default="v15-eventmae-size-sweep")
...
def main() -> None:
    ...
    print("=" * 96, flush=True)
    print("v15 event-token MAE model-size profile sweep", flush=True)
...
def normalize_combo(combo: tuple[str, int, int] | tuple[str, int, int, str] | tuple[str, int, int, str, int], default_decoder_chunk_size: int) -> tuple[str, int, int, str, int]:
    if len(combo) == 3:
        ...
        return model_size, embedding_dim, batch_size, "byte_zscore", 0
    if len(combo) == 4:
    ...
    if input_representation != "byte_zscore":
...
def build_explicit_runs(args: argparse.Namespace, combos: tuple[tuple[str, int, int] | tuple[str, int, int, str] | tuple[str, int, int, str, int], ...]) -> list[SweepRun]:
    ...
    for index, combo in enumerate(combos, start=1):
        ...
        representation_suffix = "-eventmae-bytezscore"
        decoder_suffix = ""
    ...
    for model_size in model_sizes:
        for embedding_dim in embedding_dims:
            for batch_size in batch_sizes:
                index += 1
                run_name = f"{args.run_prefix}-{model_size}-eventmae-bytezscore-emb{embedding_dim}-bs{batch_size}"
                run_root = resolve_run_root(args, run_name)
                runs.append(
                    SweepRun(
                        index=index,
                        total=total,
                        model_size=model_size,
                        input_representation="byte_zscore",
...
def build_train_command(args: argparse.Namespace, run: SweepRun) -> list[str]:
    ...
    command = [
        sys.executable,
        "-u",
        str(REPO_ROOT / "research" / "masked_event_model" / "v15" / "train.py"),
        "--data-source",
        "sample_cache",
        "--sample-cache-root",
        args.cache_root,
