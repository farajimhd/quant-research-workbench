...
from research.masked_event_model.v24.config import ModelConfig
from research.masked_event_model.v24.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class FixedGridChunkBottleneck(nn.Module):
    """Create the exported embedding from a fixed CLS/header/event grid.
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        self.fixed_grid_to_chunk_embedding = nn.Sequential(
            OrderedDict(
                [
                    ("fixed_grid_flattened_to_hidden_width", nn.Linear(self.fixed_token_count * config.d_model, config.d_model * 2)),
                    ("fixed_grid_hidden_gelu", nn.GELU()),
                    ("fixed_grid_hidden_layer_norm", nn.LayerNorm(config.d_model * 2)),
                    ("fixed_grid_hidden_to_embedding_width", nn.Linear(config.d_model * 2, config.embedding_dim)),
                    ("fixed_grid_embedding_gelu", nn.GELU()),
                    ("fixed_grid_embedding_layer_norm", nn.LayerNorm(config.embedding_dim)),
                ]
            )
        )
...
class ResidualDecoderBlock(nn.Module):
    """Small residual MLP block used only by the disposable reconstruction decoder."""

    def __init__(self, *, d_model: int, ff_dim: int, dropout: float) -> None:
        super().__init__()
        self.position_memory_residual_norm = nn.LayerNorm(d_model)
        self.position_memory_residual_mlp = nn.Sequential(
            OrderedDict(
                [
                    ("position_memory_expand_to_ffn_width", nn.Linear(d_model, ff_dim)),
                    ("position_memory_gelu", nn.GELU()),
                    ("position_memory_dropout_after_gelu", nn.Dropout(dropout)),
                    ("position_memory_contract_to_model_width", nn.Linear(ff_dim, d_model)),
                    ("position_memory_dropout_after_contract", nn.Dropout(dropout)),
                ]
            )
        )

    def forward(self, position_memory: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, M, D]. Output shape: [B, M, D].
        return position_memory + self.position_memory_residual_mlp(self.position_memory_residual_norm(position_memory))
...
class PerMaskedEventResidualMlpDecoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        self.position_memory_residual_mlp_decoder = nn.Sequential(
            OrderedDict(
                (
                    (
                        f"position_memory_residual_block_{block_idx:02d}",
                        ResidualDecoderBlock(d_model=config.d_model, ff_dim=config.ff_dim, dropout=config.dropout),
                    )
                    for block_idx in range(self.residual_block_count)
                )
            )
        )
        self.position_memory_to_bit_logits = nn.Sequential(
            OrderedDict(
                [
                    ...
                    # Deliberately no activation here: BCE-with-logits expects unconstrained raw bit logits.
    ...
    def forward(self, chunk_embedding: torch.Tensor, masked_event_indices: torch.Tensor) -> torch.Tensor:
        ...
        # Input shape: [B, M, D]. Output shape: [B, M, D].
        residual_memory = self.position_memory_residual_mlp_decoder(decoder_input)
        # Input shape: [B, M, D]. Output shape: [B, M, 128].
        logits = self.position_memory_to_bit_logits(residual_memory)
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v24 currently supports input_representation='bit' only")
