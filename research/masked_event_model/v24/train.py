...
from research.masked_event_model.v24.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v24.losses import masked_event_bce_loss
from research.masked_event_model.v24.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v24.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v24.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v24 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v24; reconstruction uses a residual per-masked-event MLP decoder.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v24; reconstruction uses a residual per-masked-event MLP decoder.", flush=True)
    ...
    metric_logger = JsonlMetricLogger(run_paths.metrics_path, wandb_run, wandb_key_mapper=v24_wandb_metric_key)
    write_run_manifest(
...
def validation_metric_key(train_key: str) -> str:
    ...
    checkpoint monitors. W&B receives a second mapping in `v24_wandb_metric_key`
...
def v24_wandb_metric_key(metric_key: str) -> str:
    """Put v24 pretraining metrics into explicit W&B panel groups."""
...
def save_model_artifacts(
    ...
    details = {
        ...
        "decoder": "per_masked_event_residual_mlp",
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v24",
        "=" * 80,
        ...
        f"Decoder: projected chunk embedding + masked event position embedding -> residual per-masked-event logits",
...
def build_model_mermaid(config: ExperimentConfig) -> str:
    ...
    MPOS --> ADD
    ADD --> HEAD[\"residual per-masked-event MLP<br/>B x {masked} x 16 x 8 logits\"]
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v24-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
