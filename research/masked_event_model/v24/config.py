...
class MaskConfig:
    ...
    # v24 keeps v9's fixed-mask schedule: every training batch masks the same
...
class ModelConfig:
    ...
    # Accepted for launcher compatibility with earlier versions. v24 ignores
    # this value because reconstruction uses a residual per-masked-event MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v24 is the explicit residual MLP decoder experiment, so its default keeps
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v24-residual-decoder"
