# Masked Event Model v29

v29 is v28 with one isolated input-representation change: byte inputs are
unpacked to literal `0/1` bit features instead of centered `-1/+1` bit
features. It keeps the same main transformer interface as v28:

```text
[CLS] + one pooled header token + visible event tokens
```

The header-mask architecture/objective is otherwise identical to v28: each
batch removes a shared subset of compact-header bit positions before the one
header token is pooled. The header decoder reconstructs only those removed
header bits from the exported chunk embedding. Header bits are not added as
separate transformer tokens.

The training inputs are:

```text
header_uint8: [B, 14]       50% header bits visible by default
...
existing `min_masked_events` clamp. Header masking is batch-shared: all samples
in the batch use the same visible/masked header bit positions, which keeps
tensor shapes rectangular and lets the masked header positions carry learned
decoder position embeddings. Header and event inputs are read as `0/1` bit
features; masked header bits are removed before pooling, not zero-filled.
Visible event bytes can still receive low-rate bit corruption for robustness.

All encoded tokens pass through the exported chunk embedding bottleneck before
the decoder sees the representation. v29 scatters the variable visible-token
...
The v29 bottleneck tests whether preserving coarse semantic groups helps the
...
The header decoder predicts only the removed compact-header bits, including
time-related header fields when those bit positions are selected:
...
v29 does not multiply semantic bit weights into the objective. The event loss
uses the PyTorch default mean reduction over all masked event bits, and the
header loss uses the same mean reduction over the removed header bits. Semantic
...
which sees all 112 header bits pooled into the one header token and all 128
events:
...
input bit values: 0/1
header bit corruption: disabled
...
W&B project: June2026-event-token-mae-v29-header-mask
```

By default v29 generates a fresh effective seed at process start, then records
...
```powershell
python research\masked_event_model\\v29\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\\v29\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\\v29\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v29\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v29\train_10shard_long.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v29\train_full_pretrain_mixed_cache.py
```
...
The full dual-cache v29 header-reconstruction pretrain launcher starts a fresh
...
```powershell
python research\masked_event_model\\v29\train_full_pretrain_dual_cache.py
```
...
```powershell
python research\masked_event_model\\v29\train_full_pretrain_xonly_continue.py
```
...
```text
\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes\masked_event_model\\v29\pretrain\v29-fullpretrain-mixedcache-fixedmask070-emb32-bs8192-lr4e4-3epochs-freshrng-from-step260352\checkpoints\checkpoint_latest.pt
```
...
Use this focused launcher to compare v29 against the v20 capacity-test runs
without running the whole four-variant sweep. It runs only:

```text
v29 grouped bottleneck
embedding_dim=32
AMP dtype=bf16
batch_size=4096
...
```powershell
python D:\TradingML\codes\masked_event_model\\v29\research\masked_event_model\\v29\run_grouped_bottleneck_probe.py
```

Dry run:

```powershell
python D:\TradingML\codes\masked_event_model\\v29\research\masked_event_model\\v29\run_grouped_bottleneck_probe.py --print-only
```
...
```powershell
python D:\TradingML\codes\masked_event_model\\v29\research\masked_event_model\\v29\run_embedding_precision_probe.py
```
...
By default this v29 launcher uses BF16 AMP and keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\\v29\train_10shard_long.py --fresh-start --run-name v29-fixedgrid-fp16decoder-fixedmask070-emb32-bs8192-10shards --amp-dtype fp16
```
