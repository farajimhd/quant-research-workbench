...
from research.masked_event_model.v29.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v29.losses import masked_event_bce_loss
from research.masked_event_model.v29.masking import EventMaskBatch, HeaderBitMaskBatch, build_event_masks, build_header_bit_masks
from research.masked_event_model.v29.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v29.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v29 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v29; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v29; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    ...
    metric_logger = JsonlMetricLogger(run_paths.metrics_path, wandb_run, wandb_key_mapper=v29_wandb_metric_key)
    write_run_manifest(
...
def validation_metric_key(train_key: str) -> str:
    ...
    checkpoint monitors. W&B receives a second mapping in `v29_wandb_metric_key`
...
def v29_wandb_metric_key(metric_key: str) -> str:
    """Put v29 pretraining metrics into explicit W&B panel groups."""
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v29",
        "=" * 80,
...
def build_model_mermaid(config: ExperimentConfig) -> str:
    ...
    header_bits = 14 * 8
    masked_header_bits = max(config.masks.min_masked_header_bits, int(round(header_bits * config.masks.header_mask_ratio)))
    visible_header_bits = header_bits - masked_header_bits
    return f"""flowchart TD
    H[\"header_uint8<br/>B x 14\"] --> HM[\"batch-shared header bit mask<br/>{visible_header_bits} visible / {masked_header_bits} masked\"]
    HM --> HV[\"visible header bits<br/>B x {visible_header_bits}\"]
    ...
    HV --> HP[\"visible header bit value + position pooling<br/>one d_model token\"]
    ...
    EMB --> HDEC[\"masked-header-bit MLP decoder<br/>B x {masked_header_bits} logits\"]
    HM --> HPOS[\"decoder-only masked header bit position embedding\"]
    HPOS --> HDEC
...
class MaskedTrainingSummaryWrapper(torch.nn.Module):
    ...
    def forward(self, header_uint8: torch.Tensor, events_uint8: torch.Tensor) -> torch.Tensor:
        ...
        total_header_bits = 14 * 8
        masked_header_count = max(1, int(round(total_header_bits * 0.50)))
        visible_header_count = total_header_bits - masked_header_count
        visible_header_bits = torch.arange(visible_header_count, device=events_uint8.device)
        masked_header_bits = torch.arange(visible_header_count, total_header_bits, device=events_uint8.device)
        masks = EventMaskBatch(
        ...
        header_masks = HeaderBitMaskBatch(
            visible_header_bit_indices=visible_header_bits,
            masked_header_bit_indices=masked_header_bits,
            visible_count=visible_header_count,
            masked_count=masked_header_count,
            header_bit_count=total_header_bits,
            requested_mask_ratio=float(masked_header_count / max(1, total_header_bits)),
            actual_mask_ratio=float(masked_header_count / max(1, total_header_bits)),
        )
        selected_events_uint8, selected_event_indices = self.visible_event_token_selector(events_uint8, masks.visible_event_indices)
        header_token = self.header_token_encoder(header_uint8, header_masks.visible_header_bit_indices)
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v29-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
