...
from research.masked_event_model.v29.config import ModelConfig
from research.masked_event_model.v29.masking import (
    EventMaskBatch,
    HeaderBitMaskBatch,
...
class EventMAEOutput:
    ...
    # Shape: [B, HM]. Raw decoder logits for masked compact-header bits.
    header_bit_logits: torch.Tensor
    # Shape: [HM]. Original compact-header bit positions reconstructed by the decoder.
    masked_header_bit_indices: torch.Tensor
    # Shape: [B, HM]. Original 0/1 compact-header bits gathered at masked positions.
    ...
    # Scalar. Effective masked-header-bit ratio after integer rounding.
    header_actual_mask_ratio: float
    # Scalar. Header mask ratio requested by the active masking policy.
...
def build_signed_bit_lookup() -> torch.Tensor:
    """Create a `[256, 8]` little-endian 0/1 lookup for byte inputs.
    ...
    # v29's isolated change: keep input bits in {0, 1}; do not center them to {-1, +1}.
    return bits


def unpack_bits_01(values_uint8: torch.Tensor, *, flatten_from_byte_axis: bool) -> torch.Tensor:
    """Unpack uint8 bytes into little-endian 0/1 bits for reconstruction targets."""
    ...
    if flatten_from_byte_axis:
        return bits.flatten(1)
    return bits.flatten(2)


class UInt8BytesToSignedBitFeatures(nn.Module):
    """Convert packed bytes into 0/1 bit features that the linear layers can read.
    ...
    the eight binary decisions inside that byte. v29 keeps those decisions as
    literal 0/1 inputs to isolate input-centering as the only change from v28.
    """
...
class HeaderBitPositionEmbedding(nn.Embedding):
    """Original-position embedding for compact-header bits pooled into one token."""


class MaskedHeaderBitPositionEmbedding(nn.Embedding):
    """Decoder-only position embedding for removed compact-header bits."""
...
class HeaderTokenEncoder(nn.Module):
    """Pool visible compact-header bits into one transformer header token.

    v29 intentionally keeps v25's transformer interface: the main encoder sees
    exactly one header token. Header masking happens before this pooling step by
    removing a batch-shared subset of bit positions from the token contents.
    """

    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.header_bytes_to_signed_bits = UInt8BytesToSignedBitFeatures(flatten_from_byte_axis=True)
        self.header_bit_value_to_model_width = nn.Sequential(
            OrderedDict(
                [
                    ("header_visible_bit_value_to_model_width", nn.Linear(1, config.d_model)),
                    ("header_visible_bit_value_gelu", nn.GELU()),
                    ("header_visible_bit_value_layer_norm", nn.LayerNorm(config.d_model)),
                ]
            )
        )
        self.header_bit_position_embedding = HeaderBitPositionEmbedding(HEADER_BYTES * BITS_PER_BYTE, config.d_model)
        self.visible_header_bits_to_header_token = nn.Sequential(
            OrderedDict(
                [
                    ("visible_header_bit_pool_to_model_width", nn.Linear(config.d_model, config.d_model)),
    ...
    def forward(self, header_uint8: torch.Tensor, visible_header_bit_indices: torch.Tensor | None) -> torch.Tensor:
        ...
        if visible_header_bit_indices is None:
            visible_header_bit_indices = torch.arange(header_bits.shape[1], device=header_uint8.device)
        # Input shapes: bits [B, 112], visible indices [VH]. Output shape: [B, VH, 1].
        visible_header_bits = header_bits.index_select(dim=1, index=visible_header_bit_indices).unsqueeze(-1)
        # Input shape: [B, VH, 1]. Output shape: [B, VH, D].
        visible_header_bit_tokens = self.header_bit_value_to_model_width(visible_header_bits)
        # Input shape: [VH]. Output shape after unsqueeze: [1, VH, D].
        header_position_tokens = self.header_bit_position_embedding(visible_header_bit_indices).unsqueeze(0)
        # Input shape: [B, VH, D]. Output shape after mean: [B, D].
        pooled_visible_header_bits = (visible_header_bit_tokens + header_position_tokens).mean(dim=1)
        # Input shape: [B, D]. Output shape after unsqueeze: [B, 1, D].
        header_token = self.visible_header_bits_to_header_token(pooled_visible_header_bits).unsqueeze(1)
...
class FixedGridChunkBottleneck(nn.Module):
    ...
    v29 differs from v20 after this scatter step: instead of flattening all 130
...
class HeaderMlpDecoder(nn.Module):
    """Reconstruct masked compact-header bits from only the reusable embedding.

    The header decoder is intentionally separate from the event decoder and
    receives no raw header token. Its position signal is the original bit
    index of each masked header bit, so the loss is aligned with the header bits
    removed before the pooled header token was built.
    """
    ...
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.masked_header_bit_position_embedding_for_decoder = MaskedHeaderBitPositionEmbedding(
            HEADER_BYTES * BITS_PER_BYTE,
            config.d_model,
        )
        self.header_position_memory_to_bit_logit = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("header_decoder_to_bit_logit", nn.Linear(config.d_model, 1)),
                ]
            )
        )
    ...
    def forward(self, chunk_embedding: torch.Tensor, masked_header_bit_indices: torch.Tensor) -> torch.Tensor:
        ...
        masked_position_context = self.masked_header_bit_position_embedding_for_decoder(masked_header_bit_indices).unsqueeze(0)
        # Input shapes: context [B, 1, D], positions [1, HM, D]. Output shape: [B, HM, D].
        decoder_input = chunk_context + masked_position_context
...
class EventChunkEncoder(nn.Module):
    ...
    def encode_tokens(
        ...
        *,
        ...
        # Input shapes: header [B, 14], optional visible header bits [VH]. Output shape: [B, 1, D].
        header_token = self.header_token_encoder(header_uint8, visible_header_bit_indices)
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v29 currently supports input_representation='bit' only")
    ...
    def forward(
        ...
        if mask_config is None:
            raise ValueError("v29 training requires mask_config so masked header-bit targets can be built.")
        # Input shape: header [B, 14]. Output shapes: visible header bits [VH], masked header bits [HM].
        header_masks = build_header_bit_masks(header_uint8, mask_config)
        ...
        # Input shapes: header [B, 14], events [B, E, 16], masks [B, M]/[B, V]. Output shapes: [B, T, D], [B, Z], [B, M, 16], [B, HM].
    ...
    def encode_tokens_for_training(
        ...
        # Input shape: [B, 14]. Output after gather: [B, HM].
        target_header_bits = unpack_bits_01(header_uint8, flatten_from_byte_axis=True).index_select(
            dim=1,
            index=header_masks.masked_header_bit_indices,
        )
    ...
    def decode_header(self, chunk_embedding: torch.Tensor, header_masks: HeaderBitMaskBatch) -> torch.Tensor:
        # Input shapes: chunk embedding [B, Z], masked header bits [HM]. Output shape: [B, HM].
