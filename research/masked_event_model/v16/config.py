...
class MaskConfig:
    ...
    # v16 keeps v9's fixed-mask schedule: every training batch masks the same
...
class ModelConfig:
    ...
    d_byte: int = 24
    d_model: int = 128
    # Kept for CLI/backward compatibility with v12-style launchers. v16's
    # exported representation is token-wise and uses `event_embedding_features`
    # instead of a single pooled embedding width.
    embedding_dim: int = 32
    event_embedding_features: int = 1
    decoder_bottleneck_tokens: int = 40
    n_heads: int = 4
    encoder_layers: int = 6
    # Accepted for launcher compatibility with earlier versions. v16 ignores
    # this value because reconstruction uses a per-masked-event MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v16 is the explicit low-cost MLP decoder experiment, so its default keeps
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v16-token-bottleneck"
