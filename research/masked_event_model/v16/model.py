...
from research.masked_event_model.v16.config import ModelConfig
from research.masked_event_model.v16.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class ChunkEmbeddingBottleneck(nn.Module):
    """Create the exported token-wise embedding from encoded context tokens.

    v16 keeps v12's fast encoder and MLP decoder, but changes the reusable
    bottleneck to the v11-style token-preserving representation. The encoder no
    longer mean-pools CLS/header/event information into one vector. Instead each
    encoded token is projected to a very small feature width. With the default
    `event_embedding_features=1`, training returns `[B, 2 + visible_events, 1]`
    and production returns `[B, 2 + events, 1]`.
    """
    ...
    def project_encoded_tokens(self, encoded_tokens: torch.Tensor) -> torch.Tensor:
        ...
        return self.encoder_token_to_event_feature_projection(encoded_tokens)
    ...
    def forward(self, encoded_tokens: torch.Tensor) -> torch.Tensor:
        ...
        return self.chunk_embedding_output(token_embeddings)
...
class PerMaskedEventMlpDecoder(nn.Module):
    ...
    exported `[B, T, F]` encoder bottleneck while the disposable decoder stays
    intentionally small.
    """
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        self.chunk_embedding_to_decoder_context = nn.Sequential(
            OrderedDict(
                [
                    (
                        "tokenwise_chunk_embedding_to_decoder_width",
                        nn.Linear(self.decoder_bottleneck_tokens * self.event_embedding_features, config.d_model),
                    ),
    ...
    def forward(self, chunk_embedding: torch.Tensor, masked_event_indices: torch.Tensor) -> torch.Tensor:
        if int(chunk_embedding.shape[1]) != self.decoder_bottleneck_tokens:
            raise ValueError(
                ...
                "For fixed 70% masking over 128 events this should be CLS + header + 38 visible events = 40."
            )
...
class EventChunkEncoder(nn.Module):
    """Standalone encoder that ends at the reusable `[B, token_count, F]` embedding.
    ...
    def forward(self, header_uint8: torch.Tensor, events_uint8: torch.Tensor) -> torch.Tensor:
        """Production path: encode all event records and return CLS + header + event embeddings."""
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    compact token-wise memory derived from `chunk_embedding`; this prevents the decoder from
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v16 currently supports input_representation='bit' only")
    ...
    def forward(
        ...
        # 2. project all encoded tokens into the exported token-wise chunk embedding,
    ...
    def decode_from_chunk_embedding(self, chunk_embedding: torch.Tensor, masks: EventMaskBatch) -> torch.Tensor:
        ...
        MLP that receives only the projected `[B, T, F]` chunk embedding and each
        masked event's position embedding.
        """

        if self.config.decoder_force_fp32 and chunk_embedding.is_cuda:
            with torch.amp.autocast("cuda", enabled=False):
                # Input shapes: chunk embedding [B, T, F], masks [B, M]. Output shape: [B, M, 16, 8].
    ...
    def decode_masked_events(self, chunk_embedding: torch.Tensor, masks: EventMaskBatch) -> torch.Tensor:
        ...
        # Input shapes: chunk embedding [B, T, F], masked indices [B, M]. Output shape: [B, M, 16, 8].
