...
from research.masked_event_model.v16.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v16.losses import masked_event_bce_loss
from research.masked_event_model.v16.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v16.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v16.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v16 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v16; the MLP decoder only processes masked events.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v16; reconstruction uses a per-masked-event MLP decoder.", flush=True)
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v16",
        "=" * 80,
        ...
        f"Chunk bottleneck: all encoded tokens -> [B, token_count, {details['event_embedding_features']}]",
        f"Decoder bridge: fixed training bottleneck [B, {details['decoder_bottleneck_tokens']}, {details['event_embedding_features']}] -> [B, d_model]",
        f"Decoder: masked position embedding + projected token-wise chunk embedding -> per-masked-event MLP",
        f"Output event bit logits: [B, masked_events, 16, 8]",
        f"Production embedding: [B, 2 + {details['events_per_chunk']}, {details['event_embedding_features']}]",
        "",
...
def build_model_mermaid(config: ExperimentConfig) -> str:
    ...
    ENC --> TOKEMB[\"project all encoded tokens<br/>B x token_count x {config.model.event_embedding_features}\"]
    TOKEMB --> FLAT[\"flatten fixed training bottleneck<br/>B x {config.model.decoder_bottleneck_tokens * config.model.event_embedding_features}\"]
    FLAT --> MEM[\"project token-wise chunk embedding<br/>B x d_model\"]
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v16-eventmae-f{args.event_embedding_features}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
