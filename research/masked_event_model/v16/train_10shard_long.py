...
from research.masked_event_model.v16.train import main as train_main  # noqa: E402
...
DEFAULTS: dict[str, Any] = {
    ...
    "compile_model": True,
    "wandb_project": "June2026-event-token-mae-v16-token-bottleneck",
    "wandb_entity": "mehdifaraji",
    "wandb_mode": "online",
    "wandb_run_name": "v16-v12mlp-v11tokenemb-f1-bs8192-10shards",
...
PROFILED_TRAINING_PATH = (
    "v16 event-token MAE, per-masked-event MLP decoder, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Long v16 pretraining over 10 sample-cache shards. Defaults target the "
            "medium f1 bs8192 token-bottleneck setup; model-size arguments are explicit so the "
...
def print_plan(
    ...
    print("v16 long pretraining over sample-cache shards", flush=True)
    ...
    print(
        f"model=d{values['d_model']} token_features={values['event_embedding_features']} "
        f"decoder_bottleneck_tokens={values['decoder_bottleneck_tokens']} compat_emb{values['embedding_dim']} heads{values['n_heads']} "
