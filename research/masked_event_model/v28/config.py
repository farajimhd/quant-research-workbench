...
class MaskConfig:
    ...
    # v28 keeps v25's fixed event-mask schedule and adds one isolated change:
    # the header token is built from only the visible header bits, while the
    # transformer still receives one pooled header token.
    ...
    # Kept for CLI/config compatibility with older versions. v28 does not flip
    # header bits; it removes a batch-shared subset before pooling the one
    # header token and reconstructs only those removed header bits.
...
class ModelConfig:
    ...
    # Accepted for launcher compatibility with earlier versions. v28 ignores
    # this value because reconstruction uses a per-masked-event MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v28 keeps the v25 decoder precision default.
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v28-header-mask"
