# Masked Event Model v28

v28 is the v25-derived isolated header-mask experiment. It keeps the same main
transformer interface as v25:

```text
[CLS] + one pooled header token + visible event tokens
```

The only intended architecture/objective change from v25 is header masking:
each batch removes a shared subset of compact-header bit positions before the
one header token is pooled. The header decoder reconstructs only those removed
...
decoder position embeddings. Header inputs are still read as `-1/+1` bit
...
the decoder sees the representation. v28 scatters the variable visible-token
...
The v28 bottleneck tests whether preserving coarse semantic groups helps the
...
v28 does not multiply semantic bit weights into the objective. The event loss
...
W&B project: June2026-event-token-mae-v28-header-mask
```

By default v28 generates a fresh effective seed at process start, then records
...
```powershell
python research\masked_event_model\\v28\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\\v28\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\\v28\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v28\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v28\train_10shard_long.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v28\train_full_pretrain_mixed_cache.py
```
...
The full dual-cache v28 header-reconstruction pretrain launcher starts a fresh
...
```powershell
python research\masked_event_model\\v28\train_full_pretrain_dual_cache.py
```
...
```powershell
python research\masked_event_model\\v28\train_full_pretrain_xonly_continue.py
```
...
```text
\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes\masked_event_model\\v28\pretrain\v28-fullpretrain-mixedcache-fixedmask070-emb32-bs8192-lr4e4-3epochs-freshrng-from-step260352\checkpoints\checkpoint_latest.pt
```
...
Use this focused launcher to compare v28 against the v20 capacity-test runs
without running the whole four-variant sweep. It runs only:

```text
v28 grouped bottleneck
embedding_dim=32
AMP dtype=bf16
batch_size=4096
...
```powershell
python D:\TradingML\codes\masked_event_model\\v28\research\masked_event_model\\v28\run_grouped_bottleneck_probe.py
```

Dry run:

```powershell
python D:\TradingML\codes\masked_event_model\\v28\research\masked_event_model\\v28\run_grouped_bottleneck_probe.py --print-only
```
...
```powershell
python D:\TradingML\codes\masked_event_model\\v28\research\masked_event_model\\v28\run_embedding_precision_probe.py
```
...
By default this v28 launcher uses BF16 AMP and keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\\v28\train_10shard_long.py --fresh-start --run-name v28-fixedgrid-fp16decoder-fixedmask070-emb32-bs8192-10shards --amp-dtype fp16
```
