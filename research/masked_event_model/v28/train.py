...
from research.masked_event_model.v28.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v28.losses import masked_event_bce_loss
from research.masked_event_model.v28.masking import EventMaskBatch, HeaderBitMaskBatch, build_event_masks, build_header_bit_masks
from research.masked_event_model.v28.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v28.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v28 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v28; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v28; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    ...
    metric_logger = JsonlMetricLogger(run_paths.metrics_path, wandb_run, wandb_key_mapper=v28_wandb_metric_key)
    write_run_manifest(
...
def validation_metric_key(train_key: str) -> str:
    ...
    checkpoint monitors. W&B receives a second mapping in `v28_wandb_metric_key`
...
def v28_wandb_metric_key(metric_key: str) -> str:
    """Put v28 pretraining metrics into explicit W&B panel groups."""
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v28",
        "=" * 80,
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v28-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
