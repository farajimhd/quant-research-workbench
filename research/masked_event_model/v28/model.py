...
from research.masked_event_model.v28.config import ModelConfig
from research.masked_event_model.v28.masking import (
    EventMaskBatch,
    HeaderBitMaskBatch,
...
class HeaderTokenEncoder(nn.Module):
    ...
    v28 intentionally keeps v25's transformer interface: the main encoder sees
...
class FixedGridChunkBottleneck(nn.Module):
    ...
    v28 differs from v20 after this scatter step: instead of flattening all 130
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v28 currently supports input_representation='bit' only")
    ...
    def forward(
        ...
        if mask_config is None:
            raise ValueError("v28 training requires mask_config so masked header-bit targets can be built.")
