...
DEFAULT_RUN_PREFIX = "v28-pooledheadermask050-emb32-1shard5ep"
DEFAULT_BATCH_SIZE = 4096
DEFAULT_EPOCHS = 5
DEFAULT_PROBE_EPOCHS = 5
DEFAULT_HEADER_LOSS_WEIGHT = 0.25
DEFAULT_HEADER_MASK_RATIO = 0.50
...
def main() -> int:
    ...
    print("v28 pooled-header-mask emb32 benchmark", flush=True)
    print("pretrain: v25 transformer path plus batch-shared masked header bits pooled into one header token", flush=True)
    print("probe: temporal v1 labeled-cache linear probe from the resulting v28 encoder checkpoint", flush=True)
    print("=" * 104, flush=True)
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description=(
            "Run the v28 isolated header-mask experiment: emb32 BF16 pretraining "
    ...
    parser.add_argument("--header-mask-ratio", type=float, default=DEFAULT_HEADER_MASK_RATIO)
