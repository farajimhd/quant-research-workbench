...
from research.masked_event_model.v11.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v11.losses import masked_event_bce_loss
from research.masked_event_model.v11.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v11.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v11.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v11 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v11; masked-query decoding only processes masked events.", flush=True)
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v11",
        "=" * 80,
        ...
        f"Production encode(): [B, 2 + {details['events_per_chunk']}, {details['event_embedding_features']}]",
        f"Production encode_events(): [B, {details['events_per_chunk']}, {details['event_embedding_features']}]",
        "",
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v11-eventmae-emb{args.embedding_dim}-f{args.event_embedding_features}-t{args.decoder_bottleneck_tokens}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
