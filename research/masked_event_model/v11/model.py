...
from research.masked_event_model.v11.config import ModelConfig
from research.masked_event_model.v11.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class ChunkEmbeddingToDecoderMemory(nn.Module):
    """Project each tokenwise embedding feature into its own decoder memory token."""
    ...
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.token_axis_layer_norm = nn.LayerNorm(self.decoder_bottleneck_tokens)
        self.token_axis_to_decoder_width = nn.Linear(self.decoder_bottleneck_tokens, config.d_model)

    def forward(self, chunk_embedding: torch.Tensor) -> torch.Tensor:
        if int(chunk_embedding.shape[1]) != self.decoder_bottleneck_tokens:
            raise ValueError(
                "v11 decoder bridge expects "
        ...
        # Input shape: [B, T, F]. Output shape: [B, F, T].
        feature_token_sequences = chunk_embedding.transpose(1, 2).contiguous()
        # Input shape: [B, F, T]. Output shape: [B, F, T].
        normalized_feature_sequences = self.token_axis_layer_norm(feature_token_sequences)
        # Input shape: [B, F, T]. Output shape: [B, F, D].
        return self.token_axis_to_decoder_width(normalized_feature_sequences)
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v11 currently supports input_representation='bit' only")
    ...
    def decode_from_chunk_embedding(self, chunk_embedding: torch.Tensor, masks: EventMaskBatch) -> torch.Tensor:
        ...
        if self.config.decoder_force_fp32 and chunk_embedding.is_cuda:
            with torch.amp.autocast("cuda", enabled=False):
                # Input shape: [B, T, F]. Output shape: [B, F, D].
                decoder_memory = self.chunk_embedding_to_decoder_memory(chunk_embedding.float())
                # Input shapes: memory [B, F, D], masks [B, M]. Output shape: [B, M, 16, 8].
    ...
    def decode_masked_events(self, decoder_memory: torch.Tensor, masks: EventMaskBatch) -> torch.Tensor:
        ...
        for decoder_layer in self.masked_query_cross_attention_decoder:
            # Input shapes: queries [B, M, D], memory [B, F, D]. Output shape: [B, M, D].
