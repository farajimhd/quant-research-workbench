# Masked Event Model v18

v18 is the event-token masked autoencoder variant that keeps the v12
per-masked-event MLP decoder path, but replaces v12's average chunk pooling with
Perceiver-style latent cross-attention pooling. It is version-local: model,
masking, loss, progress, training, and profiling code live under
`research/masked_event_model/v18`.
...
the decoder sees the representation. v18 uses a fixed bank of learned latent
queries, so the bottleneck works with both the shorter training sequence and the
full production sequence:

```text
encoded tokens [B, token_count, d_model]
  -> learned Perceiver latents [B, K, d_model]
  -> latent-to-token cross attention, Q=latents and K/V=encoded tokens
  -> flatten latents [B, K * d_model]
  -> projection [B, embedding_dim]
```
...
v18 does not multiply semantic bit weights into the objective. The training loss
...
```text
chunk_embedding: [B, embedding_dim]
event_embeddings diagnostic path: [B, 128, d_model]
```
...
sample cache root: D:\market-data\prepared\event_sample_cache\cache_20260611_195259
...
```powershell
python research\masked_event_model\v18\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\v18\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\v18\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\v18\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\v18\train_10shard_long.py --fresh-start
```

By default this v18 launcher keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\v18\train_10shard_long.py --fresh-start --run-name v18-mlpdecoder-fp16decoder-fixedmask070-emb32-bs4096-10shards --amp-dtype fp16
```
...
trainer command before starting and accepts direct overrides for model size,
latent count, batch size, shard range, validation shard, W&B run name, and
