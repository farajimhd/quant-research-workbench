...
from research.masked_event_model.v18.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v18.losses import masked_event_bce_loss
from research.masked_event_model.v18.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v18.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v18.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v18 event-token MAE on compact event sample-cache batches.")
    ...
    parser.add_argument("--perceiver-latent-tokens", type=int, default=model_defaults.perceiver_latent_tokens)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v18; the MLP decoder only processes masked events.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v18; reconstruction uses a per-masked-event MLP decoder.", flush=True)
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    return ExperimentConfig(
        ...
        model=ModelConfig(
            ...
            perceiver_latent_tokens=args.perceiver_latent_tokens,
...
def save_model_artifacts(
    ...
    details = {
        ...
        "perceiver_latent_tokens": config.model.perceiver_latent_tokens,
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v18",
        "=" * 80,
        ...
        f"Chunk bottleneck: {details['perceiver_latent_tokens']} learned latents cross-attend to encoded tokens -> [B, {details['embedding_dim']}]",
...
def build_model_mermaid(config: ExperimentConfig) -> str:
    ...
    ENC --> LATQ[\"learned Perceiver pooling latents<br/>B x {config.model.perceiver_latent_tokens} x {config.model.d_model}\"]
    LATQ --> XATTN[\"latent-to-token cross attention<br/>Q=latents, K/V=encoded tokens\"]
    ENC --> XATTN
    XATTN --> FLAT[\"flatten latents<br/>B x {config.model.perceiver_latent_tokens * config.model.d_model}\"]
    FLAT --> EMB[\"projected chunk embedding<br/>B x {config.model.embedding_dim}\"]
    EMB --> MEM[\"project chunk embedding<br/>B x d_model\"]
    M --> MPOS[\"masked event positions<br/>B x {masked}\"]
    MPOS --> MPOS_EMB[\"masked position embedding<br/>B x {masked} x d_model\"]
...
def default_run_name(args: argparse.Namespace) -> str:
    return (
        f"mem-v18-eventmae-emb{args.embedding_dim}-lat{args.perceiver_latent_tokens}-"
        f"d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
    )
