...
from research.masked_event_model.v18.config import ModelConfig
from research.masked_event_model.v18.masking import EventMaskBatch, gather_events, maybe_corrupt_header, maybe_corrupt_visible_events
...
class ChunkEmbeddingBottleneck(nn.Module):
    """Create the exported embedding with Perceiver-style latent pooling.

    The encoder sees a variable number of tokens because MAE training removes
    event tokens while production encodes the full chunk. A fixed bank of learned
    latent queries solves that shape mismatch: each latent cross-attends to all
    encoded tokens and can specialize to a different summary of the chunk. The
    latent bank is then flattened and projected back to `[B, embedding_dim]`, so
    the v12 MLP decoder interface remains unchanged.
    """

    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        if config.perceiver_latent_tokens <= 0:
            raise ValueError("perceiver_latent_tokens must be positive")
        self.perceiver_latent_tokens = int(config.perceiver_latent_tokens)
        self.learned_perceiver_pooling_latents = nn.Parameter(torch.zeros(1, self.perceiver_latent_tokens, config.d_model))
        self.perceiver_latent_layer_norm_before_cross_attention = nn.LayerNorm(config.d_model)
        self.encoded_token_layer_norm_before_latent_cross_attention = nn.LayerNorm(config.d_model)
        self.perceiver_latent_to_encoded_token_cross_attention = nn.MultiheadAttention(
            embed_dim=config.d_model,
            num_heads=config.n_heads,
            dropout=config.dropout,
            batch_first=True,
        )
        self.perceiver_cross_attention_residual_dropout = nn.Dropout(config.dropout)
        self.perceiver_latent_layer_norm_before_ffn = nn.LayerNorm(config.d_model)
        self.perceiver_latent_feed_forward = nn.Sequential(
            OrderedDict(
                [
                    ("perceiver_latent_expand_to_ffn_width", nn.Linear(config.d_model, config.ff_dim)),
                    ("perceiver_latent_ffn_gelu", nn.GELU()),
                    ("perceiver_latent_ffn_dropout_after_gelu", nn.Dropout(config.dropout)),
                    ("perceiver_latent_contract_to_model_width", nn.Linear(config.ff_dim, config.d_model)),
                    ("perceiver_latent_ffn_output_dropout", nn.Dropout(config.dropout)),
                ]
            )
        )
        self.perceiver_latents_to_chunk_embedding_projection = nn.Sequential(
            OrderedDict(
                [
                    ("flattened_perceiver_latents_to_embedding_width", nn.Linear(self.perceiver_latent_tokens * config.d_model, config.embedding_dim)),
                    ("perceiver_chunk_embedding_gelu", nn.GELU()),
                    ("perceiver_chunk_embedding_layer_norm", nn.LayerNorm(config.embedding_dim)),
                ]
            )
        )
    ...
    def reset_parameters(self) -> None:
        nn.init.normal_(self.learned_perceiver_pooling_latents, std=0.02)

    def project_encoded_tokens(self, encoded_tokens: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, T, D]. Output shape: [B, K, D].
        return self.pool_encoded_tokens_to_latents(encoded_tokens)

    def pool_encoded_tokens_to_latents(self, encoded_tokens: torch.Tensor) -> torch.Tensor:
        batch_size = int(encoded_tokens.shape[0])
        # Input shape: learned [1, K, D]. Output shape: [B, K, D].
        latent_queries = self.learned_perceiver_pooling_latents.expand(batch_size, -1, -1)
        # Input shape: [B, T, D]. Output shape: [B, T, D].
        normalized_encoded_tokens = self.encoded_token_layer_norm_before_latent_cross_attention(encoded_tokens)
        # Input shapes: latents [B, K, D], tokens [B, T, D]. Output shape: [B, K, D].
        attended_latents, _ = self.perceiver_latent_to_encoded_token_cross_attention(
            query=self.perceiver_latent_layer_norm_before_cross_attention(latent_queries),
            key=normalized_encoded_tokens,
            value=normalized_encoded_tokens,
            need_weights=False,
        )
        # Input shapes: residual [B, K, D], attention [B, K, D]. Output shape: [B, K, D].
        latent_queries = latent_queries + self.perceiver_cross_attention_residual_dropout(attended_latents)
        # Input shape: [B, K, D]. Output shape: [B, K, D].
        return latent_queries + self.perceiver_latent_feed_forward(self.perceiver_latent_layer_norm_before_ffn(latent_queries))

    def forward(self, encoded_tokens: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, T, D]. Output shape: [B, K, D].
        latent_embeddings = self.pool_encoded_tokens_to_latents(encoded_tokens)
        # Input shape: [B, K, D]. Output shape: [B, K*D].
        flattened_latents = latent_embeddings.reshape(latent_embeddings.shape[0], -1)
        # Input shape: [B, K*D]. Output shape: [B, Z].
        pooled_embedding = self.perceiver_latents_to_chunk_embedding_projection(flattened_latents)
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v18 currently supports input_representation='bit' only")
    ...
    def encode_events(self, header_uint8: torch.Tensor, events_uint8: torch.Tensor) -> torch.Tensor:
        """Return per-event encoded transformer states for diagnostics.

        v18's trained chunk bottleneck is the Perceiver latent pool, not a
        per-token projection. Returning the encoded event states avoids exposing
        an untrained token projection as if it were a learned event embedding.
        """
        # Input shapes: header [B, 14], events [B, E, 16]. Output shapes: encoded [B, 2 + E, D], embedding [B, Z].
        ...
        # Input shape: [B, 2 + E, D]. Output shape: [B, E, D].
        return encoded_tokens[:, 2:, :]
