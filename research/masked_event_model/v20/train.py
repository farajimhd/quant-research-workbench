...
from research.masked_event_model.v20.losses import masked_event_bce_loss
from research.masked_event_model.v20.masking import EventMaskBatch, build_event_masks
from research.masked_event_model.v20.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v20.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v20 event-token MAE on compact event sample-cache batches.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v20; reconstruction uses a per-masked-event MLP decoder.", flush=True)
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v20",
        "=" * 80,
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v20-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"
