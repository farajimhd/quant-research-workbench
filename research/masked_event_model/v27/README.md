# Masked Event Model v27

v27 is copied from v25 and changes only the compact-header objective. v25 always
fed the full 14-byte header to the encoder and decoded all header bits. v27
turns header reconstruction into an MAE task too:

```text
header_uint8 [B, 14]
  -> 112 header bits
  -> one batch-shared 50% visible / 50% masked split
  -> encoder sees only visible header bit values in {-1, +1}
  -> decoder reconstructs only masked header bits
```

The event side remains v25: fixed 70% event-token masking, grouped fixed-grid
bottleneck, and the cheap per-masked-event MLP decoder. v27 is version-local:
model, masking, loss, progress, training, and profiling code live under
`research/masked_event_model/v27`.

The main masking setup is:

```text
header_uint8: [B, 14]       batch-shared 50% header bits removed from encoder
events_uint8: [B, 128, 16]  fixed 70% event tokens removed from encoder
```

At training time the encoder sees:

```text
[CLS] + visible_header_bit_tokens + visible_event_tokens
```

Every batch samples one header bit mask and applies it to the whole batch for
efficiency. Header values are never represented as zero-mask placeholders:
visible header bit values are unpacked to `-1/+1`, then combined with learned
header bit-position embeddings. Masked header bits are removed from encoder
tokens and are used only as decoder targets. Visible event bytes can still
receive low-rate bit corruption for robustness.

All encoded tokens pass through the exported chunk embedding bottleneck before
the decoder sees the representation. v27 scatters the variable visible-token
encoder output back into a fixed semantic grid:

```text
encoded tokens [B, 1 + visible_header_bits + visible_events, d_model]
  -> pool encoded visible header-bit tokens to one header slot
  -> fixed grid [B, 130, d_model]
       slot 0: CLS
       slot 1: pooled visible header bits
...
The v27 bottleneck tests whether preserving coarse semantic groups helps the
exported representation. The fixed grid is split into 10 nonlinear branches:

```text
slot 0 CLS token             -> MLP -> [B, embedding_dim]
slot 1 pooled header bits    -> MLP -> [B, embedding_dim]
...
The header decoder predicts only masked compact-header bits. It receives the
exported chunk embedding plus decoder-only masked header bit-position
embeddings:

```text
chunk_embedding [B, embedding_dim]
  -> Linear + GELU + LayerNorm [B, d_model]
masked_header_bit_indices [masked_header_bits]
  -> masked_header_bit_position_embedding [masked_header_bits, d_model]
chunk_context + masked_header_bit_position_embedding
  -> header MLP decoder
  -> header_bit_logits [B, masked_header_bits]
```

Loss is ordinary binary cross entropy with logits over masked event bits plus a
weighted BCE term over masked header bits:
...
v27 does not multiply semantic bit weights into the objective. The event loss
uses the PyTorch default mean reduction over all masked event bits, and the
header loss uses the same mean reduction over the masked header bit targets.
Semantic metrics can still be emitted for diagnostics, but they are not used
for backpropagation. Production embedding uses the explicit `encode(...)` path,
which sees all 112 header bits and all 128 events:
...
header mask ratio: 0.50
header bit corruption: disabled in v27
...
W&B project: June2026-event-token-mae-v27-header-mask
```

By default v27 generates a fresh effective seed at process start, then records
...
```powershell
python research\masked_event_model\\v27\run_profile_one_shard.py --steps 50 --batch-size 4096 --fresh-start
```

Model-size sweep:

```powershell
python research\masked_event_model\\v27\run_model_size_sweep.py --steps 200 --fresh-start
```
...
```powershell
python research\masked_event_model\\v27\run_variation_profile.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v27\train_medium_bit_limited_shards.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v27\train_10shard_long.py --fresh-start
```
...
```powershell
python research\masked_event_model\\v27\train_full_pretrain_mixed_cache.py
```
...
The full dual-cache v27 header-reconstruction pretrain launcher starts a fresh
...
```powershell
python research\masked_event_model\\v27\train_full_pretrain_dual_cache.py
```
...
```powershell
python research\masked_event_model\\v27\train_full_pretrain_xonly_continue.py
```
...
```text
\\DESKTOP-SAAI85T\Workstation-D\TradingML\runtimes\masked_event_model\\v27\pretrain\v27-fullpretrain-mixedcache-fixedmask070-emb32-bs8192-lr4e4-3epochs-freshrng-from-step260352\checkpoints\checkpoint_latest.pt
```
...
Use this focused launcher to compare v27 against the v20 capacity-test runs
without running the whole four-variant sweep. It runs only:

```text
v27 grouped bottleneck
embedding_dim=32
AMP dtype=bf16
batch_size=4096
...
```powershell
python D:\TradingML\codes\masked_event_model\\v27\research\masked_event_model\\v27\run_grouped_bottleneck_probe.py
```

Dry run:

```powershell
python D:\TradingML\codes\masked_event_model\\v27\research\masked_event_model\\v27\run_grouped_bottleneck_probe.py --print-only
```
...
```powershell
python D:\TradingML\codes\masked_event_model\\v27\research\masked_event_model\\v27\run_embedding_precision_probe.py
```
...
By default this v27 launcher uses BF16 AMP and keeps the cheaper MLP decoder inside the active AMP
dtype. To run the explicit FP16 decoder experiment:

```powershell
python research\masked_event_model\\v27\train_10shard_long.py --fresh-start --run-name v27-fixedgrid-fp16decoder-fixedmask070-emb32-bs8192-10shards --amp-dtype fp16
```
