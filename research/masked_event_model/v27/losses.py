...
from research.masked_event_model.v27.config import LossConfig
from research.masked_event_model.v27.model import EventMAEOutput
...
def masked_event_bce_loss(
    ...
    *,
    ...
    header_logits = output.header_bit_logits
    target_header_bits = output.target_header_bits.to(dtype=header_logits.dtype, device=header_logits.device)
        ...
        with torch.amp.autocast("cuda", enabled=False):
            if objective == "unweighted":
                ...
                if calculate_unweighted_metric:
                    unweighted_loss = F.binary_cross_entropy_with_logits(logits.float(), target_bits.float())
            header_loss = F.binary_cross_entropy_with_logits(header_logits.float(), target_header_bits.float())
    else:
        if objective == "unweighted":
            ...
            if calculate_unweighted_metric:
                unweighted_loss = F.binary_cross_entropy_with_logits(logits, target_bits)
        header_loss = F.binary_cross_entropy_with_logits(header_logits, target_header_bits)
    event_loss = loss * float(config.event_weight)
    header_loss = header_loss * float(config.header_weight)
    loss = event_loss + header_loss

    metrics_started = time.perf_counter()
    metrics = {
        "pretrain/loss_total": float(loss.detach().cpu()),
        "pretrain/loss_event_total": float(event_loss.detach().cpu()),
        "pretrain/loss_header_total": float(header_loss.detach().cpu()),
        "pretrain/loss_header_unweighted": float((header_loss.detach() / max(float(config.header_weight), 1e-12)).cpu()),
        "pretrain/loss_header_weight": float(config.header_weight),
        ...
        "pretrain/loss_header_terms_per_sample": float(output.masked_header_bit_indices.numel()),
        ...
        "mask/header_mask_ratio_pct": float(output.header_actual_mask_ratio * 100.0),
        "mask/header_requested_mask_ratio_pct": float(output.header_requested_mask_ratio * 100.0),
        "mask/header_visible_bits": float(output.visible_header_bit_count),
        "mask/header_masked_bits": float(output.masked_header_bit_indices.numel()),
        "mask/header_bit_count": float(HEADER_BITS_PER_SAMPLE),
    }
    ...
    with torch.no_grad():
        ...
        header_probabilities = torch.sigmoid(header_logits.float())
        header_hard_bits = header_probabilities >= 0.5
        ...
        header_bit_acc = (header_hard_bits == target_header_bool).float().mean()
        header_confidence = (header_probabilities - 0.5).abs() * 2.0
        metrics.update(
            {
            ...
            "pretrain/header_bit_acc_pct": float(header_bit_acc.detach().cpu() * 100.0),
            "pretrain/header_bit_conf_mean": float(header_confidence.mean().detach().cpu()),
            "mask/event_masked_bytes": float(target_bytes.numel()),
            "mask/total_masked_bytes": float(target_bytes.numel()),
            "mask/header_masked_bit_terms": float(target_header_bits.numel()),
            }
        )
...
def masked_event_semantic_metrics(
    ...
    with torch.no_grad():
        ...
        metrics = {
            "semantic/train/masked_events": float(target.shape[0] * target.shape[1]),
            "semantic/train/valid_events": float(valid_mask.float().sum().detach().cpu()),
            "semantic/train/quote_events": float(quote_mask.float().sum().detach().cpu()),
            "semantic/train/trade_events": float(trade_mask.float().sum().detach().cpu()),
            "semantic/train/event_type_acc_pct": masked_accuracy(predicted_fields["event_type"], target_fields["event_type"], valid_mask),
            "semantic/train/event_presence_acc_pct": masked_accuracy(predicted_fields["presence"], target_fields["presence"], torch.ones_like(valid_mask, dtype=torch.bool)),
            "semantic/train/quote_time_bucket_mae": masked_mae(predicted_fields["event_delta_bucket"], target_fields["event_delta_bucket"], quote_mask),
            "semantic/train/quote_ask_delta_tick_mae": masked_mae(predicted_fields["price1_delta"], target_fields["price1_delta"], quote_mask),
            "semantic/train/quote_spread_delta_tick_mae": masked_mae(predicted_fields["price2_delta"], target_fields["price2_delta"], quote_mask),
            "semantic/train/quote_ask_tick_mae": masked_mae(predicted_fields["price1_abs_ticks"], target_fields["price1_abs_ticks"], quote_mask),
            "semantic/train/quote_spread_tick_mae": masked_mae(predicted_fields["spread_ticks"], target_fields["spread_ticks"], quote_mask),
            "semantic/train/quote_bid_tick_mae": masked_mae(predicted_fields["bid_ticks"], target_fields["bid_ticks"], quote_mask),
            "semantic/train/quote_ask_price_mae": masked_price_mae(predicted_fields["price1_abs_ticks"], target_fields["price1_abs_ticks"], target_fields["tick_size"], quote_mask),
            "semantic/train/quote_bid_price_mae": masked_price_mae(predicted_fields["bid_ticks"], target_fields["bid_ticks"], target_fields["tick_size"], quote_mask),
            "semantic/train/quote_bid_size_bucket_mae": masked_mae(predicted_fields["size1_bucket"], target_fields["size1_bucket"], quote_mask),
            "semantic/train/quote_ask_size_bucket_mae": masked_mae(predicted_fields["size2_bucket"], target_fields["size2_bucket"], quote_mask),
            "semantic/train/quote_bid_small_flag_acc_pct": masked_accuracy(predicted_fields["size1_small_flag"], target_fields["size1_small_flag"], quote_mask),
            "semantic/train/quote_ask_small_flag_acc_pct": masked_accuracy(predicted_fields["size2_small_flag"], target_fields["size2_small_flag"], quote_mask),
            "semantic/train/quote_tape_acc_pct": masked_accuracy(predicted_fields["tape"], target_fields["tape"], quote_mask),
            "semantic/train/quote_bid_exchange_acc_pct": masked_accuracy(predicted_fields["exchange1"], target_fields["exchange1"], quote_mask),
            "semantic/train/quote_ask_exchange_acc_pct": masked_accuracy(predicted_fields["exchange2"], target_fields["exchange2"], quote_mask),
            "semantic/train/quote_condition_slot_acc_pct": masked_accuracy(predicted_fields["conditions"], target_fields["conditions"], quote_mask),
            "semantic/train/quote_all_condition_slots_exact_acc_pct": masked_boolean_rate((predicted_fields["conditions"] == target_fields["conditions"]).all(dim=-1), quote_mask),
            "semantic/train/trade_time_bucket_mae": masked_mae(predicted_fields["event_delta_bucket"], target_fields["event_delta_bucket"], trade_mask),
            "semantic/train/trade_price_delta_tick_mae": masked_mae(predicted_fields["price1_delta"], target_fields["price1_delta"], trade_mask),
            "semantic/train/trade_price_tick_mae": masked_mae(predicted_fields["price1_abs_ticks"], target_fields["price1_abs_ticks"], trade_mask),
            "semantic/train/trade_price_mae": masked_price_mae(predicted_fields["price1_abs_ticks"], target_fields["price1_abs_ticks"], target_fields["tick_size"], trade_mask),
            "semantic/train/trade_size_bucket_mae": masked_mae(predicted_fields["size1_bucket"], target_fields["size1_bucket"], trade_mask),
            "semantic/train/trade_small_flag_acc_pct": masked_accuracy(predicted_fields["size1_small_flag"], target_fields["size1_small_flag"], trade_mask),
            "semantic/train/trade_tape_acc_pct": masked_accuracy(predicted_fields["tape"], target_fields["tape"], trade_mask),
            "semantic/train/trade_exchange_acc_pct": masked_accuracy(predicted_fields["exchange1"], target_fields["exchange1"], trade_mask),
            "semantic/train/trade_condition_slot_acc_pct": masked_accuracy(predicted_fields["conditions"], target_fields["conditions"], trade_mask),
            "semantic/train/trade_all_condition_slots_exact_acc_pct": masked_boolean_rate((predicted_fields["conditions"] == target_fields["conditions"]).all(dim=-1), trade_mask),
            "semantic/train/trade_correction_acc_pct": masked_accuracy(predicted_fields["correction"], target_fields["correction"], trade_mask),
            "semantic/train/predicted_quote_valid_pct": masked_boolean_rate(
