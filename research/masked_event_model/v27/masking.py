...
from research.masked_event_model.v27.config import MaskConfig
...
class HeaderBitMaskBatch:
    """Batch-shared header bit indices used by v27.

    Header masking is shared across the whole batch for efficiency: every sample
    exposes the same header bit positions to the encoder and reconstructs the
    same hidden positions in the decoder. The values themselves are still
    per-sample and stay in {-1, +1} after unpacking.
    """

    # Shape: [VH]. Sorted header bit positions visible to the encoder.
    visible_header_bit_indices: torch.Tensor
    # Shape: [MH]. Sorted header bit positions reconstructed by the decoder.
...
def build_header_bit_masks(header_uint8: torch.Tensor, config: MaskConfig) -> HeaderBitMaskBatch:
    """Select one batch-shared set of visible/masked compact-header bits."""
    ...
    mask_ratio = min(max(float(getattr(config, "header_mask_ratio", 0.0)), 0.0), 0.99)
    min_masked = max(0, int(getattr(config, "min_masked_header_bits", 1)))
    masked_count = max(min_masked, int(round(header_bit_count * mask_ratio)))
    masked_count = min(max(0, masked_count), header_bit_count - 1)
    visible_count = header_bit_count - masked_count
    scores = torch.rand((header_bit_count,), device=header_uint8.device)
    visible = torch.topk(scores, k=visible_count, dim=0, largest=False).indices.sort(dim=0).values
    masked = torch.topk(scores, k=masked_count, dim=0, largest=True).indices.sort(dim=0).values
