...
DEFAULT_RUN_PREFIX = "v27-headermask050-emb32-1shard5ep"
...
def main() -> int:
    ...
    print("v27 masked-header-bit emb32 benchmark", flush=True)
    print("pretrain: v25 architecture plus batch-shared 50% header bit masking", flush=True)
    print("probe: temporal v1 labeled-cache linear probe from the resulting v27 encoder checkpoint", flush=True)
    print("=" * 104, flush=True)
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description=(
            "Run the v27 masked-header-bit experiment: emb32 BF16 pretraining "
