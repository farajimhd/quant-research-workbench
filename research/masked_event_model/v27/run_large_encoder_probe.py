...
DEFAULT_PRETRAIN_OUTPUT_ROOT = Path(r"D:\TradingML\runtimes\masked_event_model\\v27\pretrain_capacity_tests")
...
DEFAULT_RUN_PREFIX = "v27-largeencoder-1shard5ep"
DEFAULT_D_MODEL = 384
DEFAULT_N_HEADS = 12
...
def main() -> int:
    ...
    print("v27 large-encoder embedding benchmark", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sequentially pretrain v27 large-encoder emb32 and emb128 variants on one x-only shard, "
            "then run the same temporal v1 linear probe for each resulting checkpoint. "
            "This script is self-contained so active v27 grouped/probe launchers are not changed."
        )
    )
