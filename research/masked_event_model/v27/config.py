...
class MaskConfig:
    ...
    # v27 keeps v9's fixed-mask schedule: every training batch masks the same
    ...
    # v27 masks header bits by removal, not by zeroing or corrupting values.
    # One batch-shared set of header bit positions is hidden each step, and
    # visible header bit values remain centered -1/+1 inputs with position
    # embeddings. This keeps the encoder input alphabet identical to events.
    header_mask_ratio: float = 0.50
    min_masked_header_bits: int = 1
    # Accepted for launcher compatibility with older versions. v27 does not use
    # header bit corruption because masked header bits are removed from the
    # encoder and reconstructed by position.
    header_bit_corruption_prob: float = 0.0
    header_bit_corruption_ratio: float = 0.0
...
class ModelConfig:
    ...
    # Accepted for launcher compatibility with earlier versions. v27 ignores
    # this value because reconstruction uses a per-masked-event MLP decoder.
    decoder_layers: int = 2
    ffn_mult: int = 4
    dropout: float = 0.08
    # v27 is the explicit low-cost MLP decoder experiment, so its default keeps
...
class TrainConfig:
    ...
    wandb_project: str = "June2026-event-token-mae-v27-header-mask"
