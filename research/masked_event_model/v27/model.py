...
from research.masked_event_model.v27.config import ModelConfig
from research.masked_event_model.v27.masking import (
    EventMaskBatch,
    HeaderBitMaskBatch,
    build_header_bit_masks,
    gather_events,
    maybe_corrupt_visible_events,
)
...
# T = encoder tokens (CLS + visible header bits + visible events), D = model width, Z = embedding width.
...
class EventMAEOutput:
    ...
    # Shape: [B, HM]. Raw decoder logits for each masked compact-header bit.
    header_bit_logits: torch.Tensor
    # Shape: [HM]. Original compact-header bit positions selected as targets.
    masked_header_bit_indices: torch.Tensor
    # Shape: [B, HM]. Binary target bits gathered at masked header positions.
    target_header_bits: torch.Tensor
    # Shape: [B, 14]. Original uint8 chunk header bytes.
    ...
    # Scalar. Number of header bits sent through the encoder.
    visible_header_bit_count: int
    # Scalar. Header bit mask ratio used by the current batch.
    header_actual_mask_ratio: float
    # Scalar. Header bit mask ratio requested by the current batch.
    header_requested_mask_ratio: float
    ...
    def header_bit_probs(self) -> torch.Tensor:
        # Input shape: [B, HM]. Output shape: [B, HM].
...
class HeaderRoleEmbedding(nn.Embedding):
    """Learned role vector shared by compact-header bit tokens."""


class HeaderBitPositionEmbedding(nn.Embedding):
    """Position table for the 112 compact-header bits visible to the encoder."""
...
class MaskedHeaderBitPositionEmbedding(nn.Embedding):
    """Decoder-only position embedding for masked compact-header bit locations."""
...
def unpack_header_bits_01(header_uint8: torch.Tensor) -> torch.Tensor:
    """Return compact-header target bits in {0, 1} with shape `[B, 112]`."""

    shifts = torch.arange(BITS_PER_BYTE, device=header_uint8.device, dtype=torch.long).view(1, 1, BITS_PER_BYTE)
    bits = (header_uint8.long().unsqueeze(-1) >> shifts) & 1
    return bits.flatten(1).to(torch.float32)


class HeaderBitTokenEncoder(nn.Module):
    """Project visible compact-header bits into position-aware transformer tokens.

    v27 never inserts zero-valued placeholders for hidden header bits. Visible
    bit values are unpacked to {-1, +1}, gathered by a batch-shared index set,
    and marked with learned bit-position embeddings so the transformer knows
    which original header columns were present.
    """

    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.header_bytes_to_signed_bits = UInt8BytesToSignedBitFeatures(flatten_from_byte_axis=True)
        self.header_bit_value_to_model_token = nn.Sequential(
            OrderedDict(
                [
                    ("header_signed_bit_value_to_model_width", nn.Linear(1, config.d_model)),
                    ("header_projection_gelu", nn.GELU()),
                    ("header_projection_layer_norm", nn.LayerNorm(config.d_model)),
                ]
            )
        )
        self.header_bit_position_embedding = HeaderBitPositionEmbedding(HEADER_BYTES * BITS_PER_BYTE, config.d_model)
        self.header_role_embedding = HeaderRoleEmbedding(1, config.d_model)

    def forward(self, header_uint8: torch.Tensor, visible_header_bit_indices: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        # Input shape: [B, 14]. Output shape: [B, 112].
        header_bits = self.header_bytes_to_signed_bits(header_uint8)
        if visible_header_bit_indices is None:
            visible_header_bit_indices = torch.arange(header_bits.shape[1], device=header_uint8.device, dtype=torch.long)
        visible_header_bit_indices = visible_header_bit_indices.to(device=header_uint8.device, dtype=torch.long)
        # Input shapes: bits [B, 112], positions [HV]. Output shape: [B, HV, 1].
        visible_header_bits = header_bits.index_select(dim=1, index=visible_header_bit_indices).unsqueeze(-1)
        # Input shape: [B, HV, 1]. Output shape: [B, HV, D].
        header_tokens = self.header_bit_value_to_model_token(visible_header_bits)
        # Input shape: [HV]. Output shape after unsqueeze: [1, HV, D].
        header_positions = self.header_bit_position_embedding(visible_header_bit_indices).unsqueeze(0)
        # Input shapes: tokens [B, HV, D], positions [1, HV, D], role [1, 1, D]. Output shape: [B, HV, D].
        header_tokens = header_tokens + header_positions
        header_tokens = header_tokens + single_role_vector(self.header_role_embedding, device=header_uint8.device)
        return header_tokens, visible_header_bit_indices


class HeaderVisibleBitPool(nn.Module):
    """Pool variable-count visible header-bit tokens back to one header slot."""

    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.visible_header_bits_to_header_slot = nn.Sequential(
            OrderedDict(
                [
                    ("visible_header_bit_mean_to_model_width", nn.Linear(config.d_model, config.d_model)),
                    ("visible_header_bit_pool_gelu", nn.GELU()),
                    ("visible_header_bit_pool_layer_norm", nn.LayerNorm(config.d_model)),
                ]
            )
        )

    def forward(self, encoded_header_bit_tokens: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, HV, D]. Output shape after mean: [B, D].
        pooled_header_bits = encoded_header_bit_tokens.mean(dim=1)
        # Input shape: [B, D]. Output shape after unsqueeze: [B, 1, D].
        return self.visible_header_bits_to_header_slot(pooled_header_bits).unsqueeze(1)
...
class EncoderSequenceBuilder(nn.Module):
    """Prepend the learned CLS token and concatenate header-bit plus event tokens."""
    ...
    def forward(self, header_bit_tokens: torch.Tensor, event_tokens: torch.Tensor) -> torch.Tensor:
        batch_size = int(header_bit_tokens.shape[0])
        ...
        cls_token = cls_token + single_role_vector(self.cls_role_embedding, device=header_bit_tokens.device)
        # Input shapes: CLS [B, 1, D], header bits [B, HV, D], events [B, V, D].
        # Output shape: [B, 1 + HV + V, D].
        return torch.cat([cls_token, header_bit_tokens, event_tokens], dim=1)
...
class FixedGridChunkBottleneck(nn.Module):
    ...
    v27 differs from v20 after this scatter step: instead of flattening all 130
    tokens into one dense projection, it gives CLS, header, and each 16-event
    block its own nonlinear MLP branch. The branch outputs are concatenated and
    merged back into the single exported chunk embedding `[B, Z]` expected by
    the decoder and downstream probe code.
    """
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        self.branch_embedding_dim = int(config.embedding_dim)
        self.cls_token_to_branch_embedding = nn.Sequential(
            OrderedDict(
                [
                    ("cls_token_model_width_to_branch_embedding", nn.Linear(config.d_model, self.branch_embedding_dim)),
                    ("cls_token_branch_gelu", nn.GELU()),
                    ("cls_token_branch_layer_norm", nn.LayerNorm(self.branch_embedding_dim)),
                ]
            )
        )
        self.header_token_to_branch_embedding = nn.Sequential(
            OrderedDict(
                [
                    ("header_token_model_width_to_branch_embedding", nn.Linear(config.d_model, self.branch_embedding_dim)),
                    ("header_token_branch_gelu", nn.GELU()),
                    ("header_token_branch_layer_norm", nn.LayerNorm(self.branch_embedding_dim)),
                ]
            )
        )
                ...
                (
                    ...
                    nn.Sequential(
                        OrderedDict(
                            [
                                (
                                    "event_group_flattened_tokens_to_branch_embedding",
                                    nn.Linear(self.event_group_size * config.d_model, self.branch_embedding_dim),
                                ),
                                ("event_group_branch_gelu", nn.GELU()),
                                ("event_group_branch_layer_norm", nn.LayerNorm(self.branch_embedding_dim)),
                            ]
                        )
                    ),
                )
        ...
        self.semantic_branch_embeddings_to_chunk_embedding = nn.Sequential(
            OrderedDict(
                [
                    (
                        "semantic_branch_concat_to_chunk_embedding",
                        nn.Linear((2 + self.event_group_count) * self.branch_embedding_dim, config.embedding_dim),
                    ),
                    ("semantic_branch_merge_gelu", nn.GELU()),
                    ("semantic_branch_merge_layer_norm", nn.LayerNorm(config.embedding_dim)),
                ]
            )
        )
    ...
    def project_fixed_token_grid(self, fixed_tokens: torch.Tensor) -> torch.Tensor:
        ...
        # Input shape: 10 x [B, Z]. Output shape: [B, 10 * Z].
        concatenated_branch_embeddings = torch.cat(branch_embeddings, dim=1)
        # Input shape: [B, 10 * Z]. Output shape: [B, Z].
        return self.semantic_branch_embeddings_to_chunk_embedding(concatenated_branch_embeddings)
...
class HeaderMlpDecoder(nn.Module):
    """Reconstruct only masked compact-header bits from the reusable embedding.

    The header decoder is intentionally separate from the event decoder and
    receives no raw header token. Its gradients therefore force the exported
    `[B, Z]` chunk embedding to carry anchor-price, spread, count, flag, and
    timing information that downstream models may need. v27 predicts only the
    hidden header bit positions, using decoder-only position embeddings.
    """

    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.chunk_embedding_to_header_decoder_context = nn.Sequential(
            OrderedDict(
                [
                    ("chunk_embedding_to_header_decoder_width", nn.Linear(config.embedding_dim, config.d_model)),
                    ("header_decoder_gelu_1", nn.GELU()),
                    ("header_decoder_layer_norm_1", nn.LayerNorm(config.d_model)),
                ]
            )
        )
        self.masked_header_bit_position_embedding_for_decoder = MaskedHeaderBitPositionEmbedding(HEADER_BYTES * BITS_PER_BYTE, config.d_model)
        self.header_position_memory_to_bit_logit = nn.Sequential(
            OrderedDict(
                [
                    ("header_position_memory_layer_norm", nn.LayerNorm(config.d_model)),
                    ("header_decoder_expand_to_ffn_width", nn.Linear(config.d_model, config.ff_dim)),
                    ("header_decoder_gelu_2", nn.GELU()),
                    ("header_decoder_dropout", nn.Dropout(config.dropout)),
                    ("header_decoder_contract_to_model_width", nn.Linear(config.ff_dim, config.d_model)),
                    ("header_decoder_gelu_3", nn.GELU()),
                    ("header_decoder_layer_norm_2", nn.LayerNorm(config.d_model)),
                    # BCE-with-logits needs unconstrained raw bit logits.
                    ("header_position_memory_to_bit_logit", nn.Linear(config.d_model, 1)),
                ]
            )
        )

    def forward(self, chunk_embedding: torch.Tensor, masked_header_bit_indices: torch.Tensor) -> torch.Tensor:
        # Input shape: [B, Z]. Output shape after unsqueeze: [B, 1, D].
        chunk_context = self.chunk_embedding_to_header_decoder_context(chunk_embedding).unsqueeze(1)
        # Input shape: [HM]. Output shape after unsqueeze: [1, HM, D].
        masked_header_position_context = self.masked_header_bit_position_embedding_for_decoder(masked_header_bit_indices).unsqueeze(0)
        # Input shapes: memory [B,1,D], positions [1,HM,D]. Output shape: [B,HM,D].
        decoder_input = chunk_context + masked_header_position_context
        # Input shape: [B, HM, D]. Output shape after squeeze: [B, HM].
        return self.header_position_memory_to_bit_logit(decoder_input).squeeze(-1)
...
class EventChunkEncoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        self.header_token_encoder = HeaderBitTokenEncoder(config)
        self.header_visible_bit_pool = HeaderVisibleBitPool(config)
    ...
    def forward(self, header_uint8: torch.Tensor, events_uint8: torch.Tensor) -> torch.Tensor:
        ...
        _, chunk_embedding = self.encode_tokens(
            ...
            visible_header_bit_indices=None,
            mask_config=None,
            training=False,
        )
    ...
    def encode_tokens(
        ...
        *,
        ...
        visible_header_bit_indices: torch.Tensor | None,
        mask_config,
        training: bool,
        ...
        # Input shapes: header [B, 14], optional bit positions [HV]. Output shape: [B, HV or 112, D].
        header_bit_tokens, selected_header_bit_indices = self.header_token_encoder(header_uint8, visible_header_bit_indices)
        ...
        # Input shapes: header bits [B, HV, D], events [B, V or E, D]. Output shape: [B, 1 + HV + V, D].
        encoder_input_tokens = self.encoder_sequence_builder(header_bit_tokens, visible_event_tokens)
        ...
        # Input shape: [B, 1 + HV + V, D]. Output shape: [B, 2 + V, D].
        fixed_grid_input_tokens = self.pool_encoded_header_bits(encoded_tokens, selected_header_bit_indices)
        # Input shapes: encoded [B, 2 + V, D], indices [B, V or E]. Output shape: [B, Z].
        chunk_embedding = self.chunk_embedding_bottleneck(
            fixed_grid_input_tokens,
            selected_event_indices,
        )
        return fixed_grid_input_tokens, chunk_embedding

    def pool_encoded_header_bits(self, encoded_tokens: torch.Tensor, visible_header_bit_indices: torch.Tensor) -> torch.Tensor:
        header_bit_count = int(visible_header_bit_indices.shape[0])
        # Input shape: [B, 1 + HV + V, D]. Output shapes: CLS [B,1,D], header bits [B,HV,D], events [B,V,D].
        cls_token = encoded_tokens[:, :1, :]
        encoded_header_bit_tokens = encoded_tokens[:, 1 : 1 + header_bit_count, :]
        encoded_event_tokens = encoded_tokens[:, 1 + header_bit_count :, :]
        # Input shape: [B, HV, D]. Output shape: [B, 1, D].
        pooled_header_token = self.header_visible_bit_pool(encoded_header_bit_tokens)
        # Output shape: [B, 2 + V, D], matching the fixed-grid bottleneck contract.
        return torch.cat([cls_token, pooled_header_token, encoded_event_tokens], dim=1)
...
class EventTokenMaskedAutoencoder(nn.Module):
    ...
    def __init__(self, *, events_per_chunk: int, config: ModelConfig) -> None:
        ...
        if self.input_representation != "bit":
            raise ValueError("v27 currently supports input_representation='bit' only")
        ...
        self.header_mlp_decoder = HeaderMlpDecoder(config)
        self.reset_parameters()
    ...
    def forward(
        ...
        header_masks = build_header_bit_masks(header_uint8, mask_config) if mask_config is not None else None
        if header_masks is None:
            raise ValueError("v27 training requires mask_config so header bit masks can be built.")
        encoded_tokens, chunk_embedding, target_events, target_header_bits = self.encode_tokens_for_training(
            header_uint8, events_uint8, masks, header_masks, mask_config
        )
        # Input shape: [B, Z]. Output shapes: event [B, M, 16, 8], header [B, HM].
        event_logits, header_logits = self.decode_from_chunk_embedding(chunk_embedding, masks, header_masks)
        return EventMAEOutput(
            ...
            masked_header_bit_indices=header_masks.masked_header_bit_indices,
            target_header_bits=target_header_bits,
            target_header_uint8=header_uint8,
            chunk_embedding=chunk_embedding,
            event_count=masks.event_count,
            visible_event_count=masks.visible_count,
            visible_header_bit_count=header_masks.visible_count,
            header_actual_mask_ratio=header_masks.actual_mask_ratio,
            header_requested_mask_ratio=header_masks.requested_mask_ratio,
    ...
    def decode_from_chunk_embedding(
        self,
        chunk_embedding: torch.Tensor,
        masks: EventMaskBatch,
        header_masks: HeaderBitMaskBatch,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.config.decoder_force_fp32 and chunk_embedding.is_cuda:
            with torch.amp.autocast("cuda", enabled=False):
                # Input shapes: chunk embedding [B, Z], masks [B, M]/[HM].
                # Output shapes: event [B, M, 16, 8], header [B, HM].
                return self.decode_reconstruction_targets(chunk_embedding.float(), masks, header_masks)

        # Input shapes: chunk embedding [B, Z], masks [B, M]/[HM].
        # Output shapes: event [B, M, 16, 8], header [B, HM].
        return self.decode_reconstruction_targets(chunk_embedding, masks, header_masks)
    ...
    def encode_tokens_for_training(
        ...
        # Input shapes: header [B, 14], masked header bit indices [HM]. Output shape: [B, HM].
        target_header_bits = unpack_header_bits_01(header_uint8).index_select(dim=1, index=header_masks.masked_header_bit_indices)
        # Input shapes: header [B, 14], events [B, E, 16], visible indices [B, V]/[HV]. Output shapes: [B, 2 + V, D], [B, Z].
        encoded_tokens, chunk_embedding = self._encode_tokens(
            header_uint8,
            events_uint8,
            visible_event_indices=masks.visible_event_indices,
            visible_header_bit_indices=header_masks.visible_header_bit_indices,
            mask_config=mask_config,
            training=True,
        )
        return encoded_tokens, chunk_embedding, target_events, target_header_bits
    ...
    def decode_header(self, chunk_embedding: torch.Tensor, header_masks: HeaderBitMaskBatch) -> torch.Tensor:
        # Input shape: chunk embedding [B, Z], masked header bits [HM]. Output shape: [B, HM].
        return self.header_mlp_decoder(chunk_embedding, header_masks.masked_header_bit_indices)

    def decode_reconstruction_targets(
        self,
        chunk_embedding: torch.Tensor,
        masks: EventMaskBatch,
        header_masks: HeaderBitMaskBatch,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        # Input shapes: chunk embedding [B, Z], masks [B, M].
        # Output shape: [B, M, 16, 8].
        event_logits = self.decode_masked_events(chunk_embedding, masks)
        # Input shape: chunk embedding [B, Z]. Output shape: [B, HM].
        header_logits = self.decode_header(chunk_embedding, header_masks)
        return event_logits, header_logits
