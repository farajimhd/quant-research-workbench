...
from research.masked_event_model.v27.config import DataConfig, ExperimentConfig, LossConfig, MaskConfig, ModelConfig, TrainConfig
from research.masked_event_model.v27.losses import masked_event_bce_loss
from research.masked_event_model.v27.masking import EventMaskBatch, HeaderBitMaskBatch, build_event_masks
from research.masked_event_model.v27.model import EventTokenMaskedAutoencoder
from research.masked_event_model.v27.progress import TrainingProgressState, TrainingReporter
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train v27 event-token MAE on compact event sample-cache batches.")
    ...
    parser.add_argument("--header-loss-weight", type=float, default=loss_defaults.header_weight)
    ...
    parser.add_argument("--min-masked-header-bits", type=int, default=mask_defaults.min_masked_header_bits)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if config.train.decoder_chunk_size > 0:
        print("WARN --decoder-chunk-size is ignored by v27; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    if config.model.decoder_layers != ModelConfig().decoder_layers:
        print("WARN --decoder-layers is ignored by v27; reconstruction uses a per-masked-event MLP decoder.", flush=True)
    ...
    metric_logger = JsonlMetricLogger(run_paths.metrics_path, wandb_run, wandb_key_mapper=v27_wandb_metric_key)
    write_run_manifest(
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    return ExperimentConfig(
        ...
        masks=MaskConfig(
            ...
            header_mask_ratio=args.header_mask_ratio,
            min_masked_header_bits=args.min_masked_header_bits,
        ...
        losses=LossConfig(objective=args.loss_objective, header_weight=float(args.header_loss_weight)),
...
def validation_metric_key(train_key: str) -> str:
    ...
    checkpoint monitors. W&B receives a second mapping in `v27_wandb_metric_key`
...
def v27_wandb_metric_key(metric_key: str) -> str:
    """Put v27 pretraining metrics into explicit W&B panel groups."""
...
def save_model_artifacts(
    ...
    details = {
        ...
        "header_mask_ratio": config.masks.header_mask_ratio,
...
def build_model_summary_text(model: torch.nn.Module, details: dict[str, Any], parameters: list[dict[str, Any]]) -> str:
    lines = [
        "EventTokenMaskedAutoencoder v27",
        "=" * 80,
        f"Input header_uint8: [B, 14]",
        f"Input events_uint8: [B, {details['events_per_chunk']}, 16]",
        f"Training encoder tokens: [B, 1 + visible_header_bits + visible_events, d_model]",
        f"Production encoder tokens: [B, 1 + 112 + {details['events_per_chunk']}, d_model]",
        ...
        f"Output header bit logits: [B, masked_header_bits]",
...
def build_model_mermaid(config: ExperimentConfig) -> str:
    ...
    header_bits = 14 * 8
    masked_header = max(config.masks.min_masked_header_bits, int(round(header_bits * config.masks.header_mask_ratio)))
    masked_header = min(masked_header, header_bits - 1)
    visible_header = header_bits - masked_header
    return f"""flowchart TD
    H[\"header_uint8<br/>B x 14\"] --> HM[\"batch-shared header bit mask<br/>{visible_header} visible / {masked_header} masked\"]
    ...
    HM --> HB[\"visible header bits<br/>B x {visible_header} x 1 in -1/+1\"]
    HB --> HP[\"header bit value + position projection<br/>B x {visible_header} x d_model\"]
    EC --> EP[\"event bits projection<br/>128 -> d_model\"]
    EP --> POS[\"event position + token type\"]
    HP --> TOK[\"encoder tokens<br/>CLS + visible header bits + visible events\"]
    POS --> TOK
    TOK --> ENC[\"Transformer encoder<br/>{config.model.encoder_layers} layers, d={config.model.d_model}, heads={config.model.n_heads}\"]
    ENC --> HPOOL[\"pool encoded visible header bits<br/>B x 1 x d_model\"]
    HPOOL --> GRID[\"zero-initialized fixed grid bottleneck<br/>CLS + pooled header + visible event slots\"]
    GRID --> EMB[\"chunk embedding<br/>B x {config.model.embedding_dim}\"]
    EMB --> MEM[\"project chunk embedding<br/>B x 1 x d_model\"]
    EMB --> HMEM[\"project chunk embedding for header<br/>B x 1 x d_model\"]
    HM --> HPOS[\"decoder-only masked header bit positions<br/>B x {masked_header} x d_model\"]
    HMEM --> HADD[\"chunk memory + masked header positions\"]
    HPOS --> HADD
    HADD --> HDEC[\"header MLP decoder<br/>B x {masked_header} logits\"]
    ...
    HEAD --> LOSS[\"BCE on masked event bits\"]
    HDEC --> HLOSS[\"BCE on masked header bits\"]
"""
...
class MaskedTrainingSummaryWrapper(torch.nn.Module):
    ...
    def __init__(self, model: torch.nn.Module, events_per_chunk: int) -> None:
        ...
        self.header_visible_bit_pool = model.header_visible_bit_pool
        ...
        self.header_mlp_decoder = model.header_mlp_decoder
    ...
    def forward(self, header_uint8: torch.Tensor, events_uint8: torch.Tensor) -> torch.Tensor:
        ...
        header_bit_count = 14 * 8
        masked_header_count = header_bit_count // 2
        visible_header = torch.arange(0, header_bit_count - masked_header_count, device=events_uint8.device, dtype=torch.long)
        masked_header = torch.arange(header_bit_count - masked_header_count, header_bit_count, device=events_uint8.device, dtype=torch.long)
        masks = EventMaskBatch(
        ...
        header_masks = HeaderBitMaskBatch(
            visible_header_bit_indices=visible_header,
            masked_header_bit_indices=masked_header,
            visible_count=int(visible_header.numel()),
            masked_count=int(masked_header.numel()),
            header_bit_count=header_bit_count,
            requested_mask_ratio=float(masked_header.numel() / header_bit_count),
            actual_mask_ratio=float(masked_header.numel() / header_bit_count),
        )
        selected_events_uint8, selected_event_indices = self.visible_event_token_selector(events_uint8, masks.visible_event_indices)
        header_bit_tokens, selected_header_bit_indices = self.header_token_encoder(header_uint8, header_masks.visible_header_bit_indices)
        ...
        encoded_header_bit_tokens = encoded_tokens[:, 1 : 1 + selected_header_bit_indices.numel(), :]
        encoded_event_tokens = encoded_tokens[:, 1 + selected_header_bit_indices.numel() :, :]
        pooled_header_token = self.header_visible_bit_pool(encoded_header_bit_tokens)
        fixed_grid_input_tokens = torch.cat([cls_token, pooled_header_token, encoded_event_tokens], dim=1)
        ...
        header_bit_logits = self.header_mlp_decoder(chunk_embedding, header_masks.masked_header_bit_indices)
        return chunk_embedding, event_bit_logits, header_bit_logits
...
def default_run_name(args: argparse.Namespace) -> str:
    return f"mem-v27-eventmae-emb{args.embedding_dim}-d{args.d_model}-e{args.encoder_layers}-mask{int(args.event_mask_ratio * 100)}-events{args.events_per_chunk}"


def format_metrics(step: int, metrics: dict[str, float]) -> str:
    keys = [
        "pretrain/loss_total",
        "pretrain/loss_event_total",
        "pretrain/loss_header_total",
        "pretrain/event_bit_acc_pct",
        "pretrain/header_bit_acc_pct",
        "pretrain/event_bit_acc_lift_pct",
        "pretrain/event_balanced_bit_acc_pct",
        "pretrain/event_byte_exact_acc_pct",
        "mask/header_mask_ratio_pct",
