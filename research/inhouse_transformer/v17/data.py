...
from research.inhouse_transformer.v17.config import DataConfig
...
ANCHOR_NAMES = (
    "same_minute_d1_window",
    "same_minute_d2_window",
    "same_minute_d3_window",
    "same_minute_d4_window",
    "same_minute_d5_window",
    "same_minute_d6_window",
    "same_weekday_minute_window",
    "previous_regular_close_area",
    "previous_day_full_session",
    "previous_day_regular_session",
    "previous_day_premarket_session",
    "previous_day_afterhours_session",
    "previous_week_full_session",
    "previous_week_regular_session",
    "previous_week_same_weekday_window",
    "previous_5_sessions_full",
)
    ...
    def __iter__(self) -> Iterator[dict[str, Any]]:
        ...
        for epoch in range(self.epochs):
            ...
            batch = BatchBuilder(
                ...
                macro_context_length=self.config.macro_context_length,
...
class BatchBuilder:
    def __init__(
        ...
        *,
        ...
        self.macro_15m_values = np.empty((batch_size, macro_context_length, macro_feature_count), dtype=np.float32)
        self.macro_15m_time_features = np.empty((batch_size, macro_context_length, time_feature_count), dtype=np.float32)
        self.macro_1h_values = np.empty((batch_size, macro_context_length, macro_feature_count), dtype=np.float32)
        self.macro_1h_time_features = np.empty((batch_size, macro_context_length, time_feature_count), dtype=np.float32)
        self.macro_1d_values = np.empty((batch_size, macro_context_length, macro_feature_count), dtype=np.float32)
        self.macro_1d_time_features = np.empty((batch_size, macro_context_length, time_feature_count), dtype=np.float32)
    ...
    def empty_like(self) -> "BatchBuilder":
        return BatchBuilder(
            ...
            macro_context_length=self.macro_15m_values.shape[1],
            macro_feature_count=self.macro_15m_values.shape[2],
    ...
    def add(self, arrays: dict[str, np.ndarray], origin: int, config: DataConfig, *, ticker: str) -> None:
        ...
        self.macro_15m_values[self.count], self.macro_15m_time_features[self.count] = build_macro_context(
            arrays,
            origin,
            config,
            bucket_minutes=config.macro_15m_bucket_minutes,
            current_close=current_close,
        )
        self.macro_1h_values[self.count], self.macro_1h_time_features[self.count] = build_macro_context(
            arrays,
            origin,
            config,
            bucket_minutes=config.macro_1h_bucket_minutes,
            current_close=current_close,
        )
        self.macro_1d_values[self.count], self.macro_1d_time_features[self.count] = build_macro_context(
            arrays,
            origin,
            config,
            bucket_minutes=config.macro_1d_bucket_minutes,
            current_close=current_close,
        )
    ...
    def as_torch(self) -> dict[str, Any]:
        ...
        return {
            ...
            "macro_15m_values": torch.from_numpy(self.macro_15m_values[rows].copy()),
            "macro_15m_time_features": torch.from_numpy(self.macro_15m_time_features[rows].copy()),
            "macro_1h_values": torch.from_numpy(self.macro_1h_values[rows].copy()),
            "macro_1h_time_features": torch.from_numpy(self.macro_1h_time_features[rows].copy()),
            "macro_1d_values": torch.from_numpy(self.macro_1d_values[rows].copy()),
            "macro_1d_time_features": torch.from_numpy(self.macro_1d_time_features[rows].copy()),
...
def history_tail_rows(config: DataConfig) -> int:
    # Enough rows for 1d macro buckets, anchor ranges, and extended-hours sessions.
    session_rows = max(config.anchor_history_sessions + 1, config.macro_context_length + 2, 1) * 960
    intraday_rows = max(
        config.context_length + config.horizon,
        config.macro_context_length * config.macro_1h_bucket_minutes * 2,
    )
    return max(session_rows, intraday_rows)
...
def load_initial_carryover(
    ...
    prior_session_count = max(config.anchor_history_sessions, config.macro_context_length + 2)
    prior_sessions = [session for session in all_sessions if session < first_session]
    prior_sessions = prior_sessions[-prior_session_count:]
...
def build_relative_time_features(
    ...
    *,
    ...
    age_minute_denominator = max(
        float(config.macro_context_length * config.macro_1d_bucket_minutes),
...
def build_macro_context(
    ...
    *,
    ...
    if bucket_minutes == config.macro_1d_bucket_minutes:
        return build_daily_macro_context(arrays, origin, config, current_close=current_close)

    token_count = config.macro_context_length
    values = np.zeros((token_count, len(MACRO_FEATURE_COLUMNS)), dtype=np.float32)
    time_features = np.zeros((token_count, len(config.time_feature_columns)), dtype=np.float32)
    timestamps = arrays["timestamps_ns"]
    bar_close_ns = int(timestamps[origin]) + 60_000_000_000
    bucket_ns = int(bucket_minutes * 60 * 1_000_000_000)
    latest_closed_end = (bar_close_ns // bucket_ns) * bucket_ns
    feature_names = list(MACRO_FEATURE_COLUMNS)

    for token_index in range(token_count):
        bucket_end = latest_closed_end - (token_count - 1 - token_index) * bucket_ns
        bucket_start = bucket_end - bucket_ns
        start = int(np.searchsorted(timestamps, bucket_start, side="left"))
        end = int(np.searchsorted(timestamps, bucket_end, side="left"))
    ...
    normalized = normalize_macro_values(
        values,
        config,
        feature_names,
        current_close=current_close,
...
def build_daily_macro_context(
    ...
    *,
    ...
    selected_sessions = prior_sessions[-token_count:]
    first_token = token_count - len(selected_sessions)
    for offset, session in enumerate(selected_sessions):
        rows = np.flatnonzero(arrays["sessions"][:origin] == session)
        if rows.size == 0:
            continue
        token_index = first_token + offset
        values[token_index] = aggregate_bar_slice(arrays, int(rows[0]), int(rows[-1]) + 1, feature_names)
        time_features[token_index] = build_relative_time_features(
            arrays,
            np.array([int(rows[-1])], dtype=np.int64),
            origin,
            config,
            bucket_minutes=float(config.macro_1d_bucket_minutes),
        )[0]
...
def build_anchor_context(
    ...
    *,
    ...
    for offset, session in enumerate(reversed(previous_sessions), start=1):
        ...
        rows = session_minute_window_rows(arrays, session, current_minute, config.anchor_window_minutes, before_origin=origin)
        fill_anchor_from_rows(values, time_features, arrays, anchor_index, rows, origin, anchor_index, config)

    same_week_rows = same_weekday_minute_window_rows(arrays, current_session, current_minute, origin, config)
    fill_anchor_from_rows(values, time_features, arrays, 6, same_week_rows, origin, 6, config)

    previous_session = prior_sessions[-1] if prior_sessions else ""
    if previous_session:
        fill_previous_session_anchor_groups(values, time_features, arrays, previous_session, origin, config)
        fill_previous_week_anchor_groups(values, time_features, arrays, prior_sessions, origin, config)
...
def session_minute_window_rows(
    arrays: dict[str, np.ndarray],
    session: str,
    minute: int,
    window_minutes: int,
    *,
    ...
    low_minute = minute - max(0, int(window_minutes))
    high_minute = minute + max(0, int(window_minutes))
    mask = (
        (arrays["sessions"][:before_origin] == session)
        & (arrays["minute_of_day"][:before_origin] >= low_minute)
        & (arrays["minute_of_day"][:before_origin] <= high_minute)
    )
    return np.flatnonzero(mask).astype(np.int64)


def same_weekday_minute_window_rows(
    arrays: dict[str, np.ndarray],
    current_session: str,
    minute: int,
    origin: int,
    ...
    for session in reversed(prior_sessions):
        if date.fromisoformat(session).weekday() != current_weekday:
            continue
        rows = session_minute_window_rows(arrays, session, minute, config.anchor_window_minutes, before_origin=origin)
        if rows.size:
            return rows
    return np.array([], dtype=np.int64)


def fill_previous_session_anchor_groups(
    values: np.ndarray,
    ...
    rows = np.flatnonzero(arrays["sessions"][:origin] == session).astype(np.int64)
    if rows.size == 0:
        return
    regular_rows = rows[
        (arrays["minute_of_day"][rows] >= 9 * 60 + 30) & (arrays["minute_of_day"][rows] < 16 * 60)
    ]
    premarket_rows = rows[arrays["minute_of_day"][rows] < 9 * 60 + 30]
    afterhours_rows = rows[arrays["minute_of_day"][rows] >= 16 * 60]
    close_rows = regular_rows[-5:] if regular_rows.size else rows[-5:]
    fill_anchor_from_rows(values, time_features, arrays, 7, close_rows, origin, 7, config, bucket_minutes=5.0, is_anchor_summary=True)
    fill_anchor_from_rows(values, time_features, arrays, 8, rows, origin, 8, config, bucket_minutes=1440.0, is_anchor_summary=True)
    fill_anchor_from_rows(values, time_features, arrays, 9, regular_rows, origin, 9, config, bucket_minutes=390.0, is_anchor_summary=True)
    fill_anchor_from_rows(values, time_features, arrays, 10, premarket_rows, origin, 10, config, bucket_minutes=330.0, is_anchor_summary=True)
    fill_anchor_from_rows(values, time_features, arrays, 11, afterhours_rows, origin, 11, config, bucket_minutes=480.0, is_anchor_summary=True)


def fill_previous_week_anchor_groups(
    values: np.ndarray,
    time_features: np.ndarray,
    arrays: dict[str, np.ndarray],
    ...
    if not prior_sessions:
        return
    week_sessions = prior_sessions[-5:]
    row_parts = [np.flatnonzero(arrays["sessions"][:origin] == session).astype(np.int64) for session in week_sessions]
    ...
    regular_rows = rows[
        (arrays["minute_of_day"][rows] >= 9 * 60 + 30) & (arrays["minute_of_day"][rows] < 16 * 60)
    ] if rows.size else np.array([], dtype=np.int64)
    current_session = str(arrays["sessions"][origin])
    current_minute = int(arrays["minute_of_day"][origin])
    same_weekday_rows = same_weekday_minute_window_rows(arrays, current_session, current_minute, origin, config)
    fill_anchor_from_rows(values, time_features, arrays, 12, rows, origin, 12, config, bucket_minutes=5.0 * 1440.0, is_anchor_summary=True)
    fill_anchor_from_rows(values, time_features, arrays, 13, regular_rows, origin, 13, config, bucket_minutes=5.0 * 390.0, is_anchor_summary=True)
    fill_anchor_from_rows(values, time_features, arrays, 14, same_weekday_rows, origin, 14, config, bucket_minutes=float(config.anchor_window_minutes * 2 + 1), is_anchor_summary=True)
    fill_anchor_from_rows(values, time_features, arrays, 15, rows, origin, 15, config, bucket_minutes=5.0 * 1440.0, is_anchor_summary=True)


def fill_anchor_from_rows(
    values: np.ndarray,
    time_features: np.ndarray,
    arrays: dict[str, np.ndarray],
    anchor_index: int,
    ...
    *,
    ...
    values[anchor_index] = anchor_row_from_rows(
        arrays=arrays,
        rows=rows,
        origin_ts=int(arrays["timestamps_ns"][origin]),
...
def anchor_row_from_rows(
    *,
    ...
    anchor_ts = int(arrays["timestamps_ns"][int(rows[-1])])
