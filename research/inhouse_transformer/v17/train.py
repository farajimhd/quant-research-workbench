...
from research.inhouse_transformer.v17.config import (  # noqa: E402
    DataConfig,
    ExperimentConfig,
    ModelConfig,
    TrainConfig,
)
from research.inhouse_transformer.v17.data import (  # noqa: E402
    ANCHOR_NAMES,
...
from research.inhouse_transformer.v17.metrics import MetricAccumulator, append_jsonl  # noqa: E402
...
forecast_loss = None
LOG_RULE = "*" * 96
EXPERIMENT_VERSION = "v17"
MODEL_ARCHITECTURE_NAME = "gated_macro_context_binary"
...
METRIC_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "loss": {
        ...
        "unit": "target units",
        "interpretation": "Down is better. For v17 this is binary cross entropy over sign plus magnitude bits.",
    },
    "regression_loss": {
    ...
    "h1_hard_decoded_mae_bps": {
        ...
        "unit": "bps",
        "interpretation": "Down is better. This is the legacy v17 decoded prediction error.",
    },
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--macro-context-length", type=int, default=defaults.data.macro_context_length)
    parser.add_argument("--macro-15m-bucket-minutes", type=int, default=defaults.data.macro_15m_bucket_minutes)
    parser.add_argument("--macro-1h-bucket-minutes", type=int, default=defaults.data.macro_1h_bucket_minutes)
    parser.add_argument("--macro-1d-bucket-minutes", type=int, default=defaults.data.macro_1d_bucket_minutes)
    parser.add_argument("--anchor-history-sessions", type=int, default=defaults.data.anchor_history_sessions)
    parser.add_argument("--anchor-window-minutes", type=int, default=defaults.data.anchor_window_minutes)
    ...
    parser.add_argument(
        ...
        help=(
            "Main transformer target format. v17 uses sign plus 12 magnitude bits for log-return bps."
        ),
    )
    parser.add_argument(
        "--target-columns",
    ...
    parser.add_argument("--macro-gate-init-bias", type=float, default=defaults.model.macro_gate_init_bias)
    parser.add_argument("--macro-dropout", type=float, default=defaults.model.macro_dropout)
...
def config_from_args(args: argparse.Namespace) -> ExperimentConfig:
    ...
    data = DataConfig(
        ...
        macro_context_length=args.macro_context_length,
        macro_15m_bucket_minutes=args.macro_15m_bucket_minutes,
        macro_1h_bucket_minutes=args.macro_1h_bucket_minutes,
        macro_1d_bucket_minutes=args.macro_1d_bucket_minutes,
        anchor_history_sessions=args.anchor_history_sessions,
        anchor_window_minutes=args.anchor_window_minutes,
    ...
    model = ModelConfig(
        ...
        macro_gate_init_bias=args.macro_gate_init_bias,
...
def main() -> None:
    ...
    print(
        ...
        f"macro_tokens={config.data.macro_context_length} "
        f"anchor_tokens={len(ANCHOR_NAMES)} "
        f"total_main_feature_attention_tokens={len(config.data.input_feature_columns) + len(config.data.time_feature_columns)})",
        flush=True,
    )
    ...
    model = FeatureTemporalTransformer(
        ...
        macro_context_length=config.data.macro_context_length,
...
def infer_timeline_batch_rows(
    *,
    ...
    with torch.autocast(
        ...
        prediction = model_forward(model, device_batch)
...
def model_forward(model: torch.nn.Module, batch: dict[str, Any]) -> torch.Tensor:
    return model(
        batch["values"],
        batch["time_features"],
        batch["macro_15m_values"],
        batch["macro_15m_time_features"],
        batch["macro_1h_values"],
        batch["macro_1h_time_features"],
        batch["macro_1d_values"],
        batch["macro_1d_time_features"],
        batch["anchor_values"],
        batch["anchor_time_features"],
    )
...
def config_to_dict(config: ExperimentConfig) -> dict[str, Any]:
    return {
        ...
        "data": {
            ...
            "context_length": config.data.context_length,
            "macro_context_length": config.data.macro_context_length,
            "macro_15m_bucket_minutes": config.data.macro_15m_bucket_minutes,
            "macro_1h_bucket_minutes": config.data.macro_1h_bucket_minutes,
            "macro_1d_bucket_minutes": config.data.macro_1d_bucket_minutes,
            "anchor_history_sessions": config.data.anchor_history_sessions,
            "anchor_window_minutes": config.data.anchor_window_minutes,
            "horizon": config.data.horizon,
        ...
        "model": {
            ...
            "direction_threshold_bps": config.model.direction_threshold_bps,
            "macro_gate_init_bias": config.model.macro_gate_init_bias,
            "macro_dropout": config.model.macro_dropout,
        },
        "train": {
            "batch_size": config.train.batch_size,
            "epochs": config.train.epochs,
...
def load_torch_stack() -> None:
    ...
    try:
        ...
        from research.inhouse_transformer.v17.model import (
