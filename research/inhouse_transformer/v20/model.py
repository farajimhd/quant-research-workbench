...
from research.inhouse_transformer.v20.config import ModelConfig


class FeatureTemporalTransformer(nn.Module):
    """Factorized transformer with market and time features as parallel token sets."""
    ...
    def forward(self, values: torch.Tensor, time_features: torch.Tensor) -> torch.Tensor:
        ...
        prediction = self.regression_head(last_token).reshape(
            batch_size,
            self.horizon,
            self.target_count,
