...
from research.inhouse_transformer.v20.config import DataConfig
...
def load_session_frame(config: DataConfig, session: str, tickers: tuple[str, ...]) -> pl.DataFrame:
    ...
    return add_synthetic_clock_bars(frame, config=config, session=session)


def add_synthetic_clock_bars(frame: pl.DataFrame, *, config: DataConfig, session: str) -> pl.DataFrame:
    if frame.is_empty():
        return frame.with_columns(
            pl.lit(0.0, dtype=pl.Float32).alias("is_synthetic_bar"),
            pl.lit(0.0, dtype=pl.Float32).alias("minutes_since_trade"),
        )

    session_end = frame.get_column("bar_time_market").max()
    bounds = frame.group_by("ticker").agg(
        pl.min("bar_time_market").alias("_start"),
        pl.lit(session_end).alias("_end"),
    )
    grid = (
        bounds.with_columns(pl.datetime_ranges("_start", "_end", interval="1m", closed="both").alias("bar_time_market"))
        .explode("bar_time_market")
        .with_columns(pl.col("bar_time_market").cast(frame.schema["bar_time_market"]))
        .select("ticker", "bar_time_market")
    )
    joined = grid.join(frame, on=["ticker", "bar_time_market"], how="left").sort(["ticker", "bar_time_market"])
    is_real = pl.col("close").is_not_null()
    joined = (
        joined.with_columns(
            is_real.cast(pl.Int8).alias("_is_real_bar"),
            pl.when(is_real)
            .then(pl.col("bar_time_market").dt.timestamp("ns"))
            .otherwise(None)
            .alias("_real_time_ns"),
        )
        .with_columns(pl.col("_real_time_ns").forward_fill().over("ticker").alias("_last_real_time_ns"))
        .with_columns(
            (
                (pl.col("bar_time_market").dt.timestamp("ns") - pl.col("_last_real_time_ns")).cast(pl.Float64)
                / 60_000_000_000.0
            )
            .cast(pl.Float32)
            .alias("minutes_since_trade")
        )
        .filter(pl.col("minutes_since_trade") <= float(config.max_synthetic_minutes))
    )

    last_close = pl.col("close").forward_fill().over("ticker")
    synthetic_price = last_close
    quote_columns = ("spread_bps", "quote_bid_size", "quote_ask_size", "quote_valid_ratio", "quoted_share_depth")
    return (
        joined.with_columns(
            pl.lit(session).alias("session_date"),
            (pl.col("bar_time_market").dt.hour() * 60 + pl.col("bar_time_market").dt.minute())
            .cast(pl.Int32)
            .alias("minute_of_day"),
            pl.when(is_real).then(pl.col("open")).otherwise(synthetic_price).cast(pl.Float32).alias("open"),
            pl.when(is_real).then(pl.col("high")).otherwise(synthetic_price).cast(pl.Float32).alias("high"),
            pl.when(is_real).then(pl.col("low")).otherwise(synthetic_price).cast(pl.Float32).alias("low"),
            synthetic_price.cast(pl.Float32).alias("close"),
            pl.when(is_real).then(pl.col("volume").fill_null(0.0)).otherwise(0.0).cast(pl.Float32).alias("volume"),
            pl.when(is_real).then(pl.col("transactions").fill_null(0.0)).otherwise(0.0).cast(pl.Float32).alias("transactions"),
            (1.0 - pl.col("_is_real_bar").cast(pl.Float32)).alias("is_synthetic_bar"),
            *[
                pl.col(column)
                .forward_fill()
                .over("ticker")
...
def ticker_arrays(frame: pl.DataFrame, config: DataConfig) -> dict[str, np.ndarray]:
    ...
    is_synthetic_bar = np.clip(nonnegative_array(frame, "is_synthetic_bar"), 0.0, 1.0)
    minutes_since_trade = nonnegative_array(frame, "minutes_since_trade")
...
def valid_origins(arrays: dict[str, np.ndarray], current_session: str, config: DataConfig) -> np.ndarray:
    ...
    if origins.size:
        origins = origins[contiguous_clock_window_mask(arrays["timestamps_ns"], origins, config)]
...
def contiguous_clock_window_mask(timestamps_ns: np.ndarray, origins: np.ndarray, config: DataConfig) -> np.ndarray:
    if origins.size == 0:
        return np.empty(0, dtype=bool)
    one_minute_ns = 60_000_000_000
    bad_gap = np.zeros(timestamps_ns.shape[0], dtype=np.int32)
    if timestamps_ns.shape[0] > 1:
        bad_gap[1:] = (np.diff(timestamps_ns) != one_minute_ns).astype(np.int32)
    bad_cumsum = np.cumsum(bad_gap)
    starts = origins - config.context_length + 1
    ends = origins + config.horizon
    return (bad_cumsum[ends] - bad_cumsum[starts]) == 0
