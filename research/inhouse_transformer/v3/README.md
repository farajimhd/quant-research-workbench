# In-House 1m Bar Transformer Baseline v3

This module trains a scratch PyTorch transformer baseline on provider-built 1-minute bars.

This folder is a versioned experiment. `../v2` preserves the return-bps target
experiment. v3 keeps the same target objective and model architecture, but changes
the input representation for OHLC prices.
...
Then every input column, including transformed OHLC and the non-price features,
is normalized causally with only per-window z-score:
`(value - context_column_mean) / context_column_std`. No train-wide statistics
are applied to input values.

The v3 main transformer defaults to `--target-mode return_bps`. Targets are the
...
When `--overfit-batches` is used, the script logs `overfit_timeline_predictions/*` W&B charts after the final test pass. It selects three tickers from the cached training batches, reloads their chronological session data, and overlays thin solid predicted h1 close against thicker solid target h1 close for each ticker, plus a table with the underlying values.

Run a small dry run:

```powershell
python research\inhouse_transformer\v3\train.py --dry-run --count-coverage --tickers USO --batch-size 128 --max-batches-per-session 2
```

Run the default experiment:

```powershell
python research\inhouse_transformer\v3\train.py --device cuda --batch-size 1024 --epochs 1
```

Run the main transformer one-session overfit test with wandb logging:

```powershell
python research\inhouse_transformer\v3\train.py --device cuda --overfit-session 2024-01-22 --target-columns close --horizon 1 --batch-size 1024 --epochs 200 --eval-steps 25 --logging-steps 25 --validation-window-count 8192 --test-window-count 8192 --warmup-steps 0 --wandb-entity mehdifaraji --wandb-project May2026-1m-timeseries-forecasting
```

The default W&B run name starts with `v3-` and includes `ohlc-bps-window-zscore` and `return_bps` so it can be compared directly against v2.

Run the flat MLP overfit sanity test:

```powershell
python research\inhouse_transformer\v3\train_mlp.py --device cuda --tickers USO --train-start-date 2024-01-22 --train-end-date 2024-01-22 --validation-start-date 2026-01-02 --validation-end-date 2026-01-02 --batch-size 256 --overfit-batches 4 --epochs 50 --eval-steps 25
```
...
```text
D:\TradingData\quant-research-workbench\market_data\models\inhouse_transformer\v3
```
