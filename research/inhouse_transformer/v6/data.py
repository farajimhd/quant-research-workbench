...
from research.inhouse_transformer.v6.config import DataConfig
...
def ticker_arrays(frame: pl.DataFrame, config: DataConfig) -> dict[str, np.ndarray]:
    ...
    is_premarket = (minute < 9 * 60 + 30).astype(np.float32)
    is_regular = ((minute >= 9 * 60 + 30) & (minute < 16 * 60)).astype(np.float32)
    is_afterhours = (minute >= 16 * 60).astype(np.float32)
    calendar = calendar_arrays(frame)
...
def calendar_arrays(frame: pl.DataFrame) -> dict[str, np.ndarray]:
    decoded = frame.select(
        pl.col("bar_time_market").dt.year().cast(pl.Float32).alias("year"),
        pl.col("bar_time_market").dt.month().cast(pl.Float32).alias("month"),
        pl.col("bar_time_market").dt.day().cast(pl.Float32).alias("day"),
        pl.col("bar_time_market").dt.hour().cast(pl.Float32).alias("hour"),
        pl.col("bar_time_market").dt.minute().cast(pl.Float32).alias("minute"),
        pl.col("bar_time_market").dt.second().cast(pl.Float32).alias("second"),
        pl.col("bar_time_market").dt.microsecond().cast(pl.Float32).alias("microsecond"),
        pl.col("bar_time_market").dt.ordinal_day().cast(pl.Float32).alias("day_of_year"),
        pl.col("bar_time_market").dt.weekday().cast(pl.Float32).alias("day_of_week"),
    )
    return {
...
def normalize_actual_feature_window(
    ...
    *,
    ...
    raw = anchored_log1p_inputs_to_origin(raw, config)
...
def anchored_log1p_inputs_to_origin(values: np.ndarray, config: DataConfig) -> np.ndarray:
    transformed = values.copy()
    feature_names = list(config.input_feature_columns)
    anchored_columns = (
        "volume",
    ...
    for column in anchored_columns:
        if column in feature_names:
            index = feature_names.index(column)
