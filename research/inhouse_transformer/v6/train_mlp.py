...
from research.inhouse_transformer.v6.config import DataConfig, TrainConfig  # noqa: E402
from research.inhouse_transformer.v6.data import (  # noqa: E402
...
from research.inhouse_transformer.v6.metrics import MetricAccumulator, append_jsonl  # noqa: E402
from research.inhouse_transformer.v6.model_mlp import FlatMLPForecaster  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Use --overfit-batches to repeat a fixed small train sample and verify the model can minimize train loss."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Cache this many train batches and cycle them repeatedly. Use this for the learning sanity test.",
    )
...
def main() -> None:
    ...
    print(
        ...
        f"target={data_config.horizon}x{len(data_config.target_columns)}",
        flush=True,
    )
    print(
        f"MLP model: hidden_dim={args.hidden_dim} layers={args.layers} dropout={args.dropout} "
        f"overfit_batches={args.overfit_batches}",
        flush=True,
    )
