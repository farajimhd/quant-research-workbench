...
from research.inhouse_transformer.v6.config import (  # noqa: E402
    DataConfig,
    ExperimentConfig,
    ModelConfig,
    TrainConfig,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Main transformer target format. v6 defaults to return_bps so the model predicts the next move "
...
def input_experiment_name(config: ExperimentConfig) -> str:
    if config.data.input_normalization == "window_zscore_only":
        return "calendar-values-anchored-log1p-window-zscore"
...
def main() -> None:
    ...
    print(
        f"Features={len(config.data.input_feature_columns)} "
    ...
    while True:
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=config.train.amp and device.type == "cuda"):
            prediction, direction_logits = model(batch["values"])
...
def infer_timeline_batch_rows(
    *,
    ...
    with torch.autocast(
        ...
        prediction, _ = model(device_batch["values"])
...
def load_torch_stack() -> None:
    ...
    try:
        ...
        from research.inhouse_transformer.v6.model import (
