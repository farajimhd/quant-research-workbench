...
from research.inhouse_transformer.v12.config import ModelConfig
...
class FeatureTemporalTransformer(nn.Module):
    """v12 multi-scale transformer with separate 1m, 5m, 30m, and anchor encoders."""
    ...
    def forward(
        ...
        summaries = summaries + self.scale_embedding(scale_ids).view(1, 4, -1)
        fused = self.fusion_encoder(summaries)
        fused_token = self.fusion_norm(fused.mean(dim=1))
