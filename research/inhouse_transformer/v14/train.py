...
from research.inhouse_transformer.v14.config import (  # noqa: E402
    DataConfig,
    ExperimentConfig,
    ModelConfig,
    TrainConfig,
...
METRIC_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "loss": {
        ...
        "unit": "target units",
        "interpretation": "Down is better. For v14 this is binary cross entropy over sign plus magnitude bits.",
    },
    "regression_loss": {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Main transformer target format. v14 uses sign plus 12 magnitude bits for log-return bps."
        ),
    )
    parser.add_argument(
        "--target-columns",
...
def resolve_run_identity(args: argparse.Namespace, config: ExperimentConfig, output_dir: Path) -> tuple[str, str]:
    ...
    run_name = args.wandb_run_name or str(state.get("wandb_run_name") or "") or make_wandb_run_name(args, config)
...
def main() -> None:
    ...
    run_name, run_id = resolve_run_identity(args, config, output_dir)
    write_run_state(
        output_dir,
        initial_run_state(args=args, config=config, run_name=run_name, run_id=run_id, output_dir=output_dir),
    )
    ...
    load_torch_stack()
    wandb_run = init_wandb(args, config, metadata, run_name, run_id)
    set_seed(config.train.seed)
...
def load_torch_stack() -> None:
    ...
    try:
        ...
        from research.inhouse_transformer.v14.model import (
