# In-House 1m Bar Transformer Baseline v13

This module trains a scratch PyTorch transformer baseline on provider-built 1-minute bars.

This folder is a versioned experiment. v13 starts from `../v12` and keeps the
same multi-timeframe context with separate encoders:
...
v13 keeps the v7 scaled decoded market-time fields in the separate tensor:
...
v13 also appends relative historical time features at sample-build time. These
...
The v13 main transformer defaults to `--target-mode return_bps`. Targets are the
...
the 1m, 5m, 30m, and anchor branches. v13 changes the v12 fusion by adding a
learnable branch gate initialized to favor the recent 1m branch:

```text
initial gate logits = [2.0, -1.0, -1.0, -1.0]
initial softmax weights ~= [0.87, 0.04, 0.04, 0.04]
```

The fused prediction token is:

```text
LayerNorm(one_min_summary + weighted_fusion_context)
```

This keeps v13 closer to the v7 recent-context behavior at initialization while
still allowing 5m, 30m, and anchor history to gain influence if training finds
them useful.
...
```powershell
python research\inhouse_transformer\v13\train.py --dry-run --count-coverage --tickers USO --batch-size 128 --max-batches-per-session 2
```

Run the default experiment:

```powershell
python research\inhouse_transformer\v13\train.py --device cuda --batch-size 1024 --epochs 1
```

Run the main transformer one-session overfit test with wandb logging:

```powershell
python research\inhouse_transformer\v13\train.py --device cuda --overfit-session 2024-03-01 --target-columns close --horizon 1 --batch-size 256 --overfit-window-count 8192 --epochs 200 --eval-steps 25 --logging-steps 25 --validation-window-count 8192 --test-window-count 8192 --warmup-steps 0 --wandb-entity mehdifaraji --wandb-project May2026-1m-timeseries-cache-v2
```

The default W&B run name starts with `v13-` and includes
...
```powershell
python research\inhouse_transformer\v13\train_mlp.py --device cuda --tickers USO --train-start-date 2024-01-22 --train-end-date 2024-01-22 --validation-start-date 2026-01-02 --validation-end-date 2026-01-02 --batch-size 256 --overfit-batches 4 --epochs 50 --eval-steps 25
```
...
```text
D:\TradingData\quant-research-workbench\market_data\models\inhouse_transformer\v13
```
