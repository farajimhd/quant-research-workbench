...
from research.inhouse_transformer.v4.config import DataConfig
...
def normalize_actual_feature_window(
    ...
    *,
    ...
    raw = log_compress_activity_inputs(raw, config)
...
def log_compress_activity_inputs(values: np.ndarray, config: DataConfig) -> np.ndarray:
    transformed = values.copy()
    feature_names = list(config.input_feature_columns)
    for column in ("volume", "transactions", "quote_bid_size", "quote_ask_size", "quoted_share_depth"):
        if column in feature_names:
            index = feature_names.index(column)
            transformed[:, index] = np.log1p(np.maximum(transformed[:, index], 0.0)).astype(np.float32)
    return transformed
