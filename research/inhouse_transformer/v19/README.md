# In-House 1m Bar Transformer Baseline v19

This module trains a scratch PyTorch transformer baseline on provider-built 1-minute bars.

This folder is a versioned experiment. v19 starts from `../v16` and changes only
the time-feature preprocessing. The architecture is still the v16 parallel
market/time token model, but every time-feature column is normalized causally
with a per-window z-score before it is projected into time tokens.
...
v7/v19 add scaled decoded market-time fields to that same separate tensor:
...
These added fields are computed from `bar_time_market`, not UTC. In v19, all
time-feature columns, including cyclical fields and session flags, are
normalized inside each sample's context window:

```text
time_feature = (time_feature - context_column_mean) / context_column_std
```

If a time-feature column is constant inside the window, its normalized values
become zero. This is intentional for the v19 ablation: the model sees local
relative time structure rather than absolute bounded time descriptors. Each
normalized time feature is then treated as a separate token with its own
time-feature embedding. The market and time tokens share the same
context-position embedding for each bar.

The v19 main transformer defaults to `--target-mode binary_magnitude_bps`.
...
```powershell
python research\inhouse_transformer\v19\train.py --dry-run --count-coverage --tickers USO --batch-size 128 --max-batches-per-session 2
```

Run the default experiment:

```powershell
python research\inhouse_transformer\v19\train.py --device cuda --batch-size 1024 --epochs 1
```

Run the main transformer one-session overfit test with wandb logging:

```powershell
python research\inhouse_transformer\v19\train.py --device cuda --overfit-session 2024-01-22 --batch-size 512 --epochs 200 --eval-steps 25 --logging-steps 25 --validation-window-count 5000 --test-window-count 10000 --allow-target-across-session --wandb-entity mehdifaraji --wandb-project May2026-1m-timeseries-v14-variants
```

The default W&B run name starts with `v19-` and includes
...
`v14-overfit-baseline-b512`, the script rewrites that prefix to `v19-` and
prints the normalized name before W&B init.

Run the flat MLP overfit sanity test:

```powershell
python research\inhouse_transformer\v19\train_mlp.py --device cuda --tickers USO --train-start-date 2024-01-22 --train-end-date 2024-01-22 --validation-start-date 2026-01-02 --validation-end-date 2026-01-02 --batch-size 256 --overfit-batches 4 --epochs 50 --eval-steps 25
```
...
```text
D:\TradingData\quant-research-workbench\market_data\models\inhouse_transformer\v19
```
