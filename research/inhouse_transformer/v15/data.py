...
from research.inhouse_transformer.v15.config import DataConfig
...
class BatchBuilder:
    ...
    def add(self, arrays: dict[str, np.ndarray], origin: int, config: DataConfig, *, ticker: str) -> None:
        ...
        elif config.target_mode == "ordinal_exceedance_bps":
            center = 0.0
            scale = 1.0
            targets = encode_ordinal_exceedance_targets(target_bps, thresholds_bps=config.ordinal_thresholds_bps)
        else:
...
def target_bit_count(config: DataConfig) -> int:
    if config.target_mode == "ordinal_exceedance_bps":
        return 2 * len(config.ordinal_thresholds_bps)
    return 1
...
def target_values_to_bps(
    ...
    if target_mode == "ordinal_exceedance_bps":
        return decode_ordinal_exceedance_logits_to_bps(values)
    raise ValueError(f"Unsupported target_mode: {target_mode}")


def encode_ordinal_exceedance_targets(target_bps: np.ndarray, *, thresholds_bps: tuple[float, ...]) -> np.ndarray:
    target_bps = np.asarray(target_bps, dtype=np.float32)
    thresholds = np.asarray(thresholds_bps, dtype=np.float32)
    up_bits = (target_bps[..., None] >= thresholds).astype(np.float32)
    down_bits = (target_bps[..., None] <= -thresholds).astype(np.float32)
    return np.concatenate([up_bits, down_bits], axis=-1).astype(np.float32)


def decode_ordinal_exceedance_logits_to_bps(values: np.ndarray) -> np.ndarray:
    logits = np.asarray(values, dtype=np.float64)
    if logits.ndim < 1 or logits.shape[-1] % 2 != 0:
        raise ValueError(f"Expected ordinal exceedance logits with paired up/down bit axis, got shape {logits.shape}.")
    half = logits.shape[-1] // 2
    thresholds = ordinal_thresholds_for_count(half)
    widths = np.diff(np.concatenate([[0.0], thresholds]))
    probabilities = sigmoid_np(logits)
    up_expected = (probabilities[..., :half] * widths).sum(axis=-1)
    down_expected = (probabilities[..., half:] * widths).sum(axis=-1)
    return up_expected - down_expected


def ordinal_thresholds_for_count(count: int) -> np.ndarray:
    thresholds = np.asarray(DataConfig().ordinal_thresholds_bps, dtype=np.float64)
    if count != thresholds.shape[0]:
        raise ValueError(f"Expected {thresholds.shape[0]} ordinal thresholds, got {count}.")
    return thresholds
