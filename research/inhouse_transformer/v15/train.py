...
from research.inhouse_transformer.v15.config import (  # noqa: E402
    DataConfig,
    ExperimentConfig,
    ModelConfig,
    TrainConfig,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--target-mode",
        choices=["ordinal_exceedance_bps"],
        default=defaults.data.target_mode,
        help=(
            "Main transformer target format. v15 uses signed ordinal exceedance bits for log-return bps."
        ),
    )
    parser.add_argument(
        "--target-columns",
...
def prediction_and_target_bps(
    ...
    if config.data.target_mode == "ordinal_exceedance_bps":
        current_close = batch["current_close"].detach().cpu().numpy()
...
def config_to_dict(config: ExperimentConfig) -> dict[str, Any]:
    return {
        ...
        "data": {
            ...
            "ordinal_thresholds_bps": list(config.data.ordinal_thresholds_bps),
...
def load_torch_stack() -> None:
    ...
    try:
        ...
        from research.inhouse_transformer.v15.model import (
