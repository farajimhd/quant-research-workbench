...
def command_generation_source(version: str) -> str:
    ...
    return (
        ...
        "        f\"$logPath = Join-Path {ps_quote(log_dir)} ('{log_prefix}_' + $runStamp + '.log')\\n\"\n"
        ...
        "        f\"{command} 2>&1 | Tee-Object -FilePath $logPath\\n\"\n"
        "        \"if ($LASTEXITCODE -ne 0) { throw \\\"Command failed with exit code $LASTEXITCODE\\\" }\\n\"\n"
        "    )\n"
        "    ps1_path.write_text(ps1, encoding='utf-8')\n"
        "    return ps1_path, log_dir / f'{log_prefix}_<timestamp>.log', command\n"
        "\n"
