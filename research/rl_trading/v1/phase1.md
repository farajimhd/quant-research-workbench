...
Use `build_phase1.py` for the Phase 1 → Phase 2 workflow.
...
python -B research/rl_trading/v1/build_phase1.py run --start 2026-08-18 --end 2026-09-18 --workers 4
```
...
Every decision grid includes the 19:58 liquidation row and post-cutoff rows through 20:00. Where a current price
...
The canary figures below describe earlier, immutable outputs under the old source paths. They do not certify a newly relocated package run. The package's current SQL authority was preflighted read-only against a completed workstation V5 build for 2026-08-21/AAPL; the focused local test suite covers the label and campaign contracts.
