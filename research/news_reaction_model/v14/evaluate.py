...
from research.news_reaction_model.v14 import HORIZONS, MODEL_VERSION
...
from research.news_reaction_model.v14.inference import opportunity_predictions
from research.news_reaction_model.v14.metrics import balanced_accuracy, macro_f1
...
SELECT p.canonical_news_id AS source_id, p.ticker, p.published_at_utc,
 ...
 p.numeric_ids, p.numeric_weights, p.numeric_dense, p.stock_state,
 p.publication_session, p.horizon_codes, p.return_targets,
 a.anchor_values
...
class ClickHouseEvaluationDataset:
    ...
    def iter_batches(self) -> Iterator[EvaluationBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v14-eval-{index}", daemon=True)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate V14 three-class opportunities and the midpoint-return P&L proxy."
    )
