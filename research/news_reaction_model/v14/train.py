...
from research.news_reaction_model.v14.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
...
from research.news_reaction_model.v14.evaluate import evaluate_checkpoint
from research.news_reaction_model.v14.losses import compute_loss
from research.news_reaction_model.v14.metrics import (
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train V14: V10 targets and training with sparse TF-IDF token attention."
    )
    ...
    parser.add_argument("--word-vocab-size", type=int, default=loader.word_vocab_size)
    parser.add_argument("--char-vocab-size", type=int, default=loader.char_vocab_size)
    ...
    parser.add_argument("--max-word-tokens", type=int, default=loader.max_word_tokens)
    parser.add_argument("--max-char-tokens", type=int, default=loader.max_char_tokens)
    parser.add_argument("--max-numeric-tokens", type=int, default=loader.max_numeric_tokens)
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    loader = LoaderConfig(
        ...
        word_vocab_size=max(1, args.word_vocab_size),
        char_vocab_size=max(1, args.char_vocab_size),
        numeric_vocab_size=max(1, args.numeric_vocab_size),
        numeric_dense_dim=max(1, args.numeric_dense_dim),
        stock_state_dim=max(1, args.stock_state_dim),
        max_word_tokens=max(1, args.max_word_tokens),
        max_char_tokens=max(1, args.max_char_tokens),
        max_numeric_tokens=max(1, args.max_numeric_tokens),
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 50:
        raise ValueError("V14 training requires --epochs between 1 and 50 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V14 train and validation rows were built by different representations.")
        manifest_path = config.loader.representation_artifact_root / "manifest.json"
        if not manifest_path.exists():
            raise RuntimeError(f"V14 representation manifest is missing: {manifest_path}")
        ...
        if train_audit["representation_sha256"] != feature_manifest["representation_sha256"]:
            raise RuntimeError(
                "Prepared V14 rows do not match the checksummed V7 TF-IDF-plus-stock-state representation."
            )
        print(
    ...
    raw_model = NewsReactionModelV14(config.model).to(device)
    ...
    write_run_manifest(paths.manifest_path, repo_root=REPO_ROOT, model_family=MODEL_FAMILY, version=MODEL_VERSION,
                       ...
                       data_roots={"prepared_dataset": f"{config.loader.dataset_database}.{config.loader.dataset_table}",
                                   ...
                                   "source_model_version": "v7",
                                   "representation": config.loader.representation_name,
                                   "feature_artifacts": str(config.loader.representation_artifact_root),
                                   "v5_feature_artifacts": str(config.loader.v5_feature_artifact_root),
                                   "v6_feature_artifacts": str(config.loader.v6_feature_artifact_root),
    ...
    write_model_artifacts(
        ...
        input_contract={
            "word_ids": ["B", config.loader.max_word_tokens],
            "word_weights": ["B", config.loader.max_word_tokens],
            "word_mask": ["B", config.loader.max_word_tokens],
            "char_ids": ["B", config.loader.max_char_tokens],
            "char_weights": ["B", config.loader.max_char_tokens],
            "char_mask": ["B", config.loader.max_char_tokens],
            "numeric_ids": ["B", config.loader.max_numeric_tokens],
            "numeric_weights": ["B", config.loader.max_numeric_tokens],
            "numeric_mask": ["B", config.loader.max_numeric_tokens],
            "numeric_dense": ["B", config.loader.numeric_dense_dim],
        ...
        summary_notes=(
            "V14 preserves corrected V10 targets, equal-horizon loss, deterministic "
            "shuffling, scheduler, evaluation, and resume semantics while replacing "
            "the OpenAI vector with bounded V7 sparse TF-IDF tokens and horizon cross-attention."
        ),
...
def restore(
    ...
    *,
    ...
    if state.get("resume_contract") != expected_contract:
        raise ValueError(
            "Checkpoint predates or violates the exact V14 resume contract."
        )
