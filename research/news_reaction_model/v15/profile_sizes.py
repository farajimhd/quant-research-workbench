...
from research.news_reaction_model.v15.data import PreparedNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
...
from research.news_reaction_model.v15.train import amp_dtype, set_seed
...
def profile_configuration(source: NewsReactionBatch, batch_size: int, d_model: int, layers: int, device: torch.device, args: argparse.Namespace, loader: LoaderConfig | None = None) -> dict[str, object]:
    ...
    try:
        model = NewsReactionModelV15(config).to(device); optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4)
