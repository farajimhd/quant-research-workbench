...
from research.news_reaction_model.v11.stock_state import STOCK_STATE_DIM
from research.news_reaction_model.v11.time_features import TIME_FEATURE_DIM
...
class LoaderConfig:
    ...
    # V11 reuses the exact prepared V8 rows; its packed-market V1 time channel
    # is derived losslessly from published_at_utc during loading.
...
class TrainConfig:
    ...
    # Keep the V3-V10 project so V11 remains directly comparable.
...
def default_run_name(config: ExperimentConfig) -> str:
    ...
    return (
        f"news-v11-opportunity-openai-stock-state-causalv1-time-d{config.model.d_model}"
