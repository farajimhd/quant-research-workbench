...
from research.news_reaction_model.v11.config import LoaderConfig, ModelConfig
from research.news_reaction_model.v11.model import NewsReactionModelV11
from research.news_reaction_model.v11.opportunity import (
    OPPORTUNITY_CLASS_NAMES,
    OpportunityClass,
)
from research.news_reaction_model.v11.time_features import encode_time_features
    ...
    def encode(self, rows: list[dict[str, Any]], *, device: torch.device) -> dict[str, torch.Tensor]:
        ...
        for source in rows:
            ...
            if not isinstance(embedding, (list, tuple)) or len(embedding) != self.loader_config.openai_embedding_dim:
                raise ValueError(
                    f"V11 live inference requires a {self.loader_config.openai_embedding_dim}-value "
            ...
            if not isinstance(state, (list, tuple)) or len(state) != self.loader_config.stock_state_dim:
                raise ValueError(
                    f"V11 live inference requires the configured {self.loader_config.stock_state_dim}-value "
            ...
            if not published_at_utc:
                raise ValueError(
                    "V11 live inference requires published_at_utc to reproduce the "
                    "packed-market V1 causal time channel."
                )
            embeddings.append(embedding_values)
            states.append(state_values)
            times.append(
                encode_time_features(published_at_utc)
            )
            masks.append(
                [
...
def load_model(checkpoint: Path, *, device: torch.device) -> tuple[NewsReactionModelV11, LoaderConfig]:
    with torch.serialization.safe_globals([type(Path())]):
        state = torch.load(checkpoint, map_location=device, weights_only=True)
    loader_config = LoaderConfig(**state["config"]["loader"])
    model = NewsReactionModelV11(ModelConfig(**state["config"]["model"])).to(device)
