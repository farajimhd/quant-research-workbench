...
from research.mlops.packed_market.time_features import CAUSAL_TIME_FEATURE_NAMES
from research.mlops.packed_market.streaming import STREAM_EVENT_FEATURE_NAMES
from research.news_reaction_model import v10, v11
from research.news_reaction_model.v10.config import ModelConfig as V10ModelConfig
from research.news_reaction_model.v10.config import TrainConfig as V10TrainConfig
from research.news_reaction_model.v11.config import (
    ExperimentConfig,
    LoaderConfig,
    ModelConfig,
    TrainConfig,
)
from research.news_reaction_model.v11.data import (
...
from research.news_reaction_model.v11.fit_diagnostic import fit_comparison
from research.news_reaction_model.v11.inference import (
    LiveFeatureEncoder,
    opportunity_predictions,
)
from research.news_reaction_model.v11.losses import compute_loss
from research.news_reaction_model.v11.metrics import TrainingLossAccumulator
from research.news_reaction_model.v11.model import (
    NewsReactionModelV11,
    NewsReactionOpportunityOutput,
)
from research.news_reaction_model.v11.memorization_test import (
    deterministic_subset_sql,
    slice_batch,
)
from research.news_reaction_model.v11.opportunity import (
    OPPORTUNITY_CLASSES,
...
from research.news_reaction_model.v11.time_features import (
    TIME_FEATURE_DIM,
    TIME_FEATURE_NAMES,
    encode_time_features,
)
from research.news_reaction_model.v11.train import (
...
class NewsReactionModelV11Tests(unittest.TestCase):
    def test_v11_retains_fixed_v10_horizons_and_prepared_dataset(self) -> None:
        loader = LoaderConfig()
        self.assertEqual(v11.MODEL_VERSION, "v11")
        self.assertEqual(v11.HORIZONS, v10.HORIZONS)
    ...
    def test_v11_changes_only_fixed_v10_time_representation(self) -> None:
        v10_model = dataclasses.asdict(V10ModelConfig())
        v11_model = dataclasses.asdict(ModelConfig())
        self.assertEqual(v11_model.pop("time_feature_dim"), TIME_FEATURE_DIM)
        self.assertEqual(v10_model.pop("time_feature_dim"), 11)
        self.assertEqual(v11_model, v10_model)
        v10_train = dataclasses.asdict(V10TrainConfig())
        v11_train = dataclasses.asdict(TrainConfig())
        self.assertIn(r"\v10\train", str(v10_train.pop("output_root")))
        self.assertIn(r"\v11\train", str(v11_train.pop("output_root")))
        self.assertEqual(v11_train, v10_train)
        self.assertEqual(TIME_FEATURE_NAMES, CAUSAL_TIME_FEATURE_NAMES)
    ...
    def test_opportunity_contract_has_exactly_three_classes(self) -> None:
        ...
        self.assertEqual(set(opportunity_contract()["rules"]), set(v11.HORIZONS))
    ...
    def test_tensor_targets_match_scalar_contract(self) -> None:
        returns = torch.zeros((5, len(v11.HORIZONS), 3), dtype=torch.float32)
        ...
        mask = torch.ones((5, len(v11.HORIZONS)), dtype=torch.bool)
    ...
    def test_invalid_labels_are_masked(self) -> None:
        returns = torch.zeros((2, len(v11.HORIZONS), 3), dtype=torch.float32)
        mask = torch.ones((2, len(v11.HORIZONS)), dtype=torch.bool)
        mask[0, 0] = False
    ...
    def test_model_has_only_one_opportunity_head_per_horizon(self) -> None:
        ...
        self.assertEqual(set(model.opportunity_heads), set(v11.HORIZONS))
    ...
    def test_forward_and_loss_use_one_label_per_horizon(self) -> None:
        ...
        self.assertEqual(set(output.logits), set(v11.HORIZONS))
        self.assertTrue(all(tuple(logits.shape) == (4, 3) for logits in output.logits.values()))
        result = compute_loss(output, batch)
        self.assertTrue(torch.isfinite(result.loss))
        self.assertEqual(result.metrics["train/valid_labels"], 4 * len(v11.HORIZONS))
    ...
    def test_inference_opens_only_dominant_direction_positions(self) -> None:
        ...
        output = NewsReactionOpportunityOutput(
            logits={horizon: logits.clone() for horizon in v11.HORIZONS},
    ...
    def test_time_features_match_packed_market_v1_timestamp_semantics(self) -> None:
        premarket = encode_time_features("2026-07-14 13:00:00")
        regular = encode_time_features("2026-07-14 14:00:00")
        self.assertEqual(len(premarket), TIME_FEATURE_DIM)
        self.assertEqual(TIME_FEATURE_DIM, 12)
        self.assertEqual(premarket[7], 9 * 3600)
        self.assertAlmostEqual(premarket[8], 0.3125)
        self.assertEqual(premarket[9:], [0.0, 1.0, 0.0])
        self.assertEqual(regular[7], 10 * 3600)
        self.assertAlmostEqual(regular[8], 0.375)
        self.assertEqual(regular[9:], [1.0, 0.0, 0.0])
        self.assertGreater(premarket[6], 26.0)
        self.assertEqual(
            STREAM_EVENT_FEATURE_NAMES[-TIME_FEATURE_DIM:],
            TIME_FEATURE_NAMES,
        )
    ...
    def test_loss_weights_each_horizon_equally_not_each_label(self) -> None:
        ...
        logits = {
            ...
            for horizon in v11.HORIZONS
        }
        logits[v11.HORIZONS[0]][:, 1] = 3.0
        logits[v11.HORIZONS[1]][0, 1] = -3.0
        result = compute_loss(
            NewsReactionOpportunityOutput(
                logits=logits,
        ...
        expected = torch.stack([
            torch.nn.functional.cross_entropy(
                logits[v11.HORIZONS[0]], targets[v11.HORIZONS[0]]
            ),
            torch.nn.functional.cross_entropy(
                logits[v11.HORIZONS[1]][:1], targets[v11.HORIZONS[1]][:1]
            ),
        ]).mean()
    ...
    def test_buffered_shuffle_is_deterministic_and_resume_exact(self) -> None:
        ...
        def identities(skip: int = 0, epoch: int = 2) -> list[str]:
            with mock.patch(
                "research.news_reaction_model.v11.data.ClickHouseNewsReactionDataset",
                FakeDataset,
            ):
                return [
                    identity
    ...
    def test_checkpoint_restores_exact_next_dropout_forward(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            model = NewsReactionModelV11(config.model)
            ...
            restored_model = NewsReactionModelV11(config.model)
