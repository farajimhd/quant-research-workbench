...
from research.news_reaction_model.v11 import HORIZONS, SESSIONS
from research.news_reaction_model.v11.config import LoaderConfig
...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "V11 intentionally reuses the completed V8 representation and does not build a separate dataset."
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(f"Prepared V11 dataset integrity check failed: {result}")
    return result
...
class ClickHouseNewsReactionDataset:
    ...
    def iter_batches(self) -> Iterator[NewsReactionBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v11-loader-{index}", daemon=True)
...
def rows_to_batch(rows: list[dict[str, Any]], config: LoaderConfig) -> NewsReactionBatch:
    ...
    time_features = torch.tensor(
        [encode_time_features(row["published_at_utc"]) for row in rows],
        dtype=torch.float32,
    )
