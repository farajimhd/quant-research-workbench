...
from research.news_reaction_model.v10.inference import opportunity_predictions
from research.news_reaction_model.v10.metrics import balanced_accuracy, macro_f1
...
class ClickHouseEvaluationDataset:
    ...
    def iter_batches(self) -> Iterator[EvaluationBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v10-eval-{index}", daemon=True)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate V10 three-class opportunities and the midpoint-return P&L proxy."
    )
