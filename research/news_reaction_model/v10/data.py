...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "V10 intentionally reuses the completed V8 representation and does not build a separate dataset."
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(f"Prepared V10 dataset integrity check failed: {result}")
    return result
...
class ClickHouseNewsReactionDataset:
    ...
    def iter_batches(self) -> Iterator[NewsReactionBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v10-loader-{index}", daemon=True)
