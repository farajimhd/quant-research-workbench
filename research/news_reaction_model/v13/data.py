...
from research.news_reaction_model.v13 import HORIZONS, SESSIONS
from research.news_reaction_model.v13.config import LoaderConfig
...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "V13 intentionally reuses the completed V8 representation and does not build a separate dataset."
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(f"Prepared V13 dataset integrity check failed: {result}")
    return result
...
class ClickHouseNewsReactionDataset:
    ...
    def iter_batches(self) -> Iterator[NewsReactionBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v13-loader-{index}", daemon=True)
