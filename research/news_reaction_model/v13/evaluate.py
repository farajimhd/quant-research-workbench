...
from research.news_reaction_model.v13 import HORIZONS, MODEL_VERSION
...
from research.news_reaction_model.v13.inference import opportunity_predictions
from research.news_reaction_model.v13.metrics import balanced_accuracy, macro_f1
...
class ClickHouseEvaluationDataset:
    ...
    def iter_batches(self) -> Iterator[EvaluationBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v13-eval-{index}", daemon=True)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate V13 three-class opportunities and the midpoint-return P&L proxy."
    )
...
def evaluate_checkpoint(
    ...
    *,
    ...
    model = NewsReactionModelV13(ModelConfig(**state["config"]["model"])).to(device)
