...
from research.news_reaction_model.v13.config import LoaderConfig, ModelConfig
from research.news_reaction_model.v13.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v13.losses import compute_loss
from research.news_reaction_model.v13.model import NewsReactionModelV13
from research.news_reaction_model.v13.train import amp_dtype, set_seed
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--model-sizes", default="96,192,288,384")
    ...
    for d_model in csv_ints(args.model_sizes):
        for layers in csv_ints(args.layers):
            for batch_size in csv_ints(args.batch_sizes):
                ...
                if row["status"] == "ok":
                    print(
                        f"PROFILE d={d_model:<4} heads={args.attention_heads} layers={layers} batch={int(row['batch_size']):<5} "
...
def profile_configuration(source: NewsReactionBatch, batch_size: int, d_model: int, layers: int, device: torch.device, args: argparse.Namespace, loader: LoaderConfig | None = None) -> dict[str, object]:
    ...
    try:
        model = NewsReactionModelV13(config).to(device); optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4)
        ...
        return {"status": "ok", "d_model": d_model, "attention_heads": args.attention_heads,
                "layers": layers, "batch_size": batch.sample_count,
