...
from research.news_reaction_model.v13.stock_state import STOCK_STATE_DIM
from research.news_reaction_model.v13.time_features import TIME_FEATURE_DIM
...
class LoaderConfig:
    ...
    # Corrected V13 still reuses the exact prepared V8 rows; its causal time
...
class TrainConfig:
    ...
    # Keep the V3-V9 project so V13 remains directly comparable.
...
def default_run_name(config: ExperimentConfig) -> str:
    ...
    return (
        f"news-v13-opportunity-horizon-attention-d{config.model.d_model}"
        f"-a{config.model.attention_heads}-l{config.model.layers}-b{config.loader.batch_size}"
    )
