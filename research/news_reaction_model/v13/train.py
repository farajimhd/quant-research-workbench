...
from research.news_reaction_model.v13 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v13.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
...
from research.news_reaction_model.v13.evaluate import evaluate_checkpoint
from research.news_reaction_model.v13.losses import compute_loss
from research.news_reaction_model.v13.metrics import (
    OpportunityAccumulator,
    TrainingLossAccumulator,
)
from research.news_reaction_model.v13.model import NewsReactionModelV13, build_model_mermaid
from research.news_reaction_model.v13.opportunity import opportunity_contract
from research.news_reaction_model.v13.time_features import contract_payload as time_contract
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train V13: V12 with horizon-query cross-attention over text, state, and time."
    )
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 50:
        raise ValueError("V13 training requires --epochs between 1 and 50 inclusive")
    ...
    if config.model.attention_heads < 1:
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V13 train and validation rows were built by different representations.")
        manifest_path = config.loader.representation_artifact_root / "manifest.json"
        if not manifest_path.exists():
            raise RuntimeError(f"V13 representation manifest is missing: {manifest_path}")
        ...
        if train_audit["representation_sha256"] != feature_manifest["representation_sha256"]:
            raise RuntimeError(
                "Prepared V13 rows do not match the checksummed OpenAI-plus-stock-state representation."
            )
        print(
    ...
    raw_model = NewsReactionModelV13(config.model).to(device)
    ...
    write_model_artifacts(
        ...
        summary_notes=(
            "V13 keeps corrected V12 training and replaces its shared scalar modality gate "
            "with parameter-matched six-head cross-attention driven by learned horizon queries."
        ),
...
def restore(
    ...
    *,
    ...
    if state.get("resume_contract") != expected_contract:
        raise ValueError(
            "Checkpoint predates or violates the exact V13 resume contract."
        )
