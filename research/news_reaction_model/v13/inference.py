...
from research.news_reaction_model.v13.time_features import encode_time_features
    ...
    def encode(self, rows: list[dict[str, Any]], *, device: torch.device) -> dict[str, torch.Tensor]:
        ...
        for source in rows:
            ...
            if not isinstance(embedding, (list, tuple)) or len(embedding) != self.loader_config.openai_embedding_dim:
                raise ValueError(
                    f"V13 live inference requires a {self.loader_config.openai_embedding_dim}-value "
            ...
            if not isinstance(state, (list, tuple)) or len(state) != self.loader_config.stock_state_dim:
                raise ValueError(
                    f"V13 live inference requires the configured {self.loader_config.stock_state_dim}-value "
            ...
            if not published_at_utc or not publication_session:
                raise ValueError(
                    "V13 live inference requires published_at_utc and publication_session "
...
def load_model(checkpoint: Path, *, device: torch.device) -> tuple[NewsReactionModelV13, LoaderConfig]:
