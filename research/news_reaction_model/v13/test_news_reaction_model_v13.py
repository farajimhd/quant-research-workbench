...
from research.news_reaction_model import v12, v13
from research.news_reaction_model.v12.config import ModelConfig as V12ModelConfig
from research.news_reaction_model.v12.config import TrainConfig as V12TrainConfig
from research.news_reaction_model.v13.config import (
    ExperimentConfig,
    LoaderConfig,
    ModelConfig,
    TrainConfig,
)
from research.news_reaction_model.v13.data import (
...
from research.news_reaction_model.v13.evaluate import (
    OpportunityLedger,
    evaluation_batch_sql,
    midpoint_proxy_pnl,
)
from research.news_reaction_model.v13.fit_diagnostic import fit_comparison
from research.news_reaction_model.v13.inference import (
    LiveFeatureEncoder,
    opportunity_predictions,
)
from research.news_reaction_model.v13.losses import compute_loss
from research.news_reaction_model.v13.metrics import TrainingLossAccumulator
from research.news_reaction_model.v13.model import (
    NewsReactionModelV13,
    NewsReactionOpportunityOutput,
)
from research.news_reaction_model.v13.memorization_test import (
    deterministic_subset_sql,
    slice_batch,
)
from research.news_reaction_model.v13.opportunity import (
    OPPORTUNITY_CLASSES,
...
from research.news_reaction_model.v13.time_features import (
    TIME_FEATURE_DIM,
    encode_time_features,
)
from research.news_reaction_model.v13.train import (
...
class NewsReactionModelV13Tests(unittest.TestCase):
    def test_v13_retains_v12_time_projection_without_layer_norm(self) -> None:
        model = NewsReactionModelV13(ModelConfig())
        self.assertEqual(
            [type(layer) for layer in model.time_projection],
            [torch.nn.Linear, torch.nn.GELU],
        )

    def test_v13_retains_v12_horizons_and_prepared_dataset(self) -> None:
        loader = LoaderConfig()
        self.assertEqual(v13.MODEL_VERSION, "v13")
        self.assertEqual(v13.HORIZONS, v12.HORIZONS)
    ...
    def test_v13_adds_only_attention_heads_to_v12_model_config(self) -> None:
        v12_model = dataclasses.asdict(V12ModelConfig())
        v13_model = dataclasses.asdict(ModelConfig())
        self.assertEqual(v13_model.pop("attention_heads"), 6)
        self.assertEqual(v13_model, v12_model)
        self.assertEqual(v13_model["time_feature_dim"], TIME_FEATURE_DIM)
        v12_train = dataclasses.asdict(V12TrainConfig())
        v13_train = dataclasses.asdict(TrainConfig())
        for key in (
            "epochs",
            "max_samples",
            "learning_rate",
            ...
            self.assertEqual(v13_train[key], v12_train[key], key)
        self.assertNotIn("ordinal_loss_weight", v13_train)

    def test_v13_replaces_shared_gate_with_horizon_cross_attention(self) -> None:
        model = NewsReactionModelV13(ModelConfig())
        self.assertFalse(hasattr(model, "chunk_gate"))
        self.assertEqual(model.modality_attention.num_heads, 6)
        self.assertEqual(model.horizon_queries.num_embeddings, len(v13.HORIZONS))
        self.assertEqual(model.horizon_queries.embedding_dim, 384)
    ...
    def test_attention_head_count_must_divide_model_width(self) -> None:
        config = ExperimentConfig()
        config.model.d_model = 128
        config.model.attention_heads = 6
    ...
    def test_opportunity_contract_has_exactly_three_classes(self) -> None:
        ...
        self.assertEqual(set(opportunity_contract()["rules"]), set(v13.HORIZONS))
    ...
    def test_tensor_targets_match_scalar_contract(self) -> None:
        returns = torch.zeros((5, len(v13.HORIZONS), 3), dtype=torch.float32)
        ...
        mask = torch.ones((5, len(v13.HORIZONS)), dtype=torch.bool)
    ...
    def test_invalid_labels_are_masked(self) -> None:
        returns = torch.zeros((2, len(v13.HORIZONS), 3), dtype=torch.float32)
        mask = torch.ones((2, len(v13.HORIZONS)), dtype=torch.bool)
        mask[0, 0] = False
    ...
    def test_model_has_only_one_opportunity_head_per_horizon(self) -> None:
        ...
        self.assertEqual(set(model.opportunity_heads), set(v13.HORIZONS))
    ...
    def test_forward_and_loss_use_one_label_per_horizon(self) -> None:
        ...
        self.assertEqual(set(output.logits), set(v13.HORIZONS))
        self.assertTrue(all(tuple(logits.shape) == (4, 3) for logits in output.logits.values()))
        result = compute_loss(output, batch)
        self.assertTrue(torch.isfinite(result.loss))
        self.assertEqual(result.metrics["train/valid_labels"], 4 * len(v13.HORIZONS))

    def test_cross_attention_excludes_masked_modalities(self) -> None:
        loader = LoaderConfig(openai_embedding_dim=16, stock_state_dim=8)
        model = NewsReactionModelV13(
            ModelConfig(
                openai_embedding_dim=16,
        ...
        batch.x["channel_mask"][:, 1] = False
        first = model(batch.x)
        changed = {key: value.clone() for key, value in batch.x.items()}
        changed["stock_state"] += 1_000.0
        second = model(changed)
        for horizon in v13.HORIZONS:
    ...
    def test_inference_opens_only_dominant_direction_positions(self) -> None:
        ...
        output = NewsReactionOpportunityOutput(
            logits={horizon: logits.clone() for horizon in v13.HORIZONS},
    ...
    def test_loss_weights_each_horizon_equally_not_each_label(self) -> None:
        ...
        logits[v13.HORIZONS[0]][:, 1] = 3.0
        logits[v13.HORIZONS[1]][0, 1] = -3.0
        result = compute_loss(
            NewsReactionOpportunityOutput(
                logits=logits,
        ...
        expected = torch.stack([
            torch.nn.functional.cross_entropy(
                logits[v13.HORIZONS[0]], targets[v13.HORIZONS[0]]
            ),
            torch.nn.functional.cross_entropy(
                logits[v13.HORIZONS[1]][:1], targets[v13.HORIZONS[1]][:1]
            ),
        ]).mean()
    ...
    def test_buffered_shuffle_is_deterministic_and_resume_exact(self) -> None:
        ...
        def identities(skip: int = 0, epoch: int = 2) -> list[str]:
            with mock.patch(
                "research.news_reaction_model.v13.data.ClickHouseNewsReactionDataset",
                FakeDataset,
            ):
                return [
                    identity
    ...
    def test_checkpoint_restores_exact_next_dropout_forward(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            model = NewsReactionModelV13(config.model)
            ...
            restored_model = NewsReactionModelV13(config.model)
