...
from research.news_reaction_model.v8 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v8.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
...
from research.news_reaction_model.v8.evaluate import evaluate_checkpoint
from research.news_reaction_model.v8.losses import compute_loss
from research.news_reaction_model.v8.metrics import RangeAccumulator
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train the V8 ablation: OpenAI text embedding plus unchanged V7 causal stock state."
    )
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 15:
        raise ValueError("V8 training requires --epochs between 1 and 15 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V8 train and validation rows were built by different representations.")
        manifest_path = config.loader.representation_artifact_root / "manifest.json"
        if not manifest_path.exists():
            raise RuntimeError(f"V8 representation manifest is missing: {manifest_path}")
        ...
        if train_audit["representation_sha256"] != feature_manifest["representation_sha256"]:
            raise RuntimeError(
                "Prepared V8 rows do not match the checksummed OpenAI-plus-stock-state representation."
            )
        print(
    ...
    raw_model = NewsReactionModelV8(config.model).to(device)
    ...
    write_run_manifest(paths.manifest_path, repo_root=REPO_ROOT, model_family=MODEL_FAMILY, version=MODEL_VERSION,
                       ...
                       data_roots={"prepared_dataset": f"{config.loader.dataset_database}.{config.loader.dataset_table}",
                                   ...
                                   "openai_embeddings": f"{config.loader.dataset_database}.{config.loader.embedding_table}",
    ...
    write_model_artifacts(
        ...
        summary_notes=(
            "V7 TF-IDF word, character, and numeric text channels are replaced by one durable OpenAI "
            "embedding. The V7 stock-state channel, prediction heads, objectives, targets, and training defaults are unchanged."
        ),
