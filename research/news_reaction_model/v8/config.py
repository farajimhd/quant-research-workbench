...
def default_v7_representation_artifact_root() -> Path:
    return Path(
        os.environ.get(
...
class LoaderConfig:
    ...
    source_dataset_table: str = "news_reaction_stock_state_dataset_v7"
    source_dataset_version: str = "news_reaction_stock_state_dataset_v7"
    ...
    source_representation_artifact_root: Path = field(default_factory=default_v7_representation_artifact_root)
...
class TrainConfig:
    ...
    # Keep the V3-V7 project so V8 remains directly comparable.
...
def default_run_name(config: ExperimentConfig) -> str:
    ...
    return f"news-v8-openai-stock-state-d{config.model.d_model}-l{config.model.layers}-b{config.loader.batch_size}"
