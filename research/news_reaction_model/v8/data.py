...
from research.news_reaction_model.v8 import HORIZONS, SESSIONS
...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Prepared V8 OpenAI dataset {config.dataset_database}.{config.dataset_table} is unavailable. "
            "Run python -m research.news_reaction_model.v8.run_prepare_data --execute after embedding extraction completes."
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(f"Prepared V8 dataset integrity check failed: {result}")
    return result
...
class ClickHouseNewsReactionDataset:
    ...
    def iter_batches(self) -> Iterator[NewsReactionBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v8-loader-{index}", daemon=True)
