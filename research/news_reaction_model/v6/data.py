...
from research.news_reaction_model.v6 import HORIZONS, SESSIONS
from research.news_reaction_model.v6.config import LoaderConfig
...
SELECT canonical_news_id AS source_id, ticker, published_at_utc,
 ...
 numeric_ids, numeric_weights, numeric_dense, publication_session,
...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Prepared V6 numeric TF-IDF dataset {config.dataset_database}.{config.dataset_table} is unavailable. "
            "Run python -m research.news_reaction_model.v6.run_prepare_data --execute first."
        ) from exc
    ...
    result = {
        ...
        "numeric_articles": int(fields[11]) if len(fields) > 11 and fields[11] else 0,
    }
    if rows == 0:
        raise RuntimeError(
...
def rows_to_batch(rows: list[dict[str, Any]], config: LoaderConfig) -> NewsReactionBatch:
    ...
    numeric_indices, numeric_offsets, numeric_weights, numeric_present = _pack_sparse_rows(rows, "numeric")
    ...
    numeric_present = numeric_present | numeric_dense.ne(0).any(dim=1)
    return NewsReactionBatch(
        x={
            ...
            "numeric_indices": numeric_indices, "numeric_offsets": numeric_offsets,
            "numeric_weights": numeric_weights, "numeric_dense": numeric_dense,
            "channel_mask": torch.stack((word_present, char_present, numeric_present), dim=1),
        },
