...
from research.news_reaction_model.v6.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v6.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v6.evaluate import evaluate_checkpoint
from research.news_reaction_model.v6.losses import compute_loss
from research.news_reaction_model.v6.metrics import RangeAccumulator
from research.news_reaction_model.v6.model import NewsReactionModelV6, build_model_mermaid
from research.news_reaction_model.v6.ranges import describe_ranges
from research.news_reaction_model.v6.numeric_features import load_representation_manifest
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train V6 from V5 lexical features plus structured financial numbers.")
    ...
    parser.add_argument("--ordinal-loss-weight", type=float, default=train.ordinal_loss_weight)
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    loader = LoaderConfig(
        ...
        numeric_vocab_size=max(2, args.numeric_vocab_size), numeric_dense_dim=max(1, args.numeric_dense_dim),
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 15:
        raise ValueError("v6 training requires --epochs between 1 and 15 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V6 train and validation rows were built by different representations.")
        feature_manifest = load_representation_manifest(config.loader)
        if train_audit["representation_sha256"] != feature_manifest["representation_sha256"]:
            raise RuntimeError(
                "Prepared V6 rows do not match the checksummed lexical-plus-numeric representation."
            )
        print(
    ...
    raw_model = NewsReactionModelV6(config.model).to(device)
    ...
    write_run_manifest(paths.manifest_path, repo_root=REPO_ROOT, model_family=MODEL_FAMILY, version=MODEL_VERSION,
                       ...
                       data_roots={"prepared_dataset": f"{config.loader.dataset_database}.{config.loader.dataset_table}",
                                   ...
                                   "source_dataset": f"{config.loader.dataset_database}.{config.loader.source_dataset_table}",
    ...
    write_model_artifacts(
        ...
        input_contract={
            ...
            "numeric_indices": ["NNZ_numeric"], "numeric_offsets": ["B+1"],
            "numeric_weights": ["NNZ_numeric"], "numeric_dense": ["B", config.loader.numeric_dense_dim],
            "channel_mask": ["B", 3],
        },
        output_contract={"range_logits": describe_ranges()},
        architecture_mermaid=build_model_mermaid(),
        summary_notes=(
            "Exact frozen V5 word/character TF-IDF features plus a structured publication-time numeric channel "
            "with typed mentions, relationships, magnitude bins, and continuous statistics; V5 heads are unchanged."
        ),
        ...
        for epoch in range(epoch_start, config.train.epochs):
            ...
            for batch in iterator:
                ...
                with torch.autocast(device_type=device.type, dtype=amp_dtype(config.train.amp_dtype), enabled=device.type == "cuda" and config.train.amp):
                    ...
                    result = compute_loss(output, batch, ordinal_weight=config.train.ordinal_loss_weight)
...
def print_validation_summary(metrics: dict[str, float]) -> None:
    print(
        ...
        f"within_one={metrics.get('val/within_one_bin_accuracy', 0.0):.3f} "
        f"log_loss={metrics.get('val/log_loss', 0.0):.4f} confidence={metrics.get('val/mean_confidence', 0.0):.3f}",
        flush=True,
    )
    horizon_parts = []
    for horizon in HORIZONS:
        count_key = f"val/{horizon}/ending/samples"
        if count_key in metrics:
            horizon_parts.append(
                f"{horizon}:{int(metrics[count_key])}/{metrics[f'val/{horizon}/ending/within_one_bin_accuracy']:.2f}"
            )
    if horizon_parts:
        print("HORIZONS ending_labels/within_one | " + "  ".join(horizon_parts), flush=True)


@torch.no_grad()
def validate(model: torch.nn.Module, config: ExperimentConfig, device: torch.device, dummy: bool, dummy_count: int) -> dict[str, float]:
    model.eval(); accumulator = RangeAccumulator(); loss_sum = 0.0; batches = 0
