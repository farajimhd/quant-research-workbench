...
from research.news_reaction_model.v6.config import LoaderConfig, ModelConfig
...
from research.news_reaction_model.v6.model import NewsReactionModelV6
from research.news_reaction_model.v6.train import amp_dtype, set_seed
...
def concatenate_batches(batches: list[NewsReactionBatch]) -> NewsReactionBatch:
    ...
    x = {
        ...
        "numeric_dense": torch.cat([batch.x["numeric_dense"] for batch in batches], dim=0),
    }
...
def slice_batch(batch: NewsReactionBatch, size: int, device: torch.device) -> NewsReactionBatch:
    ...
    x = {
        ...
        "numeric_dense": batch.x["numeric_dense"][:size].to(device),
    }
    for prefix in ("word", "char", "numeric"):
...
def profile_configuration(source: NewsReactionBatch, batch_size: int, d_model: int, layers: int, device: torch.device, args: argparse.Namespace, loader: LoaderConfig | None = None) -> dict[str, object]:
    ...
    try:
        model = NewsReactionModelV6(config).to(device); optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4)
