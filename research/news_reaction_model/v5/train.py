...
from research.news_reaction_model.v5.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v5.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v5.evaluate import evaluate_checkpoint
from research.news_reaction_model.v5.losses import compute_loss
from research.news_reaction_model.v5.metrics import RangeAccumulator
from research.news_reaction_model.v5.model import NewsReactionModelV5, build_model_mermaid
from research.news_reaction_model.v5.ranges import describe_ranges
from research.news_reaction_model.v5.text_features import load_feature_manifest
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 15:
        raise ValueError("v5 training requires --epochs between 1 and 15 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V5 train and validation rows were built by different lexical representations.")
        feature_manifest = load_feature_manifest(config.loader)
        if train_audit["representation_sha256"] != feature_manifest["bundle_sha256"]:
            raise RuntimeError(
                "Prepared V5 rows do not match the checksummed feature bundle required to reproduce them."
            )
        print(
    ...
    raw_model = NewsReactionModelV5(config.model).to(device)
    ...
    write_run_manifest(paths.manifest_path, repo_root=REPO_ROOT, model_family=MODEL_FAMILY, version=MODEL_VERSION,
                       ...
                       data_roots={"prepared_dataset": f"{config.loader.dataset_database}.{config.loader.dataset_table}",
                                   ...
                                   "feature_artifacts": str(config.loader.feature_artifact_root),
