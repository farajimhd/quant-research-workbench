...
from research.news_reaction_model.v18.evaluate import evaluate_checkpoint
...
from research.news_reaction_model.v18.model import NewsReactionModelV18, build_model_mermaid
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Train V18 episodic response model.")
    ...
    parser.add_argument("--attention-heads", type=int, default=defaults.model.attention_heads)
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest = {
        "model_family": MODEL_FAMILY,
        "model_version": MODEL_VERSION,
        "job": "single_ticker_episode_training",
        "run_name": run_name,
    ...
    model = NewsReactionModelV18(config.model).to(device)
    ...
    if args.resume:
        ...
        if checkpoint.get("model_version") != MODEL_VERSION:
            raise RuntimeError("Cannot resume a non-V18 checkpoint.")
        model.load_state_dict(checkpoint["model_state"])
        optimizer.load_state_dict(checkpoint["optimizer_state"])
        start_epoch = int(checkpoint["epoch"])
        best = float(checkpoint.get("metrics", {}).get("val/macro_head_f1", -math.inf))
        print(f"RESUME | epoch={start_epoch} checkpoint={args.resume}", flush=True)
    wandb_run = None
    if config.train.wandb_mode != "disabled":
        try:
            ...
            wandb_run = wandb.init(
                ...
                tags=["v18", "single-ticker", "episodes", "openai"],
                resume="allow",
            )
    ...
    for epoch in range(start_epoch, config.train.epochs):
        ...
        try:
            for batch_index, batch in enumerate(dataset.iter_batches(epoch=epoch)):
                ...
                peak = config.train.learning_rate * (
