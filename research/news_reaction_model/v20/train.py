...
from research.news_reaction_model.v20 import MODEL_FAMILY, MODEL_VERSION
...
from research.news_reaction_model.v20.evaluate import evaluate_checkpoint
from research.news_reaction_model.v20.losses import LossResult, compute_loss
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Train V20 over read-only V18/V15 prepared arrays."
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest = {
        "model_family": MODEL_FAMILY,
        "model_version": MODEL_VERSION,
        "job": "read_only_v18_signed_return_distribution_training",
        "run_name": run_name,
    ...
    model = NewsReactionModelV20(config.model, statistics).to(device)
    ...
    if args.resume:
        ...
        if checkpoint.get("model_version") != MODEL_VERSION:
            raise RuntimeError("Cannot resume a non-V20 checkpoint.")
        if TrainingStatistics.from_dict(checkpoint["training_statistics"]) != statistics:
            raise RuntimeError("V20 resume training-statistics drift.")
        if ModelConfig(**checkpoint["model_config"]) != config.model:
            raise RuntimeError("V20 resume model-config drift.")
    ...
    for epoch in range(start_epoch, config.train.epochs):
            ...
            for batch_index, batch in enumerate(dataset.iter_batches(epoch=epoch)):
                ...
                with torch.autocast(
                    ...
                    result: LossResult = compute_loss(
                        ...
                        moved,
                        statistics,
                        cross_entropy_weight=config.train.cross_entropy_weight,
                        ordinal_crps_weight=config.train.ordinal_crps_weight,
                        direction_weight=config.train.direction_weight,
                        expected_return_weight=config.train.expected_return_weight,
        ...
        epoch_metrics["val/selection_score"] = score
        ...
        print(
            ...
            f"return_skill={val['val/signed_return/mae_skill']:.4f} "
            f"log_skill={val['val/return_distribution/log_loss_skill']:.4f}",
            flush=True,
        )
