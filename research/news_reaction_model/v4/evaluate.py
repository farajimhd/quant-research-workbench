...
from research.news_reaction_model.v4.data import NewsReactionBatch, audit_prepared_dataset, month_ranges, q, qi, rows_to_batch
from research.news_reaction_model.v4.inference import trade_plans
...
class ClickHouseEvaluationDataset:
    ...
    def iter_batches(self) -> Iterator[EvaluationBatch]:
        ...
        threads = [threading.Thread(target=worker, name=f"news-v4-eval-{i}", daemon=True) for i in range(workers)]
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate v4 dominant-excursion target-touch plans.")
...
def evaluate_checkpoint(checkpoint: Path, *, output_dir: Path | None = None, start: str = "2026-01-01", end_exclusive: str = "2027-01-01", export_predictions: bool = True, amp: bool = True) -> dict[str, Any]:
    ...
    model = NewsReactionModelV4(ModelConfig(**state["config"]["model"])).to(device)
