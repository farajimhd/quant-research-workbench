...
from research.news_reaction_model.v4 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v4.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v4.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v4.evaluate import evaluate_checkpoint
from research.news_reaction_model.v4.losses import compute_loss
from research.news_reaction_model.v4.metrics import RangeAccumulator
from research.news_reaction_model.v4.model import NewsReactionModelV4, build_model_mermaid
from research.news_reaction_model.v4.ranges import describe_ranges
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train news_reaction_model v4 percentage-range heads from frozen Qwen embeddings.")
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 15:
        raise ValueError("v4 training requires --epochs between 1 and 15 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    raw_model = NewsReactionModelV4(config.model).to(device)
    ...
    write_model_artifacts(
        ...
        summary_notes=(
            "Frozen Qwen article embeddings with ending/high/low percentage-range heads per horizon; "
