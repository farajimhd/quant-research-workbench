...
from research.news_reaction_model.v4.config import LoaderConfig
from research.news_reaction_model.v4.data import month_ranges, q, qi, source_batch_sql
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Materialize exact single-ticker raw percentage reactions for news-reaction-model v4."
    )
...
(dataset_version, split, canonical_news_id, ticker, published_at_utc, publication_session,
 embedding_model, embedding_dim, chunks, horizon_codes, return_targets, built_at)
SELECT {q(config.dataset_version)}, {q(split_for_month(start))}, source_id, ticker, published_at_utc,
 ...
 horizon_codes, return_targets, now64(3)
FROM
(
{source}
)
...
SELECT
 ...
 countIf(length(horizon_codes) != length(return_targets)) AS invalid_targets,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    status_path = Path(args.status_path) if args.status_path else Path("runtime/news-reaction-model/v4/prepare") / f"{config.dataset_version}.jsonl"
