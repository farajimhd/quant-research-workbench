...
from research.news_reaction_model.v2.data import month_ranges, q, qi, source_batch_sql
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Materialize the shared exact single-ticker embedding/reaction dataset consumed by news-reaction-model v2."
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    status_path = Path(args.status_path) if args.status_path else Path("runtime/news-reaction-model/v2/prepare") / f"{config.dataset_version}.jsonl"
