...
@dataclass(slots=True)
class TrainConfig:
    output_root: Path = default_run_root(MODEL_FAMILY, MODEL_VERSION, "train", "embedding-regression")
    run_name: str = ""
    epochs: int = 10
    max_samples: int = 0
    ...
    wandb_project: str = "news-reaction-model-v2"
...
def default_run_name(config: ExperimentConfig) -> str:
    ...
    return f"news-v2-regression-d{config.model.d_model}-l{config.model.layers}-b{config.loader.batch_size}"
