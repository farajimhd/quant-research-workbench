...
from research.news_reaction_model.v2 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v2.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v2.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
...
from research.news_reaction_model.v2.model import NewsReactionModelV2, build_model_mermaid
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train regression-only news_reaction_model v2 from frozen Qwen article embeddings.")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if config.train.scheduler_restarts < 0 or config.train.scheduler_restarts >= config.train.epochs:
        raise SystemExit("--scheduler-restarts must be between zero and epochs - 1")
    ...
    raw_model = NewsReactionModelV2(config.model).to(device)
    ...
    write_run_manifest(paths.manifest_path, repo_root=REPO_ROOT, model_family=MODEL_FAMILY, version=MODEL_VERSION,
                       ...
                       data_roots={"prepared_dataset": f"{config.loader.dataset_database}.{config.loader.dataset_table}",
                                   ...
                                   "source_embeddings": f"{config.loader.embedding_database}.{config.loader.embedding_table}",
    ...
    write_model_artifacts(
        ...
        output_contract={"return_forecasts": ["B", 10, 3]},
        architecture_mermaid=build_model_mermaid(),
        summary_notes="Frozen Qwen article embeddings with terminal, high, and low abnormal-return regression heads.",
    ...
    scheduler = SampleCosineRestartScheduler(optimizer, config.train, planned_samples) if config.train.scheduler == "cosine" else None
    ...
    checkpointer = AsyncCheckpointManager(paths.checkpoints_dir, paths.checkpoint_manifest_path, CheckpointPolicy(
        ...
        monitor_train_key="train/mse", monitor_val_key="val/mse", clock_name="sample", archive_prefix="checkpoint_sample",
    ))
    ...
    try:
        for epoch in range(epoch_start, config.train.epochs):
            ...
            for batch in iterator:
                ...
                if scheduler is not None:
                    last_train["train/scheduler_cycle"] = float(scheduler.cycle_index(samples_seen) + 1)
                if samples_seen >= next_log:
                    logger.log(last_train, samples_seen)
                    print(
                        f"TRAIN samples={samples_seen:,} epoch={epoch + 1}/{config.train.epochs} "
                        f"mse={last_train['train/mse']:.6f} mae={last_train['train/mae']:.6f} "
    ...
    write_model_card(paths.run_root / "model_card.json", {
        ...
        "objective": "mse_actual_abnormal_target_high_low_returns",
        "scheduler_restarts": config.train.scheduler_restarts,
...
def print_validation_summary(metrics: dict[str, float]) -> None:
    print(
        ...
        f"mse={metrics.get('val/mse', 0.0):.6f} rmse={metrics.get('val/rmse', 0.0):.6f} "
        f"mae={metrics.get('val/mae', 0.0):.6f} "
        f"mse_vs_zero={metrics.get('val/mse_improvement_vs_zero', 0.0):+.6f}",
        flush=True,
    )
    horizon_parts = []
    for horizon in HORIZONS:
        ...
        if count_key in metrics:
            horizon_parts.append(
                f"{horizon}:{int(metrics[count_key])}/{metrics[f'val/{horizon}/rmse']:.4f}"
            )
    if horizon_parts:
        print("HORIZONS labels/rmse | " + "  ".join(horizon_parts), flush=True)


@torch.no_grad()
def validate(model: torch.nn.Module, config: ExperimentConfig, device: torch.device, dummy: bool, dummy_count: int) -> dict[str, float]:
    model.eval(); accumulator = RegressionAccumulator(); horizon_accumulators = [RegressionAccumulator() for _ in HORIZONS]; batches = 0
    iterator = dummy_batches(config, device, min(dummy_count, 2)) if dummy else real_batches(config, train=False)
    for batch in iterator:
        batch = batch.to(device)
        with torch.autocast(device_type=device.type, dtype=amp_dtype(config.train.amp_dtype), enabled=device.type == "cuda" and config.train.amp):
            output = model(batch.x)
        accumulator.add(output.return_forecasts, batch.return_targets, batch.label_mask)
        for horizon_index, horizon in enumerate(HORIZONS):
            horizon_accumulators[horizon_index].add(
                output.return_forecasts[:, horizon_index], batch.return_targets[:, horizon_index],
                batch.label_mask[:, horizon_index],
            )
        batches += 1
        if config.train.validation_max_batches > 0 and batches >= config.train.validation_max_batches:
            break
    metrics = accumulator.compute("val")
    for horizon, horizon_accumulator in zip(HORIZONS, horizon_accumulators):
        if horizon_accumulator.count:
            metrics.update(horizon_accumulator.compute(f"val/{horizon}"))
    metrics["val/loss"] = metrics["val/mse"]; metrics["val/batches"] = float(batches)
    return metrics
...
class SampleCosineRestartScheduler:
    def __init__(self, optimizer: torch.optim.Optimizer, config: TrainConfig, planned_samples: int) -> None:
        self.optimizer, self.base_lrs = optimizer, [float(g["lr"]) for g in optimizer.param_groups]
    ...
    def step(self, samples: int) -> None:
        ...
        cycle_end = self.planned_samples if cycle_index == self.restarts else min((cycle_index + 1) * self.cycle, self.planned_samples)
    ...
    def state_dict(self) -> dict[str, Any]:
        return {"samples": self.samples, "base_lrs": self.base_lrs, "planned_samples": self.planned_samples,
                "restarts": self.restarts, "cycle": self.cycle}

    def load_state_dict(self, state: dict[str, Any]) -> None:
        if int(state.get("planned_samples", self.planned_samples)) != self.planned_samples or int(state.get("restarts", self.restarts)) != self.restarts:
            raise ValueError("checkpoint scheduler plan does not match the current planned samples and restart count")
        self.samples = int(state.get("samples", 0)); self.base_lrs = list(state.get("base_lrs", self.base_lrs)); self.step(self.samples)
...
def restore(path: str, model: torch.nn.Module, optimizer: torch.optim.Optimizer, scheduler: Any, scaler: Any, device: torch.device) -> tuple[int, int]:
    ...
    # PyTorch 2.6+ defaults to restricted weights-only loading. Older checkpoints
    # may contain one non-default safe type: the local output-root
