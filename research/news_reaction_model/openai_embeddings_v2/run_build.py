...
from research.news_reaction_model.openai_embeddings_v2.config import HARD_MAX_COST_USD, PipelineConfig
from research.news_reaction_model.openai_embeddings_v2.pipeline import audit_pipeline, run_pipeline
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build one durable OpenAI embedding per certified v2 article and link it to every ticker."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "COMMAND python -m research.news_reaction_model.openai_embeddings_v2.run_build "
    ...
    if args.audit:
        ...
        return (
            ...
            and not result["invalid_dimensions"]
            and not result["ticker_link_gap"]
            else 5
        )
    try:
