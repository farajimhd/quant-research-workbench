...
from research.news_reaction_model.openai_embeddings_v2.openai_api import (
    OpenAIAPIError,
    OpenAIHTTPClient,
    batch_error_summary,
    is_quota_message,
...
""")
    client.execute(f"""
CREATE TABLE IF NOT EXISTS {database}.{qi(config.ticker_link_table)}
(
 embedding_version LowCardinality(String),
 canonical_news_id String,
 ...
 ticker_count UInt16,
 text_sha256 FixedString(64),
 linked_at_utc DateTime64(3, 'UTC')
)
ENGINE = ReplacingMergeTree(linked_at_utc)
PARTITION BY toYYYYMM(published_at_utc)
ORDER BY (embedding_version, published_at_utc, ticker, canonical_news_id)
SETTINGS index_granularity = 8192
""")
...
def source_rows_sql(config: PipelineConfig, start: dt.date, end: dt.date) -> str:
    return f"""
SELECT
 r.canonical_news_id, '' AS ticker, r.published_at_utc,
 'benzinga' AS provider, r.title, '' AS teaser,
 substring(r.rendered_text, 1, {config.max_text_chars}) AS body_text,
 '' AS external_text, '' AS pdf_text, '' AS channels, '' AS provider_tags
FROM {qi(config.database)}.{qi(config.source_table)} AS r FINAL
WHERE r.renderer_version = {q(config.source_version)}
 AND r.published_at_utc >= toDateTime64({q(start.isoformat())}, 9, 'UTC')
 AND r.published_at_utc < toDateTime64({q(end.isoformat())}, 9, 'UTC')
ORDER BY r.published_at_utc, r.canonical_news_id
...
WHERE renderer_version = {q(config.source_version)}
...
def assert_render_authority_ready(client: ClickHouseHttpClient, config: PipelineConfig) -> None:
    rows = json_rows(client, config, f"""
SELECT status, audit_errors
FROM {qi(config.news_database)}.{qi(config.render_authority_table)} FINAL
WHERE authority_version = {q(config.source_version)}
ORDER BY updated_at_utc DESC
LIMIT 1
FORMAT JSONEachRow
""", operation="render_authority")
    if not rows or str(rows[0].get("status")) != "ready" or int(rows[0].get("audit_errors") or 0):
        raise RuntimeError(
            "The structured-rendering v2 authority is not certified. Run the renderer rebuild/audit first; "
            "no embedding request was submitted."
        )


def sync_ticker_links(client: ClickHouseHttpClient, config: PipelineConfig) -> int:
    """Link one article embedding to all current ticker relationships without copying vectors."""
    now = clickhouse_utc_now()
    client.execute(f"""
INSERT INTO {qi(config.database)}.{qi(config.ticker_link_table)}
(
 embedding_version, canonical_news_id, provider_article_id, published_at_utc,
 ticker, ticker_index, ticker_count, text_sha256, linked_at_utc
)
SELECT
 {q(config.embedding_version)}, t.canonical_news_id, t.provider_article_id, t.published_at_utc,
 t.ticker, t.ticker_index, t.ticker_count, x.text_sha256,
 toDateTime64({q(now)}, 3, 'UTC')
FROM {qi(config.news_database)}.{qi(config.source_ticker_table)} AS t FINAL
INNER JOIN {qi(config.news_database)}.{qi(config.news_table)} AS e FINAL
 ON e.published_date=t.published_date
 AND e.provider_article_id=t.provider_article_id
 AND e.source_revision_key=t.source_revision_key
INNER JOIN {qi(config.database)}.{qi(config.embedding_table)} AS x FINAL
 ON x.embedding_version={q(config.embedding_version)}
 AND x.canonical_news_id=t.canonical_news_id
 AND x.published_at_utc=t.published_at_utc
 AND x.ticker=''
""")
    return int(execute_read(client, config, f"""
SELECT count()
FROM {qi(config.database)}.{qi(config.ticker_link_table)} FINAL
WHERE embedding_version={q(config.embedding_version)}
""", operation="ticker_link_count").strip() or "0")


def prepare_text(row: dict[str, Any], config: PipelineConfig, encoding: Any) -> PreparedText:
    # rendered_text already contains the title and explicit source boundaries.
    # No ticker is injected: one article vector must be reusable by every link.
    source_text = str(row.get("body_text") or "")[: max(1, int(config.max_text_chars))]
    if not source_text.strip():
        raise RuntimeError(f"Certified rendered article is empty: {row.get('canonical_news_id', '')}")
...
INNER JOIN {qi(config.database)}.{qi(config.source_table)} AS r FINAL
 ON r.renderer_version = {q(config.source_version)}
 AND r.canonical_news_id = i.canonical_news_id
 AND r.published_at_utc = i.published_at_utc
ORDER BY i.published_at_utc, i.canonical_news_id
SETTINGS max_threads={config.clickhouse_threads}, max_memory_usage={q(config.clickhouse_memory)}
FORMAT JSONEachRow
...
def print_status(summary: UsageSummary, config: PipelineConfig, *, focus: str) -> None:
    ...
    print(
        f"OPENAI NEWS EMBEDDINGS V2 | {focus}\n"
...
def run_pipeline(config: PipelineConfig, *, execute: bool, retry_failed: bool = False, no_wait: bool = False) -> int:
    ...
    client = ClickHouseHttpClient(default_clickhouse_url(), default_clickhouse_user(), default_clickhouse_password())
    assert_render_authority_ready(client, config)
    if not execute:
    ...
    while True:
        ...
        if summary.embedded_rows == summary.source_rows and summary.failed_rows == 0 and summary.active_rows == 0:
            ticker_links = sync_ticker_links(client, config)
            append_status(config, "complete", **asdict(summary))
            print(
                f"COMPLETED | every source article has one durable embedding; "
                f"ticker_links={ticker_links:,}; no unresolved failure",
                flush=True,
            )
            return 0
...
""", operation="audit_dimensions").strip() or "0")
    ticker_links = sync_ticker_links(client, config) if summary.embedded_rows else 0
    expected_ticker_links = int(execute_read(client, config, f"""
SELECT count()
FROM {qi(config.news_database)}.{qi(config.source_ticker_table)} AS t FINAL
INNER JOIN {qi(config.news_database)}.{qi(config.news_table)} AS e FINAL
 ON e.published_date=t.published_date
 AND e.provider_article_id=t.provider_article_id
 AND e.source_revision_key=t.source_revision_key
""", operation="expected_ticker_links").strip() or "0")
    result = {
        **asdict(summary),
        "duplicates": duplicates,
        "invalid_dimensions": invalid_dimensions,
        "ticker_links": ticker_links,
        "expected_ticker_links": expected_ticker_links,
        "ticker_link_gap": expected_ticker_links - ticker_links,
    }
