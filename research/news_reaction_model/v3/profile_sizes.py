...
from research.news_reaction_model.v3.train import amp_dtype, set_seed
    ...
    for d_model in csv_ints(args.model_sizes):
        for layers in csv_ints(args.layers):
            for batch_size in csv_ints(args.batch_sizes):
                row = profile_configuration(source, batch_size, d_model, layers, device, args)
...
def slice_batch(batch: NewsReactionBatch, size: int, device: torch.device) -> NewsReactionBatch:
    return NewsReactionBatch(
        x={key: value[:size].to(device) for key, value in batch.x.items()}, class_targets=batch.class_targets[:size].to(device),
...
def profile_configuration(source: NewsReactionBatch, batch_size: int, d_model: int, layers: int, device: torch.device, args: argparse.Namespace) -> dict[str, object]:
    config = ModelConfig(d_model=d_model, hidden_dim=d_model, layers=layers)
    try:
        model = NewsReactionModelV3(config).to(device); optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4)
