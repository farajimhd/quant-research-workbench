...
from research.news_reaction_model.v3 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v3.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v3.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v3.evaluate import evaluate_checkpoint
from research.news_reaction_model.v3.losses import compute_loss
from research.news_reaction_model.v3.metrics import HierarchicalAccumulator
from research.news_reaction_model.v3.model import NewsReactionModelV3, build_model_mermaid
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train news_reaction_model v3 from frozen Qwen article embeddings.")
    ...
    parser.add_argument("--direction-loss-weight", type=float, default=train.direction_loss_weight)
    parser.add_argument("--magnitude-loss-weight", type=float, default=train.magnitude_loss_weight)
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    ...
    train = TrainConfig(
        ...
        direction_loss_weight=args.direction_loss_weight, magnitude_loss_weight=args.magnitude_loss_weight,
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 15:
        raise ValueError("v3 training requires --epochs between 1 and 15 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    raw_model = NewsReactionModelV3(config.model).to(device)
    ...
    write_model_artifacts(
        ...
        output_contract={
            "actionable_logits": ["B", 10, 2],
            "direction_logits": ["B", 10, 2],
            "magnitude_forecasts": ["B", 10, 3],
        },
        architecture_mermaid=build_model_mermaid(),
        summary_notes=(
            "Frozen Qwen article embeddings with hierarchical actionable, conditional-direction, and magnitude heads; "
            "every input is available at publication time."
        ),
            ...
            for batch in iterator:
                ...
                with torch.autocast(device_type=device.type, dtype=amp_dtype(config.train.amp_dtype), enabled=device.type == "cuda" and config.train.amp):
                    ...
                    result = compute_loss(
                        ...
                        magnitude_weight=config.train.magnitude_loss_weight,
                    )
...
def print_validation_summary(metrics: dict[str, float]) -> None:
    print(
        ...
        f"loss={metrics.get('val/loss', 0.0):.4f} balanced_accuracy={metrics.get('val/balanced_accuracy', 0.0):.3f} "
        f"macro_f1={metrics.get('val/macro_f1', 0.0):.3f} log_loss={metrics.get('val/log_loss', 0.0):.4f} "
        f"vs_uniform={metrics.get('val/log_loss_improvement_vs_uniform', 0.0):+.4f} ece={metrics.get('val/ece_10_bin', 0.0):.3f}",
        flush=True,
    )
    horizon_parts = []
    for horizon in HORIZONS:
        ...
        if count_key in metrics:
            horizon_parts.append(
                f"{horizon}:{int(metrics[count_key])}/{metrics[f'val/{horizon}/balanced_accuracy']:.2f}"
            )
    if horizon_parts:
        print("HORIZONS labels/bal_acc | " + "  ".join(horizon_parts), flush=True)


@torch.no_grad()
def validate(model: torch.nn.Module, config: ExperimentConfig, device: torch.device, dummy: bool, dummy_count: int) -> dict[str, float]:
    model.eval(); accumulator = HierarchicalAccumulator(); horizon_accumulators = [HierarchicalAccumulator() for _ in HORIZONS]; loss_sum = 0.0; batches = 0
    ...
    for batch in iterator:
        ...
        accumulator.add(output, batch.class_targets, batch.return_targets, batch.label_mask)
        for horizon_index, horizon in enumerate(HORIZONS):
            horizon_output = type(output)(
                actionable_logits=output.actionable_logits[:, horizon_index],
                direction_logits=output.direction_logits[:, horizon_index],
                magnitude_forecasts=output.magnitude_forecasts[:, horizon_index],
                article_embedding=output.article_embedding,
                profile={},
            )
            horizon_accumulators[horizon_index].add(
                horizon_output,
                batch.class_targets[:, horizon_index],
                batch.return_targets[:, horizon_index],
