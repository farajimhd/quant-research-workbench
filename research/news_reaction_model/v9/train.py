...
from research.news_reaction_model.v9 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v9.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v9.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v9.evaluate import evaluate_checkpoint
from research.news_reaction_model.v9.losses import compute_loss
from research.news_reaction_model.v9.metrics import OpportunityAccumulator
from research.news_reaction_model.v9.model import NewsReactionModelV9, build_model_mermaid
from research.news_reaction_model.v9.opportunity import opportunity_contract
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train V9: the V8 encoder with one four-class opportunity head per horizon."
    )
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 15:
        raise ValueError("V9 training requires --epochs between 1 and 15 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V9 train and validation rows were built by different representations.")
        manifest_path = config.loader.representation_artifact_root / "manifest.json"
        if not manifest_path.exists():
            raise RuntimeError(f"V9 representation manifest is missing: {manifest_path}")
        ...
        if train_audit["representation_sha256"] != feature_manifest["representation_sha256"]:
            raise RuntimeError(
                "Prepared V9 rows do not match the checksummed OpenAI-plus-stock-state representation."
            )
        print(
    ...
    raw_model = NewsReactionModelV9(config.model).to(device)
    ...
    write_model_artifacts(
        ...
        summary_notes=(
            "V9 retains V8's OpenAI embedding, stock-state input, and encoder. Only the supervised "
            "output is replaced by one four-class opportunity head per horizon."
        ),
