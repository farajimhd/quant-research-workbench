...
from research.news_reaction_model.v7 import HORIZONS, SESSIONS
...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Prepared V7 stock-state dataset {config.dataset_database}.{config.dataset_table} is unavailable. "
            "Run python -m research.news_reaction_model.v7.run_prepare_data --execute first."
        ) from exc
...
def rows_to_batch(rows: list[dict[str, Any]], config: LoaderConfig) -> NewsReactionBatch:
    ...
    state_present = stock_state.ne(0).any(dim=1)
    return NewsReactionBatch(
        x={
            ...
            "channel_mask": torch.stack((word_present, char_present, numeric_present, state_present), dim=1),
        },
