...
from research.news_reaction_model.v7.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v7.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v7.evaluate import evaluate_checkpoint
from research.news_reaction_model.v7.losses import compute_loss
from research.news_reaction_model.v7.metrics import RangeAccumulator
from research.news_reaction_model.v7.model import NewsReactionModelV7, build_model_mermaid
from research.news_reaction_model.v7.ranges import describe_ranges
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train V7 from frozen V6 features plus causal stock state.")
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 15:
        raise ValueError("v7 training requires --epochs between 1 and 15 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V7 train and validation rows were built by different representations.")
        manifest_path = config.loader.representation_artifact_root / "manifest.json"
        if not manifest_path.exists():
            raise RuntimeError(f"V7 representation manifest is missing: {manifest_path}")
        ...
        if train_audit["representation_sha256"] != feature_manifest["representation_sha256"]:
            raise RuntimeError(
                "Prepared V7 rows do not match the checksummed stock-state representation."
            )
        print(
    ...
    raw_model = NewsReactionModelV7(config.model).to(device)
    ...
    write_model_artifacts(
        ...
        input_contract={
            ...
            "stock_state": ["B", config.loader.stock_state_dim], "channel_mask": ["B", 4],
        },
        output_contract={"range_logits": describe_ranges()},
        architecture_mermaid=build_model_mermaid(),
        summary_notes=(
            "Frozen V6 lexical and financial-number channels plus one checksummed, causal point-in-time "
            "SEC and market-state channel; prediction heads and objectives are unchanged."
        ),
