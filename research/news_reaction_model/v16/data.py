...
def _count_nonfinite_rows(
    ...
    *,
    ...
    total = 0
    for offset in range(lower, upper, max(1, int(chunk_rows))):
        total += int(
            np.count_nonzero(
                ~np.isfinite(
                    np.asarray(array[offset : min(upper, offset + chunk_rows)])
                )
            )
        )
    return total
...
def audit_prepared_dataset(
    ...
    try:
        ...
        if rows <= 0:
            raise RuntimeError(
                f"Prepared V16 dataset has no rows in [{start}, {end_exclusive})."
            )
        ...
        market_mask = np.asarray(arrays["market_context_mask"][lower:upper])
        market_indices = np.asarray(arrays["market_context_indices"][lower:upper])
        invalid_market_context = int(
            np.count_nonzero(
                (market_mask & (market_indices < 0))
                | (market_mask & (market_indices.astype(np.int64) >= row_ids))
                | ((~market_mask) & (market_indices != -1))
            )
        )
        invalid_market_features = 0
        for name in (
            "current_market_features",
            "market_context_features",
            "market_leader_features",
        ):
            invalid_market_features += _count_nonfinite_rows(
                arrays[name], lower, upper
            )
        leader_mask = np.asarray(arrays["market_leader_mask"][lower:upper])
        result = {
            "rows": rows,
            "articles": rows,
            ...
            "market_context_articles": int(
                np.count_nonzero(market_mask.any(axis=1))
            ),
            "market_context_slots": int(np.count_nonzero(market_mask)),
            "market_leader_articles": int(
                np.count_nonzero(leader_mask.any(axis=1))
            ),
            "market_leader_slots": int(np.count_nonzero(leader_mask)),
            "invalid_market_context": invalid_market_context,
            "invalid_market_feature_values": invalid_market_features,
        ...
        if (
            invalid_context
            or invalid_features
            or invalid_market_context
            or invalid_market_features
        ):
            raise RuntimeError(f"Prepared V16 dataset integrity check failed: {result}.")
        return result
    finally:
        close_arrays(arrays)
...
def batch_from_indices(
    ...
    market_context_indices = _numpy_copy(
        arrays["market_context_indices"][indices],
        dtype=np.int64,
    )
    market_context_mask = _numpy_copy(
        arrays["market_context_mask"][indices],
        dtype=np.bool_,
    )
    safe_market_context_indices = np.where(
        market_context_mask,
        market_context_indices,
        0,
    )
    market_context_embeddings = _numpy_copy(
        arrays["openai_embedding"][safe_market_context_indices],
        dtype=np.float32,
    )
    market_context_embeddings[~market_context_mask] = 0.0
    x = {
        ...
        "channel_mask": torch.stack(
            (
                ...
                torch.from_numpy(
                    _numpy_copy(
                        np.any(arrays["current_market_features"][indices] != 0, axis=1),
                        dtype=np.bool_,
                    )
                ),
            ),
        ...
        "current_market_features": torch.from_numpy(
            _numpy_copy(arrays["current_market_features"][indices], dtype=np.float32)
        ),
        "market_context_openai_embeddings": torch.from_numpy(
            market_context_embeddings
        ),
        "market_context_features": torch.from_numpy(
            _numpy_copy(arrays["market_context_features"][indices], dtype=np.float32)
        ),
        "market_context_mask": torch.from_numpy(market_context_mask),
        "market_leader_features": torch.from_numpy(
            _numpy_copy(arrays["market_leader_features"][indices], dtype=np.float32)
        ),
        "market_leader_mask": torch.from_numpy(
            _numpy_copy(arrays["market_leader_mask"][indices], dtype=np.bool_)
        ),
    }
    return NewsReactionBatch(
        x=x,
    ...
    try:
        ...
        for block_start in range(lower, upper, config.shuffle_buffer_articles):
            ...
            for offset in range(0, len(indices), config.batch_size):
                ...
                if remaining_skip:
                    raise RuntimeError(
                        "Resume offset is not aligned to a deterministic V16 batch boundary."
                    )
                yield batch_from_indices(arrays, selected, config)
        if remaining_skip:
            raise RuntimeError(
                f"Resume requested {skip_articles} articles beyond the available V16 epoch."
            )
    finally:
        close_arrays(arrays)
...
def rows_to_batch(
    ...
    current_market = np.zeros(
        (b, config.current_market_feature_dim), dtype=np.float32
    )
    market_embeddings = np.zeros(
        (b, config.market_context_size, config.openai_embedding_dim), dtype=np.float32
    )
    market_features = np.zeros(
        (b, config.market_context_size, config.market_news_feature_dim), dtype=np.float32
    )
    market_mask = np.zeros((b, config.market_context_size), dtype=np.bool_)
    leader_features = np.zeros(
        (b, config.market_leader_size, config.market_leader_feature_dim), dtype=np.float32
    )
    leader_mask = np.zeros((b, config.market_leader_size), dtype=np.bool_)
    for row_index, row in enumerate(rows):
        ...
        current = np.asarray(row.get("current_market_features", ()), dtype=np.float32)
        explicit_market_embeddings = np.asarray(
            row.get("market_context_openai_embeddings", ()), dtype=np.float32
        )
        explicit_market_features = np.asarray(
            row.get("market_context_features", ()), dtype=np.float32
        )
        explicit_market_mask = np.asarray(
            row.get("market_context_mask", ()), dtype=np.bool_
        )
        explicit_leaders = np.asarray(
            row.get("market_leader_features", ()), dtype=np.float32
        )
        explicit_leader_mask = np.asarray(
            row.get("market_leader_mask", ()), dtype=np.bool_
        )
        market_parts = (
            current,
            explicit_market_embeddings,
            explicit_market_features,
            explicit_market_mask,
            explicit_leaders,
            explicit_leader_mask,
        )
        if any(value.size for value in market_parts) and not all(
            value.size for value in market_parts
        ):
            raise ValueError(
                "V16 market context is all-or-none: current state, market-news "
                "embeddings/features/mask, and leader features/mask must be supplied together."
            )
        if current.size:
            expected = (
                (config.current_market_feature_dim,),
                (config.market_context_size, config.openai_embedding_dim),
                (config.market_context_size, config.market_news_feature_dim),
                (config.market_context_size,),
                (config.market_leader_size, config.market_leader_feature_dim),
                (config.market_leader_size,),
            )
            actual = (
                current.shape,
                explicit_market_embeddings.shape,
                explicit_market_features.shape,
                explicit_market_mask.shape,
                explicit_leaders.shape,
                explicit_leader_mask.shape,
            )
            if actual != expected:
                raise ValueError(f"Explicit V16 market context has shapes {actual}; expected {expected}.")
            current_market[row_index] = current
            market_embeddings[row_index] = explicit_market_embeddings
            market_features[row_index] = explicit_market_features
            market_mask[row_index] = explicit_market_mask
            leader_features[row_index] = explicit_leaders
            leader_mask[row_index] = explicit_leader_mask
    ...
    return NewsReactionBatch(
        x={
            ...
            "channel_mask": torch.from_numpy(
                np.stack(
                    (
                        ...
                        np.any(current_market != 0, axis=1),
                    ),
                    axis=1,
                )
            ),
            ...
            "current_market_features": torch.from_numpy(current_market),
            "market_context_openai_embeddings": torch.from_numpy(market_embeddings),
            "market_context_features": torch.from_numpy(market_features),
            "market_context_mask": torch.from_numpy(market_mask),
            "market_leader_features": torch.from_numpy(leader_features),
            "market_leader_mask": torch.from_numpy(leader_mask),
        },
...
def make_dummy_batch(
    ...
    *,
    ...
    for index in range(batch_size):
        ...
        market_embeddings = np.zeros(
            (config.market_context_size, config.openai_embedding_dim), dtype=np.float32
        )
        market_features = np.zeros(
            (config.market_context_size, config.market_news_feature_dim), dtype=np.float32
        )
        market_mask = np.zeros(config.market_context_size, dtype=np.bool_)
        market_available = min(config.market_context_size, index)
        for slot in range(market_available):
            market_embeddings[slot, (index + slot + 3) % config.openai_embedding_dim] = 1.0
            market_features[slot, -4] = float(slot + 1) / max(1, config.market_context_size)
            market_mask[slot] = True
        leader_features = np.zeros(
            (config.market_leader_size, config.market_leader_feature_dim), dtype=np.float32
        )
        leader_mask = np.zeros(config.market_leader_size, dtype=np.bool_)
        leader_available = min(config.market_leader_size, index)
        if leader_available:
            leader_features[:leader_available, 0] = 0.01
            leader_mask[:leader_available] = True
        rows.append(
            {
                ...
                "current_market_features": np.full(
                    config.current_market_feature_dim, 0.01, dtype=np.float32
                ),
                "market_context_openai_embeddings": market_embeddings,
                "market_context_features": market_features,
                "market_context_mask": market_mask,
                "market_leader_features": leader_features,
                "market_leader_mask": leader_mask,
            }
        )
