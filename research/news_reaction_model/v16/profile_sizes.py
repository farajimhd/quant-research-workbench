...
from research.news_reaction_model.v16.data import PreparedNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
...
from research.news_reaction_model.v16.train import amp_dtype, set_seed
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--model-sizes", default="192,384,576,768")
    parser.add_argument("--batch-sizes", default="256,512,1024,2048,4096,8192")
    parser.add_argument("--layers", default="1,2,4")
    parser.add_argument("--attention-heads", type=int, default=ModelConfig().attention_heads)
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.real_data:
        ...
        print(
            f"DATASET ready | articles={audit['rows']:,} | "
            f"context_articles={audit['context_articles']:,} | "
            f"market_articles={audit['market_context_articles']:,} | "
            f"leader_articles={audit['market_leader_articles']:,} | "
            f"version={loader.prepared_dataset_version}",
            flush=True,
        )
...
def load_real_sample(loader: LoaderConfig, start: str, end_exclusive: str, required_articles: int) -> NewsReactionBatch:
    ...
    dataset = PreparedNewsReactionDataset(loader, start=start, end_exclusive=end_exclusive)
...
def profile_configuration(source: NewsReactionBatch, batch_size: int, d_model: int, layers: int, device: torch.device, args: argparse.Namespace, loader: LoaderConfig | None = None) -> dict[str, object]:
    ...
    try:
        model = NewsReactionModelV16(config).to(device); optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4)
