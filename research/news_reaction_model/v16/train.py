...
from research.news_reaction_model.v16 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v16.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
...
from research.news_reaction_model.v16.evaluate import evaluate_checkpoint
from research.news_reaction_model.v16.losses import compute_loss
from research.news_reaction_model.v16.metrics import (
    OpportunityAccumulator,
    TrainingLossAccumulator,
)
from research.news_reaction_model.v16.model import NewsReactionModelV16, build_model_mermaid
...
from research.news_reaction_model.v16.market_context import (
    ...
    LEADER_FEATURE_DIM,
    MARKET_CONTEXT_MAX_SESSION_DISTANCE,
    MARKET_CONTEXT_SIZE,
    MARKET_LEADER_SIZE,
    MARKET_NEWS_FEATURE_DIM,
    contract_payload as market_context_contract,
)
from research.news_reaction_model.v16.opportunity import opportunity_contract
from research.news_reaction_model.v16.time_features import contract_payload as time_contract
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train V16: V15 opportunity model plus strictly causal cross-market "
            "news and anonymous leader attention."
        )
    )
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 50:
        raise ValueError("V16 training requires --epochs between 1 and 50 inclusive")
    ...
    if (
        ...
        raise ValueError(
            "V16 prepared context contract is fixed at "
    ...
    loader_market = (
        config.loader.market_context_size,
        config.loader.market_context_max_session_distance,
        config.loader.current_market_feature_dim,
        config.loader.market_news_feature_dim,
        config.loader.market_leader_size,
        config.loader.market_leader_feature_dim,
    )
    model_market = (
        config.model.market_context_size,
        MARKET_CONTEXT_MAX_SESSION_DISTANCE,
        config.model.current_market_feature_dim,
        config.model.market_news_feature_dim,
        config.model.market_leader_size,
        config.model.market_leader_feature_dim,
    )
    if loader_market != expected_market or model_market != expected_market:
        raise ValueError(
            f"V16 market-context contract drift: loader={loader_market}, "
            f"model={model_market}, expected={expected_market}."
        )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V16 train and validation rows were built by different representations.")
        print(
            ...
            f"market_train={train_audit['market_context_articles']:,}/"
            f"{train_audit['market_context_slots']:,} slots | "
            f"leaders_train={train_audit['market_leader_articles']:,}/"
            f"{train_audit['market_leader_slots']:,} slots | "
    ...
    raw_model = NewsReactionModelV16(config.model).to(device)
    ...
    write_model_artifacts(
        ...
        input_contract={
            ...
            "market_context_openai_embeddings": [
                "B",
                config.loader.market_context_size,
                config.loader.openai_embedding_dim,
            ],
            ...
            "market_context_mask": ["B", config.loader.market_context_size],
            "market_leader_features": [
                "B",
                config.loader.market_leader_size,
                config.loader.market_leader_feature_dim,
            ],
            "market_leader_mask": ["B", config.loader.market_leader_size],
        },
        output_contract={
        ...
        summary_notes=(
            "V16 preserves V15 and adds a fixed causal market-attention branch: "
            "the latest 100 single-ticker news items plus up to 20 anonymous market "
            "leaders. Main-news market action is strictly pre-publication; earlier "
            "news reactions are capped at the current publication time."
        ),
...
def restore(
    ...
    *,
    ...
    if state.get("resume_contract") != expected_contract:
        raise ValueError(
            "Checkpoint predates or violates the exact V16 resume contract."
        )
