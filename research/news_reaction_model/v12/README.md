# News Reaction Model V12

V12 is a strict single-variable ablation derived from the corrected V10. It
keeps V10's data, 11-value causal exchange-time vector, model, targets, loss,
shuffle, chronological split, optimizer, scheduler, checkpointing, evaluation,
and W&B project unchanged. Its only experimental change is removing the
`LayerNorm(time_feature_dim)` operation immediately before the time projection's
linear layer.

The purpose is to test whether LayerNorm helps or harms V10's already bounded
time features. V12 does not add, remove, reorder, or redefine any time value.

V12 retains the five fundamental V10 training corrections:
...
V12 checkpoints are intentionally architecture- and resume-incompatible with
V10 because the time projection parameter graph differs. Use a new V12 run;
do not resume a V10 checkpoint.

V12 deliberately reads the completed V8 prepared table. There is no V12
...
This rule is intentionally fixed before V12 training. It is not tuned against
...
```powershell
python -m research.news_reaction_model.v12.run_profile_sizes --real-data
```

Corrected training:

```powershell
python -m research.news_reaction_model.v12.run_train
```
...
V12 run and leaves the corrected V10 run intact as the comparison baseline.

Equivalent explicit command:

```powershell
python -m research.news_reaction_model.v12.run_train --epochs 50 --learning-rate 3e-4 --scheduler cosine --scheduler-restarts 49 --scheduler-cycle-decay 0.98 --scheduler-eta-min 1e-6 --shuffle-buffer-articles 32768 --run-name news-v12-opportunity-openai-stock-state-time-balanced-d384-l4-b2048-e50-cosine-r49-gamma098
```
...
```powershell
python -m research.news_reaction_model.v12.run_train --resume-checkpoint D:\TradingML\runtimes\news-reaction-model\v12\train\news-v12-opportunity-openai-stock-state-time-balanced-d384-l4-b2048-e50-cosine-r49-gamma098\checkpoints\checkpoint_latest.pt
```
...
```powershell
python -m research.news_reaction_model.v12.run_evaluate
```
...
```powershell
python -m research.news_reaction_model.v12.run_fit_diagnostic
```
...
To test whether the unchanged V12 architecture and optimizer can memorize one
fixed label population:

```powershell
python -m research.news_reaction_model.v12.run_memorization_test
```
...
```powershell
python -m unittest research.news_reaction_model.v12.test_news_reaction_model_v12 -v
```
