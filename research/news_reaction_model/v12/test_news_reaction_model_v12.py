...
from research.news_reaction_model import v9, v12
...
from research.news_reaction_model.v12.config import (
    ExperimentConfig,
    LoaderConfig,
    ModelConfig,
    TrainConfig,
)
from research.news_reaction_model.v12.data import (
...
from research.news_reaction_model.v12.evaluate import (
    OpportunityLedger,
    evaluation_batch_sql,
    midpoint_proxy_pnl,
)
from research.news_reaction_model.v12.fit_diagnostic import fit_comparison
from research.news_reaction_model.v12.inference import (
    LiveFeatureEncoder,
    opportunity_predictions,
)
from research.news_reaction_model.v12.losses import compute_loss
from research.news_reaction_model.v12.metrics import TrainingLossAccumulator
from research.news_reaction_model.v12.model import (
    NewsReactionModelV12,
    NewsReactionOpportunityOutput,
)
from research.news_reaction_model.v12.memorization_test import (
    deterministic_subset_sql,
    slice_batch,
)
from research.news_reaction_model.v12.opportunity import (
    OPPORTUNITY_CLASSES,
...
from research.news_reaction_model.v12.time_features import (
    TIME_FEATURE_DIM,
    encode_time_features,
)
from research.news_reaction_model.v12.train import (
...
class NewsReactionModelV12Tests(unittest.TestCase):
    def test_v12_removes_only_the_time_projection_layer_norm(self) -> None:
        model = NewsReactionModelV12(ModelConfig())
        self.assertEqual(
            [type(layer) for layer in model.time_projection],
            [torch.nn.Linear, torch.nn.GELU],
        )

    def test_v12_retains_v9_horizons_and_prepared_dataset(self) -> None:
        loader = LoaderConfig()
        self.assertEqual(v12.MODEL_VERSION, "v12")
        self.assertEqual(v12.HORIZONS, v9.HORIZONS)
    ...
    def test_corrected_v12_preserves_v9_capacity_while_adding_time_channel(self) -> None:
        v9_model = dataclasses.asdict(V9ModelConfig())
        v12_model = dataclasses.asdict(ModelConfig())
        self.assertEqual(v12_model.pop("time_feature_dim"), TIME_FEATURE_DIM)
        self.assertEqual(v12_model, v9_model)
        v9_train = dataclasses.asdict(V9TrainConfig())
        v12_train = dataclasses.asdict(TrainConfig())
        for key in (
            "epochs",
            "max_samples",
            "learning_rate",
            ...
            self.assertEqual(v12_train[key], v9_train[key], key)
        self.assertNotIn("ordinal_loss_weight", v12_train)
    ...
    def test_opportunity_contract_has_exactly_three_classes(self) -> None:
        ...
        self.assertEqual(set(opportunity_contract()["rules"]), set(v12.HORIZONS))
    ...
    def test_tensor_targets_match_scalar_contract(self) -> None:
        returns = torch.zeros((5, len(v12.HORIZONS), 3), dtype=torch.float32)
        ...
        mask = torch.ones((5, len(v12.HORIZONS)), dtype=torch.bool)
    ...
    def test_invalid_labels_are_masked(self) -> None:
        returns = torch.zeros((2, len(v12.HORIZONS), 3), dtype=torch.float32)
        mask = torch.ones((2, len(v12.HORIZONS)), dtype=torch.bool)
        mask[0, 0] = False
    ...
    def test_model_has_only_one_opportunity_head_per_horizon(self) -> None:
        ...
        self.assertEqual(set(model.opportunity_heads), set(v12.HORIZONS))
    ...
    def test_forward_and_loss_use_one_label_per_horizon(self) -> None:
        ...
        self.assertEqual(set(output.logits), set(v12.HORIZONS))
        self.assertTrue(all(tuple(logits.shape) == (4, 3) for logits in output.logits.values()))
        result = compute_loss(output, batch)
        self.assertTrue(torch.isfinite(result.loss))
        self.assertEqual(result.metrics["train/valid_labels"], 4 * len(v12.HORIZONS))
    ...
    def test_inference_opens_only_dominant_direction_positions(self) -> None:
        ...
        output = NewsReactionOpportunityOutput(
            logits={horizon: logits.clone() for horizon in v12.HORIZONS},
    ...
    def test_loss_weights_each_horizon_equally_not_each_label(self) -> None:
        ...
        logits = {
            ...
            for horizon in v12.HORIZONS
        }
        logits[v12.HORIZONS[0]][:, 1] = 3.0
        logits[v12.HORIZONS[1]][0, 1] = -3.0
        result = compute_loss(
            NewsReactionOpportunityOutput(
                logits=logits,
        ...
        expected = torch.stack([
            torch.nn.functional.cross_entropy(
                logits[v12.HORIZONS[0]], targets[v12.HORIZONS[0]]
            ),
            torch.nn.functional.cross_entropy(
                logits[v12.HORIZONS[1]][:1], targets[v12.HORIZONS[1]][:1]
            ),
        ]).mean()
    ...
    def test_buffered_shuffle_is_deterministic_and_resume_exact(self) -> None:
        ...
        def identities(skip: int = 0, epoch: int = 2) -> list[str]:
            with mock.patch(
                "research.news_reaction_model.v12.data.ClickHouseNewsReactionDataset",
                FakeDataset,
            ):
                return [
                    identity
    ...
    def test_checkpoint_restores_exact_next_dropout_forward(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            model = NewsReactionModelV12(config.model)
            ...
            restored_model = NewsReactionModelV12(config.model)
