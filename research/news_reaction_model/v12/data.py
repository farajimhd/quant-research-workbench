...
from research.news_reaction_model.v12 import HORIZONS, SESSIONS
from research.news_reaction_model.v12.config import LoaderConfig
...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "V12 intentionally reuses the completed V8 representation and does not build a separate dataset."
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(f"Prepared V12 dataset integrity check failed: {result}")
    return result
...
class ClickHouseNewsReactionDataset:
    ...
    def iter_batches(self) -> Iterator[NewsReactionBatch]:
        ...
        threads = [
            threading.Thread(target=worker, name=f"news-v12-loader-{index}", daemon=True)
