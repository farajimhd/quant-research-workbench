...
from research.news_reaction_model.v12.stock_state import STOCK_STATE_DIM
from research.news_reaction_model.v12.time_features import TIME_FEATURE_DIM
...
class LoaderConfig:
    ...
    # Corrected V12 still reuses the exact prepared V8 rows; its causal time
...
class TrainConfig:
    ...
    # Keep the V3-V9 project so V12 remains directly comparable.
...
def default_run_name(config: ExperimentConfig) -> str:
    ...
    return (
        f"news-v12-opportunity-openai-stock-state-time-balanced-d{config.model.d_model}"
