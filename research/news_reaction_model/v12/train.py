...
from research.news_reaction_model.v12 import HORIZONS, MODEL_FAMILY, MODEL_VERSION
from research.news_reaction_model.v12.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
...
from research.news_reaction_model.v12.evaluate import evaluate_checkpoint
from research.news_reaction_model.v12.losses import compute_loss
from research.news_reaction_model.v12.metrics import (
    OpportunityAccumulator,
    TrainingLossAccumulator,
)
from research.news_reaction_model.v12.model import NewsReactionModelV12, build_model_mermaid
from research.news_reaction_model.v12.opportunity import opportunity_contract
from research.news_reaction_model.v12.time_features import contract_payload as time_contract
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train V12: the V8 encoder with one three-class opportunity head per horizon."
    )
...
def validate_config(config: ExperimentConfig) -> None:
    if not 1 <= config.train.epochs <= 50:
        raise ValueError("V12 training requires --epochs between 1 and 50 inclusive")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.dummy_data:
        ...
        if train_audit["representation_sha256"] != validation_audit["representation_sha256"]:
            raise RuntimeError("V12 train and validation rows were built by different representations.")
        manifest_path = config.loader.representation_artifact_root / "manifest.json"
        if not manifest_path.exists():
            raise RuntimeError(f"V12 representation manifest is missing: {manifest_path}")
        ...
        if train_audit["representation_sha256"] != feature_manifest["representation_sha256"]:
            raise RuntimeError(
                "Prepared V12 rows do not match the checksummed OpenAI-plus-stock-state representation."
            )
        print(
    ...
    raw_model = NewsReactionModelV12(config.model).to(device)
    ...
    write_model_artifacts(
        ...
        summary_notes=(
            "Corrected V12 adds a causal exchange-time channel, equal horizon loss, "
...
def restore(
    ...
    *,
    ...
    if state.get("resume_contract") != expected_contract:
        raise ValueError(
            "Checkpoint predates or violates the exact V12 resume contract."
        )
