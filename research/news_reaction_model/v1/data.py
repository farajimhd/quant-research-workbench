...
from research.news_reaction_model.v1 import HORIZONS, SESSIONS
from research.news_reaction_model.v1.config import LoaderConfig
...
class NewsReactionBatch:
    ...
    def to(self, device: torch.device, *, non_blocking: bool = True) -> "NewsReactionBatch":
        return NewsReactionBatch(
            ...
            class_targets=self.class_targets.to(device, non_blocking=non_blocking),
...
SELECT canonical_news_id AS source_id, ticker, published_at_utc, chunks, publication_session,
 horizon_codes, class_targets, return_targets
FROM {table} FINAL
...
def audit_prepared_dataset(config: LoaderConfig, start: str, end_exclusive: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run python -m research.news_reaction_model.v1.run_prepare_data --execute first."
        ) from exc
...
def rows_to_batch(rows: list[dict[str, Any]], config: LoaderConfig) -> NewsReactionBatch:
    ...
    for row_index, row in enumerate(rows):
        ...
        for code, target_class, target_returns in zip(
            row.get("horizon_codes", ()), row.get("class_targets", ()), row.get("return_targets", ())
        ):
            hi = horizon_index.get(str(code))
            if hi is not None:
                classes[row_index, hi] = int(target_class)
...
def make_dummy_batch(batch_size: int, config: LoaderConfig, *, device: torch.device | str = "cpu") -> NewsReactionBatch:
    ...
    for index in range(batch_size):
        rows.append({
            ...
            "class_targets": [index % 3 for _ in config.horizons],
