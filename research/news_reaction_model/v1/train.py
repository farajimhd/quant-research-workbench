...
from research.news_reaction_model.v1.config import ExperimentConfig, LoaderConfig, ModelConfig, TrainConfig, default_run_name, to_dict
from research.news_reaction_model.v1.data import ClickHouseNewsReactionDataset, NewsReactionBatch, audit_prepared_dataset, make_dummy_batch
from research.news_reaction_model.v1.losses import compute_loss
from research.news_reaction_model.v1.metrics import ClassificationAccumulator
from research.news_reaction_model.v1.model import NewsReactionModelV1, build_model_mermaid
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train news_reaction_model v1 from frozen Qwen article embeddings.")
    ...
    parser.add_argument("--return-loss-weight", type=float, default=train.return_loss_weight)
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    ...
    train = TrainConfig(
        ...
        return_loss_weight=args.return_loss_weight, scheduler=args.scheduler,
        scheduler_cycle_samples=args.scheduler_cycle_samples, scheduler_eta_min=args.scheduler_eta_min,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    raw_model = NewsReactionModelV1(config.model).to(device)
        ...
        for epoch in range(epoch_start, config.train.epochs):
            ...
            for batch in iterator:
                ...
                with torch.autocast(device_type=device.type, dtype=amp_dtype(config.train.amp_dtype), enabled=device.type == "cuda" and config.train.amp):
                    ...
                    result = compute_loss(output, batch, return_weight=config.train.return_loss_weight)
...
def validate(model: torch.nn.Module, config: ExperimentConfig, device: torch.device, dummy: bool, dummy_count: int) -> dict[str, float]:
    model.eval(); accumulator = ClassificationAccumulator(); horizon_accumulators = [ClassificationAccumulator() for _ in HORIZONS]; loss_sum = 0.0; batches = 0
    iterator = dummy_batches(config, device, min(dummy_count, 2)) if dummy else real_batches(config, train=False)
    for batch in iterator:
        batch = batch.to(device)
        with torch.autocast(device_type=device.type, dtype=amp_dtype(config.train.amp_dtype), enabled=device.type == "cuda" and config.train.amp):
            output = model(batch.x); result = compute_loss(output, batch, return_weight=config.train.return_loss_weight)
        accumulator.add(output.class_logits, batch.class_targets, output.return_forecasts, batch.return_targets, batch.label_mask)
        for horizon_index, horizon in enumerate(HORIZONS):
            horizon_accumulators[horizon_index].add(
                output.class_logits[:, horizon_index], batch.class_targets[:, horizon_index],
...
class SampleCosineScheduler:
    def __init__(self, optimizer: torch.optim.Optimizer, config: TrainConfig) -> None:
        ...
        self.cycle, self.eta_min, self.samples = max(1, config.scheduler_cycle_samples), config.scheduler_eta_min, 0
    def step(self, samples: int) -> None:
        self.samples = samples; position = (samples % self.cycle) / self.cycle
    ...
    def state_dict(self) -> dict[str, Any]: return {"samples": self.samples, "base_lrs": self.base_lrs}
