...
from research.news_reaction_model.v1.data import rows_to_batch
from research.news_reaction_model.v1.model import NewsReactionModelV1


def load_model(checkpoint_path: str | Path, *, device: str | torch.device = "cpu") -> NewsReactionModelV1:
    state = torch.load(checkpoint_path, map_location=device)
    raw_config = state.get("config", {}).get("model", {})
    config = ModelConfig(**raw_config) if raw_config else ModelConfig()
    model = NewsReactionModelV1(config).to(device)
...
def forecast_rows(
    ...
    *,
    ...
    probabilities = torch.softmax(output.class_logits.float(), dim=-1).cpu()
    ...
    for index in range(batch.sample_count):
        ...
        for horizon_index, horizon in enumerate(HORIZONS):
            forecasts[horizon] = {
                "probability_negative": float(probabilities[index, horizon_index, 0]),
                "probability_neutral": float(probabilities[index, horizon_index, 1]),
                "probability_positive": float(probabilities[index, horizon_index, 2]),
