...
class DailyIndexStreamingDashboard:
    ...
    def _render(self) -> object:
        ...
        summary_body = Group(
            ...
            self._wide_progress(
                ...
                total_units,
                label="Overall",
                detail=f"{progress * 100.0:.2f}%  units={done_units:,}/{total_units:,} rate={rate:,.0f}/s eta={format_seconds(eta)}",
            ),
        )

        panels: list[Any] = [
            Panel(summary_body, title="Daily-Indexed Streaming Cache", box=box.ROUNDED, border_style="red" if last_error else "green", padding=(0, 1))
        ]
    ...
    def _modality_panel(self, name: str, modality: dict[str, Any], *, compact: bool) -> object:
        ...
        overall_bar = self._wide_progress(
            written,
            expected,
            label="Overall",
            detail=f"{progress * 100.0:.1f}%  {written:,}/{expected:,}  {queue_detail(modality)}",
        )
        separator = Rule(style="dim blue")
        if compact:
            ...
            return Panel(Group(overall_bar, separator, table), title=name, box=box.ROUNDED, border_style="blue", padding=(0, 1))
    ...
    @staticmethod
    def _wide_progress(completed: int | float, total: int | float, *, label: str, detail: str) -> object:
        from rich.progress import BarColumn, Progress, TextColumn

        progress = Progress(
            TextColumn(f"[cyan]{label}", justify="left"),
            BarColumn(bar_width=None),
            TextColumn(detail, justify="right"),
            expand=True,
        )
        total_value = max(1.0, float(total or 0))
        completed_value = min(max(0.0, float(completed or 0)), total_value)
        progress.add_task(label, total=total_value, completed=completed_value)
        return progress
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--label-process-workers", type=int, default=32)
    parser.add_argument("--label-write-workers", type=int, default=8)
    parser.add_argument("--macro-fetch-workers", type=int, default=4)
    parser.add_argument("--macro-process-workers", type=int, default=1)
    parser.add_argument("--macro-write-workers", type=int, default=4)
    parser.add_argument("--news-fetch-workers", type=int, default=3)
    parser.add_argument("--news-process-workers", type=int, default=1)
    parser.add_argument("--news-write-workers", type=int, default=3)
    parser.add_argument("--sec-fetch-workers", type=int, default=3)
    parser.add_argument("--sec-process-workers", type=int, default=1)
    parser.add_argument("--sec-write-workers", type=int, default=3)
    parser.add_argument("--xbrl-fetch-workers", type=int, default=4)
    parser.add_argument("--xbrl-process-workers", type=int, default=1)
    parser.add_argument("--xbrl-write-workers", type=int, default=3)
