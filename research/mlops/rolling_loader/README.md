...
| `ticker_intraday_bars.trade_values.open/close/high/low` | Same-session historical trade stream before the origin | `intraday_context_bars_part_*.parquet`, decoded float32 list columns | `bar_inputs["ticker_intraday_bars"]["trade_values"][..., price_field]` | Backward intraday X context. Uses completed grid buckets and clips to the 04:00 New York session start. If no completed bar exists for that resolution, it falls back to session open through the origin only. |
| `ticker_intraday_bars.trade_values.size_sum/event_count` | Same-session historical trade stream before the origin | `intraday_context_bars_part_*.parquet`, float/count list columns | `bar_inputs["ticker_intraday_bars"]["trade_values"][..., size_sum/event_count]` | Same 6-field schema as future trade bars, but used as context. |
| `ticker_intraday_bars.quote_bid_values.open/close/high/low` | Same-session historical quote bid stream before the origin | `intraday_context_bars_part_*.parquet`, decoded float32 list columns | `bar_inputs["ticker_intraday_bars"]["quote_bid_values"][..., price_field]` | Bid-side backward intraday X context. |
| `ticker_intraday_bars.quote_bid_values.size_open/size_close/size_high/size_low/event_count` | Same-session historical quote bid stream before the origin | `intraday_context_bars_part_*.parquet`, float/count list columns | `bar_inputs["ticker_intraday_bars"]["quote_bid_values"][..., field]` | Same 9-field schema as future bid bars, but used as context. |
| `ticker_intraday_bars.quote_ask_values.open/close/high/low` | Same-session historical quote ask stream before the origin | `intraday_context_bars_part_*.parquet`, decoded float32 list columns | `bar_inputs["ticker_intraday_bars"]["quote_ask_values"][..., price_field]` | Ask-side backward intraday X context. |
| `ticker_intraday_bars.quote_ask_values.size_open/size_close/size_high/size_low/event_count` | Same-session historical quote ask stream before the origin | `intraday_context_bars_part_*.parquet`, float/count list columns | `bar_inputs["ticker_intraday_bars"]["quote_ask_values"][..., field]` | Same 9-field schema as future ask bars, but used as context. |
| `ticker_intraday_bars.*_mask` | Family availability in each backward horizon | `*_available` list columns | `bar_inputs["ticker_intraday_bars"]["{family}_mask"] [B,H]` | False when no event of that family exists in the backward horizon. |
| `ticker_intraday_bars.*_time_features` | Backward context grid start and origin timestamp | computed by loader from `context_grid_start_timestamp_us` | `[B,H,bar_time_features]` | UTC start-time features plus age from origin to context-window start; zero where the family mask is false. |
...
| Intraday backward trade bars | Same-session trade events before each origin | `intraday_context_bars_part_*.parquet` list columns `trade_open`, `trade_close`, `trade_high`, `trade_low`, `trade_size_sum`, `trade_event_count`, `trade_available` | `bar_inputs["ticker_intraday_bars"]["trade_values"] [B,H,6]`, mask `[B,H]` | X context only. Normal rows use completed grid buckets ending before the origin bucket; if there is no completed bucket yet for that resolution, all horizons use the same session-open-to-origin window. |
| Intraday backward quote bid bars | Same-session quote events before each origin | `intraday_context_bars_part_*.parquet` list columns `quote_bid_*`, `quote_bid_available` | `bar_inputs["ticker_intraday_bars"]["quote_bid_values"] [B,H,9]`, mask `[B,H]` | Same schema as future bid bars, clipped backward to session start with the same first-bucket fallback and no lookahead. |
| Intraday backward quote ask bars | Same-session quote events before each origin | `intraday_context_bars_part_*.parquet` list columns `quote_ask_*`, `quote_ask_available` | `bar_inputs["ticker_intraday_bars"]["quote_ask_values"] [B,H,9]`, mask `[B,H]` | Same schema as future ask bars, clipped backward to session start with the same first-bucket fallback and no lookahead. |
