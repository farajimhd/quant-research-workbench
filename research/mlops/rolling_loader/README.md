...
| Intraday backward trade bars | Same-session trade events before each origin | `intraday_context_bars_part_*.parquet` list columns `trade_open`, `trade_close`, `trade_high`, `trade_low`, `trade_size_sum`, `trade_event_count`, `trade_available` | `bar_inputs["ticker_intraday_bars"]["trade_values"] [B,H,6]`, mask `[B,H]` | X context only. The builder uses completed grid buckets ending before the origin bucket; it never includes future events. |
| Intraday backward quote bid bars | Same-session quote events before each origin | `intraday_context_bars_part_*.parquet` list columns `quote_bid_*`, `quote_bid_available` | `bar_inputs["ticker_intraday_bars"]["quote_bid_values"] [B,H,9]`, mask `[B,H]` | Same schema as future bid bars, but clipped backward to session start. |
| Intraday backward quote ask bars | Same-session quote events before each origin | `intraday_context_bars_part_*.parquet` list columns `quote_ask_*`, `quote_ask_available` | `bar_inputs["ticker_intraday_bars"]["quote_ask_values"] [B,H,9]`, mask `[B,H]` | Same schema as future ask bars, but clipped backward to session start. |
