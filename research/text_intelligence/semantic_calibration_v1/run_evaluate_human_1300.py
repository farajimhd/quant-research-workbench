...
CONTRACT = "news_human_1300_deterministic_v9_regression_v2"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Regress the current deterministic News V9 over the certified 1,300-item authority."
    )
    parser.add_argument("--human-root", type=Path, default=base / "news_1300_v1")
    parser.add_argument(
        "--output-root",
        type=Path,
        default=base / "news_1300_v1" / "current_v9_regression",
    )
    ...
    generate_v9_predictions(items, prediction_dir, issuer_resolver=load_v9_issuer_authority())
