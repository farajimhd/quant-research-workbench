...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_BASE / "openai_gold_100_v4" / "comparison.json",
    )
    parser.add_argument(
        "--shared-root", type=Path, default=DEFAULT_BASE / "oss_gold_100_v1" / "shared"
    )
    args = parser.parse_args(argv)
    manifest = prepare_bundle(
        collection_root=args.collection_root,
        selection_path=args.selection_path,
        openai_comparison_path=args.openai_comparison_path,
