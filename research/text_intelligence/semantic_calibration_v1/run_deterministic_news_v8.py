...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build and evaluate deterministic News V8.")
    ...
    output = args.runtime_root / "deterministic_v8"
    ...
    if args.phase == "frozen-acceptance" and metrics_path.exists():
        raise RuntimeError("Frozen V8 acceptance already exists; create a new authority version.")
    ...
    for index, item in enumerate(selected, 1):
        ...
        if index % 100 == 0 or index == len(selected):
            print(f"V8 {args.phase} {index:,}/{len(selected):,}", flush=True)
...
def _predict(item) -> dict:
    ...
    return classify_news_document_v8(
        document,
