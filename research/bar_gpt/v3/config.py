...
from research.bar_gpt.v3.targets import AUTOREGRESSIVE_TARGET_NAMES, TARGET_NAMES
...
BAR_GPT_WANDB_PROJECT = "bar gpt v3"
BAR_GPT_MODEL_COMPARISON_WANDB_PROJECT = "bar gpt v3 model comparison"
BAR_GPT_FULL_TRAINING_WANDB_PROJECT = "bar gpt v3 full training"
...
@dataclass(slots=True)
class TrainConfig:
    output_root: Path = Path(r"D:\TradingML\runtimes\bar_gpt\v3\train")
    run_name: str = "bar-gpt-v3"
    # Production v3 makes two complete passes over the unchanged catalog.
    epochs: int = 2
    max_samples: int = 0
    ...
    # This authoritative clock is independent of chunks and outer epochs.
    # Every boundary owns an immutable sample-numbered checkpoint.
    global_validation_interval_samples: int = 500_000_000
    ...
    def validate(self) -> None:
        ...
        if self.global_validation_interval_samples <= 0:
            raise ValueError("global_validation_interval_samples must be positive")
