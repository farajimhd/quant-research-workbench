...
from research.bar_gpt.v3.objectives import BarGPTLoss
...
def _flat_confusion(
    ...
    *,
    ...
    if labels.shape != predictions.shape or labels.shape != mask.shape:
        raise ValueError("flat confusion labels, predictions, and mask must align")
    encoded = labels[mask] * int(classes) + predictions[mask]
    return torch.bincount(encoded, minlength=classes * classes).reshape(classes, classes).cpu()
...
class ValidationAccumulator:
    ...
    autoregressive_return_confusion_by_gap: dict[str, torch.Tensor] = field(default_factory=dict)
    autoregressive_gap_confusion: dict[str, torch.Tensor] = field(default_factory=dict)
    ...
    def update(self, output: BarGPTOutput, batch: BarGPTBatch, result: BarGPTLoss) -> None:
        ...
        for view, prediction in output.autoregressive.items():
            target = batch.autoregressive_targets[view][
                :, : prediction.shape[1], :RETURN_TARGET_COUNT
            ...
            confusion = _confusion(
                ...
                autoregressive_return_class_labels(predicted_return.detach(), view),
                valid,
            )
            ...
            gap_logits = output.autoregressive_gap_logits[view].detach()
            gap_labels = batch.autoregressive_gap_labels[view][:, : gap_logits.shape[1]]
            gap_mask = batch.autoregressive_gap_mask[view][:, : gap_logits.shape[1]]
            gap_confusion = _flat_confusion(
                gap_labels,
                gap_logits.argmax(dim=-1),
                gap_mask,
                classes=NEXT_EVENT_GAP_CLASS_COUNT,
            )
            previous_gap = self.autoregressive_gap_confusion.get(view)
            self.autoregressive_gap_confusion[view] = (
                gap_confusion if previous_gap is None else previous_gap + gap_confusion
            )
            actual_direction = autoregressive_return_class_labels(target, view)
            predicted_direction = autoregressive_return_class_labels(
                predicted_return.detach(), view
            )
            by_gap = torch.stack(
                [
                    _confusion(
                        actual_direction,
                        predicted_direction,
                        valid & (gap_labels == gap_class).unsqueeze(-1),
                    )
                    for gap_class in range(NEXT_EVENT_GAP_CLASS_COUNT)
                ]
            )
            previous_by_gap = self.autoregressive_return_confusion_by_gap.get(view)
            self.autoregressive_return_confusion_by_gap[view] = (
                by_gap if previous_by_gap is None else previous_by_gap + by_gap
            )
        ...
        confusion = torch.stack([
            _confusion(
                ...
                physical_return_class_labels(return_prediction, batch.horizons_us)[
                    :, :, horizon_index, CLOSE_RETURN_TARGET_INDICES
                ],
    ...
    def _finalize_losses(self, metrics: dict[str, float]) -> None:
        if not self.include_loss_metrics:
            return
        groups = {
            "ar_regression": [], "ar_categorical": [], "ar_time_to_event": [],
            "horizon_regression": [], "horizon_categorical": [],
        }
        ...
        ar = group_means["ar_regression"] + group_means["ar_categorical"] + group_means["ar_time_to_event"]
        horizon = group_means["horizon_regression"] + group_means["horizon_categorical"]
        metrics.update({
            f"{self.namespace}_loss/ar_regression": group_means["ar_regression"],
            f"{self.namespace}_loss/ar_categorical": group_means["ar_categorical"],
            f"{self.namespace}_loss/ar_time_to_event": group_means["ar_time_to_event"],
            f"{self.namespace}_loss/horizon_quantile": group_means["horizon_regression"],
            f"{self.namespace}_loss/horizon_categorical": group_means["horizon_categorical"],
    ...
    def finalize(self) -> dict[str, float]:
        ...
        for view, by_gap in sorted(self.autoregressive_return_confusion_by_gap.items()):
            for gap_index, gap_name in enumerate(NEXT_EVENT_GAP_CLASS_NAMES):
                balanced: list[float] = []
                mcc: list[float] = []
                support = 0.0
                for target_confusion in by_gap[gap_index]:
                    scores = multiclass_scores(target_confusion)
                    balanced.append(scores[1])
                    mcc.append(scores[3])
                    support += float(target_confusion.sum())
                prefix = f"{self.namespace}_ar_close_by_gap_{view}_{gap_name}"
                metrics[f"{prefix}/balanced_accuracy_macro"] = _macro(balanced)
                metrics[f"{prefix}/mcc_macro"] = _macro(mcc)
                metrics[f"{prefix}/support"] = support

        gap_accuracies: list[float] = []
        for view, matrix in sorted(self.autoregressive_gap_confusion.items()):
            total = float(matrix.sum())
            accuracy = float(matrix.diag().sum()) / total if total else float("nan")
            metrics[f"{self.namespace}_ar_time_to_event_{view}/accuracy"] = accuracy
            gap_accuracies.append(accuracy)
            if self.include_class_diagnostics:
                for class_index, class_name in enumerate(NEXT_EVENT_GAP_CLASS_NAMES):
                    metrics[f"{self.namespace}_ar_time_to_event_{view}_support/{class_name}"] = float(
                        matrix[class_index].sum()
                    )
        metrics[f"{self.namespace}_ar_time_to_event_summary/accuracy_macro"] = _macro(gap_accuracies)
        ...
        endpoint_mae: dict[str, list[float]] = {family: [] for family in PRICE_FAMILIES}
        range_mae: dict[str, list[float]] = {family: [] for family in PRICE_FAMILIES}
        for horizon_index, horizon_us in enumerate(self.horizons_us):
            ...
            for target_index, target_name in enumerate(RETURN_TARGET_NAMES):
                ...
                if field == "close":
                    endpoint_mae[family].append(value * 100.0)
                elif field in {"high", "low"}:
                    range_mae[family].append(value * 100.0)
        ...
        for family, values in family_values.items():
            ...
            metrics[f"{self.namespace}_{family}_close_summary/mae_bps_macro"] = _macro(endpoint_mae[family])
            metrics[f"{self.namespace}_{family}_range_summary/mae_bps_macro"] = _macro(range_mae[family])
            if self.include_confidence_metrics:
