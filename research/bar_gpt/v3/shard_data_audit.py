...
from research.bar_gpt.v3 import assert_checkpoint_version

from research.bar_gpt.v3.config import BarGPTConfig, DataConfig, ExperimentConfig, TrainConfig
from research.bar_gpt.v3.data import AUTOREGRESSIVE_VIEW_NAMES, collate_examples
from research.bar_gpt.v3.direct_event_shards import DirectEventShardDataset
from research.bar_gpt.v3.features import MODEL_FEATURE_NAMES
...
from research.bar_gpt.v3.train import _forward
...
def selected_predictions(
    ...
    *,
    ...
    model = BarGPTV3(model_config).to(resolved_device).eval()
    ...
    assert output.horizon_quantiles is not None
    median_index = min(range(len(model_config.quantiles)), key=lambda index: abs(model_config.quantiles[index] - 0.5))
    predicted_classes = physical_return_class_labels(
        output.horizon_quantiles[..., : len(DIRECTION_TARGET_NAMES), median_index],
        tuple(data_config.horizons_us),
    )
    rows: list[dict[str, Any]] = []
    for horizon_index, horizon_us in enumerate(data_config.horizons_us):
        ...
        for target_index, target_name in enumerate(DIRECTION_TARGET_NAMES):
            ...
            predicted_class = int(predicted_classes[0, origin_offset, horizon_index, target_index])
            row.update({
                ...
                f"{target_name}_predicted_class": RETURN_CLASS_NAMES[predicted_class],
                f"{target_name}_class_source": "continuous_median_return",
            })
        rows.append(row)
    return rows
