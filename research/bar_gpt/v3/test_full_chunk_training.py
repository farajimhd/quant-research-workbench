...
from research.bar_gpt.v3.model_discovery import DISCOVERY_CONTRACT_VERSION
from research.bar_gpt.v3.offline_shards import OfflineBlockRef, OfflineShardUnit
from research.bar_gpt.v3.progress import TrainingProgressState, TrainingReporter
from research.bar_gpt.v3.run_train_full_chunks import (
    DEFAULT_MODEL_SIZE,
...
from research.bar_gpt.v3.train import (
    ...
    _global_validation_checkpoint_name,
    _global_checkpoint_promotes,
    _next_global_validation_boundary,
...
class FullChunkLauncherTest(unittest.TestCase):
    def test_global_validation_checkpoint_name_is_origin_exact(self) -> None:
        self.assertEqual(
            _global_validation_checkpoint_name(500_123_456),
            "checkpoint_global_validation_origins_000500123456.pt",
        )
        self.assertEqual(_next_global_validation_boundary(-1, 500_000_000), 500_000_000)
        self.assertEqual(
            _next_global_validation_boundary(500_123_456, 500_000_000),
            1_000_000_000,
        )

    def test_global_promotion_requires_endpoint_and_all_non_regression_gates(self) -> None:
        incumbent = {
            "trade_close_mae_bps": 2.0,
            "trade_range_mae_bps": 4.0,
            "close_mcc": 0.2,
            "trade_calibration": 0.1,
        }
        self.assertTrue(
            _global_checkpoint_promotes(
                {**incumbent, "trade_close_mae_bps": 1.9}, incumbent
            )
        )
        self.assertFalse(
            _global_checkpoint_promotes(
    ...
    def test_defaults_to_medium_and_preserves_samples_seen_as_training_clock(self) -> None:
        ...
        args.initialize_from_v2_checkpoint = r"D:\runtime\checkpoint_v2.pt"
        argv = trainer_argv(
            args,
        ...
        self.assertEqual(
            parsed.initialize_from_v2_checkpoint,
            r"D:\runtime\checkpoint_v2.pt",
        )
        self.assertEqual(parsed.global_validation_interval_samples, 500_000_000)
        ...
        self.assertEqual(parsed.learning_rate, 3e-4)
        self.assertEqual(parsed.weight_decay, 0.1)
        self.assertEqual(parsed.grad_clip_norm, 1.0)
        self.assertEqual(parsed.amp_dtype, "bf16")
        self.assertTrue(parsed.amp)
