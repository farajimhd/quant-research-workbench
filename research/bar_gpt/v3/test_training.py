...
from research.bar_gpt.v3.data import FixedBucketHistoryCache, PATHWAY_ID_BY_NAME, TIMEFRAME_US_BY_NAME, BarView, collate_examples
...
from research.bar_gpt.v3.sampling import CoverageCursor, SESSION_PHASES, coverage_plan_summary, select_stratified_examples
...
from research.bar_gpt.v3.metrics import ValidationAccumulator, _direction_scores, multiclass_scores
from research.bar_gpt.v3.features import MODEL_FEATURE_NAMES, project_stationary_features
from research.bar_gpt.v3.model import BarGPTV3
from research.bar_gpt.v3.linear_probe import fit_ridge_probes
from research.bar_gpt.v3.objectives import (
    compute_loss,
    masked_huber_loss,
...
from research.bar_gpt.v3.lock_offline_shard_catalog import lock_catalog
from research.bar_gpt.v3.progress import TrainingProgressState, TrainingReporter, _format_value, _ratio_markup
...
from research.bar_gpt.v3.train import (
...
from research.bar_gpt.v3.run_build_conditions_1s import default_argv as condition_builder_argv
...
from research.bar_gpt.v3.run_profile_train import DEFAULT_ARGS as profile_launcher_args
from research.bar_gpt.v3.run_pilot_offline_shards import commands as pilot_commands, parse_args as parse_pilot_args
from research.bar_gpt.v3.overfit_pilot import (
...
from research.bar_gpt.v3.run_train import DEFAULT_ARGS as training_launcher_args
from research.bar_gpt.v3.run_train_model_comparison import (
...
from research.bar_gpt.v3.run_profile_model_performance import parse_args as parse_performance_profile_args, profiler_argv
from research.bar_gpt.v3.run_profile_length_buckets import (
...
class LoaderTrainerContractTest(unittest.TestCase):
    ...
    def test_offline_controller_exception_is_durable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with patch(
                "research.bar_gpt.v3.offline_shards._run_main",
    ...
    def test_arrow_retry_discards_partial_attempt_before_exposing_rows(self) -> None:
        ...
        with patch("research.bar_gpt.v3.loader.request.urlopen", side_effect=responses), patch(
    ...
    def test_validation_plan_uses_bounded_sequential_blocks(self) -> None:
        ...
        with patch("research.bar_gpt.v3.loader.ArrowStreamClient", FakeClient):
    ...
    def data_config(self) -> DataConfig:
        return DataConfig(
            tickers=("AAA", "BBB", "CCC"),
            horizons_us=(5_000_000, 30_000_000),
            maximum_target_horizon_us=30_000_000,
            context_bars_1s=4,
    ...
    def test_overfit_population_is_bounded_and_three_class_scores_are_independent(self) -> None:
        ...
        perfect = torch.eye(RETURN_CLASS_COUNT, dtype=torch.float64) * 10
    ...
    def test_strict_2026_audit_rejects_masked_calendar_context(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(audit_shard(sidecar, verify_sha256=True, max_absolute_return_bps=1e9)["status"], "passed")
            with self.assertRaisesRegex(RuntimeError, "required calendar context is unavailable"):
                audit_shard(
                    sidecar, verify_sha256=True, require_calendar_context=True,
                    max_absolute_return_bps=1e9,
                )
    ...
    def test_pilot_launcher_builds_direct_shards_then_audits_automatically(self) -> None:
        ...
        self.assertIn("research.bar_gpt.v3.audit_offline_shards", audit)
        self.assertIn("--verify-sha256", audit)
        self.assertNotIn("--require-calendar-context", audit)
        self.assertIn("--verify-direct-source", audit)
        self.assertIn("research.bar_gpt.v3.run_audit_shard_data", sampled_audit)
    ...
    def test_complete_offline_dataset_launcher_builds_one_catalog_then_audits(self) -> None:
        ...
        self.assertIn(
            "research.bar_gpt.v3.run_audit_shard_data",
    ...
    def test_immutable_catalog_blocks_execute_before_build_diagnostics(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("research.bar_gpt.v3.offline_shards.BuildRunLog") as build_log:
    ...
    def test_sequential_coverage_explicitly_includes_holdout_and_derived_warmup(self) -> None:
        ...
        test_case = self
        with patch("research.bar_gpt.v3.train.ArrowStreamClient", FakeStream):
    ...
    def test_long_run_defaults_use_profiled_shape_and_fractional_warmup(self) -> None:
        ...
        self.assertEqual(training_launcher_args["--epochs"], "2")
    ...
    def test_checkpoint_policy_does_not_promote_on_composite_loss(self) -> None:
        ...
        self.assertFalse(policy.save_best_val)
    ...
    def test_condition_builder_is_sparse_resumable_and_uses_runtime_authority(self) -> None:
        ...
        self.assertEqual(args[output_index], r"D:\TradingML\runtimes\bar_gpt\v3\build_conditions_1s")
    ...
    def test_collated_batch_runs_complete_mixed_objective(self) -> None:
        ...
        self.assertEqual(set(output.autoregressive_gap_logits), set(output.autoregressive))
        self.assertTrue(
            all(logits.shape[-1] == 6 for logits in output.autoregressive_gap_logits.values())
        )
        ...
        self.assertIn("train/loss_ar_time_to_event", loss.metrics)
        self.assertNotIn("train/loss_ar_return_class", loss.metrics)
        self.assertNotIn("train/loss_horizon_return_class", loss.metrics)
        reweighted = deepcopy(batch)
        reweighted.sample_weights = torch.tensor([1.0, 1000.0])
        reweighted_loss = compute_loss(output, reweighted, TrainConfig(), model_config.quantiles)
        torch.testing.assert_close(reweighted_loss.loss, loss.loss)
        loss.loss.backward()
        self.assertGreater(float(sum(parameter.grad.abs().sum() for parameter in model.parameters() if parameter.grad is not None)), 0.0)
        self.assertGreater(float(model.autoregressive_gap_head.weight.grad.abs().sum()), 0.0)
        self.assertGreater(float(model.autoregressive_gap_condition.weight.grad.abs().sum()), 0.0)
        self.assertFalse(hasattr(model, "autoregressive_return_class_head"))
        self.assertFalse(hasattr(model, "horizon_return_class_head"))
    ...
    def test_endpoint_mae_is_reported_in_inverse_transformed_basis_points(self) -> None:
        ...
        self.assertAlmostEqual(
            metrics["validation_trade_open_return_error/mae_bps_5s"],
    ...
    def test_one_epoch_comparison_runs_match_profiled_winners(self) -> None:
        ...
        for model_size, values in expected.items():
            ...
            self.assertTrue(parsed.experiment_manifest.endswith("fixed_panels_v2.json"))
    ...
    def test_comparison_manifest_requires_all_tickers_and_disjoint_evaluation_dates(self) -> None:
        ...
        manifest = {
            ...
            "cohorts": {
                "training_tickers": ["AAA", "BBB", "CCC"],
                "evaluation_tickers": ["AAA", "BBB", "CCC"],
                "evaluation_available_ticker_dates": {"AAA": 2, "BBB": 2, "CCC": 1},
            },
            "panels": {
                "train": [
                    row("AAA", "2020-01-02", 49_999_999),
                    row("BBB", "2020-01-02", 50_000_000),
                    row("CCC", "2020-01-02", 1),
                ],
                "monitor": [
                ...
                "validation": [
                    row("AAA", "2026-01-05", 2_499_912),
                    row("BBB", "2026-01-05", 2_500_000),
                    row("CCC", "2026-01-05", 88),
                ],
            },
        }
    ...
    def test_all_model_comparison_runs_fresh_processes_sequentially(self) -> None:
        ...
        with patch(
            "research.bar_gpt.v3.run_train_model_comparison.subprocess.run",
    ...
    def test_training_dashboard_keeps_a_fixed_complete_schema(self) -> None:
        ...
        state = TrainingProgressState(
            run_name="smoke", device="cuda", precision="bf16", output_dir=r"D:\TradingML\runtimes\bar_gpt\v3\train\smoke",
        ...
        reporter.validation(
            {
                ...
                "validation_trade_summary/mae_bps_macro": 7.5,
                "validation_close_return_class_summary/balanced_accuracy_macro": 0.56,
                "validation_close_return_class_summary/mcc_macro": 0.12,
        ...
        reporter.validation(
            {
                ...
                "monitor_close_return_class_summary/balanced_accuracy_macro": 0.57,
                "monitor_close_return_class_summary/mcc_macro": 0.14,
            }
        )
        self.assertEqual(reporter.state.validation_metrics["monitor_loss/total"], 0.25)
        self.assertEqual(
            reporter.state.validation_metrics["validation_close_return_class_summary/mcc_macro"],
            0.14,
        )
