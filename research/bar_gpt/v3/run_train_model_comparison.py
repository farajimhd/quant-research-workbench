...
# These are provisional v2 launch baselines inherited from the completed v1
# end-to-end workstation sweep. Replace them only with a completed v2 grid;
# accumulation retains at least 32 blocks per optimizer update.
...
DEFAULT_OUTPUT_ROOT = Path(r"D:\TradingML\runtimes\bar_gpt\v3\model_comparison")
...
def comparison_run_name(model_size: str, run_stamp: str) -> str:
    ...
    return (
        f"bar-gpt-v3-epoch1-{model_size}-micro{run.microbatch}-"
...
def _launcher_command(
    ...
    *,
    ...
    command = [
        ...
        "-B",
        "-m",
        "research.bar_gpt.v3.run_train_model_comparison",
        "--model-size",
        model_size,
        "--run-stamp",
        run_stamp,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    equivalent = [sys.executable, "-B", "-m", "research.bar_gpt.v3.train", *resolved]
