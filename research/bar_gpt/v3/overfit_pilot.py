...
from research.bar_gpt.v3.metrics import ValidationAccumulator, multiclass_scores
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Deliberately overfit warm-started BarGPT v3 on a tiny certified v12 panel."
    )
...
def _evaluate(model: BarGPTV3, batches: list[BarGPTBatch], config: ExperimentConfig, namespace: str) -> dict[str, float]:
    model.eval()
...
def _quick_scores(
    ...
    median_index = min(
        range(len(quantiles)),
        key=lambda index: abs(quantiles[index] - 0.5),
    )
    predictions = physical_return_class_labels(
        output.horizon_quantiles[..., :RETURN_TARGET_COUNT, median_index].detach(),
        batch.horizons_us,
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    output_path = args.output or (
        Path(r"D:\TradingML\runtimes\bar_gpt\v3\overfit_pilot_3class_1bp_v1")
