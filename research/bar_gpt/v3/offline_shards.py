...
from research.bar_gpt.v3.data import (
    ...
    build_target_clock_features,
)
from research.bar_gpt.v3.features import project_stationary_features
from research.bar_gpt.v3.loader import BarGPTIterableDataset, month_units
from research.bar_gpt.v3.schema import FEATURE_VERSION
...
def collate_compiled_blocks(
    ...
    *,
    ...
    origin_timestamps_us = _pad_first_dimension(
        [block.origin_timestamps_us for block in blocks]
    )
    batch = BarGPTBatch(
        views=views,
        view_mask=view_mask,
        autoregressive_gap_labels={
            name: _pad_first_dimension([
                next_event_gap_class_labels(
                    block.view_start_us[name],
                    timeframe_us=TIMEFRAME_US_BY_NAME[name],
                    bar_mask=block.view_mask[name],
                )
                for block in blocks
            ])
            for name in AUTOREGRESSIVE_VIEW_NAMES
        },
        autoregressive_gap_mask={
            name: _pad_first_dimension([
                block.autoregressive_mask[name].any(dim=-1)
                for block in blocks
            ], fill=False)
            for name in AUTOREGRESSIVE_VIEW_NAMES
        },
...
def _run_main(args: argparse.Namespace, run_log: BuildRunLog | None) -> int:
    from research.bar_gpt.v3.train import _stream_config, preflight, sequential_coverage_counts
