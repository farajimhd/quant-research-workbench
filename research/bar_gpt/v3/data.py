...
from research.bar_gpt.v3.schema import FEATURE_INDEX, FEATURE_NAMES, FEATURE_SPECS
...
TARGET_CLOCK_FEATURE_COUNT = 8
_NEW_YORK = ZoneInfo("America/New_York")


def build_target_clock_features(
    origin_timestamps_us: torch.Tensor,
    horizons_us: tuple[int, ...],
) -> torch.Tensor:
    """Build known New York target-clock features for physical horizons."""
    if origin_timestamps_us.ndim != 2:
        raise ValueError("origin timestamps must have shape [B,N]")
    if not horizons_us or any(int(value) <= 0 for value in horizons_us):
        raise ValueError("target-clock horizons must be nonempty and positive")

    def local_components(timestamps_us: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        seconds = torch.div(timestamps_us, 1_000_000, rounding_mode="floor")
        utc_hours = torch.div(seconds, 3_600, rounding_mode="floor")
        unique_hours, inverse = torch.unique(
            utc_hours, sorted=True, return_inverse=True
        )
        offsets = []
        for hour in unique_hours.detach().cpu().tolist():
            instant = dt.datetime.fromtimestamp(
                int(hour) * 3_600, tz=dt.timezone.utc
            )
        ...
        offset_tensor = torch.as_tensor(
            offsets, dtype=seconds.dtype, device=seconds.device
        )
        local = seconds + offset_tensor[inverse]
        return torch.remainder(local, 86_400), torch.div(
            local, 86_400, rounding_mode="floor"
        )

    horizon_tensor = torch.as_tensor(
        horizons_us,
        dtype=origin_timestamps_us.dtype,
        device=origin_timestamps_us.device,
    )
    target_us = origin_timestamps_us.unsqueeze(-1) + horizon_tensor.view(1, 1, -1)
    target_seconds, target_days = local_components(target_us)
    _origin_seconds, origin_days = local_components(origin_timestamps_us)

    seconds = target_seconds.float()
    day_phase = seconds * (2.0 * math.pi / 86_400.0)
    weekday = torch.remainder(target_days + 3, 7).float()
    week_phase = (weekday + seconds / 86_400.0) * (2.0 * math.pi / 7.0)
    session_progress = ((seconds - 34_200.0) / 23_400.0).clamp(0.0, 1.0)
    session_phase = session_progress * (2.0 * math.pi)
    output = torch.stack(
        (
            day_phase.sin(),
            day_phase.cos(),
            ...
            session_phase.sin(),
            session_phase.cos(),
            ((57_600.0 - seconds) / 23_400.0).clamp(-2.0, 2.0),
            (origin_days.unsqueeze(-1) != target_days).float(),
        ),
        dim=-1,
    )
    return output * (origin_timestamps_us > 0).unsqueeze(-1).unsqueeze(-1)
...
class BarGPTBatch:
    ...
    autoregressive_gap_labels: dict[str, torch.Tensor]
    autoregressive_gap_mask: dict[str, torch.Tensor]
    masked_context_views: tuple[str, ...]
    origin_indices: torch.Tensor
    origin_timestamps_us: torch.Tensor
    horizon_clock_features: torch.Tensor
    ...
    def record_stream(self, stream: torch.Stream) -> None:
        ...
        for values in (
            ...
            self.autoregressive_gap_labels,
            self.autoregressive_gap_mask,
        ):
    ...
    def pin_memory(self) -> "BarGPTBatch":
        ...
        return BarGPTBatch(
            ...
            autoregressive_gap_labels=pin_map(self.autoregressive_gap_labels),
            autoregressive_gap_mask=pin_map(self.autoregressive_gap_mask),
            ...
            horizon_clock_features=self.horizon_clock_features.pin_memory(),
    ...
    def to(self, device: torch.device | str, *, non_blocking: bool = True) -> "BarGPTBatch":
        ...
        return BarGPTBatch(
            ...
            autoregressive_gap_labels=move_map(self.autoregressive_gap_labels),
            autoregressive_gap_mask=move_map(self.autoregressive_gap_mask),
            masked_context_views=self.masked_context_views,
            ...
            horizon_clock_features=self.horizon_clock_features.to(device, non_blocking=non_blocking),
...
def collate_examples(examples: Sequence[BarGPTExample], *, balance_activity_regimes: bool = True) -> BarGPTBatch:
    ...
    ar_gap_labels: dict[str, torch.Tensor] = {}
    ar_gap_masks: dict[str, torch.Tensor] = {}
    for name, values in raw_by_view.items():
        ...
        gap_labels = [
            next_event_gap_class_labels(
                example.raw_view_start_us[name],
                timeframe_us=TIMEFRAME_US_BY_NAME[name],
                bar_mask=example.raw_view_mask[name],
            )
            for example in examples
        ]
        ar_gap_labels[name] = _pad_first_dimension(gap_labels)
        ar_gap_masks[name] = ar_masks[name].any(dim=-1)
    ...
    return BarGPTBatch(
        views=views,
        view_mask=view_mask,
        autoregressive_gap_labels=ar_gap_labels,
        autoregressive_gap_mask=ar_gap_masks,
        ...
        horizon_clock_features=build_target_clock_features(
            origin_timestamps_us,
            examples[0].horizons_us,
        ),
