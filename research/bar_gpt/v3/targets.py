...
NEXT_EVENT_GAP_CLASS_NAMES: tuple[str, ...] = (
    "one_interval",
    "two_intervals",
    "three_to_five_intervals",
    "six_to_thirty_intervals",
    "more_than_thirty_intervals",
    "cross_session",
)
NEXT_EVENT_GAP_CLASS_COUNT = len(NEXT_EVENT_GAP_CLASS_NAMES)
...
def next_event_gap_class_labels(
    bar_start_us: torch.Tensor,
    *,
    timeframe_us: int,
    bar_mask: torch.Tensor | None = None,
) -> torch.Tensor:
    """Classify the causal wall-clock gap to each next stored nonempty bar.

    The final class is based on an exact New York local-date transition.  The
    remaining classes describe integral multiples of the view timeframe.
    Labels are derived during CPU collation from timestamps already present in
    v12 shards, so v3 does not require a shard rebuild.
    """
    if bar_start_us.ndim != 1:
        raise ValueError("next-event timestamps must be one-dimensional")
    if int(timeframe_us) <= 0:
        raise ValueError("next-event timeframe must be positive")
    if bar_start_us.numel() < 2:
        return torch.empty(0, dtype=torch.long, device=bar_start_us.device)
    if bar_mask is not None and bar_mask.shape != bar_start_us.shape:
        raise ValueError("next-event timestamp mask must align with timestamps")
    valid_transition = (
        torch.ones(bar_start_us.numel() - 1, dtype=torch.bool, device=bar_start_us.device)
        if bar_mask is None
        else bar_mask[:-1] & bar_mask[1:]
    )
    if torch.any((bar_start_us[1:] <= bar_start_us[:-1]) & valid_transition):
        raise ValueError("next-event timestamps must be strictly increasing")

    starts = bar_start_us.detach().to(device="cpu", dtype=torch.long)
    valid_cpu = valid_transition.detach().to(device="cpu")
    gap = torch.where(
        valid_cpu,
        starts[1:] - starts[:-1],
        torch.full_like(starts[:-1], int(timeframe_us)),
    )
    multiple = torch.div(
        gap + int(timeframe_us) - 1,
        int(timeframe_us),
        rounding_mode="floor",
    )
    labels = torch.full_like(multiple, 4)
    labels = torch.where(multiple <= 30, torch.full_like(labels, 3), labels)
    labels = torch.where(multiple <= 5, torch.full_like(labels, 2), labels)
    labels = torch.where(multiple <= 2, torch.full_like(labels, 1), labels)
    labels = torch.where(multiple <= 1, torch.zeros_like(labels), labels)

    seconds = torch.div(starts, 1_000_000, rounding_mode="floor")
    utc_hours = torch.div(seconds, 3_600, rounding_mode="floor")
    unique_hours, inverse = torch.unique(utc_hours, sorted=True, return_inverse=True)
    offsets = []
    for hour in unique_hours.tolist():
        instant = dt.datetime.fromtimestamp(int(hour) * 3_600, tz=dt.timezone.utc)
        offset = instant.astimezone(_NEW_YORK).utcoffset()
        offsets.append(int(offset.total_seconds()) if offset is not None else 0)
    local_days = torch.div(
        seconds + torch.as_tensor(offsets, dtype=seconds.dtype)[inverse],
        86_400,
        rounding_mode="floor",
    )
    cross_session = local_days[1:] != local_days[:-1]
    labels[cross_session & valid_cpu] = NEXT_EVENT_GAP_CLASS_COUNT - 1
    return labels.to(device=bar_start_us.device)
