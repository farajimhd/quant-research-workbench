...
from research.bar_gpt.v3.run_train import default_argv
from research.bar_gpt.v3.train import main as train_main


DEFAULT_OUTPUT_ROOT = Path(r"D:\TradingML\runtimes\bar_gpt\v3\full_training")
DEFAULT_V2_VALIDATION_MANIFEST = Path(
    r"D:\TradingML\runtimes\bar_gpt\v2\full_training\full_catalog_chunks_v2.json"
)
DEFAULT_V2_INITIALIZATION_CHECKPOINT = Path(
    r"D:\TradingML\runtimes\bar_gpt\v2\full_training\bar-gpt-v2-full-medium-chunks30m-epoch10-chunkepochs20-chunkcosine-decay95-micro10-accum4-bucket16-production\checkpoints\checkpoint_latest.pt"
)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train warm-started BarGPT v3 over every eligible 2019-2025 shard block using "
    ...
    parser.add_argument(
        "--manifest",
        default=str(DEFAULT_V2_VALIDATION_MANIFEST),
        help="reuse the immutable v2 full-catalog manifest by default",
    )
    parser.add_argument(
        "--initialize-from-v2-checkpoint",
        default=str(DEFAULT_V2_INITIALIZATION_CHECKPOINT),
        help="required for a new run; ignored when the named v3 run already has a resumable checkpoint",
    )
    parser.add_argument(
...
def ensure_full_training_manifest(args: argparse.Namespace) -> Path:
    ...
    if output.is_file():
        ...
        if output == DEFAULT_V2_VALIDATION_MANIFEST:
            raise FileNotFoundError(
                "the immutable v2 full-catalog manifest is unavailable; "
                "do not silently rebuild the v3 validation authority"
            )
        print(
...
def trainer_argv(args: argparse.Namespace, *, resolved_manifest: Path) -> list[str]:
    ...
    run_name = (
        f"bar-gpt-v3-full-{args.model_size}-chunks{int(args.chunk_target_origins) // 1_000_000}m-"
        f"epoch{args.epochs}-"
        f"{minimum_tag}"
    ...
    elif args.initialize_from_v2_checkpoint:
        argv.extend(("--initialize-from-v2-checkpoint", str(args.initialize_from_v2_checkpoint)))
    else:
        raise ValueError(
            "a new v3 full-training run requires --initialize-from-v2-checkpoint"
        )
    return argv
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    print(
        ...
        + " ".join(
            shlex.quote(item)
            for item in (sys.executable, "-B", "-m", "research.bar_gpt.v3.train", *preview_args)
        ),
        flush=True,
    )
