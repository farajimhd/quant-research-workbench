...
from research.bar_gpt.v3.schema import (  # noqa: E402
    FEATURE_SPECS,
    FEATURE_VERSION,
    ONE_SECOND_US,
    SCHEMA_VERSION,
...
from research.bar_gpt.v3.cohort import (  # noqa: E402
    BAR_GPT_COHORT_2TB_MANIFEST_TABLE,
    BAR_GPT_COHORT_2TB_TABLE,
)
from research.bar_gpt.v3.trade_correction_overlay import (  # noqa: E402
...
DEFAULT_RUNTIME_ROOT = Path(r"D:\TradingML\runtimes\bar_gpt\v3\build_1s")
...
WITH
    ...
    (SELECT groupArray(toUInt8(token_id)) FROM {quote_ident(args.database)}.{quote_ident(condition_reference_table)} WHERE source_family = 'trade_conditions' AND is_join_canonical = 1 AND modifier_int = 12) AS trade_model_ineligible_tokens,
    (SELECT groupArray(toUInt8(token_id)) FROM {quote_ident(args.database)}.{quote_ident(condition_reference_table)} WHERE source_family = 'quote_conditions' AND is_join_canonical = 1 AND modifier_int IN (-1, 12, 15, 19, 20, 80, 83, 84)) AS quote_origin_ineligible_tokens,
