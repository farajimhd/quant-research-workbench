...
from research.bar_gpt.v3.analyze_return_classes import _rows
from research.bar_gpt.v3.config import TrainConfig
from research.bar_gpt.v3.metrics import multiclass_scores
...
class V3LearningContractTest(unittest.TestCase):
    def test_checkpoint_contract_fails_closed_across_versions(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "checkpoint version mismatch"):
            assert_checkpoint_version({"model_family": "bar_gpt", "model_version": "v3"})
        with self.assertRaisesRegex(RuntimeError, "checkpoint version mismatch"):
            assert_checkpoint_version({})
    ...
    def test_train_config_has_no_v1_loss_coefficients(self) -> None:
        ...
        self.assertEqual(config.global_validation_interval_samples, 500_000_000)
        for removed in (
