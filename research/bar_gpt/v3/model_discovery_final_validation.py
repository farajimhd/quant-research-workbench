...
from research.bar_gpt.v3.offline_shards import verify_shard_catalog_lock
...
def _write_consolidated_summary(output_root: Path, summaries: list[dict[str, Any]]) -> None:
    ...
    ordered = sorted(
        ...
        key=lambda item: (
            ...
            -close_mcc(item),
            -float(item.get("final_validation_ar_close_return_class_summary/mcc_macro", float("-inf"))),
        ),
    )
    _atomic_json(
    ...
    columns = (
        ...
        "final_validation_trade_summary/mae_bps_macro",
        "final_validation_close_return_class_summary/balanced_accuracy_macro",
        "final_validation_close_return_class_summary/mcc_macro",
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if campaign_state.get("learning_contract") != LEARNING_CONTRACT:
        raise RuntimeError("final validation requires the active v3 learning contract")
