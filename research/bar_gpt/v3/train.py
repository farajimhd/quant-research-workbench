...
from research.bar_gpt.v3 import (
    LEARNING_CONTRACT,
    MODEL_FAMILY,
    MODEL_VERSION,
...
from research.bar_gpt.v3.data import AUTOREGRESSIVE_VIEW_NAMES, PATHWAY_ID_BY_NAME, TIMEFRAME_US_BY_NAME, BarGPTBatch, BarGPTExample, BarView, collate_examples
from research.bar_gpt.v3.loader import (
    ArrowStreamClient,
...
from research.bar_gpt.v3.model_discovery import load_discovery_manifest, panel_refs
from research.bar_gpt.v3.prefetch import DeviceBatchPrefetcher
from research.bar_gpt.v3.sampling import CoverageCursor, coverage_plan_summary
from research.bar_gpt.v3.model import BarGPTV3, build_model_mermaid
...
from research.bar_gpt.v3.objectives import BarGPTLoss, compute_loss
from research.bar_gpt.v3.progress import TrainingProgressState, TrainingReporter
...
from research.mlops.checkpoints import (
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Warm-start BarGPT v3 from v2 on the certified v12 offline shards.")
    ...
    parser.add_argument(
        "--global-validation-interval-samples",
        type=int,
        default=train.global_validation_interval_samples,
        help="save an immutable checkpoint and run full fixed validation on this sample clock",
    )
    parser.add_argument(
        "--full-chunk-training",
        action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument(
        ...
        default="",
        help="warm-start a new v3 run from an immutable v2 checkpoint; optimizer state is not copied",
    )
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    ...
    train = TrainConfig(
        ...
        global_validation_interval_samples=int(args.global_validation_interval_samples),
...
def _chunk_advance_decision(
    *,
    ...
    if not 1 <= minimum_repetitions <= maximum_repetitions:
        raise ValueError("invalid chunk repetition bounds")
...
def _forward(
    ...
    *,
    ...
    output = model(
        ...
        target_clock_features=batch.horizon_clock_features,
    )
...
def _wandb_metric_key(key: str) -> str:
    ...
    train_groups = {
        ...
        "chunk_cosine_cycle": "train_optimization/chunk_cosine_cycle",
...
def checkpoint_payload(
    ...
    *,
    ...
    last_global_validation_samples: int = -1,
    ...
    return {
        ...
        "last_global_validation_samples": last_global_validation_samples,
...
def _checkpoint_policy(config: TrainConfig) -> CheckpointPolicy:
    return CheckpointPolicy(
        ...
        # Global sample-clock checkpoints own promotion through endpoint skill
        # and non-regression gates. Never promote on composite total loss.
        save_best_val=False,
...
def _global_validation_checkpoint_name(samples_seen: int) -> str:
    if samples_seen <= 0:
        raise ValueError("global-validation checkpoint requires positive origins")
    return f"checkpoint_global_validation_origins_{samples_seen:012d}.pt"


def _next_global_validation_boundary(
    last_successful_samples: int,
    interval_samples: int,
) -> int:
    if last_successful_samples < -1 or interval_samples <= 0:
        raise ValueError("invalid global-validation sample clock")
    return (
        (max(0, last_successful_samples) // interval_samples) + 1
    ) * interval_samples


def _global_checkpoint_quality(metrics: Mapping[str, float]) -> dict[str, float]:
    names = {
        "trade_close_mae_bps": "global_validation_trade_close_summary/mae_bps_macro",
        "trade_range_mae_bps": "global_validation_trade_range_summary/mae_bps_macro",
        "close_mcc": "global_validation_close_return_class_summary/mcc_macro",
        "trade_calibration": "global_validation_trade_summary/calibration_macro",
    }
    quality = {name: float(metrics[key]) for name, key in names.items()}
    if not all(math.isfinite(value) for value in quality.values()):
        raise ValueError("global checkpoint quality metrics must all be finite")
    return quality


def _global_checkpoint_promotes(
    candidate: Mapping[str, float],
    incumbent: Mapping[str, float] | None,
) -> bool:
    """Use close MAE as primary score with strict non-regression gates."""
    if incumbent is None:
        return True
    return bool(
        candidate["trade_close_mae_bps"] < incumbent["trade_close_mae_bps"]
        and candidate["close_mcc"] >= incumbent["close_mcc"]
        and candidate["trade_range_mae_bps"] <= incumbent["trade_range_mae_bps"]
        and candidate["trade_calibration"] <= incumbent["trade_calibration"]
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.resume_checkpoint and args.initialize_from_v2_checkpoint:
        raise ValueError("choose v3 resume or v2 initialization, not both")
    if not args.dummy_data and not args.resume_checkpoint and not args.initialize_from_v2_checkpoint:
        raise ValueError(
            "a new v3 run requires --initialize-from-v2-checkpoint; "
            "random production initialization is forbidden"
        )
    run_name = args.run_name or f"bar-gpt-v3-{time.strftime('%Y%m%d-%H%M%S')}"
    ...
    model: torch.nn.Module = BarGPTV3(config.model).to(device)
    migration_report: dict[str, Any] | None = None
    if args.initialize_from_v2_checkpoint:
        v2_source = Path(args.initialize_from_v2_checkpoint)
        v2_snapshot = snapshot_v2_checkpoint(
            v2_source,
            paths.checkpoints_dir / "checkpoint_v2_initialization_source.pt",
        )
        migration_report = load_v2_weights(
            model,
            v2_snapshot,
            map_location=device,
        )
        migration_report["original_source_checkpoint"] = str(v2_source.resolve())
        migration_report["source_snapshot_method"] = "same_volume_hard_link"
        (paths.artifacts_dir / "v2_to_v3_migration.json").write_text(
    ...
    def model_artifact_dummy() -> tuple[tuple[Any, ...], dict[str, Any]]:
        ...
        return (), {
            ...
            "target_clock_features": torch.zeros(
                batch,
                1,
                len(config.data.horizons_us),
                8,
                device=device,
            ),
        }
    ...
    global_diagnostic_caches: dict[str, ReusableValidationBatches] = {}
    if discovery_manifest is not None:
        validation_workers = min(
            DISCOVERY_VALIDATION_WORKERS,
            int(config.data.loader_workers),
        ...
        validation_refs = panel_refs(discovery_manifest, "validation")
        diagnostic_groups: dict[str, tuple[OfflineBlockRef, ...]] = {
            "liquidity_low": tuple(
                ref for ref in validation_refs if ref.activity_regime == 0
            ),
            "liquidity_moderate": tuple(
                ref for ref in validation_refs if ref.activity_regime == 1
            ),
            "liquidity_high": tuple(
                ref for ref in validation_refs if ref.activity_regime == 2
            ),
            **{
                f"session_{phase}": tuple(
                    ref for ref in validation_refs if ref.session_phase == phase
                )
                for phase in (
                    "premarket",
                    "regular_open",
            ...
            "unusual_conditions": tuple(
                ref for ref in validation_refs if ref.has_condition_target
            ),
        }
        global_diagnostic_caches = {
            name: _offline_reference_evaluation_cache(
                config,
                units=offline_validation_units,
                refs=refs,
                seed=config.train.seed,
            )
            for name, refs in diagnostic_groups.items()
            if refs
        }
    ...
    write_run_manifest(
        ...
        config={
            **to_dict(config),
            ...
            "global_validation_interval_samples": config.train.global_validation_interval_samples,
            "v2_to_v3_migration": migration_report,
            "global_diagnostic_panels": sorted(global_diagnostic_caches),
            "global_diagnostic_grouping_contract": {
                "liquidity": "frozen validation ref activity_regime 0/1/2",
                "session": "frozen validation ref session_phase premarket/regular_open/regular_midday/regular_close/after_hours",
                "unusual_conditions": "frozen validation ref has_condition_target",
                "price_and_market_cap": "requires separate authoritative point-in-time metadata manifest",
            },
        ...
        source_checkpoint=(
            ...
            else Path(args.initialize_from_v2_checkpoint)
            if args.initialize_from_v2_checkpoint
            else None
        ),
        wandb_info={
    ...
    last_global_validation_samples = int(
        restored.get("last_global_validation_samples", -1)
    )
    # Drive the next boundary from the last *successful* global evaluation,
    # not merely from training progress. A failed evaluation is retried after
    # resume before training advances past its missing authority point.
    next_global_validation = _next_global_validation_boundary(
        last_global_validation_samples,
        config.train.global_validation_interval_samples,
    )
    ...
    def run_global_validation() -> None:
        """Checkpoint and evaluate the complete frozen panel on the 500M clock."""
        nonlocal last_global_validation_samples, next_global_validation
        nonlocal validation_evaluations_completed, last_val
        boundary = next_global_validation
        checkpoint_name = _global_validation_checkpoint_name(samples_seen)
        ...
        if checkpoint_path.exists():
            ...
            assert_checkpoint_version(existing)
            if (
                int(existing.get("samples_seen", -1)) != samples_seen
                or str(existing.get("plan_hash", "")) != plan_hash
            ):
                raise RuntimeError(
                    "existing immutable global-validation checkpoint does not "
                    "match the pending sample/coverage state"
                )
            del existing
            reporter.message(
                f"Reusing immutable checkpoint for retried global validation: {checkpoint_name}"
            )
        else:
            reporter.message(
                "Saving immutable global-validation checkpoint "
                f"at {samples_seen:,} origins for boundary {boundary:,}"
            )
            snapshot_cursors = dict(durable_cursors)
            ...
            atomic_torch_save(to_cpu_payload(payload), checkpoint_path)
        checkpoint_sha256 = _file_sha256(checkpoint_path)

        reporter.phase("validating_global")
        global_metrics = validate(
            model,
            full_validation_cache,
            config,
            device,
            namespace="global_validation",
            max_batches=None,
            max_origins=(
                None
                if discovery_manifest is not None
                else config.train.monitor_evaluation_origins
            ),
        )
        global_metrics.update(
            {
                "global_validation/checkpoint_samples": float(samples_seen),
                "global_validation/target_boundary_samples": float(boundary),
            }
        )
        all_global_metrics = dict(global_metrics)
        for diagnostic_name, diagnostic_cache in sorted(
            global_diagnostic_caches.items()
        ):
            reporter.message(
                f"Running fixed global diagnostic panel: {diagnostic_name}"
            )
            diagnostic_metrics = validate(
                model,
                diagnostic_cache,
                config,
                device,
                namespace=f"global_diagnostic_{diagnostic_name}",
                max_batches=None,
                max_origins=None,
            )
            all_global_metrics.update(diagnostic_metrics)
        quality = _global_checkpoint_quality(global_metrics)
        selection_path = paths.artifacts_dir / "global_checkpoint_selection.json"
        incumbent_record: dict[str, Any] | None = None
        if selection_path.is_file():
            incumbent_record = json.loads(selection_path.read_text(encoding="utf-8"))
            if not isinstance(incumbent_record.get("quality"), dict):
                raise RuntimeError("global checkpoint selection artifact is malformed")
        promotes = _global_checkpoint_promotes(
            quality,
            incumbent_record["quality"] if incumbent_record is not None else None,
        )
        global_metrics["global_validation/checkpoint_promoted"] = float(promotes)
        all_global_metrics["global_validation/checkpoint_promoted"] = float(promotes)
        if promotes:
            best_path = paths.checkpoints_dir / "checkpoint_best_global.pt"
            temporary_best = best_path.with_suffix(
                best_path.suffix + f".tmp.{os.getpid()}.{time.time_ns()}"
            )
            os.link(checkpoint_path, temporary_best)
            os.replace(temporary_best, best_path)
            selection = {
                "selection_contract": "endpoint_pareto_non_regression_v1",
                "checkpoint": str(checkpoint_path),
                "checkpoint_sha256": checkpoint_sha256,
                "samples_seen": samples_seen,
                "quality": quality,
                "gates": {
                    "primary": "strictly lower trade close MAE",
                    "non_regression": [
                        "close MCC",
                        "trade high/low range MAE",
                        "trade quantile calibration",
                    ],
                },
                "promoted_at": dt.datetime.now(dt.timezone.utc).isoformat(),
            }
            temporary_selection = selection_path.with_suffix(
                selection_path.suffix + f".tmp.{os.getpid()}"
            )
        ...
        metrics_logger.log(all_global_metrics, samples_seen)
        ...
        record = {
            ...
            "learning_contract": LEARNING_CONTRACT,
            "samples_seen": samples_seen,
            "target_boundary_samples": boundary,
            "checkpoint": str(checkpoint_path),
            "checkpoint_sha256": checkpoint_sha256,
            "experiment_manifest": str(args.experiment_manifest),
            "experiment_manifest_hash": (
                str(discovery_manifest.get("manifest_hash", ""))
                if discovery_manifest is not None
                else ""
            ),
        ...
        with (paths.artifacts_dir / "global_validation_runs.jsonl").open(
            "a", encoding="utf-8"
        ) as handle:
            handle.write(json.dumps(record, sort_keys=True) + "\n")
        reporter.validation(global_metrics)
        last_val = global_metrics
        last_global_validation_samples = samples_seen
        validation_evaluations_completed += 1
        while next_global_validation <= samples_seen:
            next_global_validation += config.train.global_validation_interval_samples
        reporter.message(
            f"Global validation is reproducible from {checkpoint_name} "
    ...
    def evaluate_completed_chunk_epoch(active_chunk: Any) -> bool:
        ...
        control_metrics = {
            ...
            "chunk_control/minimum_repetitions": float(
                config.train.min_chunk_epochs
            ),
            "chunk_control/minimum_repetitions_completed": float(
                completed_repetitions >= config.train.min_chunk_epochs
            ),
    ...
    try:
                    ...
                    if full_update or partial_final_update:
                            ...
                            update_metadata.update(
                                {
                                    ...
                                    "train/chunk_cosine_cycle": float(
                                        scheduler.chunk_cycle_index + 1
                                    ),
                                }
                            )
                        ...
                        if chunk_due:
                            ...
                            if not exhausted:
                                ...
                                active_iterator = iterator
                            checkpoint_after_validation()
                        if samples_seen >= next_global_validation:
        ...
        try:
            ...
            for diagnostic_cache in global_diagnostic_caches.values():
                diagnostic_cache.close()
