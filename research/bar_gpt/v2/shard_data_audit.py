...
from research.bar_gpt.v2.data import AUTOREGRESSIVE_VIEW_NAMES, collate_examples
from research.bar_gpt.v2.direct_event_shards import DirectEventShardDataset
...
def selected_predictions(
    ...
    *,
    ...
    model = BarGPTV2(model_config).to(resolved_device).eval()
    ...
    assert output.horizon_quantiles is not None and output.horizon_return_class_logits is not None
    ...
    for horizon_index, horizon_us in enumerate(data_config.horizons_us):
        ...
        for target_index, target_name in enumerate(DIRECTION_TARGET_NAMES):
            ...
            logits = output.horizon_return_class_logits[0, origin_offset, horizon_index, target_index].double().cpu()
            ...
            row.update({
                ...
                **{
                    f"{target_name}_probability_{class_name}": float(probabilities[class_index])
                    for class_index, class_name in enumerate(RETURN_CLASS_NAMES)
                },
                f"{target_name}_predicted_class": RETURN_CLASS_NAMES[int(logits.argmax())],
            })
        rows.append(row)
    return rows
