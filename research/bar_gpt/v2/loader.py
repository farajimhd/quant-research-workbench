...
from research.bar_gpt.v2.corporate_actions import (
    SplitAction,
...
from research.bar_gpt.v2.features import project_stationary_features
from research.bar_gpt.v2.schema import FEATURE_INDEX, FEATURE_NAMES, SESSION_END_SECOND, SESSION_START_SECOND, SESSION_TIMEZONE
from research.bar_gpt.v2.sampling import CoverageCursor, has_condition_target, select_stratified_examples, session_phase
from research.bar_gpt.v2.targets import build_physical_horizon_targets
...
def prepare_session_context(
    *,
    ...
    calendar_views = _calendar_views(
        ...
        max_daily_rows=int(config.calendar_warmup_daily_bars),
        anchor_us=session_anchor,
...
def _calendar_views(
    ...
    *,
    ...
    filtered_dates = [value for value in dates if value not in excluded and value < cutoff_local_date]
    filtered_dates = filtered_dates[-max(1, int(max_daily_rows)):]
    keep = torch.as_tensor(
        [value in set(filtered_dates) for value in dates], dtype=torch.bool, device=daily_view.features.device
    )
        ...
        def raw_examples() -> Iterator[BarGPTExample]:
            ...
            for unit_index, unit in units:
                ...
                warmup_days = max(
                    14,
                    math.ceil(
                        self.data_config.intraday_warmup_bars_1s
                ...
                for chunk_index in range(0, len(schedule), fetch_blocks):
                    ...
                    for window, (session, prior_session) in zip(windows, fetched, strict=True):
                        target_start = max(
                            SESSION_START_SECOND,
                            int(window.origin_bucket) - int(self.data_config.intraday_warmup_bars_1s),
                        )
                        target_end = (
                        ...
                        built = list(
                            build_session_examples(
                                ...
                                split_actions=split_actions,
                                config=self.data_config,
                                origin_start_index=max(0, int(window.origin_bucket) - target_start),
                            )
                        )
                        if len(built) > 1:
