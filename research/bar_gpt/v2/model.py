...
from research.bar_gpt.v2.config import BarGPTConfig
from research.bar_gpt.v2.data import AUTOREGRESSIVE_VIEW_NAMES
...
class BarGPTOutput:
    ...
    autoregressive_return_class_logits: dict[str, torch.Tensor]
    horizon_quantiles: torch.Tensor | None
    horizon_availability_logits: torch.Tensor | None
    horizon_return_class_logits: torch.Tensor | None
...
class BarGPTV2(nn.Module):
    ...
    def __init__(self, config: BarGPTConfig) -> None:
        ...
        self.autoregressive_return_class_head = nn.Linear(
            config.d_model, RETURN_TARGET_COUNT * RETURN_CLASS_COUNT, bias=True
        )
        ...
        self.horizon_return_class_head = nn.Linear(
            config.horizon_rank, RETURN_TARGET_COUNT * RETURN_CLASS_COUNT, bias=True
        )
    ...
    def forward(
        ...
        *,
        ...
        autoregressive_weight = torch.cat((
            ...
            self.autoregressive_return_class_head.weight,
        ), dim=0)
        availability_bias = self.autoregressive_availability_head.bias
        return_class_bias = self.autoregressive_return_class_head.bias
        if availability_bias is None or return_class_bias is None:
        ...
        for name in AUTOREGRESSIVE_VIEW_NAMES:
            ...
            continuous, availability, return_classes = _fused_linear_group(
                state[:, :-1],
                self.autoregressive_continuous_head,
                self.autoregressive_availability_head,
                self.autoregressive_return_class_head,
            ...
            autoregressive_return_class_logits[name] = return_classes.view(
                *return_classes.shape[:-1], RETURN_TARGET_COUNT, RETURN_CLASS_COUNT
            )
        quantiles = None
        availability_logits = None
        return_class_logits = None
        if horizon_ids is not None:
            ...
            raw, availability_logits, return_class_logits = _fused_linear_group(
                conditioned,
                self.horizon_head,
                self.horizon_availability_head,
                self.horizon_return_class_head,
            )
        ...
        return BarGPTOutput(
            ...
            autoregressive_return_class_logits=autoregressive_return_class_logits,
            horizon_quantiles=quantiles,
            horizon_availability_logits=availability_logits,
            horizon_return_class_logits=(
                None
                if return_class_logits is None
                else return_class_logits.view(
                    *return_class_logits.shape[:-1], RETURN_TARGET_COUNT, RETURN_CLASS_COUNT
                )
            ),
        )
