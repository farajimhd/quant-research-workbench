...
from research.bar_gpt.v2.audit_offline_shards import audit_shard
...
from research.bar_gpt.v2.data import FixedBucketHistoryCache, PATHWAY_ID_BY_NAME, TIMEFRAME_US_BY_NAME, BarView, collate_examples
...
from research.bar_gpt.v2.sampling import CoverageCursor, SESSION_PHASES, coverage_plan_summary, select_stratified_examples
from research.bar_gpt.v2.shard_data_audit import (
    AuditBlockRef,
    LoadedAuditSample,
...
from research.bar_gpt.v2.metrics import ValidationAccumulator, _direction_scores, multiclass_scores
...
from research.bar_gpt.v2.linear_probe import fit_ridge_probes
from research.bar_gpt.v2.objectives import (
    compute_loss,
    masked_huber_loss,
...
from research.bar_gpt.v2.lock_offline_shard_catalog import lock_catalog
from research.bar_gpt.v2.progress import TrainingProgressState, TrainingReporter, _format_value, _ratio_markup
...
from research.bar_gpt.v2.profile_train import (
...
from research.bar_gpt.v2.run_build_conditions_1s import default_argv as condition_builder_argv
from research.bar_gpt.v2.run_build_offline_dataset import (
    commands as offline_dataset_commands,
    parse_args as parse_offline_dataset_args,
    required_free_bytes,
)
from research.bar_gpt.v2.run_profile_train import DEFAULT_ARGS as profile_launcher_args
from research.bar_gpt.v2.run_pilot_offline_shards import commands as pilot_commands, parse_args as parse_pilot_args
from research.bar_gpt.v2.overfit_pilot import (
...
from research.bar_gpt.v2.run_profile_model_performance import parse_args as parse_performance_profile_args, profiler_argv
from research.bar_gpt.v2.run_profile_length_buckets import (
...
class LoaderTrainerContractTest(unittest.TestCase):
    ...
    def test_offline_controller_exception_is_durable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with patch(
                "research.bar_gpt.v2.offline_shards._run_main",
    ...
    def test_arrow_retry_discards_partial_attempt_before_exposing_rows(self) -> None:
        ...
        with patch("research.bar_gpt.v2.loader.request.urlopen", side_effect=responses), patch(
    ...
    def test_validation_plan_uses_bounded_sequential_blocks(self) -> None:
        ...
        with patch("research.bar_gpt.v2.loader.ArrowStreamClient", FakeClient):
    ...
    def test_pilot_launcher_builds_direct_shards_then_audits_automatically(self) -> None:
        ...
        self.assertIn("research.bar_gpt.v2.audit_offline_shards", audit)
        self.assertIn("--verify-sha256", audit)
        self.assertNotIn("--require-calendar-context", audit)
        self.assertIn("--verify-direct-source", audit)
        self.assertIn("research.bar_gpt.v2.run_audit_shard_data", sampled_audit)
    ...
    def test_complete_offline_dataset_launcher_builds_one_catalog_then_audits(self) -> None:
        ...
        self.assertIn(
            "research.bar_gpt.v2.run_audit_shard_data",
    ...
    def test_immutable_catalog_blocks_execute_before_build_diagnostics(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("research.bar_gpt.v2.offline_shards.BuildRunLog") as build_log:
    ...
    def test_sequential_coverage_explicitly_includes_holdout_and_derived_warmup(self) -> None:
        ...
        test_case = self
        with patch("research.bar_gpt.v2.train.ArrowStreamClient", FakeStream):
    ...
    def test_condition_builder_is_sparse_resumable_and_uses_runtime_authority(self) -> None:
        ...
        self.assertEqual(args[output_index], r"D:\TradingML\runtimes\bar_gpt\v2\build_conditions_1s")
    ...
    def test_collated_batch_runs_complete_mixed_objective(self) -> None:
        ...
        self.assertEqual(set(output.autoregressive_return_class_logits), set(output.autoregressive))
        self.assertEqual(
            output.horizon_return_class_logits.shape,
            (*batch.horizon_targets.shape[:-1], RETURN_TARGET_COUNT, RETURN_CLASS_COUNT),
        )
    ...
    def test_all_model_comparison_runs_fresh_processes_sequentially(self) -> None:
        ...
        with patch(
            "research.bar_gpt.v2.run_train_model_comparison.subprocess.run",
    ...
    def test_training_dashboard_keeps_a_fixed_complete_schema(self) -> None:
        ...
        state = TrainingProgressState(
            run_name="smoke", device="cuda", precision="bf16", output_dir=r"D:\TradingML\runtimes\bar_gpt\v2\train\smoke",
