...
| `ticker_intraday_trade_values` | `bar_inputs["ticker_intraday_bars"]["trade_values"]` | `float32 [B, H, 6]` | same-session completed backward trade bars ending before the origin bucket | optional price normalization to origin/recent event; optional `log1p`/standardization for size/count | intraday bar encoder |
| `ticker_intraday_quote_bid_values` | `bar_inputs["ticker_intraday_bars"]["quote_bid_values"]` | `float32 [B, H, 9]` | same-session completed backward bid bars ending before the origin bucket | optional price normalization to origin/recent event; optional size transforms | intraday bar encoder |
| `ticker_intraday_quote_ask_values` | `bar_inputs["ticker_intraday_bars"]["quote_ask_values"]` | `float32 [B, H, 9]` | same-session completed backward ask bars ending before the origin bucket | optional price normalization to origin/recent event; optional size transforms | intraday bar encoder |
...
| `ticker_intraday_bar_family_masks` | `trade_mask`, `quote_bid_mask`, `quote_ask_mask` | each `bool [B, H]` | true for available completed backward bars in each family | none | intraday bar attention mask |
