...
```powershell
python D:\TradingCodes\quant-research-workbench\pipelines\sec\edgar\sec_historical_feed_download.py
```

Bounded pipeline script:

```powershell
python D:\TradingCodes\quant-research-workbench\pipelines\sec\edgar\sec_historical_feed_pipeline.py
```
...
```powershell
python D:\TradingCodes\quant-research-workbench\pipelines\sec\edgar\sec_historical_feed_download.py --existing-extracted-dir C:\Users\g835l\Downloads\20260605.nc.tar\20260605.nc --existing-archive-date 2026-06-05 --header-concurrency 4 --sec-request-min-interval-seconds 0.11
```

Skip SEC header fetch if you only want to validate local `.nc` parsing:

```powershell
python D:\TradingCodes\quant-research-workbench\pipelines\sec\edgar\sec_historical_feed_download.py --existing-extracted-dir C:\Users\g835l\Downloads\20260605.nc.tar\20260605.nc --existing-archive-date 2026-06-05 --no-header-fetch
```
...
- `accepted_at_edgar_raw` is normalized to `accepted_at_utc`; compact 14-digit SGML values are interpreted in `America/New_York`, while explicit `Z` or offset-bearing values retain their declared timezone.
