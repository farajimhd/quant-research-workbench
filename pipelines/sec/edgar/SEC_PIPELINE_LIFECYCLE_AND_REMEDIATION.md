...
| An empty targeted parent set triggered the bulk parent lookup | Parallel identity-repair workers ran broad `sec_filing_v3 FINAL` scans and exhausted ClickHouse memory | Make parent resolution an explicit worker contract: normal rebuilds use `database_window`; targeted repairs use `supplied_only` and derive any missing parent from authoritative archive SGML without a database fallback |
| A completed worker removed shared Parquet directories | Another active repair worker failed opening its temporary shard after the shared directory disappeared | Worker cleanup deletes only its own durable files; the coordinator prunes empty directories after the process pool exits, and each writer reasserts its output directory before opening a shard |
| Entity integrity required SGML rows for metadata-only filings | Finalization failed even though every archive-backed accession had complete entity relationships | Enforce entity completeness only for accessions present in the parsed daily-archive inventory; report filings absent from that inventory separately as metadata-only |
| Source-text creation paths disagreed on physical partitioning | A fresh DDL or live-created table could use CIK hashing while the completed historical table and rebuild validator required monthly archive partitions | Define source-text partition and sorting keys once, render them into generic DDL and live table creation, reuse them in rebuild validation, and fail the integrity audit on physical-layout drift |
...
| A live document row was treated as accession completion | A crash after documents but before rendered text, skips, or XBRL made the partial accession permanently skippable | Make `sec_filing_live_ingest_manifest_v3` the only live completion authority and allow same-revision replay until its final transition |
| Supported source text has no rendered derivative | Earlier renderer policy classified structured fund XML as model-excluded even though complete source text was retained | Renderer v9 renders supported XML in live and historical paths; `sec_missing_render_repair.py` discovers the sparse source/render anti-join and repairs exact source revisions in bounded resumable units without rebuilding raw SEC data |
| Cached SEC payloads could predate a newly discovered accession | Missing exact acceptance metadata or Company Facts became permanent | Refresh once on accession miss, expire negative Company Facts cache entries, and retain a durable source retry time |
| Live schema bootstrap could clone v1/v2 layouts into v3 names | Fresh write databases could violate the canonical v3 contract | Remove legacy schema fallback and require an existing or explicit v3 schema authority |
