...
VERSION = "hindsight-greedy-fractional-v1"
...
def coefficients(frame: pl.DataFrame, gamma: float = 0.99,
    ...
    for side, sign in (("long", 1), ("short", -1)):
        ...
        available = ((pl.col(f"{side}_status") == "available")
                     ...
                     & (target > 0) & new_ok).fill_null(False)
        ...
        item = frame.select(
            ...
            pl.when(available).then(discount).alias("discount"),
            pl.when(available).then(sign * (target - entry)).alias("open_profit_per_share"),
            pl.when(available).then(sign * (target - close)).alias("hold_profit_per_share"),
...
class ActionTable:
    ...
    def __init__(self, rows: list[dict], positions: list[Position], *,
        ...
        self._held_missing = [key for key in self.positions
                              if not self.rows[key]["value_available"]]
    ...
    def evaluate(self, changes: Mapping[str, float]) -> dict:
        ...
        missing = [key for key, q in quantities.items()
                   if q > 1e-10 and not self.rows[key]["value_available"]]
        total = raw = None
        if feasible and not missing:
            ...
            for key, q in quantities.items():
                ...
                retained = q - added
                total += retained * row["hold_value_per_share"] + added * row["open_value_per_share"]
                raw += retained * row["hold_profit_per_share"] + added * row["open_profit_per_share"]
