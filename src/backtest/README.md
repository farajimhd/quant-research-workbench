...
the requested size is larger, the engine records a `PARTIAL` fill tag and marks
the remainder for strategy resubmission instead of keeping the original order
open. Strategy-generated `PARTIAL_ENTRY_REST` and `PARTIAL_EXIT_REST` orders are
treated as aggressive reprices and are not capped again. When partial fills are
disabled, crossing orders fill for the full requested quantity subject to cash
and normal order checks.
...
- `portfolio_candles.parquet`: mark-to-market P/L and equity OHLC candles plus
  open-unrealized and drawdown OHLC fields for `1h`, `2h`, `4h`, and `1d`.
  Candle timestamps are market-clock timestamps and API payloads must convert
  naive values as `America/New_York` so the chart x-axis remains in exchange
  time.
