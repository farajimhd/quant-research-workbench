...
from src.strategies.liquidity_pullback_reversal.v1.config import LiquidityPullbackReversalConfig
from src.strategies.liquidity_pullback_reversal.v1.presentation import chart_presentation
...
class LiquidityPullbackReversalStrategy:
    name = "liquidity_pullback_reversal"

    def __init__(self, config: LiquidityPullbackReversalConfig | None = None):
        self.config = config or LiquidityPullbackReversalConfig()
    ...
    def _update_states(self, context: BarContext) -> None:
        ...
        for raw in context.updates.iter_rows(named=True):
            ...
            if state is None:
                state = PullbackState(ticker=ticker, recent_closes=deque(maxlen=max_window))
            ...
            row.update(
                {
                    ...
                    "volume_z20_value": self._float(row.get("volume_z20")),
                }
            )
    ...
    def _entry_open(
        ...
        if ticker in set(self.config.excluded_symbols):
        ...
        if self._float(row.get("volume_z20_value")) < self.config.min_volume_z20:
            return False, "volume_z"
        if self._float(row.get("vwap_bps")) < self.config.min_vwap_bps:
            return False, "too_far_below_vwap"
        if self._float(row.get("vwap_bps")) > self.config.max_vwap_bps:
        ...
        if self._float(row.get("ret15_bps")) < self.config.min_ret15_bps:
            return False, "recent_damage"
        if self._float(row.get("ret15_bps")) > self.config.max_ret15_bps:
            return False, "recent_extended"
        if self._float(row.get("tema_spread_bps")) > self.config.max_weak_tema_spread_bps:
            return False, "tema_already_extended"
        if self._float(row.get("macd_hist_bps")) > self.config.max_weak_macd_hist_bps:
            return False, "macd_already_extended"
        if self._float(row.get("return_1_bps")) <= self.config.min_reversal_return_bps:
            return False, "not_green_reversal"
    ...
    def _entry_request(self, candidate: dict, context: BarContext, portfolio: Portfolio) -> OrderRequest | None:
        ...
        return OrderRequest(
            ...
            quantity=quantity,
            order_type="MARKET",
            reason="PULLBACK_REVERSAL",
            tag=(
                f"ENTRY|rule=LIQUIDITY_PULLBACK_REVERSAL|rank={candidate.get('entry_rank') or candidate.get('rank')}"
                ...
                f"|vwap={self._float(candidate.get('vwap_bps')):.1f}|macdd={self._float(candidate.get('macd_hist_delta_bps')):.2f}"
            ),
        )
    ...
    def _exit_requests(self, context: BarContext, portfolio: Portfolio) -> list[OrderRequest]:
        ...
        for symbol, position in list(portfolio.positions.items()):
            ...
            if self._float(enriched_bar.get("low")) <= trailing_stop:
                ...
                reason = "TIME_EXIT"
            elif held_minutes >= self.config.min_hold_minutes and self._reversal_failed(enriched_bar):
    ...
    def _score_row(self, row: dict) -> dict[str, float]:
        ...
        ret15_bps = self._float(row.get("ret15_bps"))
        ...
        liquidity_score = min(20.0, log1p(dollar_volume / max(self.config.min_dollar_volume_sma20, 1.0)) * 10.0)
        liquidity_score += min(8.0, relative_dollar_volume * 3.0)
        vwap_mid = (self.config.min_vwap_bps + self.config.max_vwap_bps) / 2.0
        vwap_width = max(1.0, (self.config.max_vwap_bps - self.config.min_vwap_bps) / 2.0)
        pullback_score = max(0.0, 20.0 * (1.0 - abs(vwap_bps - vwap_mid) / vwap_width))
        reversal_score = (close_location * 18.0) + min(10.0, return_1_bps / 1.5) + min(16.0, macd_delta * 5.0) + min(8.0, tema_delta * 2.0)
        day_score = 12.0 if self.config.min_day_return_bps <= day_return_bps <= self.config.max_day_return_bps else 0.0
        recent_score = 10.0 if self.config.min_ret15_bps <= ret15_bps <= self.config.max_ret15_bps else 0.0
        shock_score = max(0.0, min(6.0, self._float(row.get("price_volume_shock_score")) * 4.0))
        scanner_score = liquidity_score + pullback_score + reversal_score + day_score + recent_score + shock_score
        estimated_edge_bps = scanner_score * 0.78 - self.config.estimated_round_trip_cost_bps
        return {
    ...
    def _reversal_failed(self, bar: dict) -> bool:
        return (
            self._float(bar.get("return_1_bps")) <= -self.config.failure_return_bps
            and self._float(bar.get("macd_hist_delta_bps")) < 0
            and self._float(bar.get("vwap_bps")) < 0
        )
    ...
    def _record_scanner(
        ...
        if not self.observability or not rows:
            return
        self.observability.scanner(timestamp=context.timestamp, rows=rows, score_key="scanner_score", stage="pullback_reversal_scanner")
    ...
    def _trace_entry(
        ...
        self.observability.trace(
            ...
            reason_code="PULLBACK_REVERSAL",
            reason="Liquid pullback row passed reversal, edge, liquidity, and risk gates",
            values={
                "quantity": quantity,
                "price": self._float(candidate.get("close")),
                "stop": stop_price,
                ...
                "macd_hist_delta_bps": candidate.get("macd_hist_delta_bps"),
            },
    ...
    def _exit_tag(self, reason: str, position, bar: dict, meta: dict) -> str:
        ...
        return (
            ...
            f"|maxu={position.max_unrealized_profit:.2f}|macdd={self._float(bar.get('macd_hist_delta_bps')):.2f}"
        )
    ...
    def _lookback_return_bps(self, state: PullbackState, close: float, lookback: int) -> float:
