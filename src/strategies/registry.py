...
def create_orb_5m_momentum_v8(params: dict | None = None) -> OrbFiveMinuteMomentumV8Strategy:
    return OrbFiveMinuteMomentumV8Strategy(OrbMomentumV8Config.from_dict(params))


def default_orb_5m_momentum_v8_params() -> dict:
...
def create_long_momentum_v2(params: dict | None = None) -> LongMomentumV2Strategy:
    return LongMomentumV2Strategy(LongMomentumV2Config.from_dict(params))


def default_long_momentum_v2_params() -> dict:
...
def create_long_momentum_v4(params: dict | None = None) -> LongMomentumV4Strategy:
    return LongMomentumV4Strategy(LongMomentumV4Config.from_dict(params))


def default_long_momentum_v4_params() -> dict:
...
def create_long_momentum_v6(params: dict | None = None) -> LongMomentumV6Strategy:
    return LongMomentumV6Strategy(LongMomentumV6Config.from_dict(params))


def default_long_momentum_v6_params() -> dict:
...
def create_long_momentum_v9(params: dict | None = None) -> LongMomentumV9Strategy:
    return LongMomentumV9Strategy(LongMomentumV9Config.from_dict(params))


def default_long_momentum_v9_params() -> dict:
...
STRATEGY_VERSION_DESCRIPTIONS: dict[tuple[str, str], str] = {
    ...
    ("long_momentum", "v9"): (
        ...
        "cash, no-cash First Entries rotate out existing positions, and lower-priority VWAP/TEMA reentries remain "
