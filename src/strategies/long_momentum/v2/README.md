...
- `last_tema_open == true`
- `last_macd_line > 0` OR `current_open > last_vwap`. In v2, reclaiming or
  holding above VWAP can satisfy the MACD-line gate because it indicates price
  acceptance above the session VWAP even when the MACD line has not crossed
  positive yet.
