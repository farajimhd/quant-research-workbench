...
from src.strategies.long_momentum.v4.presentation import chart_presentation


@dataclass
class LongMomentumV4SymbolState:
    ticker: str
...
class LongMomentumV4Strategy:
    ...
    def __init__(self, config: LongMomentumV4Config | None = None):
        self.config = config or LongMomentumV4Config()
        self.session_date = None
        self.states: dict[str, LongMomentumV4SymbolState] = {}
    ...
    def _update_states(self, context: BarContext) -> None:
        for raw in context.updates.iter_rows(named=True):
            ...
            if state is None:
                state = LongMomentumV4SymbolState(ticker=ticker)
        ...
        for row in rows:
            ...
            if setup_open and state is not None and not pullback_reclaim_entry:
                ...
                if not active_setup or body_high > state.setup_body_high:
                    ...
                    state.setup_expires_bar = bar_index + max(1, int(self.config.pullback_reclaim_valid_bars))
    ...
    def _pullback_reclaim_entry_open(self, row: dict, state: LongMomentumV4SymbolState | None, bar_index: int) -> bool:
        ...
        if state.setup_low > 0 and current_open <= state.setup_low:
            return False
        return self._pullback_reclaim_filters_ok(row)

    def _pullback_reclaim_filters_ok(self, row: dict) -> bool:
        ...
        if self._float(row.get("last_transactions")) < self.config.min_transactions:
            return False
        if not bool(row.get("_lm_spread_ok")) or not bool(row.get("_lm_tema_open")):
            return False
    ...
    def _initial_stop_price(self, row: dict, entry_price: float) -> float:
        setup_stop_low = self._float(row.get("setup_stop_low"))
        if setup_stop_low > 0 and setup_stop_low < entry_price:
            return max(0.01, setup_stop_low)
    ...
    def _entry_block_reason(self, row: dict) -> str:
        ...
        if not bool(row.get("long_momentum_body_break_entry_open")) and not bool(row.get("long_momentum_pullback_reclaim_entry_open")):
    ...
    def _record_scanner(
        ...
        if not self.observability or not rows:
            return
        self.observability.scanner(timestamp=context.timestamp, rows=rows, score_key="scanner_score", stage="long_momentum_v4_scanner")
    ...
    def _trace_entry(self, timestamp: datetime, candidate: dict, quantity: int, entry_price: float, stop_price: float) -> None:
        ...
        self.observability.trace(
            ...
            reason_code="LONG_MOMENTUM_V4",
            reason="Eligible long momentum v4 scanner candidate",
            values={
                "quantity": quantity,
