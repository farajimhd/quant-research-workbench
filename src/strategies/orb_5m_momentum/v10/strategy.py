...
from src.strategies.orb_5m_momentum.v10.config import OrbMomentumConfig
from src.strategies.orb_5m_momentum.v10.presentation import chart_presentation
...
class OrbFiveMinuteMomentumV10Strategy:
    ...
    def _next_entry_request(
        ...
        while self.pending_candidates:
            ...
            if bar is not None and self._is_red_bar(bar):
                ...
                continue
            if bar is None or not self._macd_entry_filter_passes(bar):
                self._reject(
                    context.timestamp,
                    symbol,
                    "macd_spread_below_threshold",
                    {
                        **candidate,
                        "macd_line": bar.get("macd_line") if bar is not None else None,
                        "macd_signal": bar.get("macd_signal") if bar is not None else None,
                        "macd_signal_buffer": self.config.macd_signal_buffer,
                    },
                )
                continue
        ...
        self.rejection_events.append(
            {
                ...
                "macd_signal_buffer": setup.get("macd_signal_buffer"),
            }
        )
    ...
    def _macd_entry_filter_passes(self, bar: dict) -> bool:
        try:
            return (float(bar.get("macd_line")) - float(bar.get("macd_signal"))) > self.config.macd_signal_buffer
        except (TypeError, ValueError):
            return False
