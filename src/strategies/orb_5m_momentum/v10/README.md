...
This version starts from v9 and tightens the MACD entry confirmation. An entry
candidate is allowed only when the current completed `1m` MACD spread is greater
than the configured threshold:

```text
macd_line - macd_signal > macd_signal_buffer
```

The default `macd_signal_buffer` is `0.001`. Missing or invalid MACD values block
entry instead of passing the guard.
...
- skip the candidate unless `macd_line - macd_signal > macd_signal_buffer` on
  the completed `1m` entry-evaluation bar
...
the completed `1m` MACD spread as an extra entry guard and keeps the v8/v9
profit-pocketing behavior.
