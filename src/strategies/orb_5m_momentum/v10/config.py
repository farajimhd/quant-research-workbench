...
class OrbMomentumConfig:
    ...
    take_profit_pocket_pct: float = 0.01
    macd_signal_buffer: float = 0.001

    @classmethod
    def from_dict(cls, raw: dict[str, Any] | None) -> "OrbMomentumConfig":
        if raw is None:
            return cls()
        if "take_profit_reentry_pct" in raw and "take_profit_pocket_pct" not in raw:
            raw = {**raw, "take_profit_pocket_pct": raw["take_profit_reentry_pct"]}
