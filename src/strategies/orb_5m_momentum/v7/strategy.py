...
from src.strategies.orb_5m_momentum.v7.config import OrbMomentumConfig
from src.strategies.orb_5m_momentum.v7.presentation import chart_presentation
...
class OrbFiveMinuteMomentumV7Strategy:
    ...
    def _next_reentry_request(
        ...
        while self.pending_reentries:
            ...
            if self._is_red_bar(bar):
                ...
                self._trace_reentry_skip(
                    context.timestamp,
                    reentry,
                    "red_reentry_bar",
                    self._float_or_none(bar.get("tema9")),
                    self._float_or_none(reentry.get("pocket_tema9")),
                )
                continue
            self.pending_reentries.pop(0)
    ...
    def _trace_reentry_skip(
        ...
        if self.observability:
            reason = (
                "Profit reentry skipped because the next completed candle was red"
                if reason_code == "red_reentry_bar"
                else "Profit reentry skipped because 1m TEMA9 did not improve after pocketing profit"
            )
            self.observability.trace(
                timestamp=timestamp,
                ticker=symbol,
