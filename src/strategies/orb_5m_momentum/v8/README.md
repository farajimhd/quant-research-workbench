...
This version starts from v7 and changes only the profit-taking behavior. When a
position reaches the configured pocketing threshold, the strategy exits to lock
the profit and does not schedule an immediate same-size reentry. After that, the
normal scanner queue is responsible for selecting the next opportunity.
...
This version intentionally does not use 5-minute MACD/TEMA gates. It also
removes the v6/v7 immediate profit-reentry path.
