...
from src.strategies.orb_5m_momentum.v6.config import OrbMomentumConfig
from src.strategies.orb_5m_momentum.v6.presentation import chart_presentation
...
class OrbFiveMinuteMomentumV6Strategy:
    ...
    def __init__(self, config: OrbMomentumConfig | None = None):
        ...
        self.pending_reentries: list[dict] = []
    ...
    def on_bar(self, context: BarContext, portfolio: Portfolio, pending_orders: list[Order]) -> list[OrderRequest]:
        ...
        request = self._next_reentry_request(context, portfolio, pending_orders, exiting_symbols)
        if request is None:
    ...
    def _next_reentry_request(
        ...
        pending_symbols = {order.symbol for order in pending_orders if order.status == "OPEN"}
        while self.pending_reentries:
            reentry = self.pending_reentries[0]
            symbol = reentry["ticker"]
            if symbol in portfolio.positions or symbol in pending_symbols or symbol in exiting_symbols:
                return None
            bar = context.updates_by_symbol.get(symbol)
            if bar is None:
                return None
            self.pending_reentries.pop(0)
            if not self._passes_reentry_tema_confirmation(reentry, context, bar):
                continue
            return self._reentry_request(reentry, context, bar)
        return None

    def _passes_reentry_tema_confirmation(self, reentry: dict, context: BarContext, bar: dict) -> bool:
        current_tema9 = self._float_or_none(bar.get("tema9"))
        pocket_tema9 = self._float_or_none(reentry.get("pocket_tema9"))
        if current_tema9 is None or pocket_tema9 is None:
            self._trace_reentry_skip(context.timestamp, reentry, "tema9_missing", current_tema9, pocket_tema9)
            return False
        if current_tema9 <= pocket_tema9:
            self._trace_reentry_skip(context.timestamp, reentry, "tema9_not_higher", current_tema9, pocket_tema9)
            return False
        return True
    ...
    def _entry_request(self, candidate: dict, context: BarContext, portfolio: Portfolio) -> OrderRequest | None:
        ...
        self.signal_events.append(
            {
                ...
                "event": "ORB_ENTRY_STOP_SUBMITTED",
                "rank": candidate["rank"],
                "entry": entry,
                "stop": stop,
                "quantity": quantity,
        ...
        return OrderRequest(
            ...
            order_type="STOP",
            stop_price=entry,
            reason="ORB_STOP_ENTRY",
            tag=(
                f"ENTRY|type=STOP|rule=QC_ORB_SINGLE|rank={candidate['rank']}|qty={quantity}"
    ...
    def _reentry_request(self, reentry: dict, context: BarContext, bar: dict) -> OrderRequest:
        symbol = reentry["ticker"]
        quantity = int(reentry["quantity"])
        entry = float(bar["close"])
        stop = max(0.01, entry - float(reentry["risk_per_share"]))
        setup_rank = int(reentry.get("setup_rank") or 0)
        score = float(reentry.get("setup_score") or 0.0)
        self.active_symbol = symbol
        self.entry_order_metadata[symbol] = {
            "setup_rank": setup_rank,
            "live_rank": setup_rank,
            "setup_score": score,
            "live_score": score,
            "stop_price": stop,
        }
        self.signal_events.append(
            {
                "timestamp": context.timestamp,
                "ticker": symbol,
                "event": "PROFIT_REENTRY_SUBMITTED",
                "rank": setup_rank,
                "entry": entry,
                "stop": stop,
                "quantity": quantity,
                "pocket_tema9": reentry.get("pocket_tema9"),
                "reentry_tema9": bar.get("tema9"),
                "source_exit_time": reentry.get("source_exit_time"),
            }
        )
        if self.observability:
            self.observability.trace(
                ...
                ticker=symbol,
                stage="order_request",
                event_type="reentry_intent",
                decision="submit_order",
                reason_code="PROFIT_REENTRY",
                reason="Reentering after a prior profit pocket on the previous bar",
                values={
                    "quantity": quantity,
                    "entry": entry,
                    "stop": stop,
                    "prior_entry_price": reentry.get("prior_entry_price"),
                    "profit_exit_price": reentry.get("profit_exit_price"),
                    "pocket_tema9": reentry.get("pocket_tema9"),
                    "reentry_tema9": bar.get("tema9"),
                },
        ...
        return OrderRequest(
            ...
            quantity=quantity,
            order_type="MARKET",
            reason="PROFIT_REENTRY",
            tag=(
                f"ENTRY|type=MARKET|rule=PROFIT_REENTRY|rank={setup_rank}|qty={quantity}"
                f"|entry={entry:.2f}|stop={stop:.2f}|prior={float(reentry['profit_exit_price']):.2f}"
                f"|pocket_tema9={float(reentry.get('pocket_tema9') or 0.0):.4f}|tema9={float(bar.get('tema9') or 0.0):.4f}"
            ),
        )
    ...
    def _position_exit_requests(self, context: BarContext, portfolio: Portfolio) -> list[OrderRequest]:
        ...
        for symbol, position in list(portfolio.positions.items()):
            ...
            if bar is None:
                continue
            take_profit_price = position.entry_price * (1.0 + self.config.take_profit_reentry_pct)
            if (
                self.config.take_profit_reentry_pct > 0
                and context.timestamp > position.entry_time
                and float(bar["close"]) >= take_profit_price
            ):
                self._schedule_profit_reentry(context.timestamp, symbol, position, bar)
                self._trace_exit(context.timestamp, symbol, "PROFIT_REENTRY", position, bar)
                requests.append(
                    OrderRequest(
                        symbol=symbol,
                        side="SELL",
                        ...
                        order_type="MARKET",
                        reason="PROFIT_REENTRY",
                        tag=self._exit_tag("PROFIT_REENTRY", position, bar),
                    )
                )
                continue
    ...
    def _schedule_profit_reentry(self, timestamp: datetime, symbol: str, position, bar: dict) -> None:
        risk_per_share = max(0.01, abs(position.entry_price - position.stop_price))
        self.pending_reentries.append(
            {
                "ticker": symbol,
                "quantity": position.quantity,
                ...
                "prior_entry_price": position.entry_price,
                "profit_exit_price": float(bar["close"]),
                "pocket_tema9": self._float_or_none(bar.get("tema9")),
                "source_exit_time": timestamp.isoformat(),
            }
        )

    def _trace_reentry_skip(
        self,
        timestamp: datetime,
        reentry: dict,
        reason_code: str,
        current_tema9: float | None,
        pocket_tema9: float | None,
    ) -> None:
        symbol = str(reentry.get("ticker") or "")
        self.rejection_events.append(
            {
                "timestamp": timestamp,
                "ticker": symbol,
                "reject_reason": reason_code,
                "setup_rank": reentry.get("setup_rank"),
                "orb_relative_volume": reentry.get("setup_score"),
                "trigger": None,
                "stop": None,
                "current_tema9": current_tema9,
                "pocket_tema9": pocket_tema9,
            }
        )
        if self.observability:
            self.observability.trace(
                ...
                event_type="reentry_rejected",
                decision="skip",
                reason_code=reason_code,
                reason="Profit reentry skipped because 1m TEMA9 did not improve after pocketing profit",
                values={
                    "current_tema9": current_tema9,
                    "pocket_tema9": pocket_tema9,
                    "profit_exit_price": reentry.get("profit_exit_price"),
    ...
    def _trace_entry(self, timestamp: datetime, candidate: dict, quantity: int, entry: float, stop: float) -> None:
        ...
        self.observability.trace(
            ...
            decision="submit_order",
            reason_code="ORB_STOP_ENTRY",
            reason="Ranked QuantConnect ORB candidate submitted as a buy stop",
            values={
