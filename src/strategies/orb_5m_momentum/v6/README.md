...
This version starts from v5 and adds a 1-minute TEMA9 confirmation before
reentering after a profit-pocket exit.
...
- on the next candle for that ticker, reenter the same symbol with the same size
  only when 1-minute `tema9` is higher than it was on the profit-pocket bar
