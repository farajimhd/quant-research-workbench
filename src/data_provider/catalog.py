...
def session_feature_knowledge(lower: str, group: str, category: str, title: str) -> dict[str, Any]:
    details = {
        ...
        "session_bar_count": (
            "Number of bars observed so far in the session.",
        ...
        "day_dollar_volume_so_far": (
            "Cumulative session dollar volume.",
            "Day dollar volume so far sums close times volume from the beginning of the ticker/session through the current bar.",
            "$$DayDollarVolumeSoFar_t=\\sum_{i\\le t, session_i=session_t} Close_i\\cdot Volume_i$$",
            {"Close_i": "Close price for bar i", "Volume_i": "Share volume for bar i"},
        ),
        "prev_close": (
...
def price_action_knowledge(lower: str, group: str, category: str, title: str) -> dict[str, Any]:
    details = {
        ...
        "green_bar_count_so_far": ("Cumulative green candle count.", "Green bar count so far counts how many candles have closed above their open from the beginning of the ticker/session through the current bar.", "$$GreenBarsSoFar_t=\\sum_{i\\le t, session_i=session_t}I(Close_i>Open_i)$$"),
        "red_bar_count_so_far": ("Cumulative red candle count.", "Red bar count so far counts how many candles have closed below their open from the beginning of the ticker/session through the current bar.", "$$RedBarsSoFar_t=\\sum_{i\\le t, session_i=session_t}I(Close_i<Open_i)$$"),
        "green_bars_occurrence": ("Green candle occurrence ratio.", "Green bars occurrence divides green_bar_count_so_far by session_bar_count, so it shows what fraction of completed session bars have closed green so far.", "$$GreenBarsOccurrence_t=\\frac{GreenBarsSoFar_t}{SessionBarCount_t}$$"),
        "green_body_sum_so_far": ("Cumulative green candle body size.", "Green body sum so far adds absolute candle body size only for green candles in the same ticker/session up to the current bar.", "$$GreenBodySum_t=\\sum_{i\\le t}I(Close_i>Open_i)|Close_i-Open_i|$$"),
        "red_body_sum_so_far": ("Cumulative red candle body size.", "Red body sum so far adds absolute candle body size only for red candles in the same ticker/session up to the current bar.", "$$RedBodySum_t=\\sum_{i\\le t}I(Close_i<Open_i)|Close_i-Open_i|$$"),
        "green_body_avg": ("Average green candle body size.", "Green body average divides cumulative green body size by green_bar_count_so_far, so it measures the average body size of green candles seen so far in the ticker/session.", "$$GreenBodyAvg_t=\\frac{GreenBodySumSoFar_t}{GreenBarsSoFar_t}$$"),
        "red_body_avg": ("Average red candle body size.", "Red body average divides cumulative red body size by red_bar_count_so_far, so it measures the average body size of red candles seen so far in the ticker/session.", "$$RedBodyAvg_t=\\frac{RedBodySumSoFar_t}{RedBarsSoFar_t}$$"),
        "green_range_sum_so_far": ("Cumulative green candle range.", "Green range sum so far adds high-low range only for green candles in the same ticker/session up to the current bar.", "$$GreenRangeSum_t=\\sum_{i\\le t}I(Close_i>Open_i)(High_i-Low_i)$$"),
        "red_range_sum_so_far": ("Cumulative red candle range.", "Red range sum so far adds high-low range only for red candles in the same ticker/session up to the current bar.", "$$RedRangeSum_t=\\sum_{i\\le t}I(Close_i<Open_i)(High_i-Low_i)$$"),
        "net_body_sum_so_far": ("Cumulative signed candle body.", "Net body sum so far accumulates close minus open across the ticker/session. Positive values mean green body movement has dominated red body movement so far.", "$$NetBodySum_t=\\sum_{i\\le t}(Close_i-Open_i)$$"),
