...
def _query_plans() -> tuple[QueryPlanDefinition, ...]:
    ...
    return (
        ...
        QueryPlanDefinition(
            ...
            "canonical news ID plus stable provider article identity",
        ...
        QueryPlanDefinition(
            ...
            "canonical news ID and stable provider article identity",
        ...
        QueryPlanDefinition(
            ...
            "canonical News ID with stable provider article identity",
...
FIELD_OPERATOR_DOCUMENTATION: dict[str, dict[str, object]] = {
    ...
    "market.previous_close": {
        "source": "The completed prior session close from QMD, converted to current share units by the causal Reference Gateway stock-split publication when required.",
        "calculation": "Selects the final eligible trade price from the preceding completed session and applies split_from / split_to only when the split executes on the evaluated session.",
        "inputs": (),
        "timeframes": ("session",),
    },
    "market.change_pct": {
        "source": "QMD last price and the split-adjusted completed previous-session close available at the same market clock.",
        "calculation": "session_change.v1: ((last price / adjusted previous close) - 1) x 100. Missing, stale, or invalid price and corporate-action references fail closed.",
        "inputs": ("market.last_price", "market.previous_close"),
        "timeframes": ("1s", "10s", "30s", "1m"),
    },
    "market.change_actual": {
        "source": "QMD last price and the split-adjusted completed previous-session close available at the same market clock.",
        "calculation": "session_change.v1: last price minus adjusted previous-session close. Missing, stale, or invalid references fail closed.",
        "inputs": ("market.last_price", "market.previous_close"),
        "timeframes": ("1s", "10s", "30s", "1m"),
    },
    "market.volume": {
...
DISCOVERY_FIELD_PRESENTATIONS = (
    ...
    DiscoveryFieldPresentation("market.previous_close", "market.previous_close", "previous_close", "Previous close", "Split-adjusted completed prior-session close available at the scanner clock.", "reference", True, True, True, ("greater_or_equal", "greater_than", "less_or_equal", "less_than", "equals"), ("session",)),
    DiscoveryFieldPresentation("market.change_actual", "market.change_actual", "change_actual", "Session price change", "Last price minus the split-adjusted completed previous-session close.", "market_data", True, True, True, ("greater_or_equal", "greater_than", "less_or_equal", "less_than", "equals"), ("session",)),
    DiscoveryFieldPresentation("market.change_pct", "market.change_pct", "change_pct", "Session change %", "Percentage change from the split-adjusted completed previous-session close.", "market_data", True, True, True, ("greater_or_equal", "greater_than", "less_or_equal", "less_than", "equals"), ("session",)),
    ...
    DiscoveryFieldPresentation("indicator.vwap.execution_value", "market.execution_vwap", "execution_vwap", "VWAP", "Sole causal session VWAP authority, computed from volume-eligible trades inside the prevailing NBBO no more than one second old.", "indicator", True, True, True, ("greater_or_equal", "greater_than", "less_or_equal", "less_than", "equals", "above_by_bps"), ("session",)),
    ...
    DiscoveryFieldPresentation("news.composite_sentiment", "news.composite_sentiment", "news_composite_sentiment", "Synthesis direction", "Issuer-specific News Synthesis direction. It is decision-authorized only inside the protected positive-direction plus DeepFM-eligible signal contract.", "context", False, True, True, ("equals", "not_equals"), ("event",)),
