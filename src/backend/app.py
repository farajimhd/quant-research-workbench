...
from src.backend.query_plans.news_canvas_asof_v1 import trading_news_queries
...
def trading_news_rows(
    ...
    query, facet_query = trading_news_queries(
        before=before,
        before_id=before_id,
        cutoff=cutoff,
        cursor=cursor,
        ...
        safe_eligibility=safe_eligibility,
        safe_kind=safe_kind,
        safe_label_state=safe_label_state,
...
def service_sec_identity_rows_by_cik(database: str, ciks: list[str]) -> dict[str, list[dict[str, Any]]]:
    ...
    if not normalized_ciks:
        return {}
    if database != "q_live":
        raise ValueError("SEC operational identity plan is bound to q_live")
    rows = clickhouse_json_each_row(sec_identity_rows_by_cik_query(normalized_ciks))
...
@app.get("/api/trading/ticker-facts/{symbol}/splits")
def trading_ticker_split_events(symbol: str, as_of: str | None = None) -> dict[str, Any]:
    try:
        return ticker_split_events_payload(symbol, as_of=as_of)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    except RuntimeError as error:
        raise HTTPException(status_code=503, detail=f"Ticker split events are temporarily unavailable: {error}") from error
...
def market_discovery_signal_stream_runtime(
    ...
    if not as_of.strip() and not run_id.strip():
        from src.backend.real_live_trading_service import (
            enrich_halt_signal_stream_snapshot,
        )
...
async def _backtest_result_projection(
    ...
    *,
    ...
    # Canvas already carries the consequential, lifecycle-scoped strategy
    # activity. Re-reading the complete journal here retransmitted thousands of
    # partial-fill bookkeeping rows that no Backtest result consumer uses.
    activity = list(trading.get("strategy_activity") or [])
    return {
        "schema_version": 2,
...
async def trading_backtest_run(run_id: str, compact: bool = False) -> dict[str, Any]:
    try:
        controller = backtest_run_service.get(run_id)
        return controller.stream_snapshot() if compact else await asyncio.to_thread(controller.snapshot)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Backtest run not found") from exc
...
async def trading_backtest_typed_running_page(
    ...
    """Read only the committed V2 running prefix; never claim terminal review."""
    try:
        normalized = str(uuid.UUID(run_id))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Invalid Backtest run id") from exc
    ...
    def read_page() -> dict[str, Any]:
        from contextlib import closing
        from src.trading_runtime.arte_journal_reader import readonly_typed_journal_client
        from src.backend.typed_backtest_review_core import load_typed_backtest_running_page
        with closing(readonly_typed_journal_client()) as client:
            return load_typed_backtest_running_page(
