...
from src.trading_runtime.early_squeeze_price import STRICT_CONTRACT as CONTRACT

LABEL = 'Early Squeeze / resistance new high / trade-price trail v10'
DESCRIPTION = (
    ...
    'resistance. Only reentry at that resistance also requires a new traded-price high since its first breakout. No separate recovery path. '
    ...
    'Stops anchor below the broken lower band; additions ratchet protection beneath their newly broken bands. '
...
def create():
    ...
    existing = next((c for c in candidates if c['candidate_revision'] == 327), None)
    if existing:
        if existing['label'] != LABEL:
            raise ValueError('Candidate 327 is occupied; refusing to replace it')
        return configuration_candidate(existing['candidate_id'], required=True)
    if max((c['candidate_revision'] for c in candidates), default=0) != 326:
        raise ValueError('Next candidate must be exactly 327')
    ...
    if result['candidate_revision'] != 327:
