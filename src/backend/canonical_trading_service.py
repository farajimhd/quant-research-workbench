...
def performance_snapshot(
    ...
    *,
    ...
    supporting_metrics = [
        ...
        _performance_metric("open_position_count", "Open positions", open_positions, "count", "neutral", "Current non-zero positions across the selected accounts."),
        _performance_metric("available_cash", "Available cash", Decimal(str(available_cash or 0)), "money", "neutral", "Broker available funds, with total cash used only when available funds are absent."),
    ]
    return json_safe(
        {
            "schema_version": 3,
            ...
            "metrics": [*headline_metrics, *supporting_metrics],
        }
    )
