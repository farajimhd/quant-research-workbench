...
class ReplayRunController:
    ...
    def __init__(
        ...
        *,
        ...
        self._quote_geometry_enabled = any(
            ...
            for row in pressure_profiles)
        self._trade_volume_trackers: dict[str, TradeVolumeTracker] = {}
        self._trade_volume_enabled = any(
            dict(row.get("parameters") or {}).get("historical_hod", {}).get("setup_minimum_volume_ratio")
            for row in pressure_profiles)
        self._session_relative_volume_enabled = any(
            dict(row.get('parameters') or {}).get('historical_hod', {}).get('setup_minimum_session_relative_volume')
            for row in pressure_profiles)
    ...
    async def _can_skip_to_navigation_target(self) -> bool:
        ...
        if self._quote_geometry_enabled or self._trade_volume_enabled or self._session_relative_volume_enabled:
    ...
    def _navigation_skip_boundary(self) -> datetime | None:
        if (
            self._quote_geometry_enabled or self._trade_volume_enabled or self._session_relative_volume_enabled
    ...
    def _restart_checkpoint_state(self, *, reference_authority=False) -> dict[str, Any]:
        ...
        return {
            ...
            "controller": {
                ...
                'session_volume_trackers': {ticker: tracker.checkpoint() for ticker, tracker in self._session_volume_trackers.items()},
                'session_relative_volume_artifacts': dict(self._session_relative_volume_store.identities),
                "trade_volume_trackers": {ticker: tracker.checkpoint() for ticker, tracker in self._trade_volume_trackers.items()},
    ...
    def _restore_restart_checkpoint(self, *, review_only: bool = False) -> None:
        ...
        if self._session_relative_volume_enabled and ('session_volume_trackers' not in controller or 'session_relative_volume_artifacts' not in controller):
            raise ValueError('Enabled session RVOL requires checkpoint history and baseline artifacts')
        self._session_volume_trackers = {ticker: SessionVolumeTracker(self.definition.session_start, saved)
        ...
        self._trade_volume_trackers = {ticker: TradeVolumeTracker(saved)
            for ticker, saved in controller.get("trade_volume_trackers", {}).items()}
    ...
    def _market_pressure_snapshot(self, ticker, at):
        ...
        if self._trade_volume_enabled:
            volume = self._trade_volume_trackers.get(ticker)
            if volume is not None:
                result['trade_volume'] = volume.snapshot(at)
        if self._session_relative_volume_enabled:
            volume = self._session_volume_trackers.get(ticker)
            if volume is not None and ticker in self._session_relative_volume_store.cache:
                baseline = self._session_relative_volume_store.cached(ticker)
                result['session_relative_volume'] = volume.snapshot(at, baseline, ticker)
        return result
    ...
    async def _process_market_event(
        ...
        *,
        ...
        if self._trade_volume_enabled:
            volume = self._trade_volume_trackers.get(event.ticker)
            if volume is None:
                volume = self._trade_volume_trackers[event.ticker] = TradeVolumeTracker()
            volume.observe(event)
        if self._session_relative_volume_enabled:
            volume = self._session_volume_trackers.get(event.ticker)
            if volume is None:
                volume = self._session_volume_trackers[event.ticker] = SessionVolumeTracker(self.definition.session_start)
            volume.observe(event)
    ...
    async def _observe_episode_candle(self, frame: ReplayDerivedFrame) -> None:
        ...
        if parameters.get('early_squeeze_breakout_contract') in ('early-squeeze-r1-100ms-v7', 'early-squeeze-r1-100ms-v8', 'early-squeeze-r1-price-gap-v9', 'early-squeeze-r1-price-high-v10', 'early-squeeze-r1-price-episode-v11', 'early-squeeze-r1-price-resistance-ceiling-v12') and frame.timeframe == '100ms':
    ...
    async def _apply_strategy_frame(self, frame: ReplayDerivedFrame) -> bool:
        ...
        if (
            ...
            if self._processed_frames % 64 == 0:
                await asyncio.sleep(0)
            return False
        await self._ensure_session_relative_volume(frame.ticker)
        ...
        if frame.ticker in getattr(self, '_candle_detector_states', {}):
            ...
            if 'structural_recovery' in detector_stream:
                ...
                if parameters.get('early_squeeze_breakout_contract') in ('early-squeeze-r1-100ms-v8', 'early-squeeze-r1-price-gap-v9', 'early-squeeze-r1-price-high-v10', 'early-squeeze-r1-price-episode-v11', 'early-squeeze-r1-price-resistance-ceiling-v12'):
    ...
    async def _process_strategy_market_event(self, event: MarketEvent) -> bool:
        ...
        if isinstance(event, QuoteEvent) and event.ticker in self._strategy_engaged_tickers:
            assignments = tuple(a for a in self._ticker_assignments(event.ticker)
                                if a.parameters.get('hindsight_long_contract') or a.parameters.get('early_squeeze_breakout_contract') in ('early-squeeze-r1-100ms-v7', 'early-squeeze-r1-100ms-v8', 'early-squeeze-r1-price-gap-v9', 'early-squeeze-r1-price-high-v10', 'early-squeeze-r1-price-episode-v11', 'early-squeeze-r1-price-resistance-ceiling-v12'))
        ...
        if self.definition.experimental_structure_book or any(assignment.strategy_revision >= 37
            ...
            if any(a.parameters.get('early_squeeze_breakout_contract') in ('early-squeeze-r1-price-gap-v9', 'early-squeeze-r1-price-high-v10', 'early-squeeze-r1-price-episode-v11', 'early-squeeze-r1-price-resistance-ceiling-v12')
                   for a in ticker_assignments):
        ...
        if any(a.parameters.get('early_squeeze_breakout_contract') in ('early-squeeze-r1-price-gap-v9', 'early-squeeze-r1-price-high-v10', 'early-squeeze-r1-price-episode-v11', 'early-squeeze-r1-price-resistance-ceiling-v12')
            ...
            # event. A quiet interval does not invalidate cumulative VWAP;
            # retain its source time and separately certify the consumed prefix.
            vwap_sample = source_values.get('indicator.vwap.execution_value@100ms', {})
            base = replace(base, structural_detector_state={**(base.structural_detector_state or {}),
        ...
        await self._ensure_session_relative_volume(event.ticker)
        observation = replace(
            base,
    ...
    async def _event_structure_context(self, event: TradeEvent) -> dict[str, Any]:
        ...
        if self.definition.experimental_structure_book:
            ...
            if self.definition.configuration_revision['payload'].get('strategy', {}).get('parameters', {}).get('early_squeeze_breakout_contract') in ('early-squeeze-r1-price-gap-v9', 'early-squeeze-r1-price-high-v10', 'early-squeeze-r1-price-episode-v11', 'early-squeeze-r1-price-resistance-ceiling-v12'):
            ...
            async with permits:
                if stream.get("historical_occurrence_artifact"):
                    ...
                    try:
                        ...
                        parameters = self.definition.configuration_revision['payload'].get('strategy', {}).get('parameters', {})
                        if parameters.get('early_squeeze_breakout_contract') not in ('early-squeeze-r1-fixed-trail-v6', 'early-squeeze-r1-100ms-v7', 'early-squeeze-r1-100ms-v8', 'early-squeeze-r1-price-gap-v9', 'early-squeeze-r1-price-high-v10', 'early-squeeze-r1-price-episode-v11', 'early-squeeze-r1-price-resistance-ceiling-v12'):
                            raise
