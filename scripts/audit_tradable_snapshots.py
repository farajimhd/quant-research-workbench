...
from services.reference_gateway.tradable_snapshots import (
    COVERAGE, ensure_schema, publish_retained_snapshot, query, record_missing_sessions, sql_literal, target_session,
)
...
def audit(client: ClickHouseHttpClient, start: date, end: date, *, execute: bool) -> dict:
    ...
    sources = query(client, f"""SELECT universe_date,uniqExact(source_run_id) AS runs,
        min(inserted_at) AS first_insert,max(inserted_at) AS last_insert,count() AS rows
    ...
    for source in sources:
        ...
        if session in candidates:
            candidates[session].append(source)
    certificates = {}
    if execute:
        ...
        for day in sessions:
            ...
            try:
                publish_retained_snapshot(client,"q_live",date.fromisoformat(source["universe_date"]),expected_session=day)
    ...
    if coverage_exists:
        rows = query(client, f"SELECT session_date,snapshot_id,source_universe_date,captured_at_utc,row_count,tradable_count,source_hash,revision,status FROM q_live.{COVERAGE} FINAL WHERE session_date BETWEEN toDate({sql_literal(start)}) AND toDate({sql_literal(end)})")
        certificates = {date.fromisoformat(row["session_date"]):row for row in rows if row["status"]=="certified"}
    outcomes = []
    for day in sessions:
    ...
    return dict(start_date=str(start),end_date=str(end),execute=execute,revision="preopen-tradable-snapshot-v2",
