    ...
    try:
        for sequence,stamp,raw in connection.execute("select sequence,event_time,payload_json from journal where category='execution' and entity_type='fill' order by sequence"):
            ...
            if active is None:
                ...
                active=dict(opened_at=stamp,entry_price=price,quantity=0.,buy=0.,sell=0.,fees=0.,fills=[])
...
def summarize(root):
    ...
    lines=['# Strategy 222 replay refinement','',
        'Same-day development evidence; hindsight selects evaluation windows only. Each run starts at 04:00 with $10,000 and one symbol. Portfolio competition from the original multi-symbol run is absent. Net results include simulated fill commissions; open positions are excluded from closed net.', '',
...
def compare_positions(root,trials):
    ...
    for review in reviews:
        ...
        if not best:
            row['hindsight_status']='No positive tested hindsight label';cases.append(row);continue
        key=review['group']
        if key not in quotes:quotes[key]=np.load(study/f'quotes-{key}.npz')['data']
        data=quotes[key];future=data[data[:,0]/1e6>=best['t']]
        ...
        for trial in trials:
            if trial['symbol']!=review['s']:continue
    ...
    (root/'position-comparison.json').write_text(json.dumps(dict(
        method='Hindsight best tested ask to maximum subsequent bid in the frozen review window. This is an offline diagnostic, not an executable target or proof of profit. Shared opportunity IDs must be deduplicated when aggregating.',
        cases=cases),indent=2),encoding='utf-8')
    print(f"Position audit: {len(cases)} original positions; {sum(bool(c['variants']) for c in cases)} have a completed comparable replay window",flush=True)
