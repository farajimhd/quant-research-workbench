...
class ServiceConfig:
    ...
    def from_env(cls) -> "ServiceConfig":
        ...
        releases = _release_configs(os.environ)
        return cls(
            bind=bind,
            device=os.environ.get("BAR_GPT_DEVICE", "auto").strip().lower(),
            dtype=os.environ.get("BAR_GPT_DTYPE", "bfloat16").strip().lower(),
            ...
            maximum_batch_size=max(1, int(os.environ.get("BAR_GPT_MAX_BATCH_SIZE", "64"))),
            maximum_tickers=max(1, int(os.environ.get("BAR_GPT_MAX_TICKERS", "500"))),
            maximum_batch_delay_ms=max(0, int(os.environ.get("BAR_GPT_MAX_BATCH_DELAY_MS", "20"))),
            prediction_history=max(1, int(os.environ.get("BAR_GPT_PREDICTION_HISTORY", "2048"))),
            minimum_warm_1s_bars=max(1, int(os.environ.get("BAR_GPT_MINIMUM_WARM_1S_BARS", "64"))),
            queue_capacity=max(1, int(os.environ.get("BAR_GPT_QUEUE_CAPACITY", "4096"))),
            warm_concurrency=max(1, int(os.environ.get("BAR_GPT_WARM_CONCURRENCY", "4"))),
            releases=releases,
            connect_qmd=_bool_env("BAR_GPT_CONNECT_QMD", True),
        )
