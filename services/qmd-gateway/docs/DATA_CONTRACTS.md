...
| `qmd_decision_signal`, `qmd_decision_confidence`, `qmd_decision_action`, `qmd_decision_reason` | Canonical short-horizon Buy/Sell/Wait decision. The timeframe-native microstructure trigger contributes 78% and event-native structural context contributes 22%; material opposing context vetoes the action to Wait. Confidence is stored on a 0-1 scale. | Calculated once per closed bar from merged sufficient statistics and the structure state sampled at that bar end. |
...
| `qmd_structure_session_*`, `qmd_structure_opening_range_*`, `qmd_structure_trade_volume_poc` | Event-native reference levels. | Eligible trades own the 04:00-20:00 New York session extrema and exact trade-volume POC; eligible regular-session trades own the opening range. |
